/*
 * libgit3 multi-pack-index fuzzer target.
 *
...
{
	git3_midx_file idx = {{0}};
	git3_midx_entry e;
	git3_str midx_buf = GIT3_STR_INIT;
	...
	if (append_hash) {
		if (git3_str_init(&midx_buf, size + GIT3_HASH_SHA1_SIZE) < 0)
			goto cleanup;
		...
		memcpy(midx_buf.ptr + size, hash, GIT3_HASH_SHA1_SIZE);

		memcpy(oid.id, hash, GIT3_OID_SHA1_SIZE);
	} else {
		git3_str_attach_notowned(&midx_buf, (char *)data, size);
	}

	if (git3_midx_parse(&idx, (const unsigned char *)git3_str_cstr(&midx_buf), git3_str_len(&midx_buf)) < 0)
		goto cleanup;

	/* Search for any oid, just to exercise that codepath. */
	if (git3_midx_entry_find(&e, &idx, &oid, GIT3_OID_SHA1_HEXSIZE) < 0)
		goto cleanup;

cleanup:
	git3_midx_close(&idx);
	git3_str_dispose(&midx_buf);
	return 0;
}
