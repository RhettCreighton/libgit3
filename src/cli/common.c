...
{
	git3_str kv = GIT3_STR_INIT, env = GIT3_STR_INIT;
	git3_vector *cmdline_config = data;
	int error = 0;

	if (opt->spec && opt->spec->alias == 'c') {
		if (git3_str_puts(&kv, opt->value) < 0) {
			error = cli_error_git();
			goto done;
		}
	}
		...
		if (git3_str_put(&kv, opt->value, (val - opt->value)) < 0) {
			error = cli_error_git();
			goto done;
		}
		...
		if ((error = git3__getenv(&env, val)) == GIT3_ENOTFOUND) {
		...
		if (git3_str_putc(&kv, '=') < 0 ||
		    git3_str_puts(&kv, env.ptr) < 0) {
			error = cli_error_git();
			goto done;
		}
	}
	...
	if (kv.size > 0 &&
	    git3_vector_insert(cmdline_config, git3_str_detach(&kv)) < 0)
		error = cli_error_git();
	...
	cli_repository_open_options *opts)
{
	...
	git3_config_backend_memory_options config_opts =
		GIT3_CONFIG_BACKEND_MEMORY_OPTIONS_INIT;
	git3_vector cmdline = GIT3_VECTOR_INIT;
	...
	if (git3_vector_length(&cmdline) == 0)
		goto done;

	if (git3_repository_config(&config, repo) < 0 ||
	    git3_config_backend_from_values(&backend,
			(const char **)cmdline.contents, cmdline.length,
			&config_opts) < 0 ||
	    git3_config_add_backend(config, backend, GIT3_CONFIG_LEVEL_APP,
			repo, 0) < 0)
		error = cli_error_git();
	...
	git3_vector_dispose_deep(&cmdline);
	return error;
}
	...
	cli_repository_open_options *opts)
{
	...
	if (git3_repository_open_ext(&repo, ".", GIT3_REPOSITORY_OPEN_FROM_ENV, NULL) < 0)
		return -1;
 ...
 */
int cli_resolve_path(git3_str *out, git3_repository *repo, const char *given_path)
{
	git3_str path = GIT3_STR_INIT;
	int error = 0;

	if (!git3_fs_path_is_absolute(given_path)) {
		char cwd[GIT3_PATH_MAX];

		if (p_getcwd(cwd, GIT3_PATH_MAX) < 0)
			error = cli_error_os();
		else if (git3_str_puts(&path, cwd) < 0 ||
		         git3_fs_path_apply_relative(&path, given_path) < 0)
			error = cli_error_git();

		if (error)
			goto done;
	} else if (git3_str_puts(&path, given_path) < 0) {
		error = cli_error_git();
		goto done;
	}

	error = git3_fs_path_make_relative(&path, git3_repository_workdir(repo));

	if (error == GIT3_ENOTFOUND)
		error = cli_error("path '%s' is not inside the git repository '%s'",
			given_path, git3_repository_workdir(repo));
	else if (error < 0)
		error = cli_error_git();
	else
