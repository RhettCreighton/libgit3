...
static int merge_env(
	...
	bool exclude_env)
{
	git3_vector merged = GIT3_VECTOR_INIT;
	char **kv, *dup;
	size_t max, cnt;
	int error = 0;
	...
	if ((error = git3_vector_init(&merged, max, NULL)) < 0)
		goto on_error;
	...
	for (cnt = 0; env && cnt < env_len; cnt++) {
		...
		dup = git3__strdup(env[cnt]);
		GIT3_ERROR_CHECK_ALLOC(dup);

		if ((error = git3_vector_insert(&merged, dup)) < 0)
			goto on_error;
	}

	if (!exclude_env) {
		for (kv = environ; *kv; kv++) {
			if (env && git3_strlist_contains_key(env, env_len, *kv, '='))
				continue;
	...
	git3_vector_insert(&merged, NULL);
...
on_error:
	git3_vector_dispose_deep(&merged);
	return error;
}
	...
	git3_process_options *opts)
{
	...
	*out = NULL;
	...
	process = git3__calloc(sizeof(git3_process), 1);
	GIT3_ERROR_CHECK_ALLOC(process);

	if (git3_strlist_copy_with_null(&process->args, args, args_len) < 0 ||
	...
	if (opts) {
		...
		if (opts->cwd) {
			process->cwd = git3__strdup(opts->cwd);
			GIT3_ERROR_CHECK_ALLOC(process->cwd);
		}
	}
...
static int try_read_status(size_t *out, int fd, void *buf, size_t len)
{
	...
	while (ret && read_len < len) {
		...
		if (ret < 0 && errno != EAGAIN && errno != EINTR) {
			git3_error_set(GIT3_ERROR_OS, "could not read child status");
			return -1;
		}
...
static int read_status(int fd)
{
	...
	if (read_len < status_len) {
		git3_error_set(GIT3_ERROR_INVALID, "child status truncated");
		return -1;
	}
	...
	if (fn_error) {
		errno = os_error;
		git3_error_set(GIT3_ERROR_OS, "could not %s", fn[0] ? fn : "(unknown)");
	}
...
int git3_process_start(git3_process *process)
{
	...
	switch (pid = fork()) {
	case -1:
		git3_error_set(GIT3_ERROR_OS, "could not fork");
		goto on_error;
...
static ssize_t process_read(int fd, void *buf, size_t count)
{
	...
	if ((ret = read(fd, buf, count)) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not read from child process");
		return -1;
	}
...
{
	GIT3_ASSERT_ARG(process);
	GIT3_ASSERT(process->capture_out);

	return process_read(process->child_out, buf, count);
}

ssize_t git3_process_read_err(git3_process *process, void *buf, size_t count)
{
	GIT3_ASSERT_ARG(process);
	GIT3_ASSERT(process->capture_err);
...
GIT3_INLINE(int) disable_signals(sigset_t *saved_mask)
{
	...
	if (pthread_sigmask(SIG_BLOCK, &sigpipe_mask, saved_mask) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not configure signal mask");
		return -1;
	}
...
GIT3_INLINE(int) restore_signals(sigset_t *saved_mask)
{
	...
	if (sigpending(&pending) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not examine pending signals");
		return -1;
	}

	if (sigismember(&pending, SIGPIPE) == 1 &&
	    sigwait(&sigpipe_mask, &signal) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not wait for (blocking) signal delivery");
		return -1;
	}
...
GIT3_INLINE(int) disable_signals(struct sigaction *saved_handler)
{
	...
	if (sigaction(SIGPIPE, &ign_handler, saved_handler) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not configure signal handler");
		return -1;
	}
...
GIT3_INLINE(int) restore_signals(struct sigaction *saved_handler)
{
...
ssize_t git3_process_write(git3_process *process, const void *buf, size_t count)
{
	...
	GIT3_ASSERT_ARG(process);
	GIT3_ASSERT(process->capture_in);
	...
	if ((ret = write(process->child_in, buf, count)) < 0)
		git3_error_set(GIT3_ERROR_OS, "could not write to child process");
...
int git3_process_wait(git3_process_result *result, git3_process *process)
{
	...
	if (waitpid(process->pid, &state, 0) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not wait for child");
		return -1;
	}
	...
	if (result) {
		if (WIFEXITED(state)) {
			...
			result->status = GIT3_PROCESS_STATUS_ERROR;
...
void git3_process_free(git3_process *process)
{
	...
	git3_strlist_free_with_null(process->args);
	git3_strlist_free_with_null(process->env);
	git3__free(process);
}
