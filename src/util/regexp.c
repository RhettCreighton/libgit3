...
int git3_regexp_compile(git3_regexp *r, const char *pattern, int flags)
{
	...
	if (flags & GIT3_REGEXP_ICASE)
		cflags |= PCRE_CASELESS;

	if ((*r = pcre_compile(pattern, cflags, &error, &erroffset, NULL)) == NULL) {
		git3_error_set_str(GIT3_ERROR_REGEX, error);
...
int git3_regexp_match(const git3_regexp *r, const char *string)
{
	...
	if ((error = pcre_exec(*r, NULL, string, (int) strlen(string), 0, 0, NULL, 0)) < 0)
		return (error == PCRE_ERROR_NOMATCH) ? GIT3_ENOTFOUND : GIT3_EINVALIDSPEC;
	return 0;
}
...
int git3_regexp_search(const git3_regexp *r, const char *string, size_t nmatches, git3_regmatch *matches)
{
	...
	if (nmatches <= ARRAY_SIZE(static_ovec) / 3)
		...
		ovec = git3__calloc(nmatches * 3, sizeof(*ovec));
	GIT3_ERROR_CHECK_ALLOC(ovec);
...
int git3_regexp_compile(git3_regexp *r, const char *pattern, int flags)
{
	...
	if ((*r = pcre2_compile((const unsigned char *) pattern, PCRE2_ZERO_TERMINATED,
		...
		git3_error_set_str(GIT3_ERROR_REGEX, (char *) errmsg);
		return GIT3_EINVALIDSPEC;
	}
...
int git3_regexp_match(const git3_regexp *r, const char *string)
{
	...
	if (error < 0)
		return (error == PCRE2_ERROR_NOMATCH) ? GIT3_ENOTFOUND : GIT3_EINVALIDSPEC;
...
# endif
	{
		...
		git3_error_set_str(GIT3_ERROR_REGEX, errmsg);
		return GIT3_EINVALIDSPEC;
	}
...
int git3_regexp_match(const git3_regexp *r, const char *string)
{
	...
	if ((error = regexec(r, string, 0, NULL, 0)) != 0)
		return (error == REG_NOMATCH) ? GIT3_ENOTFOUND : GIT3_EINVALIDSPEC;
	return 0;
}
...
int git3_regexp_search(const git3_regexp *r, const char *string, size_t nmatches, git3_regmatch *matches)
{
	...
	if (nmatches <= ARRAY_SIZE(static_m))
		...
		m = git3__calloc(nmatches, sizeof(*m));
