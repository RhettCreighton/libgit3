...
#if defined(GIT3_REGEX_BUILTIN) || defined(GIT3_REGEX_PCRE)
# include "pcre.h"
typedef pcre *git3_regexp;
# define GIT3_REGEX_INIT NULL
#elif defined(GIT3_REGEX_PCRE2)
# define PCRE2_CODE_UNIT_WIDTH 8
# include <pcre2.h>
typedef pcre2_code *git3_regexp;
# define GIT3_REGEX_INIT NULL
#elif defined(GIT3_REGEX_REGCOMP) || defined(GIT3_REGEX_REGCOMP_L)
# include <regex.h>
typedef regex_t git3_regexp;
# define GIT3_REGEX_INIT { 0 }
#else
# error "No regex backend"
#endif

/** Options supported by @git3_regexp_compile. */
typedef enum {
	/** Enable case-insensitive matching */
	GIT3_REGEXP_ICASE = (1 << 0)
} git3_regexp_flags_t;
 ...
 * be cleaned up afterwards with `git3_regexp_dispose`.
 *
 ...
 *              0 for defaults, otherwise see @git3_regexp_flags_t.
 * @return 0 on success, otherwise a negative return value.
 */
int git3_regexp_compile(git3_regexp *r, const char *pattern, int flags);
 ...
 */
void git3_regexp_dispose(git3_regexp *r);
 ...
 *         otherwise. GIT3_ENOTFOUND if no match was found,
 *         GIT3_EINVALIDSPEC if the regular expression matching
 *         was invalid.
 */
int git3_regexp_match(const git3_regexp *r, const char *string);

/**
 * Search for matches inside of a given string.
 *
 * Given a regular expression with capturing groups, this
 * function will populate provided @git3_regmatch structures with
 * offsets for each of the given matches. Non-matching groups
 * will have start and end values of the respective @git3_regmatch
 * structure set to -1.
 *
 * @param r Compiled regular expression.
 * @param string String to match against the regular expression.
 * @param nmatches Number of @git3_regmatch structures provided by
 *                 the user.
 * @param matches Pointer to an array of @git3_regmatch structures.
 * @return 0 if the string matches, a negative error code
 *         otherwise. GIT3_ENOTFOUND if no match was found,
 *         GIT3_EINVALIDSPEC if the regular expression matching
 *         was invalid.
 */
int git3_regexp_search(const git3_regexp *r, const char *string, size_t nmatches, git3_regmatch *matches);
