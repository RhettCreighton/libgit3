 ...
 */
GIT3_INLINE(int) git3_error_set_after_callback_function(
	int error_code, const char *action)
{
	if (error_code) {
		if (!git3_error_exists())
			git3_error_set(GIT3_ERROR_CALLBACK,
...
#ifdef GIT3_WIN32
#define git3_error_set_after_callback(code) \
	git3_error_set_after_callback_function((code), __FUNCTION__)
#else
#define git3_error_set_after_callback(code) \
	git3_error_set_after_callback_function((code), __func__)
#endif
 ...
 */
extern int git3_error_save(git3_error **out);

/**
 * Restore thread error state to the given value. The given value is
 * freed and `git3_error_free` need not be called on it.
 */
extern int git3_error_restore(git3_error *error);

/** Free an error state. */
extern void git3_error_free(git3_error *error);
