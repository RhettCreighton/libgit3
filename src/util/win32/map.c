...
int git3__mmap_alignment(size_t *page_size)
{
...
int p_mmap(git3_map *out, size_t len, int prot, int flags, int fd, off64_t offset)
{
	...
	if (fh == INVALID_HANDLE_VALUE) {
		errno = EBADF;
		git3_error_set(GIT3_ERROR_OS, "failed to mmap. Invalid handle value");
		return -1;
	}

	if (prot & GIT3_PROT_WRITE)
		fmap_prot |= PAGE_READWRITE;
	else if (prot & GIT3_PROT_READ)
		fmap_prot |= PAGE_READONLY;

	if (prot & GIT3_PROT_WRITE)
		view_prot |= FILE_MAP_WRITE;
	if (prot & GIT3_PROT_READ)
	...
	if (page_offset != 0) { /* offset must be multiple of the allocation granularity */
		errno = EINVAL;
		git3_error_set(GIT3_ERROR_OS, "failed to mmap. Offset must be multiple of allocation granularity");
		return -1;
	}
	...
	if (!out->data) {
		git3_error_set(GIT3_ERROR_OS, "failed to mmap. No data written");
		CloseHandle(out->fmh);
		out->fmh = NULL;
		return -1;
	}
...
int p_munmap(git3_map *map)
{
	...
	if (map->data) {
		if (!UnmapViewOfFile(map->data)) {
			git3_error_set(GIT3_ERROR_OS, "failed to munmap. Could not unmap view of file");
			error = -1;
		}
		map->data = NULL;
	}
	...
	if (map->fmh) {
		if (!CloseHandle(map->fmh)) {
			git3_error_set(GIT3_ERROR_OS, "failed to munmap. Could not close handle");
			error = -1;
		}
		map->fmh = NULL;
	}
