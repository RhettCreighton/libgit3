...
GIT3_INLINE(int) handle_wc_error(void)
{
...
int git3_str_put_w(git3_str *buf, const wchar_t *string_w, size_t len_w)
{
	...
	GIT3_ASSERT(utf8_len > 0);

	GIT3_ERROR_CHECK_ALLOC_ADD(&new_size, buf->size, (size_t)utf8_len);
	GIT3_ERROR_CHECK_ALLOC_ADD(&new_size, new_size, 1);

	if (git3_str_grow(buf, new_size) < 0)
		return -1;
	...
	GIT3_ASSERT(utf8_write_len == utf8_len);
