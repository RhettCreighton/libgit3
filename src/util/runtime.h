...
typedef int (*git3_runtime_init_fn)(void);
typedef void (*git3_runtime_shutdown_fn)(void);
 ...
 * handlers using `git3_runtime_shutdown_register` to be notified
 ...
 * `git3_runtime_init` minus the number of calls sto `git3_runtime_shutdown`).
 ...
 * by a function invoked by `git3_runtime_init` to ensure that the
