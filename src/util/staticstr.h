...
#define git3_staticstr_with_size(__size) \
	struct { \
		size_t len; \
		size_t size; \
		char data[__size]; \
...
#define git3_staticstr_init(__str, __size) \
	do { \
		(__str)->len = 0; \
		(__str)->size = __size; \
		(__str)->data[0] = '\0'; \
...
#define git3_staticstr_offset(__str) \
	((__str)->data + (__str)->len)

#define git3_staticstr_remain(__str) \
	((__str)->len > (__str)->size ? 0 : ((__str)->size - (__str)->len))

#define git3_staticstr_increase(__str, __len) \
	do { ((__str)->len += __len); } while(0)

#define git3_staticstr_consume_bytes(__str, __len) \
	do { git3_staticstr_consume(__str, (__str)->data + __len); } while(0)

#define git3_staticstr_consume(__str, __end) \
	do { \
		if (__end > (__str)->data && \
...
#define git3_staticstr_clear(__str) \
	do { \
		(__str)->len = 0; \
		(__str)->data[0] = 0; \
	} while(0)
