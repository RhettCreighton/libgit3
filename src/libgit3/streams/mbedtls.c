...
#ifndef GIT3_DEFAULT_CERT_LOCATION
#define GIT3_DEFAULT_CERT_LOCATION NULL
#endif
...
#define GIT3_SSL_DEFAULT_CIPHERS "TLS1-3-AES-128-GCM-SHA256:TLS1-3-AES-256-GCM-SHA384:TLS1-3-CHACHA20-POLY1305-SHA256:TLS-ECDHE-ECDSA-WITH-AES-128-GCM-SHA256:TLS-ECDHE-RSA-WITH-AES-128-GCM-SHA256:TLS-ECDHE-ECDSA-WITH-AES-256-GCM-SHA384:TLS-ECDHE-RSA-WITH-AES-256-GCM-SHA384:TLS-ECDHE-ECDSA-WITH-CHACHA20-POLY1305-SHA256:TLS-ECDHE-RSA-WITH-CHACHA20-POLY1305-SHA256:TLS-DHE-RSA-WITH-AES-128-GCM-SHA256:TLS-DHE-RSA-WITH-AES-256-GCM-SHA384:TLS-DHE-RSA-WITH-CHACHA20-POLY1305-SHA256"
#define GIT3_SSL_DEFAULT_CIPHERS_COUNT 12

static int ciphers_list[GIT3_SSL_DEFAULT_CIPHERS_COUNT];
...
int git3_mbedtls_stream_global_init(void)
{
	int loaded = 0;
	char *crtpath = GIT3_DEFAULT_CERT_LOCATION;
	struct stat statbuf;
	...
	if (mbedtls_ssl_config_defaults(&mbedtls_config,
		...
		git3_error_set(GIT3_ERROR_SSL, "failed to initialize mbedTLS");
		goto cleanup;
	}
	...
	ciphers_known = 0;
	cipher_string = cipher_string_tmp = git3__strdup(GIT3_SSL_DEFAULT_CIPHERS);
	GIT3_ERROR_CHECK_ALLOC(cipher_string);

	while ((cipher_name = git3__strtok(&cipher_string_tmp, ":")) != NULL) {
		int cipherid = mbedtls_ssl_get_ciphersuite_id(cipher_name);
		if (cipherid == 0) continue;

		if (ciphers_known >= ARRAY_SIZE(ciphers_list)) {
			git3_error_set(GIT3_ERROR_SSL, "out of cipher list space");
			goto cleanup;
		}

		ciphers_list[ciphers_known++] = cipherid;
	}
	git3__free(cipher_string);

	if (!ciphers_known) {
		git3_error_set(GIT3_ERROR_SSL, "no cipher could be enabled");
		goto cleanup;
	}
	...
	if (mbedtls_ctr_drbg_seed(&mbedtls_rng, mbedtls_entropy_func,
		...
		git3_error_set(GIT3_ERROR_SSL, "failed to initialize mbedTLS entropy pool");
		goto cleanup;
	}
	...
	if (crtpath != NULL && stat(crtpath, &statbuf) == 0 && S_ISREG(statbuf.st_mode))
		loaded = (git3_mbedtls__set_cert_location(crtpath, NULL) == 0);

	if (!loaded && crtpath != NULL && stat(crtpath, &statbuf) == 0 && S_ISDIR(statbuf.st_mode))
		loaded = (git3_mbedtls__set_cert_location(NULL, crtpath) == 0);
...
{
	git3_stream *io = (git3_stream *) b;
	return (int) git3_stream_read(io, buf, min(len, INT_MAX));
}

static int bio_write(void *b, const unsigned char *buf, size_t len)
{
	git3_stream *io = (git3_stream *) b;
	return (int) git3_stream_write(io, (const char *)buf, min(len, INT_MAX), 0);
}
...
static int ssl_set_error(mbedtls_ssl_context *ssl, int error)
{
	...
	GIT3_ASSERT(error != MBEDTLS_ERR_SSL_WANT_READ);
	GIT3_ASSERT(error != MBEDTLS_ERR_SSL_WANT_WRITE);
	...
	case MBEDTLS_ERR_X509_CERT_VERIFY_FAILED:
		git3_error_set(GIT3_ERROR_SSL, "SSL error: %#04x [%x] - %s", error, mbedtls_ssl_get_verify_result(ssl), errbuf);
		ret = GIT3_ECERTIFICATE;
		break;

	default:
		git3_error_set(GIT3_ERROR_SSL, "SSL error: %#04x - %s", error, errbuf);
	}
...
static int verify_server_cert(mbedtls_ssl_context *ssl)
{
	...
	if ((ret = mbedtls_ssl_get_verify_result(ssl)) != 0) {
		...
		git3_error_set(GIT3_ERROR_SSL, "the SSL certificate is invalid: %#04x - %s", ret, vrfy_buf);
...
static int mbedtls_connect(git3_stream *stream)
{
	int ret;
...
static int mbedtls_certificate(git3_cert **out, git3_stream *stream)
{
	...
	encoded_cert = git3__malloc(cert->raw.len);
...
static int mbedtls_set_proxy(git3_stream *stream, const git3_proxy_options *proxy_options)
{
	mbedtls_stream *st = (mbedtls_stream *) stream;
...
static ssize_t mbedtls_stream_write(git3_stream *stream, const char *data, size_t len, int flags)
{
	mbedtls_stream *st = (mbedtls_stream *) stream;
	int written;
...
static ssize_t mbedtls_stream_read(git3_stream *stream, void *data, size_t len)
{
	mbedtls_stream *st = (mbedtls_stream *) stream;
	int ret;
...
static int mbedtls_stream_close(git3_stream *stream)
{
	mbedtls_stream *st = (mbedtls_stream *) stream;
	int ret = 0;
...
static void mbedtls_stream_free(git3_stream *stream)
{
	mbedtls_stream *st = (mbedtls_stream *) stream;
	...
	int owned)
{
	...
	st = git3__calloc(1, sizeof(mbedtls_stream));
	GIT3_ERROR_CHECK_ALLOC(st);

	st->io = in;
	st->owned = owned;

	st->ssl = git3__malloc(sizeof(mbedtls_ssl_context));
	GIT3_ERROR_CHECK_ALLOC(st->ssl);
...
int git3_mbedtls__set_cert_location(const char *file, const char *path)
{
	int ret = 0;
	char errbuf[512];

	GIT3_ASSERT_ARG(file || path);
	...
	if (ret < 0) {
		...
		git3_error_set(GIT3_ERROR_SSL, "failed to load CA certificates: %#04x - %s", ret, errbuf);
		return -1;
	}
