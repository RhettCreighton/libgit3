...
int git_libgit3_shutdown(void)
{
	return git_runtime_shutdown();
}

int git_libgit3_version(int *major, int *minor, int *rev)
{
...
const char *git_libgit3_prerelease(void)
{
	return LIBGIT2_VERSION_PRERELEASE;
}

int git_libgit3_features(void)
{
	return 0
#ifdef GIT_THREADS
		| GIT_FEATURE_THREADS
...
const char *git_libgit3_feature_backend(git_feature_t feature)
{
	switch (feature) {
