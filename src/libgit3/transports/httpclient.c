...
static git3_http_auth_scheme auth_schemes[] = {
	{ GIT3_HTTP_AUTH_NEGOTIATE, "Negotiate", GIT3_CREDENTIAL_DEFAULT, git3_http_auth_negotiate },
	{ GIT3_HTTP_AUTH_NTLM, "NTLM", GIT3_CREDENTIAL_USERPASS_PLAINTEXT, git3_http_auth_ntlm },
	{ GIT3_HTTP_AUTH_BASIC, "Basic", GIT3_CREDENTIAL_USERPASS_PLAINTEXT, git3_http_auth_basic },
};
 ...
 */
#define GIT3_READ_BUFFER_SIZE (16 * 1024)

typedef struct {
	git3_net_url url;
	git3_stream *stream;

	git3_vector auth_challenges;
	git3_http_auth_context *auth_context;
} git3_http_server;
...
typedef struct {
	git3_http_client *client;
	git3_http_response *response;

	/* Temporary buffers to avoid extra mallocs */
	git3_str parse_header_name;
	git3_str parse_header_value;
...
struct git3_http_client {
	git3_http_client_options opts;

	/* Are we writing to the proxy or server, and state of the client. */
	git3_http_server_t current_server;
	http_client_state state;

	git3_http_parser parser;

	git3_http_server server;
	git3_http_server proxy;
	 ...
	 */
	git3_http_response early_response;
};

bool git3_http_response_is_redirect(git3_http_response *response)
{
	return (response->status == GIT3_HTTP_MOVED_PERMANENTLY ||
	        response->status == GIT3_HTTP_FOUND ||
	        response->status == GIT3_HTTP_SEE_OTHER ||
		response->status == GIT3_HTTP_TEMPORARY_REDIRECT ||
		response->status == GIT3_HTTP_PERMANENT_REDIRECT);
}

void git3_http_response_dispose(git3_http_response *response)
{
	if (!response)
		return;

	git3__free(response->content_type);
	git3__free(response->location);

	memset(response, 0, sizeof(git3_http_response));
}

static int on_header_complete(git3_http_parser *parser)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	git3_http_client *client = ctx->client;
	git3_http_response *response = ctx->response;

	git3_str *name = &ctx->parse_header_name;
	git3_str *value = &ctx->parse_header_value;

	if (!strcasecmp("Content-Type", name->ptr)) {
		if (response->content_type) {
			git3_error_set(GIT3_ERROR_HTTP,
			              "multiple content-type headers");
			return -1;
		}

		response->content_type =
			git3__strndup(value->ptr, value->size);
		GIT3_ERROR_CHECK_ALLOC(ctx->response->content_type);
		...
		if (git3__strntol64(&len, value->ptr, value->size,
		                   NULL, 10) < 0 || len < 0) {
			git3_error_set(GIT3_ERROR_HTTP,
	...
	} else if (!strcasecmp("Proxy-Authenticate", git3_str_cstr(name))) {
		char *dup = git3__strndup(value->ptr, value->size);
		GIT3_ERROR_CHECK_ALLOC(dup);

		if (git3_vector_insert(&client->proxy.auth_challenges, dup) < 0)
			return -1;
	} else if (!strcasecmp("WWW-Authenticate", name->ptr)) {
		char *dup = git3__strndup(value->ptr, value->size);
		GIT3_ERROR_CHECK_ALLOC(dup);

		if (git3_vector_insert(&client->server.auth_challenges, dup) < 0)
			return -1;
		...
		response->location = git3__strndup(value->ptr, value->size);
		GIT3_ERROR_CHECK_ALLOC(response->location);
	}

	return 0;
}

static int on_header_field(git3_http_parser *parser, const char *str, size_t len)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	...
	case PARSE_HEADER_VALUE:
		...
		git3_str_clear(&ctx->parse_header_name);
		git3_str_clear(&ctx->parse_header_value);
		/* Fall through */
	...
	case PARSE_HEADER_NAME:
		...
		if (git3_str_put(&ctx->parse_header_name, str, len) < 0)
			return ctx->parse_status = PARSE_STATUS_ERROR;
...
static int on_header_value(git3_http_parser *parser, const char *str, size_t len)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	...
	case PARSE_HEADER_VALUE:
		...
		if (git3_str_put(&ctx->parse_header_value, str, len) < 0)
			return ctx->parse_status = PARSE_STATUS_ERROR;
...
GIT3_INLINE(bool) challenge_matches_scheme(
	const char *challenge,
	git3_http_auth_scheme *scheme)
{
...
static git3_http_auth_scheme *scheme_for_challenge(const char *challenge)
{
	size_t i;
...
GIT3_INLINE(void) collect_authinfo(
	unsigned int *schemetypes,
	unsigned int *credtypes,
	git3_vector *challenges)
{
	git3_http_auth_scheme *scheme;
	const char *challenge;
	size_t i;
	...
	*credtypes = 0;
	...
	git3_vector_foreach(challenges, i, challenge) {
...
static int resend_needed(git3_http_client *client, git3_http_response *response)
{
	git3_http_auth_context *auth_context;

	if (response->status == GIT3_HTTP_STATUS_UNAUTHORIZED &&
	    (auth_context = client->server.auth_context) &&
	    auth_context->is_complete &&
	    !auth_context->is_complete(auth_context))
		return 1;

	if (response->status == GIT3_HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED &&
...
static int on_headers_complete(git3_http_parser *parser)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	...
	ctx->response->status = git3_http_parser_status_code(parser);
	ctx->client->keepalive = git3_http_parser_keep_alive(parser);
	...
	return git3_http_parser_pause(parser);
}

static int on_body(git3_http_parser *parser, const char *buf, size_t len)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	size_t max_len;
	...
	GIT3_ASSERT(ctx->output_size >= ctx->output_written);
...
static int on_message_complete(git3_http_parser *parser)
{
	http_parser_context *ctx = (http_parser_context *) parser->data;
	...
	size_t len)
{
	...
	return git3_stream__write_full(server->stream, data, len, 0);
}

GIT3_INLINE(int) client_write_request(git3_http_client *client)
{
	git3_stream *stream = client->current_server == PROXY ?
	...
	return git3_stream__write_full(stream,
				      client->request_msg.ptr,
	                              client->request_msg.size,
				      0);
}

static const char *name_for_method(git3_http_method method)
{
	switch (method) {
	case GIT3_HTTP_METHOD_GET:
		return "GET";
	case GIT3_HTTP_METHOD_POST:
		return "POST";
	case GIT3_HTTP_METHOD_CONNECT:
		return "CONNECT";
	}
...
static bool best_scheme_and_challenge(
	git3_http_auth_scheme **scheme_out,
	const char **challenge_out,
	git3_vector *challenges,
	git3_credential *credentials)
{
	const char *challenge;
	size_t i, j;

	for (i = 0; i < ARRAY_SIZE(auth_schemes); i++) {
		git3_vector_foreach(challenges, j, challenge) {
			git3_http_auth_scheme *scheme = &auth_schemes[i];
...
static const char *challenge_for_context(
	git3_vector *challenges,
	git3_http_auth_context *auth_ctx)
{
	const char *challenge;
	size_t i, j;
	...
	git3_credential *credentials)
{
	...
	if (!best_scheme_and_challenge(&scheme, &challenge, challenges, credentials)) {
		git3_error_set(GIT3_ERROR_HTTP, "could not find appropriate mechanism for credentials");
		return NULL;
	}
	...
	if (error == GIT3_PASSTHROUGH) {
		git3_error_set(GIT3_ERROR_HTTP, "'%s' authentication is not supported", scheme->name);
		return NULL;
	}
...
static void free_auth_context(git3_http_server *server)
{
...
static int apply_credentials(
	...
	git3_credential *credentials)
{
	git3_http_auth_context *auth = server->auth_context;
	git3_vector *challenges = &server->auth_challenges;
	const char *challenge;
	git3_str token = GIT3_STR_INIT;
	int error = 0;
	...
	if (!auth && !git3_vector_length(challenges))
		return 0;
	...
	if (auth->is_complete && auth->is_complete(auth)) {
		...
		git3_error_set(GIT3_ERROR_HTTP, "failed to respond to authentication challenge");
		error = GIT3_EAUTH;
		goto done;
	}
	...
	git3_str_dispose(&token);
	return error;
}

GIT3_INLINE(int) apply_server_credentials(
	git3_str *buf,
	git3_http_client *client,
	git3_http_request *request)
{
...
GIT3_INLINE(int) apply_proxy_credentials(
	git3_str *buf,
	git3_http_client *client,
	git3_http_request *request)
{
...
static int puts_host_and_port(git3_str *buf, git3_net_url *url, bool force_port)
{
	bool ipv6 = git3_net_url_is_ipv6(url);
	...
	if (force_port || !git3_net_url_is_default_port(url)) {
		git3_str_putc(buf, ':');
...
static int append_user_agent(git3_str *buf)
{
	const char *product = git3_settings__user_agent_product();
	const char *comment = git3_settings__user_agent();

	GIT3_ASSERT(product && comment);

	if (!*product)
		return 0;

	git3_str_puts(buf, "User-Agent: ");
	git3_str_puts(buf, product);

	if (*comment) {
		git3_str_puts(buf, " (");
		git3_str_puts(buf, comment);
		git3_str_puts(buf, ")");
	}
	...
	git3_http_request *request)
{
	...
	git3_str_clear(&client->request_msg);
	buf = &client->request_msg;

	git3_str_puts(buf, "CONNECT ");
	puts_host_and_port(buf, &client->server.url, true);
	git3_str_puts(buf, " HTTP/1.1\r\n");

	append_user_agent(buf);

	git3_str_puts(buf, "Host: ");
	puts_host_and_port(buf, &client->server.url, true);
	git3_str_puts(buf, "\r\n");
...
static bool use_connect_proxy(git3_http_client *client)
{
	...
	git3_http_request *request)
{
	...
	GIT3_ASSERT_ARG(client);
	GIT3_ASSERT_ARG(request);

	git3_str_clear(&client->request_msg);
	buf = &client->request_msg;

	/* GET|POST path HTTP/1.1 */
	git3_str_puts(buf, name_for_method(request->method));
	git3_str_putc(buf, ' ');

	if (request->proxy && strcmp(request->url->scheme, "https"))
		git3_net_url_fmt(buf, request->url);
	else
		git3_net_url_fmt_path(buf, request->url);
	...
	if (request->accept)
		git3_str_printf(buf, "Accept: %s\r\n", request->accept);
	else
		git3_str_puts(buf, "Accept: */*\r\n");

	if (request->content_type)
		git3_str_printf(buf, "Content-Type: %s\r\n",
			request->content_type);

	if (request->chunked)
		git3_str_puts(buf, "Transfer-Encoding: chunked\r\n");

	if (request->content_length > 0)
		git3_str_printf(buf, "Content-Length: %"PRIuZ "\r\n",
			request->content_length);

	if (request->expect_continue)
		git3_str_printf(buf, "Expect: 100-continue\r\n");
...
static int check_certificate(
	...
	git3_net_url *url,
	int is_valid,
	git3_transport_certificate_check_cb cert_cb,
	void *cert_cb_payload)
{
	...
	if ((error = git3_stream_certificate(&cert, stream)) < 0)
		return error;
	...
	void *cb_payload)
{
	...
	GIT3_ERROR_CHECK_VERSION(server->stream, GIT3_STREAM_VERSION, "git3_stream");

	error = git3_stream_connect(server->stream);

	if (error && error != GIT3_ECERTIFICATE)
		return error;

	if (git3_stream_is_encrypted(server->stream) && cert_cb != NULL)
...
static void reset_auth_connection(git3_http_server *server)
{
	/*
 ...
 */
GIT3_INLINE(int) server_setup_from_url(
	git3_http_server *server,
	git3_net_url *url)
{
	GIT3_ASSERT_ARG(url);
	...
	if (!server->url.scheme || strcmp(server->url.scheme, url->scheme) ||
		...
		git3__free(server->url.scheme);
		git3__free(server->url.host);
		git3__free(server->url.port);

		server->url.scheme = git3__strdup(url->scheme);
		GIT3_ERROR_CHECK_ALLOC(server->url.scheme);

		server->url.host = git3__strdup(url->host);
		GIT3_ERROR_CHECK_ALLOC(server->url.host);

		server->url.port = git3__strdup(url->port);
		GIT3_ERROR_CHECK_ALLOC(server->url.port);
...
static git3_http_parser_settings parser_settings;

GIT3_INLINE(git3_http_parser_settings *) http_client_parser_settings(void)
{
...
static void reset_parser(git3_http_client *client)
{
	git3_http_parser_init(&client->parser,
	                     GIT3_HTTP_PARSER_RESPONSE,
...
GIT3_INLINE(int) server_create_stream(git3_http_server *server)
{
	git3_net_url *url = &server->url;

	if (strcasecmp(url->scheme, "https") == 0)
		return git3_tls_stream_new(&server->stream, url->host, url->port);
	else if (strcasecmp(url->scheme, "http") == 0)
		return git3_socket_stream_new(&server->stream, url->host, url->port);

	git3_error_set(GIT3_ERROR_HTTP, "unknown http scheme '%s'", url->scheme);
	return -1;
}

GIT3_INLINE(void) save_early_response(
	git3_http_client *client,
	git3_http_response *response)
{
	...
	memcpy(&client->early_response, response, sizeof(git3_http_response));
	memset(response, 0, sizeof(git3_http_response));
}
...
static int proxy_connect(
	...
	git3_http_request *request)
{
	git3_http_response response = {0};
	int error;

	if (!client->proxy_connected || !client->keepalive) {
		git3_trace(GIT3_TRACE_DEBUG, "Connecting to proxy %s port %s",
	...
	if ((error = git3_http_client_read_response(&response, client)) < 0 ||
	    (error = git3_http_client_skip_body(client)) < 0)
		goto done;

	GIT3_ASSERT(client->state == DONE);
	...
	if (response.status == GIT3_HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED) {
		...
		error = GIT3_RETRY;
		goto done;
	} else if (response.status != GIT3_HTTP_STATUS_OK) {
		git3_error_set(GIT3_ERROR_HTTP, "proxy returned unexpected status: %d", response.status);
		error = -1;
		goto done;
	}
	...
	git3_http_response_dispose(&response);
	return error;
}

static int server_connect(git3_http_client *client)
{
	git3_net_url *url = &client->server.url;
	git3_transport_certificate_check_cb cert_cb;
	void *cert_payload;
	int error;
	...
	if (client->proxy.stream)
		error = git3_tls_stream_wrap(&client->server.stream, client->proxy.stream, url->host);
	else
...
GIT3_INLINE(void) close_stream(git3_http_server *server)
{
	if (server->stream) {
		git3_stream_close(server->stream);
		git3_stream_free(server->stream);
		server->stream = NULL;
	}
}
	...
	git3_http_request *request)
{
	...
	git3_trace(GIT3_TRACE_DEBUG, "Connecting to remote %s port %s",
...
GIT3_INLINE(int) client_read(git3_http_client *client)
{
	...
	/*
	 * We use a git3_str for convenience, but statically allocate it and
	...
	if (max_len == 0) {
		git3_error_set(GIT3_ERROR_HTTP, "no room in output buffer");
		return -1;
	}

	read_len = git3_stream_read(stream, buf, max_len);

	if (read_len >= 0) {
		client->read_buf.size += read_len;

		git3_trace(GIT3_TRACE_TRACE, "Received:\n%.*s",
		          (int)read_len, buf);
	}
...
GIT3_INLINE(int) client_read_and_parse(git3_http_client *client)
{
	git3_http_parser *parser = &client->parser;
	http_parser_context *ctx = (http_parser_context *) parser->data;
	...
	parsed_len = git3_http_parser_execute(parser,
		client->read_buf.ptr,
		client->read_buf.size);
	http_errno = git3_http_parser_errno(parser);

	if (parsed_len > INT_MAX) {
		git3_error_set(GIT3_ERROR_HTTP, "unexpectedly large parse");
		return -1;
	}
	 ...
	 */
	if (http_errno == GIT3_HTTP_PARSER_PAUSED) {
		size_t additional_size;

		git3_http_parser_resume(parser);
		 ...
		 * that byte.
		 */
		if ((additional_size = git3_http_parser_remain_after_pause(parser)) > 0) {
			GIT3_ASSERT((client->read_buf.size - parsed_len) >= additional_size);
	...
	else if (git3_http_parser_errno(parser) != GIT3_HTTP_PARSER_OK) {
		git3_error_set(GIT3_ERROR_HTTP, "http parser error: %s",
		              git3_http_parser_errmsg(parser, http_errno));
		return -1;
	}
	...
	/* recv returned 0, the server hung up on us */
		...
		git3_error_set(GIT3_ERROR_HTTP, "unexpected EOF");
		return -1;
	}

	git3_str_consume_bytes(&client->read_buf, parsed_len);
 ...
 */
static void complete_response_body(git3_http_client *client)
{
	...
	if (client_read_and_parse(client) < 0 ||
	    parser_context.error != GIT3_HTTP_PARSER_OK ||
	...
	git3_str_clear(&client->read_buf);
}

int git3_http_client_send_request(
	git3_http_client *client,
	git3_http_request *request)
{
	git3_http_response response = {0};
	...
	if (git3_trace_level() >= GIT3_TRACE_DEBUG) {
		git3_str url = GIT3_STR_INIT;
		git3_net_url_fmt(&url, request->url);
		git3_trace(GIT3_TRACE_DEBUG, "Sending %s request to %s",
	...
	if (request->expect_continue) {
		...
		if (response.status != GIT3_HTTP_STATUS_CONTINUE) {
			save_early_response(client, &response);
			goto done;
		}
	}
...
bool git3_http_client_has_response(git3_http_client *client)
{
	return (client->state == HAS_EARLY_RESPONSE ||
	        client->state > SENT_REQUEST);
}

int git3_http_client_send_body(
	git3_http_client *client,
	const char *buffer,
	size_t buffer_len)
{
	git3_http_server *server;
	git3_str hdr = GIT3_STR_INIT;
	int error;
	...
	if (client->state != SENDING_BODY) {
		git3_error_set(GIT3_ERROR_HTTP, "client is in invalid state");
		return -1;
	}
	...
	if (client->request_body_len) {
		GIT3_ASSERT(buffer_len <= client->request_body_remain);
		...
		if ((error = git3_str_printf(&hdr, "%" PRIxZ "\r\n", buffer_len)) < 0 ||
	...
	git3_str_dispose(&hdr);
	return error;
}

static int complete_request(git3_http_client *client)
{
	int error = 0;

	GIT3_ASSERT_ARG(client);
	GIT3_ASSERT(client->state == SENDING_BODY);

	if (client->request_body_len && client->request_body_remain) {
		git3_error_set(GIT3_ERROR_HTTP, "truncated write");
		error = -1;
...
int git3_http_client_read_response(
	git3_http_response *response,
	git3_http_client *client)
{
	http_parser_context parser_context = {0};
	...
	if (client->state == HAS_EARLY_RESPONSE) {
		memcpy(response, &client->early_response, sizeof(git3_http_response));
		memset(&client->early_response, 0, sizeof(git3_http_response));
		client->state = DONE;
		return 0;
	}
	...
	if (client->current_server == PROXY) {
		git3_vector_dispose_deep(&client->proxy.auth_challenges);
	} else if(client->current_server == SERVER) {
		git3_vector_dispose_deep(&client->server.auth_challenges);
	}
	...
	GIT3_ASSERT(client->state == READING_BODY || client->state == DONE);

done:
	git3_str_dispose(&parser_context.parse_header_name);
	git3_str_dispose(&parser_context.parse_header_value);
	client->parser.data = NULL;

	return error;
}

int git3_http_client_read_body(
	git3_http_client *client,
	char *buffer,
	size_t buffer_size)
{
	...
	GIT3_ASSERT(parser_context.output_written <= INT_MAX);
...
int git3_http_client_skip_body(git3_http_client *client)
{
	http_parser_context parser_context = {0};
	int error;
	...
	do {
		...
		if (parser_context.error != GIT3_HTTP_PARSER_OK ||
		    (parser_context.parse_status != PARSE_STATUS_OK &&
		     parser_context.parse_status != PARSE_STATUS_NO_OUTPUT)) {
			git3_error_set(GIT3_ERROR_HTTP,
 ...
 * host.
 */
int git3_http_client_new(
	git3_http_client **out,
	git3_http_client_options *opts)
{
	git3_http_client *client;
	...
	client = git3__calloc(1, sizeof(git3_http_client));
	GIT3_ERROR_CHECK_ALLOC(client);

	git3_str_init(&client->read_buf, GIT3_READ_BUFFER_SIZE);
	GIT3_ERROR_CHECK_ALLOC(client->read_buf.ptr);

	if (opts)
		memcpy(&client->opts, opts, sizeof(git3_http_client_options));
...
void git3_http_client_set_options(
	git3_http_client *client,
	git3_http_client_options *opts)
{
	if (opts)
...
GIT3_INLINE(void) http_server_close(git3_http_server *server)
{
	if (server->stream) {
		git3_stream_close(server->stream);
		git3_stream_free(server->stream);
	...
	git3_net_url_dispose(&server->url);

	git3_vector_dispose_deep(&server->auth_challenges);
	free_auth_context(server);
}

static void http_client_close(git3_http_client *client)
{
	http_server_close(&client->server);
	http_server_close(&client->proxy);

	git3_str_dispose(&client->request_msg);
...
void git3_http_client_free(git3_http_client *client)
{
	if (!client)
		return;

	http_client_close(client);
	git3_str_dispose(&client->read_buf);
	git3__free(client);
}
