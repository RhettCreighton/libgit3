...
#define GIT3_HTTP_STATUS_CONTINUE                      100
#define GIT3_HTTP_STATUS_OK                            200
#define GIT3_HTTP_MOVED_PERMANENTLY                    301
#define GIT3_HTTP_FOUND                                302
#define GIT3_HTTP_SEE_OTHER                            303
#define GIT3_HTTP_TEMPORARY_REDIRECT                   307
#define GIT3_HTTP_PERMANENT_REDIRECT                   308
#define GIT3_HTTP_STATUS_UNAUTHORIZED                  401
#define GIT3_HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED 407

typedef struct git3_http_client git3_http_client;

/** Method for the HTTP request */
typedef enum {
	GIT3_HTTP_METHOD_GET,
	GIT3_HTTP_METHOD_POST,
	GIT3_HTTP_METHOD_CONNECT
} git3_http_method;

/** An HTTP request */
typedef struct {
	git3_http_method method;            /**< Method for the request */
	git3_net_url *url;                  /**< Full request URL */
	git3_net_url *proxy;                /**< Proxy to use */

	/* Headers */
	const char *accept;                /**< Contents of the Accept header */
	const char *content_type;          /**< Content-Type header (for POST) */
	git3_credential *credentials;       /**< Credentials to authenticate with */
	git3_credential *proxy_credentials; /**< Credentials for proxy */
	git3_strarray *custom_headers;      /**< Additional headers to deliver */
...
typedef struct {
	...
	git3_transport_certificate_check_cb server_certificate_check_cb;
	void *server_certificate_check_payload;

	/** Certificate check callback for the proxy */
	git3_transport_certificate_check_cb proxy_certificate_check_cb;
	void *proxy_certificate_check_payload;
} git3_http_client_options;
 ...
 */
extern int git3_http_client_new(
 ...
 */
extern void git3_http_client_set_options(
 ...
 * git3_http_client_send_body.
 *
 * @param client the client to write the request to
 * @param request the request to send
 */
extern int git3_http_client_send_request(
	git3_http_client *client,
 ...
 */
extern bool git3_http_client_has_response(git3_http_client *client);
 ...
 */
extern int git3_http_client_send_body(
	git3_http_client *client,
	const char *buffer,
	size_t buffer_len);
 ...
 * can be read by calling git3_http_client_read_body.  Callers must free
 * the response with git3_http_response_dispose.
 *
 * @param response pointer to the response object to fill
 * @param client the client to read the response from
 */
extern int git3_http_client_read_response(
 ...
 */
extern int git3_http_client_read_body(
	git3_http_client *client,
	char *buffer,
	size_t buffer_size);
 ...
 */
extern int git3_http_client_skip_body(git3_http_client *client);
 ...
 */
extern bool git3_http_response_is_redirect(git3_http_response *response);
 ...
 */
extern void git3_http_response_dispose(git3_http_response *response);
 ...
 */
extern void git3_http_client_free(git3_http_client *client);
