...
typedef struct {
	...
	git3_diff *diff;
	git3_checkout_options opts;
	bool opts_free_baseline;
	char *pfx;
	git3_index *index;
	git3_pool pool;
	git3_vector removes;
	git3_vector remove_conflicts;
	git3_vector update_conflicts;
	git3_vector *update_reuc;
	git3_vector *update_names;
	git3_str target_path;
	size_t target_len;
	git3_str tmp;
	...
	git3_checkout_perfdata perfdata;
	git3_hashset_str mkdir_pathcache;
...
static int checkout_notify(
	...
	const git3_index_entry *wditem)
{
	git3_diff_file wdfile;
	const git3_diff_file *baseline = NULL, *target = NULL, *workdir = NULL;
	...
	if (wditem) {
		...
		git3_oid_cpy(&wdfile.id, &wditem->id);
		wdfile.path = wditem->path;
		wdfile.size = wditem->file_size;
		wdfile.flags = GIT3_DIFF_FLAG_VALID_ID;
	...
	if (delta) {
		switch (delta->status) {
		case GIT3_DELTA_UNMODIFIED:
		case GIT3_DELTA_MODIFIED:
		case GIT3_DELTA_TYPECHANGE:
		default:
...

	{
		...
		return git3_error_set_after_callback_function(
			error, "git3_checkout notification");
	}
}

GIT3_INLINE(bool) is_workdir_base_or_new(
	const git3_oid *workdir_id,
	const git3_diff_file *baseitem,
	const git3_diff_file *newitem)
{
	return (git3_oid__cmp(&baseitem->id, workdir_id) == 0 ||
		git3_oid__cmp(&newitem->id, workdir_id) == 0);
}

GIT3_INLINE(bool) is_filemode_changed(git3_filemode_t a, git3_filemode_t b, int respect_filemode)
{
	/* If core.filemode = false, ignore links in the repository and executable bit changes */
	if (!respect_filemode) {
		if (a == S_IFLNK)
			a = GIT3_FILEMODE_BLOB;
		if (b == S_IFLNK)
			b = GIT3_FILEMODE_BLOB;
	...
	const git3_index_entry *wditem)
{
	...
	if (wditem->mode == GIT3_FILEMODE_COMMIT) {
		git3_submodule *sm;
		unsigned int sm_status = 0;
		const git3_oid *sm_oid = NULL;
		bool rval = false;

		if (git3_submodule_lookup(&sm, data->repo, wditem->path) < 0) {
			git3_error_clear();
			return true;
		}

		if (git3_submodule_status(&sm_status, data->repo, wditem->path, GIT3_SUBMODULE_IGNORE_UNSPECIFIED) < 0 ||
		    GIT3_SUBMODULE_STATUS_IS_WD_DIRTY(sm_status))
			rval = true;
		else if ((sm_oid = git3_submodule_wd_id(sm)) == NULL)
			rval = false;
		else
			rval = (git3_oid__cmp(&baseitem->id, sm_oid) != 0);
	 ...
	 */
	ie = git3_index_get_bypath(data->index, wditem->path, 0);

	if (ie != NULL &&
	    !git3_index_entry_newer_than_index(ie, data->index) &&
	    git3_index_time_eq(&wditem->mtime, &ie->mtime) &&
	...
	if (git3_diff__oid_for_entry(&oid, data->diff, wditem, wditem->mode, NULL) < 0)
		return false;
...
#define CHECKOUT_ACTION_IF(FLAG,YES,NO) \
	((data->strategy & GIT3_CHECKOUT_##FLAG) ? CHECKOUT_ACTION__##YES : CHECKOUT_ACTION__##NO)

static int checkout_action_common(
	int *action,
	checkout_data *data,
	const git3_diff_delta *delta,
	const git3_index_entry *wd)
{
	git3_checkout_notify_t notify = GIT3_CHECKOUT_NOTIFY_NONE;

	if ((data->strategy & GIT3_CHECKOUT_UPDATE_ONLY) != 0)
	...
	if ((*action & CHECKOUT_ACTION__UPDATE_BLOB) != 0) {
		...
		if (delta->new_file.mode == GIT3_FILEMODE_LINK && wd != NULL)
			*action |= CHECKOUT_ACTION__REMOVE;

		/* if the file is on disk and doesn't match our mode, force update */
		if (wd &&
		    GIT3_PERMS_IS_EXEC(wd->mode) != GIT3_PERMS_IS_EXEC(delta->new_file.mode))
			*action |= CHECKOUT_ACTION__REMOVE;

		notify = GIT3_CHECKOUT_NOTIFY_UPDATED;
	}

	if ((*action & CHECKOUT_ACTION__CONFLICT) != 0)
		notify = GIT3_CHECKOUT_NOTIFY_CONFLICT;
	...
	const git3_diff_delta *delta)
{
	...
	*action = CHECKOUT_ACTION__NONE;
	...
	if ((data->strategy & GIT3_CHECKOUT_NONE))
		return 0;

	switch (delta->status) {
	case GIT3_DELTA_UNMODIFIED: /* case 12 */
		error = checkout_notify(data, GIT3_CHECKOUT_NOTIFY_DIRTY, delta, NULL);
		if (error)
			return error;
		*action = CHECKOUT_ACTION_IF(RECREATE_MISSING, UPDATE_BLOB, NONE);
		break;
	case GIT3_DELTA_ADDED:    /* case 2 or 28 (and 5 but not really) */
		*action = CHECKOUT_ACTION__UPDATE_BLOB;
		break;
	case GIT3_DELTA_MODIFIED: /* case 13 (and 35 but not really) */
		*action = CHECKOUT_ACTION_IF(RECREATE_MISSING, UPDATE_BLOB, CONFLICT);
		break;
	case GIT3_DELTA_TYPECHANGE: /* case 21 (B->T) and 28 (T->B)*/
		if (delta->new_file.mode == GIT3_FILEMODE_TREE)
			*action = CHECKOUT_ACTION__UPDATE_BLOB;
		break;
	case GIT3_DELTA_DELETED: /* case 8 or 25 */
		*action = CHECKOUT_ACTION__REMOVE;
		break;
	default: /* impossible */
		break;
...
static int checkout_target_fullpath(
	git3_str **out, checkout_data *data, const char *path)
{
	git3_str_truncate(&data->target_path, data->target_len);

	if (path && git3_str_puts(&data->target_path, path) < 0)
		return -1;

	if (git3_path_validate_str_length(data->repo, &data->target_path) < 0)
		return -1;
...
static bool wd_item_is_removable(
	checkout_data *data, const git3_index_entry *wd)
{
	git3_str *full;

	if (wd->mode != GIT3_FILEMODE_TREE)
		return true;

	if (checkout_target_fullpath(&full, data, wd->path) < 0)
		return false;

	return !full || !git3_fs_path_contains(full, DOT_GIT);
}

static int checkout_queue_remove(checkout_data *data, const char *path)
{
	char *copy = git3_pool_strdup(&data->pool, path);
	GIT3_ERROR_CHECK_ALLOC(copy);
	return git3_vector_insert(&data->removes, copy);
}

/* note that this advances the iterator over the wd item */
static int checkout_action_wd_only(
	checkout_data *data,
	git3_iterator *workdir,
	const git3_index_entry **wditem,
	git3_vector *pathspec)
{
	...
	bool remove = false;
	git3_checkout_notify_t notify = GIT3_CHECKOUT_NOTIFY_NONE;
	const git3_index_entry *wd = *wditem;

	if (!git3_pathspec__match(
			pathspec, wd->path,
			(data->strategy & GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH) != 0,
			git3_iterator_ignore_case(workdir), NULL, NULL)) {

		if (wd->mode == GIT3_FILEMODE_TREE)
			return git3_iterator_advance_into(wditem, workdir);
		else
			return git3_iterator_advance(wditem, workdir);
	}
	...
	if (data->index != NULL) {
		...
		error = git3_index__find_pos(
			&pos, data->index, wd->path, 0, GIT3_INDEX_STAGE_ANY);

		if (wd->mode != GIT3_FILEMODE_TREE) {
			if (!error) { /* found by git3_index__find_pos call */
				notify = GIT3_CHECKOUT_NOTIFY_DIRTY;
				remove = ((data->strategy & GIT3_CHECKOUT_FORCE) != 0);
			} else if (error != GIT3_ENOTFOUND)
				return error;
			else
				error = 0; /* git3_index__find_pos does not set error msg */
		} else {
			/* for tree entries, we have to see if there are any index
			 * entries that are contained inside that tree
			 */
			const git3_index_entry *e = git3_index_get_byindex(data->index, pos);
	...
	if (notify != GIT3_CHECKOUT_NOTIFY_NONE) {
		...
		if (untracked_state == GIT3_ITERATOR_STATUS_IGNORED) {
			notify = GIT3_CHECKOUT_NOTIFY_IGNORED;
			remove = ((data->strategy & GIT3_CHECKOUT_REMOVE_IGNORED) != 0);
		} else {
			notify = GIT3_CHECKOUT_NOTIFY_UNTRACKED;
			remove = ((data->strategy & GIT3_CHECKOUT_REMOVE_UNTRACKED) != 0);
		}
	...
	const char *path)
{
	...
	if (git3_submodule_lookup(&sm, data->repo, path) < 0)
		return true;

	if (git3_submodule_location(&sm_loc, sm) < 0 ||
		sm_loc == GIT3_SUBMODULE_STATUS_IN_CONFIG)
		rval = true;
...
static bool checkout_is_empty_dir(checkout_data *data, const char *path)
{
	...
	return git3_fs_path_is_empty_dir(fullpath->ptr);
}
	...
	const git3_index_entry *wd)
{
	*action = CHECKOUT_ACTION__NONE;
	...
	switch (delta->status) {
	case GIT3_DELTA_UNMODIFIED: /* case 14/15 or 33 */
		if (checkout_is_workdir_modified(data, &delta->old_file, &delta->new_file, wd)) {
			GIT3_ERROR_CHECK_ERROR(
				checkout_notify(data, GIT3_CHECKOUT_NOTIFY_DIRTY, delta, wd) );
			*action = CHECKOUT_ACTION_IF(FORCE, UPDATE_BLOB, NONE);
		}
		break;
	case GIT3_DELTA_ADDED: /* case 3, 4 or 6 */
		if (git3_iterator_current_is_ignored(workdir))
			*action = CHECKOUT_ACTION_IF(DONT_OVERWRITE_IGNORED, CONFLICT, UPDATE_BLOB);
		else
			*action = CHECKOUT_ACTION_IF(FORCE, UPDATE_BLOB, CONFLICT);
		break;
	case GIT3_DELTA_DELETED: /* case 9 or 10 (or 26 but not really) */
		if (checkout_is_workdir_modified(data, &delta->old_file, &delta->new_file, wd))
			*action = CHECKOUT_ACTION_IF(FORCE, REMOVE, CONFLICT);
		else
			*action = CHECKOUT_ACTION__REMOVE;
		break;
	case GIT3_DELTA_MODIFIED: /* case 16, 17, 18 (or 36 but not really) */
		if (wd->mode != GIT3_FILEMODE_COMMIT &&
			checkout_is_workdir_modified(data, &delta->old_file, &delta->new_file, wd))
			*action = CHECKOUT_ACTION_IF(FORCE, UPDATE_BLOB, CONFLICT);
		else
			*action = CHECKOUT_ACTION__UPDATE_BLOB;
		break;
	case GIT3_DELTA_TYPECHANGE: /* case 22, 23, 29, 30 */
		if (delta->old_file.mode == GIT3_FILEMODE_TREE) {
			if (wd->mode == GIT3_FILEMODE_TREE)
				/* either deleting items in old tree will delete the wd dir,
				 * or we'll get a conflict when we attempt blob update...
				 */
				*action = CHECKOUT_ACTION__UPDATE_BLOB;
			else if (wd->mode == GIT3_FILEMODE_COMMIT) {
	...
	const git3_index_entry *wd)
{
	*action = CHECKOUT_ACTION__NONE;
	...
	switch (delta->status) {
	case GIT3_DELTA_UNMODIFIED: /* case 19 or 24 (or 34 but not really) */
		GIT3_ERROR_CHECK_ERROR(
			checkout_notify(data, GIT3_CHECKOUT_NOTIFY_DIRTY, delta, NULL));
		GIT3_ERROR_CHECK_ERROR(
			checkout_notify(data, GIT3_CHECKOUT_NOTIFY_UNTRACKED, NULL, wd));
		*action = CHECKOUT_ACTION_IF(FORCE, REMOVE_AND_UPDATE, NONE);
		break;
	case GIT3_DELTA_ADDED:/* case 4 (and 7 for dir) */
	case GIT3_DELTA_MODIFIED: /* case 20 (or 37 but not really) */
		if (delta->old_file.mode == GIT3_FILEMODE_COMMIT)
			/* expected submodule (and maybe found one) */;
		else if (delta->new_file.mode != GIT3_FILEMODE_TREE)
			*action = git3_iterator_current_is_ignored(workdir) ?
				CHECKOUT_ACTION_IF(DONT_OVERWRITE_IGNORED, CONFLICT, REMOVE_AND_UPDATE) :
				CHECKOUT_ACTION_IF(FORCE, REMOVE_AND_UPDATE, CONFLICT);
		break;
	case GIT3_DELTA_DELETED: /* case 11 (and 27 for dir) */
		if (delta->old_file.mode != GIT3_FILEMODE_TREE)
			GIT3_ERROR_CHECK_ERROR(
				checkout_notify(data, GIT3_CHECKOUT_NOTIFY_UNTRACKED, NULL, wd));
		break;
	case GIT3_DELTA_TYPECHANGE: /* case 24 or 31 */
		if (delta->old_file.mode == GIT3_FILEMODE_TREE) {
	...
	git3_vector *pathspec)
{
	...
	int (*advance)(const git3_index_entry **, git3_iterator *) = NULL;
	...
	while (1) {
		...
		if (cmp < 0) {
			...
			if (cmp == 0) {
				if (wd->mode == GIT3_FILEMODE_TREE) {
					...
					error = git3_iterator_advance_into(wditem, workdir);
		...
		if (cmp == 0) { /* case 5 */
			...
			if (delta->status == GIT3_DELTA_TYPECHANGE) {
				if (delta->old_file.mode == GIT3_FILEMODE_TREE) {
					error = checkout_action_with_wd(action, data, delta, workdir, wd);
					advance = git3_iterator_advance_into;
					goto done;
				}

				if (delta->new_file.mode == GIT3_FILEMODE_TREE ||
					delta->new_file.mode == GIT3_FILEMODE_COMMIT ||
					delta->old_file.mode == GIT3_FILEMODE_COMMIT)
				{
					error = checkout_action_with_wd(action, data, delta, workdir, wd);
					advance = git3_iterator_advance;
					goto done;
...
GIT3_INLINE(int) checkout_idxentry_cmp(
	const git3_index_entry *a,
	const git3_index_entry *b)
{
	if (!a && !b)
		return 0;
	else if (!a && b)
...
static int checkout_conflictdata_empty(
	const git3_vector *conflicts, size_t idx, void *payload)
{
	checkout_conflictdata *conflict;

	GIT3_UNUSED(payload);

	if ((conflict = git3_vector_get(conflicts, idx)) == NULL)
		return -1;
...
GIT3_INLINE(bool) conflict_pathspec_match(
	checkout_data *data,
	git3_iterator *workdir,
	git3_vector *pathspec,
	const git3_index_entry *ancestor,
	...
	const git3_index_entry *theirs)
{
	...
	if (ours && git3_pathspec__match(pathspec, ours->path,
		(data->strategy & GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH) != 0,
		git3_iterator_ignore_case(workdir), NULL, NULL))
		return true;

	if (theirs && git3_pathspec__match(pathspec, theirs->path,
		(data->strategy & GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH) != 0,
		git3_iterator_ignore_case(workdir), NULL, NULL))
		return true;

	if (ancestor && git3_pathspec__match(pathspec, ancestor->path,
		(data->strategy & GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH) != 0,
		git3_iterator_ignore_case(workdir), NULL, NULL))
		return true;
...
GIT3_INLINE(int) checkout_conflict_detect_submodule(checkout_conflictdata *conflict)
{
...
GIT3_INLINE(int) checkout_conflict_detect_binary(git3_repository *repo, checkout_conflictdata *conflict)
{
	git3_blob *ancestor_blob = NULL, *our_blob = NULL, *their_blob = NULL;
	int error = 0;
	...
	if (conflict->ancestor) {
		if ((error = git3_blob_lookup(&ancestor_blob, repo, &conflict->ancestor->id)) < 0)
			goto done;

		conflict->binary = git3_blob_is_binary(ancestor_blob);
	}

	if (!conflict->binary && conflict->ours) {
		if ((error = git3_blob_lookup(&our_blob, repo, &conflict->ours->id)) < 0)
			goto done;

		conflict->binary = git3_blob_is_binary(our_blob);
	}

	if (!conflict->binary && conflict->theirs) {
		if ((error = git3_blob_lookup(&their_blob, repo, &conflict->theirs->id)) < 0)
			goto done;

		conflict->binary = git3_blob_is_binary(their_blob);
	}

done:
	git3_blob_free(ancestor_blob);
	git3_blob_free(our_blob);
	git3_blob_free(their_blob);
	...
	void *payload)
{
	...
	conflict = git3__calloc(1, sizeof(checkout_conflictdata));
	GIT3_ERROR_CHECK_ALLOC(conflict);
	...
	if (git3_vector_insert(&data->update_conflicts, conflict))
		return -1;
...
static int checkout_conflicts_foreach(
	...
	git3_iterator *workdir,
	git3_vector *pathspec,
	int (*cb)(const git3_index_entry *, const git3_index_entry *, const git3_index_entry *, void *),
	void *payload)
{
	git3_index_conflict_iterator *iterator = NULL;
	const git3_index_entry *ancestor, *ours, *theirs;
	int error = 0;

	if ((error = git3_index_conflict_iterator_new(&iterator, index)) < 0)
		goto done;

	/* Collect the conflicts */
	while ((error = git3_index_conflict_next(&ancestor, &ours, &theirs, iterator)) == 0) {
	...
	git3_index_conflict_iterator_free(iterator);

	return error;
}

static int checkout_conflicts_load(checkout_data *data, git3_iterator *workdir, git3_vector *pathspec)
{
	git3_index *index;

	/* Only write conflicts from sources that have them: indexes. */
	if ((index = git3_iterator_index(data->target)) == NULL)
		return 0;
...
GIT3_INLINE(int) checkout_conflicts_cmp_entry(
	const char *path,
	...
	const char *path)
{
	...
	if (git3_vector_bsearch2(&pos, &data->update_conflicts, checkout_conflicts_cmp_ancestor, path) < 0)
		return NULL;

	return git3_vector_get(&data->update_conflicts, pos);
}
	...
	const char *path)
{
	...
	git3_vector_foreach(&data->update_conflicts, i, conflict) {
		int cmp = -1;
...
static int checkout_conflicts_load_byname_entry(
	...
	checkout_data *data,
	const git3_index_name_entry *name_entry)
{
	...
	*theirs_out = NULL;
	...
	if (!name_entry->ancestor) {
		git3_error_set(GIT3_ERROR_INDEX, "a NAME entry exists without an ancestor");
		error = -1;
		goto done;
	}

	if (!name_entry->ours && !name_entry->theirs) {
		git3_error_set(GIT3_ERROR_INDEX, "a NAME entry exists without an ours or theirs");
		error = -1;
		goto done;
	}
	...
	checkout_data *data)
{
	...
	names = git3_index_name_entrycount(index);

	for (i = 0; i < names; i++) {
		name_entry = git3_index_name_get_byindex(index, i);
	...
	checkout_data *data)
{
	...
	len = git3_index_entrycount(index);
	...
	git3_vector_foreach(&data->update_conflicts, i, conflict) {
		...
		if ((error = git3_index_find(&j, index, path)) < 0) {
		...
		for (; j < len; j++) {
			if ((entry = git3_index_get_byindex(index, j)) == NULL) {
				git3_error_set(GIT3_ERROR_INDEX,
			...
			prefixed = git3_fs_path_equal_or_prefixed(path, entry->path, NULL);

			if (prefixed == GIT3_FS_PATH_EQUAL)
				continue;

			if (prefixed == GIT3_FS_PATH_PREFIX)
	...
	git3_vector *pathspec)
{
	...
	if (data->strategy & GIT3_CHECKOUT_SKIP_UNMERGED)
		return 0;
	...
	void *payload)
{
	...
	GIT3_ASSERT_ARG(ancestor || ours || theirs);

	if (ancestor)
		name = git3__strdup(ancestor->path);
	else if (ours)
		name = git3__strdup(ours->path);
	else if (theirs)
		name = git3__strdup(theirs->path);
	else
		abort();

	GIT3_ERROR_CHECK_ALLOC(name);

	return git3_vector_insert(&data->remove_conflicts, (char *)name);
}
...
static int checkout_get_remove_conflicts(
	...
	git3_vector *pathspec)
{
	if ((data->strategy & GIT3_CHECKOUT_DONT_UPDATE_INDEX) != 0)
		return 0;
...
static int checkout_verify_paths(
	...
	git3_diff_delta *delta)
{
	unsigned int flags = GIT3_PATH_REJECT_WORKDIR_DEFAULTS;

	if (action & CHECKOUT_ACTION__REMOVE) {
		if (!git3_path_is_valid(repo, delta->old_file.path, delta->old_file.mode, flags)) {
			git3_error_set(GIT3_ERROR_CHECKOUT, "cannot remove invalid path '%s'", delta->old_file.path);
			return -1;
		}
	}

	if (action & ~CHECKOUT_ACTION__REMOVE) {
		if (!git3_path_is_valid(repo, delta->new_file.path, delta->new_file.mode, flags)) {
			git3_error_set(GIT3_ERROR_CHECKOUT, "cannot checkout to invalid path '%s'", delta->new_file.path);
			return -1;
		}
	}
	...
	git3_iterator *workdir)
{
	int error = 0, act;
	const git3_index_entry *wditem;
	git3_vector pathspec = GIT3_VECTOR_INIT, *deltas;
	git3_pool pathpool;
	git3_diff_delta *delta;
	size_t i, *counts = NULL;
	uint32_t *actions = NULL;

	if (git3_pool_init(&pathpool, 1) < 0)
		return -1;

	if (data->opts.paths.count > 0 &&
	    git3_pathspec__vinit(&pathspec, &data->opts.paths, &pathpool) < 0)
		return -1;

	if ((error = git3_iterator_current(&wditem, workdir)) < 0 &&
	    error != GIT3_ITEROVER)
		goto fail;

	deltas = &data->diff->deltas;

	*counts_ptr = counts = git3__calloc(CHECKOUT_ACTION__MAX+1, sizeof(size_t));
	*actions_ptr = actions = git3__calloc(
	...
	git3_vector_foreach(deltas, i, delta) {
	...
	if (counts[CHECKOUT_ACTION__CONFLICT] > 0 &&
	    (data->strategy & GIT3_CHECKOUT_ALLOW_CONFLICTS) == 0) {
		git3_error_set(GIT3_ERROR_CHECKOUT, "%"PRIuZ" %s checkout",
			counts[CHECKOUT_ACTION__CONFLICT],
	...
	counts[CHECKOUT_ACTION__REMOVE_CONFLICT] = git3_vector_length(&data->remove_conflicts);
	counts[CHECKOUT_ACTION__UPDATE_CONFLICT] = git3_vector_length(&data->update_conflicts);

	git3_pathspec__vfree(&pathspec);
	git3_pool_clear(&pathpool);
...
static bool should_remove_existing(checkout_data *data)
{
	...
	if (git3_repository__configmap_lookup(&ignorecase, data->repo, GIT3_CONFIGMAP_IGNORECASE) < 0) {
		ignorecase = 0;
	}

	return (ignorecase &&
		(data->strategy & GIT3_CHECKOUT_DONT_REMOVE_EXISTING) == 0);
}

#define MKDIR_NORMAL \
	GIT3_MKDIR_PATH | GIT3_MKDIR_VERIFY_DIR
#define MKDIR_REMOVE_EXISTING \
	MKDIR_NORMAL | GIT3_MKDIR_REMOVE_FILES | GIT3_MKDIR_REMOVE_SYMLINKS
...
static int checkout_mkdir(
	...
	unsigned int flags)
{
	struct git3_futils_mkdir_options mkdir_opts = {0};
	int error;

	if (git3_pool_is_initialized(&data->pool)) {
	...
	checkout_data *data, const char *path, unsigned int mode)
{
	...
	if (remove_existing) {
		...
		if (p_lstat(path, &st) == 0) {
			 ...
			 */
			error = git3_futils_rmdir_r(path, NULL, GIT3_RMDIR_REMOVE_FILES);
		} else if (errno != ENOENT) {
			git3_error_set(GIT3_ERROR_OS, "failed to stat '%s'", path);
			return GIT3_EEXISTS;
		} else {
			git3_error_clear();
		}
...
static int checkout_stream_write(
	git3_writestream *s, const char *buffer, size_t len)
{
	struct checkout_stream *stream = (struct checkout_stream *)s;
	int ret;

	if ((ret = p_write(stream->fd, buffer, len)) < 0)
		git3_error_set(GIT3_ERROR_OS, "could not write to '%s'", stream->path);

	return ret;
}

static int checkout_stream_close(git3_writestream *s)
{
	struct checkout_stream *stream = (struct checkout_stream *)s;

	GIT3_ASSERT_ARG(stream);
	GIT3_ASSERT_ARG(stream->open);

	stream->open = 0;
	return p_close(stream->fd);
}

static void checkout_stream_free(git3_writestream *s)
{
	GIT3_UNUSED(s);
}
	...
	mode_t entry_filemode)
{
	...
	git3_filter_session filter_session = GIT3_FILTER_SESSION_INIT;
	...
	if (!(mode = file_mode))
		mode = GIT3_FILEMODE_BLOB;

	if ((fd = p_open(path, flags, mode)) < 0) {
		git3_error_set(GIT3_ERROR_OS, "could not open '%s' for writing", path);
		return fd;
	}
	...
	if (!data->opts.disable_filters &&
		(error = git3_filter_list__load(
			&fl, data->repo, blob, hint_path,
			GIT3_FILTER_TO_WORKTREE, &filter_session))) {
		p_close(fd);
		return error;
	}
	...
	error = git3_filter_list_stream_blob(fl, blob, &writer.base);

	GIT3_ASSERT(writer.open == 0);
...
static int blob_content_to_link(
	...
	const char *path)
{
	git3_str linktarget = GIT3_STR_INIT;
	int error;

	if ((error = mkpath2file(data, path, data->opts.dir_mode)) < 0)
		return error;

	if ((error = git3_blob__getbuf(&linktarget, blob)) < 0)
		return error;

	if (data->can_symlink) {
		if ((error = p_symlink(git3_str_cstr(&linktarget), path)) < 0)
			git3_error_set(GIT3_ERROR_OS, "could not create symlink %s", path);
	} else {
		error = git3_futils_fake_symlink(git3_str_cstr(&linktarget), path);
	}
	...
	if (!error) {
		...
		if ((error = p_lstat(path, st)) < 0)
			git3_error_set(GIT3_ERROR_CHECKOUT, "could not stat symlink %s", path);

		st->st_mode = GIT3_FILEMODE_LINK;
	}

	git3_str_dispose(&linktarget);
	...
	struct stat *st)
{
	...
	git3_index_entry__init_from_stat(&entry, st, true);
	git3_oid_cpy(&entry.id, &file->id);

	return git3_index_add(data->index, &entry);
}
	...
	const git3_diff_file *file)
{
	...
	if (p_stat(fullpath->ptr, &st) < 0) {
		git3_error_set(
			GIT3_ERROR_CHECKOUT, "could not stat submodule %s\n", file->path);
		return GIT3_ENOTFOUND;
	}

	st.st_mode = GIT3_FILEMODE_COMMIT;
	...
	const git3_diff_file *file)
{
	...
	if ((error = git3_submodule_lookup(NULL, data->repo, file->path)) < 0) {
	...
	struct stat *st)
{
	...
	if ((error = git3_blob_lookup(&blob, data->repo, oid)) < 0)
		return error;
	 ...
	 */
	if ((data->strategy & GIT3_CHECKOUT_ALLOW_CONFLICTS) != 0 &&
		(error == GIT3_ENOTFOUND || error == GIT3_EEXISTS))
	{
		git3_error_clear();
		error = 0;
	}
	...
	const git3_diff_file *file)
{
	...
	/* update the index unless prevented */
	if (!error && (data->strategy & GIT3_CHECKOUT_DONT_UPDATE_INDEX) == 0)
	...
	checkout_data *data)
{
	...
	size_t i;
	git3_str *fullpath;
	uint32_t flg = GIT3_RMDIR_EMPTY_PARENTS |
		GIT3_RMDIR_REMOVE_FILES | GIT3_RMDIR_REMOVE_BLOCKERS;

	if (data->opts.checkout_strategy & GIT3_CHECKOUT_SKIP_LOCKED_DIRECTORIES)
		flg |= GIT3_RMDIR_SKIP_NONEMPTY;

	if (checkout_target_fullpath(&fullpath, data, NULL) < 0)
		return -1;

	git3_vector_foreach(&data->diff->deltas, i, delta) {
		if (actions[i] & CHECKOUT_ACTION__REMOVE) {
			error = git3_futils_rmdir_r(
			...
			if ((actions[i] & CHECKOUT_ACTION__UPDATE_BLOB) == 0 &&
				(data->strategy & GIT3_CHECKOUT_DONT_UPDATE_INDEX) == 0 &&
				data->index != NULL)
			{
				(void)git3_index_remove(data->index, delta->old_file.path, 0);
			}
		}
	}

	git3_vector_foreach(&data->removes, i, str) {
		error = git3_futils_rmdir_r(str, fullpath->ptr, flg);
		if (error < 0)
			return error;
		...
		{
			if (str[strlen(str) - 1] == '/')
				(void)git3_index_remove_directory(data->index, str, 0);
			else
				(void)git3_index_remove(data->index, str, 0);
		}
	}
...
static int checkout_lookup_head_tree(git3_tree **out, git3_repository *repo)
{
	int error = 0;
	git3_reference *ref = NULL;
	git3_object *head;

	if (!(error = git3_repository_head(&ref, repo)) &&
		!(error = git3_reference_peel(&head, ref, GIT3_OBJECT_TREE)))
		*out = (git3_tree *)head;
...
static int conflict_entry_name(
	...
	const char *filename)
{
	if (git3_str_puts(out, side_name) < 0 ||
		git3_str_putc(out, ':') < 0 ||
		git3_str_puts(out, filename) < 0)
		return -1;

	return 0;
}

static int checkout_path_suffixed(git3_str *path, const char *suffix)
{
	size_t path_len;
	int i = 0, error = 0;

	if ((error = git3_str_putc(path, '~')) < 0 || (error = git3_str_puts(path, suffix)) < 0)
		return -1;

	path_len = git3_str_len(path);

	while (git3_fs_path_exists(git3_str_cstr(path)) && i < INT_MAX) {
		git3_str_truncate(path, path_len);

		if ((error = git3_str_putc(path, '_')) < 0 ||
			(error = git3_str_printf(path, "%d", i)) < 0)
			return error;
	...
	if (i == INT_MAX) {
		...
		git3_error_set(GIT3_ERROR_CHECKOUT, "could not write '%s': working directory file exists", path->ptr);
		return GIT3_EEXISTS;
	}
	...
	const git3_index_entry *side)
{
	...
	GIT3_ASSERT(side == conflict->ours || side == conflict->theirs);

	if (checkout_target_fullpath(&fullpath, data, side->path) < 0)
		return -1;

	if ((conflict->name_collision || conflict->directoryfile) &&
		(data->strategy & GIT3_CHECKOUT_USE_OURS) == 0 &&
		(data->strategy & GIT3_CHECKOUT_USE_THEIRS) == 0) {
...
static int checkout_merge_path(
	...
	checkout_data *data,
	checkout_conflictdata *conflict,
	git3_merge_file_result *result)
{
	const char *our_label_raw, *their_label_raw, *suffix;
	int error = 0;

	if ((error = git3_str_joinpath(out, data->opts.target_directory, result->path)) < 0 ||
	    (error = git3_path_validate_str_length(data->repo, out)) < 0)
		return error;
...
static int checkout_write_merge(
	...
	checkout_conflictdata *conflict)
{
	git3_str our_label = GIT3_STR_INIT, their_label = GIT3_STR_INIT,
		path_suffixed = GIT3_STR_INIT, path_workdir = GIT3_STR_INIT,
		in_data = GIT3_STR_INIT, out_data = GIT3_STR_INIT;
	...
	if (data->opts.checkout_strategy & GIT3_CHECKOUT_CONFLICT_STYLE_DIFF3)
		opts.flags |= GIT3_MERGE_FILE_STYLE_DIFF3;

	if (data->opts.checkout_strategy & GIT3_CHECKOUT_CONFLICT_STYLE_ZDIFF3)
	...
	if (conflict->ours && conflict->theirs &&
		...
		opts.our_label = git3_str_cstr(&our_label);
		opts.their_label = git3_str_cstr(&their_label);
	}

	if ((error = git3_merge_file_from_index(&result, data->repo,
		conflict->ancestor, conflict->ours, conflict->theirs, &opts)) < 0)
		goto done;

	if (result.path == NULL || result.mode == 0) {
		git3_error_set(GIT3_ERROR_CHECKOUT, "could not merge contents of file");
		error = GIT3_ECONFLICT;
		goto done;
	}
	...
	if ((data->strategy & GIT3_CHECKOUT_UPDATE_ONLY) != 0 &&
		(error = checkout_safe_for_update_only(data, git3_str_cstr(&path_workdir), result.mode)) <= 0)
		goto done;
	...
	if (!data->opts.disable_filters) {
		...
		if ((error = git3_filter_list__load(
				&fl, data->repo, NULL, result.path,
				GIT3_FILTER_TO_WORKTREE, &filter_session)) < 0 ||
			(error = git3_filter_list__convert_buf(&out_data, fl, &in_data)) < 0)
			goto done;
	} else {
	...
	if ((error = mkpath2file(data, path_workdir.ptr, data->opts.dir_mode)) < 0 ||
		(error = git3_filebuf_open(&output, git3_str_cstr(&path_workdir), GIT3_FILEBUF_DO_NOT_BUFFER, result.mode)) < 0 ||
		(error = git3_filebuf_write(&output, out_data.ptr, out_data.size)) < 0 ||
		(error = git3_filebuf_commit(&output)) < 0)
		goto done;

done:
	git3_filter_list_free(fl);

	git3_str_dispose(&out_data);
	git3_str_dispose(&our_label);
	git3_str_dispose(&their_label);

	git3_merge_file_result_free(&result);
	git3_str_dispose(&path_workdir);
	git3_str_dispose(&path_suffixed);
...
static int checkout_conflict_add(
	checkout_data *data,
	const git3_index_entry *conflict)
{
	int error = git3_index_remove(data->index, conflict->path, 0);
	...
	return git3_index_add(data->index, conflict);
}
...
static int checkout_create_conflicts(checkout_data *data)
{
	...
	git3_vector_foreach(&data->update_conflicts, i, conflict) {
		...
		else if ((data->strategy & GIT3_CHECKOUT_USE_OURS) &&
			conflict->ours)
			error = checkout_write_entry(data, conflict, conflict->ours);
		else if ((data->strategy & GIT3_CHECKOUT_USE_THEIRS) &&
			conflict->theirs)
			error = checkout_write_entry(data, conflict, conflict->theirs);
...
static int checkout_remove_conflicts(checkout_data *data)
{
	...
	git3_vector_foreach(&data->remove_conflicts, i, conflict) {
		if (git3_index_conflict_remove(data->index, conflict) < 0)
			return -1;
...
{
	const git3_index_reuc_entry *reuc_entry;
	const git3_index_name_entry *name_entry;
	size_t i;
	int error = 0;
	...
	if (data->update_reuc) {
		git3_vector_foreach(data->update_reuc, i, reuc_entry) {
			if ((error = git3_index_reuc_add(data->index, reuc_entry->path,
	...
	if (data->update_names) {
		git3_vector_foreach(data->update_names, i, name_entry) {
			if ((error = git3_index_name_add(data->index, name_entry->ancestor,
...
static void checkout_data_clear(checkout_data *data)
{
	if (data->opts_free_baseline) {
		git3_tree_free(data->opts.baseline);
		data->opts.baseline = NULL;
	}

	git3_vector_dispose(&data->removes);
	git3_pool_clear(&data->pool);

	git3_vector_dispose_deep(&data->remove_conflicts);
	git3_vector_dispose_deep(&data->update_conflicts);

	git3__free(data->pfx);
	data->pfx = NULL;

	git3_str_dispose(&data->target_path);
	git3_str_dispose(&data->tmp);

	git3_index_free(data->index);
	data->index = NULL;

	git3_hashset_str_dispose(&data->mkdir_pathcache);

	git3_attr_session__free(&data->attr_session);
}
...
static int validate_target_directory(checkout_data *data)
{
	...
	if ((error = git3_path_validate_length(data->repo, data->opts.target_directory)) < 0)
		return error;

	if (git3_fs_path_isdir(data->opts.target_directory))
		return 0;

	error = checkout_mkdir(data, data->opts.target_directory, NULL,
	                       GIT3_DIR_MODE, GIT3_MKDIR_VERIFY_DIR);
...
static int checkout_data_init(
	checkout_data *data,
	git3_iterator *target,
	const git3_checkout_options *proposed)
{
	int error = 0;
	git3_repository *repo = git3_iterator_owner(target);

	memset(data, 0, sizeof(*data));

	if (!repo) {
		git3_error_set(GIT3_ERROR_CHECKOUT, "cannot checkout nothing");
		return -1;
	}

	if ((!proposed || !proposed->target_directory) &&
		(error = git3_repository__ensure_not_bare(repo, "checkout")) < 0)
		return error;
	...
	GIT3_ERROR_CHECK_VERSION(
		proposed, GIT3_CHECKOUT_OPTIONS_VERSION, "git3_checkout_options");

	if (!proposed)
		GIT3_INIT_STRUCTURE(&data->opts, GIT3_CHECKOUT_OPTIONS_VERSION);
	else
		memmove(&data->opts, proposed, sizeof(git3_checkout_options));

	if (!data->opts.target_directory)
		data->opts.target_directory = git3_repository_workdir(repo);
	else if ((error = validate_target_directory(data)) < 0)
		goto cleanup;

	if ((error = git3_repository_index(&data->index, data->repo)) < 0)
		goto cleanup;

	/* refresh config and index content unless NO_REFRESH is given */
	if ((data->opts.checkout_strategy & GIT3_CHECKOUT_NO_REFRESH) == 0) {
		git3_config *cfg;
		 ...
		 */
		if (data->index != git3_iterator_index(target)) {
			if (data->opts.checkout_strategy & GIT3_CHECKOUT_FORCE) {
				/* When forcing, we can blindly re-read the index */
				if ((error = git3_index_read(data->index, false)) < 0)
					goto cleanup;
			} else {
				/*
				 ...
				 * proceeding.
				 */
				if (git3_index_has_conflicts(data->index)) {
					error = GIT3_ECONFLICT;
					git3_error_set(GIT3_ERROR_CHECKOUT,
						"unresolved conflicts exist in the index");
					goto cleanup;
				}

				if ((error = git3_index_read_safely(data->index)) < 0)
					goto cleanup;
			}

			/* clean conflict data in the current index */
			git3_index_name_clear(data->index);
			git3_index_reuc_clear(data->index);
		}
	}

	/* if you are forcing, allow all safe updates, plus recreate missing */
	if ((data->opts.checkout_strategy & GIT3_CHECKOUT_FORCE) != 0)
		data->opts.checkout_strategy |= GIT3_CHECKOUT_RECREATE_MISSING;
	...
	if (!data->opts.dir_mode)
		data->opts.dir_mode = GIT3_DIR_MODE;

	if (!data->opts.file_open_flags)
		data->opts.file_open_flags = O_CREAT | O_TRUNC | O_WRONLY;

	data->pfx = git3_pathspec_prefix(&data->opts.paths);

	if ((error = git3_repository__configmap_lookup(
			 &data->can_symlink, repo, GIT3_CONFIGMAP_SYMLINKS)) < 0)
		goto cleanup;

	if ((error = git3_repository__configmap_lookup(
			 &data->respect_filemode, repo, GIT3_CONFIGMAP_FILEMODE)) < 0)
		goto cleanup;
	...
	if ((data->opts.checkout_strategy &
		(GIT3_CHECKOUT_CONFLICT_STYLE_MERGE | GIT3_CHECKOUT_CONFLICT_STYLE_DIFF3)) == 0) {
		git3_config_entry *conflict_style = NULL;
		git3_config *cfg = NULL;

		if ((error = git3_repository_config__weakptr(&cfg, repo)) < 0 ||
			(error = git3_config_get_entry(&conflict_style, cfg, "merge.conflictstyle")) < 0 ||
			error == GIT3_ENOTFOUND)
			;
		else if (error)
			goto cleanup;
		else if (strcmp(conflict_style->value, "merge") == 0)
			data->opts.checkout_strategy |= GIT3_CHECKOUT_CONFLICT_STYLE_MERGE;
		else if (strcmp(conflict_style->value, "diff3") == 0)
			data->opts.checkout_strategy |= GIT3_CHECKOUT_CONFLICT_STYLE_DIFF3;
		else if (strcmp(conflict_style->value, "zdiff3") == 0)
			data->opts.checkout_strategy |= GIT3_CHECKOUT_CONFLICT_STYLE_ZDIFF3;
		else {
			git3_error_set(GIT3_ERROR_CHECKOUT, "unknown style '%s' given for 'merge.conflictstyle'",
				conflict_style->value);
			error = -1;
			git3_config_entry_free(conflict_style);
			goto cleanup;
		}
		git3_config_entry_free(conflict_style);
	}

	if ((error = git3_pool_init(&data->pool, 1)) < 0 ||
	    (error = git3_vector_init(&data->removes, 0, git3__strcmp_cb)) < 0 ||
	    (error = git3_vector_init(&data->remove_conflicts, 0, NULL)) < 0 ||
	    (error = git3_vector_init(&data->update_conflicts, 0, NULL)) < 0 ||
	    (error = git3_str_puts(&data->target_path, data->opts.target_directory)) < 0 ||
	    (error = git3_fs_path_to_dir(&data->target_path)) < 0)
		goto cleanup;

	data->target_len = git3_str_len(&data->target_path);

	git3_attr_session__init(&data->attr_session, data->repo);
...
#define CHECKOUT_INDEX_DONT_WRITE_MASK \
	(GIT3_CHECKOUT_DONT_UPDATE_INDEX | GIT3_CHECKOUT_DONT_WRITE_INDEX)

GIT3_INLINE(void) setup_pathspecs(
	git3_iterator_options *iter_opts,
...
int git3_checkout_iterator(
	git3_iterator *target,
	git3_index *index,
	const git3_checkout_options *opts)
{
	int error = 0;
	git3_iterator *baseline = NULL, *workdir = NULL;
	git3_iterator_options baseline_opts = GIT3_ITERATOR_OPTIONS_INIT,
		workdir_opts = GIT3_ITERATOR_OPTIONS_INIT;
	...
	diff_opts.flags =
		...
		GIT3_DIFF_INCLUDE_UNREADABLE |
		GIT3_DIFF_INCLUDE_UNTRACKED |
		GIT3_DIFF_RECURSE_UNTRACKED_DIRS | /* needed to match baseline */
		GIT3_DIFF_INCLUDE_IGNORED |
		GIT3_DIFF_INCLUDE_TYPECHANGE |
		GIT3_DIFF_INCLUDE_TYPECHANGE_TREES |
		GIT3_DIFF_SKIP_BINARY_CHECK |
		GIT3_DIFF_INCLUDE_CASECHANGE;
	if (data.opts.checkout_strategy & GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH)
		diff_opts.flags |= GIT3_DIFF_DISABLE_PATHSPEC_MATCH;
	if (data.opts.paths.count > 0)
		diff_opts.pathspec = data.opts.paths;

	/* set up iterators */

	workdir_opts.flags = git3_iterator_ignore_case(target) ?
		GIT3_ITERATOR_IGNORE_CASE : GIT3_ITERATOR_DONT_IGNORE_CASE;
	workdir_opts.flags |= GIT3_ITERATOR_DONT_AUTOEXPAND;
	workdir_opts.start = data.pfx;
	workdir_opts.end = data.pfx;

	setup_pathspecs(&workdir_opts, opts);

	if ((error = git3_iterator_reset_range(target, data.pfx, data.pfx)) < 0 ||
		(error = git3_iterator_for_workdir_ext(
			&workdir, data.repo, data.opts.target_directory, index, NULL,
			&workdir_opts)) < 0)
		goto cleanup;

	baseline_opts.flags = git3_iterator_ignore_case(target) ?
		GIT3_ITERATOR_IGNORE_CASE : GIT3_ITERATOR_DONT_IGNORE_CASE;
	...
	if (data.opts.baseline_index) {
		if ((error = git3_iterator_for_index(
				&baseline, git3_index_owner(data.opts.baseline_index),
	...
	GIT3_ASSERT(git3_iterator_ignore_case(workdir) == git3_iterator_ignore_case(baseline));

	/* Generate baseline-to-target diff which will include an entry for
	 * every possible update that might need to be made.
	 */
	if ((error = git3_diff__from_iterators(
	...
	if (data.strategy & GIT3_CHECKOUT_DRY_RUN)
		goto cleanup;
	...
	GIT3_ASSERT(data.completed_steps == data.total_steps);
...
cleanup:
	if (!error && data.index != NULL &&
		...
		error = git3_index_write(data.index);

	git3_diff_free(data.diff);
	git3_iterator_free(workdir);
	git3_iterator_free(baseline);
	git3__free(actions);
	git3__free(counts);
	checkout_data_clear(&data);
	...
	const git3_checkout_options *opts)
{
	...
	if (index && repo &&
		...
		git3_index_owner(index) != repo) {
		git3_error_set(GIT3_ERROR_CHECKOUT,
			"index to checkout does not match repository");
		return -1;
	} else if(index && repo && !git3_index_owner(index)) {
		GIT3_REFCOUNT_OWN(index, repo);
		owned = 1;
	}

	if (!repo)
		repo = git3_index_owner(index);

	if (!index && (error = git3_repository_index__weakptr(&index, repo)) < 0)
		return error;
	GIT3_REFCOUNT_INC(index);

	setup_pathspecs(&iter_opts, opts);

	if (!(error = git3_iterator_for_index(&index_i, repo, index, &iter_opts)))
		error = git3_checkout_iterator(index_i, index, opts);

	if (owned)
		GIT3_REFCOUNT_OWN(index, NULL);

	git3_iterator_free(index_i);
	git3_index_free(index);

	return error;
}

int git3_checkout_tree(
	git3_repository *repo,
	...
	const git3_checkout_options *opts)
{
	...
	if (treeish && repo && git3_object_owner(treeish) != repo) {
		git3_error_set(GIT3_ERROR_CHECKOUT,
	...
	if (!repo)
		repo = git3_object_owner(treeish);

	if (treeish) {
		if (git3_object_peel((git3_object **)&tree, treeish, GIT3_OBJECT_TREE) < 0) {
			git3_error_set(
				GIT3_ERROR_CHECKOUT, "provided object cannot be peeled to a tree");
			return -1;
		}
	}
	else {
	...
	if (!(error = git3_iterator_for_tree(&tree_i, tree, &iter_opts)))
		error = git3_checkout_iterator(tree_i, index, opts);

	git3_iterator_free(tree_i);
	git3_index_free(index);
	git3_tree_free(tree);
...
int git3_checkout_head(
	git3_repository *repo,
	const git3_checkout_options *opts)
{
	GIT3_ASSERT_ARG(repo);

	return git3_checkout_tree(repo, NULL, opts);
}

int git3_checkout_options_init(git3_checkout_options *opts, unsigned int version)
{
	GIT3_INIT_STRUCTURE_FROM_TEMPLATE(
		opts, version, git3_checkout_options, GIT3_CHECKOUT_OPTIONS_INIT);
	return 0;
}

#ifndef GIT3_DEPRECATE_HARD
int git3_checkout_init_options(git3_checkout_options *opts, unsigned int version)
{
	return git3_checkout_options_init(opts, version);
}
#endif
