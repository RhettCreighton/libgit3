...
static int git3_parse_err(const char *fmt, ...) GIT3_FORMAT_PRINTF(1, 2);
static int git3_parse_err(const char *fmt, ...)
{
	va_list ap;
...
static size_t header_path_len(git3_patch_parse_ctx *ctx)
{
	bool inquote = 0;
	bool quoted = git3_parse_ctx_contains_s(&ctx->parse_ctx, "\"");
	size_t len;

	for (len = quoted; len < ctx->parse_ctx.line_len; len++) {
		if (!quoted && git3__isspace(ctx->parse_ctx.line[len]))
			break;
...
static int parse_header_path_buf(git3_str *path, git3_patch_parse_ctx *ctx, size_t path_len)
{
	int error;

	if ((error = git3_str_put(path, ctx->parse_ctx.line, path_len)) < 0)
		return error;

	git3_parse_advance_chars(&ctx->parse_ctx, path_len);

	git3_str_rtrim(path);

	if (path->size > 0 && path->ptr[0] == '"' &&
	    (error = git3_str_unquote(path)) < 0)
		return error;

	git3_fs_path_squash_slashes(path);

	if (!path->size)
		return git3_parse_err("patch contains empty path at line %"PRIuZ,
				     ctx->parse_ctx.line_num);

	return 0;
}

static int parse_header_path(char **out, git3_patch_parse_ctx *ctx)
{
...
static int parse_header_git_oldpath(
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	git3_str old_path = GIT3_STR_INIT;
	int error;

	if (patch->old_path) {
		error = git3_parse_err("patch contains duplicate old path at line %"PRIuZ,
				      ctx->parse_ctx.line_num);
		goto out;
	}
	...
	patch->old_path = git3_str_detach(&old_path);
	...
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	...
	if (patch->new_path) {
		error = git3_parse_err("patch contains duplicate new path at line %"PRIuZ,
				      ctx->parse_ctx.line_num);
		goto out;
	}
	...
	if ((error = parse_header_path_buf(&new_path, ctx, ctx->parse_ctx.line_len - 1)) <  0)
		goto out;
	patch->new_path = git3_str_detach(&new_path);
...
static int parse_header_mode(uint16_t *mode, git3_patch_parse_ctx *ctx)
{
	int64_t m;

	if ((git3_parse_advance_digit(&m, &ctx->parse_ctx, 8)) < 0)
		return git3_parse_err("invalid file mode at line %"PRIuZ, ctx->parse_ctx.line_num);
	...
	git3_patch_parse_ctx *ctx)
{
	...
	hexsize = git3_oid_hexsize(ctx->opts.oid_type);

	for (len = 0;
	     len < ctx->parse_ctx.line_len && len < hexsize;
	     len++) {
		if (!git3__isxdigit(ctx->parse_ctx.line[len]))
			break;
	}

	if (len < GIT3_OID_MINPREFIXLEN || len > hexsize ||
		git3_oid_from_prefix(oid, ctx->parse_ctx.line, len, ctx->opts.oid_type) < 0)
		return git3_parse_err("invalid hex formatted object id at line %"PRIuZ,
			ctx->parse_ctx.line_num);

	git3_parse_advance_chars(&ctx->parse_ctx, len);
	...
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	...
	if (parse_header_oid(&patch->base.delta->old_file.id,
		...
		git3_parse_advance_expected_str(&ctx->parse_ctx, "..") < 0 ||
		parse_header_oid(&patch->base.delta->new_file.id,
			&patch->base.delta->new_file.id_abbrev, ctx) < 0)
		return -1;

	if (git3_parse_peek(&c, &ctx->parse_ctx, 0) == 0 && c == ' ') {
		uint16_t mode = 0;

		git3_parse_advance_chars(&ctx->parse_ctx, 1);
...
static int parse_header_git_deletedfilemode(
	git3_patch_parsed *patch,
	git3_patch_parse_ctx *ctx)
{
	git3__free((char *)patch->base.delta->new_file.path);

	patch->base.delta->new_file.path = NULL;
	patch->base.delta->status = GIT3_DELTA_DELETED;
...
static int parse_header_git_newfilemode(
	...
	git3_patch_parse_ctx *ctx)
{
	git3__free((char *)patch->base.delta->old_file.path);

	patch->base.delta->old_file.path = NULL;
	patch->base.delta->status = GIT3_DELTA_ADDED;
	patch->base.delta->nfiles = 1;
...
static int parse_header_renamefrom(
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT3_DELTA_RENAMED;
...
static int parse_header_copyfrom(
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT3_DELTA_COPIED;
	return parse_header_rename(&patch->rename_old_path, ctx);
}
...
static int parse_header_percent(uint16_t *out, git3_patch_parse_ctx *ctx)
{
	int64_t val;

	if (git3_parse_advance_digit(&val, &ctx->parse_ctx, 10) < 0)
		return -1;

	if (git3_parse_advance_expected_str(&ctx->parse_ctx, "%") < 0)
		return -1;
	...
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	if (parse_header_percent(&patch->base.delta->similarity, ctx) < 0)
		return git3_parse_err("invalid similarity percentage at line %"PRIuZ,
			ctx->parse_ctx.line_num);
...
static int parse_header_start(git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	if (parse_header_path(&patch->header_old_path, ctx) < 0)
		return git3_parse_err("corrupt old path in git diff header at line %"PRIuZ,
			ctx->parse_ctx.line_num);

	if (git3_parse_advance_ws(&ctx->parse_ctx) < 0 ||
		parse_header_path(&patch->header_new_path, ctx) < 0)
		return git3_parse_err("corrupt new path in git diff header at line %"PRIuZ,
			ctx->parse_ctx.line_num);
	 ...
	 * for us.
	 */
	if (!git3_parse_ctx_contains(&ctx->parse_ctx, "\n", 1) &&
	    !git3_parse_ctx_contains(&ctx->parse_ctx, "\r\n", 2)) {
		git3_parse_advance_chars(&ctx->parse_ctx, ctx->parse_ctx.line_len - 1);

		git3__free(patch->header_old_path);
		patch->header_old_path = NULL;
		git3__free(patch->header_new_path);
...
typedef struct {
	...
	int(*fn)(git3_patch_parsed *, git3_patch_parse_ctx *);
	...
	git3_patch_parse_ctx *ctx)
{
	...
	for (; ctx->parse_ctx.remain_len > 0; git3_parse_advance_line(&ctx->parse_ctx)) {
		bool found = false;
		...
		for (i = 0; i < ARRAY_SIZE(transitions); i++) {
			...
			if (transition->expected_state != state ||
			    git3__prefixcmp(ctx->parse_ctx.line, transition->str) != 0)
				continue;
			...
			git3_parse_advance_chars(&ctx->parse_ctx, op_len);

			if ((error = transition->fn(patch, ctx)) < 0)
				goto done;

			git3_parse_advance_ws(&ctx->parse_ctx);

			if (git3_parse_advance_expected_str(&ctx->parse_ctx, "\n") < 0 ||
			    ctx->parse_ctx.line_len > 0) {
				error = git3_parse_err("trailing data at line %"PRIuZ, ctx->parse_ctx.line_num);
				goto done;
			}
		...
		if (!found) {
			error = git3_parse_err("invalid patch header at line %"PRIuZ,
				ctx->parse_ctx.line_num);
			goto done;
		}
	}
	...
	if (state != STATE_END) {
		error = git3_parse_err("unexpected header line %"PRIuZ, ctx->parse_ctx.line_num);
		goto done;
	}
...
static int parse_int(int *out, git3_patch_parse_ctx *ctx)
{
	int64_t num;

	if (git3_parse_advance_digit(&num, &ctx->parse_ctx, 10) < 0 || !git3__is_int(num))
		return -1;
	...
	git3_patch_parse_ctx *ctx)
{
	...
	git3_parse_advance_line(&ctx->parse_ctx);

	if (!hunk->hunk.old_lines && !hunk->hunk.new_lines)
		goto fail;

	hunk->hunk.header_len = ctx->parse_ctx.line - header_start;
	if (hunk->hunk.header_len > (GIT3_DIFF_HUNK_HEADER_SIZE - 1))
		return git3_parse_err("oversized patch hunk header at line %"PRIuZ,
			ctx->parse_ctx.line_num);
...
fail:
	git3_error_set(GIT3_ERROR_PATCH, "invalid patch hunk header at line %"PRIuZ,
		ctx->parse_ctx.line_num);
	return -1;
}

static int eof_for_origin(int origin) {
	if (origin == GIT3_DIFF_LINE_ADDITION)
		return GIT3_DIFF_LINE_DEL_EOFNL;
	if (origin == GIT3_DIFF_LINE_DELETION)
		return GIT3_DIFF_LINE_ADD_EOFNL;
	return GIT3_DIFF_LINE_CONTEXT_EOFNL;
}
	...
	git3_patch_parse_ctx *ctx)
{
	...
	for (;
		...
		(oldlines || newlines) &&
		!git3_parse_ctx_contains_s(&ctx->parse_ctx, "@@ -");
		git3_parse_advance_line(&ctx->parse_ctx)) {

		int old_lineno, new_lineno, origin, prefix = 1;
		char c;

		if (git3__add_int_overflow(&old_lineno, hunk->hunk.old_start, hunk->hunk.old_lines) ||
		    git3__sub_int_overflow(&old_lineno, old_lineno, oldlines) ||
		    git3__add_int_overflow(&new_lineno, hunk->hunk.new_start, hunk->hunk.new_lines) ||
		    git3__sub_int_overflow(&new_lineno, new_lineno, newlines)) {
			error = git3_parse_err("unrepresentable line count at line %"PRIuZ,
					      ctx->parse_ctx.line_num);
			goto done;
		}

		if (ctx->parse_ctx.line_len == 0 || ctx->parse_ctx.line[ctx->parse_ctx.line_len - 1] != '\n') {
			error = git3_parse_err("invalid patch instruction at line %"PRIuZ,
				ctx->parse_ctx.line_num);
			goto done;
		}

		git3_parse_peek(&c, &ctx->parse_ctx, 0);
		...
		case ' ':
			origin = GIT3_DIFF_LINE_CONTEXT;
			oldlines--;
			newlines--;
			break;

		case '-':
			origin = GIT3_DIFF_LINE_DELETION;
			oldlines--;
			new_lineno = -1;
			break;

		case '+':
			origin = GIT3_DIFF_LINE_ADDITION;
			newlines--;
			old_lineno = -1;
			break;
		...
		default:
			error = git3_parse_err("invalid patch hunk at line %"PRIuZ, ctx->parse_ctx.line_num);
			goto done;
		}

		line = git3_array_alloc(patch->base.lines);
		GIT3_ERROR_CHECK_ALLOC(line);

		memset(line, 0x0, sizeof(git3_diff_line));

		line->content_len = ctx->parse_ctx.line_len - prefix;
		line->content = git3__strndup(ctx->parse_ctx.line + prefix, line->content_len);
		GIT3_ERROR_CHECK_ALLOC(line->content);
	 ...
	 */
	if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "\\ ") &&
		git3_array_size(patch->base.lines) > 0) {

		line = git3_array_get(patch->base.lines, git3_array_size(patch->base.lines) - 1);

		if (line->content_len < 1) {
			error = git3_parse_err("last line has no trailing newline");
			goto done;
		}
		...
		line->content = git3__strndup(ctx->parse_ctx.line, line->content_len);
		GIT3_ERROR_CHECK_ALLOC(line->content);
		line->content_offset = ctx->parse_ctx.content_len - ctx->parse_ctx.remain_len;
	...
	git3_patch_parse_ctx *ctx)
{
	...
	for (; ctx->parse_ctx.remain_len > 0; git3_parse_advance_line(&ctx->parse_ctx)) {
		...
		if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "@@ -")) {
			...
			*/
			...
			error = git3_parse_err("invalid hunk header outside patch at line %"PRIuZ,
				line_num);
			goto done;
		}
		...
		/* A proper git patch */
		if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "diff --git ")) {
	...
	git3_error_set(GIT3_ERROR_PATCH, "no patch found");
...
static int parse_patch_binary_side(
	git3_diff_binary_file *binary,
	git3_patch_parse_ctx *ctx)
{
	git3_diff_binary_t type = GIT3_DIFF_BINARY_NONE;
	git3_str base85 = GIT3_STR_INIT, decoded = GIT3_STR_INIT;
	int64_t len;
	int error = 0;

	if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "literal ")) {
		type = GIT3_DIFF_BINARY_LITERAL;
		git3_parse_advance_chars(&ctx->parse_ctx, 8);
	} else if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "delta ")) {
		type = GIT3_DIFF_BINARY_DELTA;
		git3_parse_advance_chars(&ctx->parse_ctx, 6);
	} else {
		error = git3_parse_err(
	...
	if (git3_parse_advance_digit(&len, &ctx->parse_ctx, 10) < 0 ||
	    git3_parse_advance_nl(&ctx->parse_ctx) < 0 || len < 0) {
		error = git3_parse_err("invalid binary size at line %"PRIuZ, ctx->parse_ctx.line_num);
		goto done;
	}
	...
	while (ctx->parse_ctx.line_len) {
		...
		if (!decoded_len) {
			error = git3_parse_err("invalid binary length at line %"PRIuZ, ctx->parse_ctx.line_num);
			goto done;
		}
		...
		if (!encoded_len || !ctx->parse_ctx.line_len || encoded_len > ctx->parse_ctx.line_len - 1) {
			error = git3_parse_err("truncated binary data at line %"PRIuZ, ctx->parse_ctx.line_num);
			goto done;
		}

		if ((error = git3_str_decode_base85(
		...
		git3_parse_advance_chars(&ctx->parse_ctx, encoded_len);

		if (git3_parse_advance_nl(&ctx->parse_ctx) < 0) {
			error = git3_parse_err("trailing data at line %"PRIuZ, ctx->parse_ctx.line_num);
			goto done;
		}
	}
	...
	binary->data = git3_str_detach(&decoded);

done:
	git3_str_dispose(&base85);
	git3_str_dispose(&decoded);
	return error;
}
	...
	git3_patch_parse_ctx *ctx)
{
	...
	if (git3_parse_advance_expected_str(&ctx->parse_ctx, "GIT binary patch") < 0 ||
		git3_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git3_parse_err("corrupt git binary header at line %"PRIuZ, ctx->parse_ctx.line_num);
	...
	if (git3_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git3_parse_err("corrupt git binary separator at line %"PRIuZ,
			ctx->parse_ctx.line_num);
	...
	if (git3_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git3_parse_err("corrupt git binary patch separator at line %"PRIuZ,
			ctx->parse_ctx.line_num);

	patch->base.binary.contains_data = 1;
	patch->base.delta->flags |= GIT3_DIFF_FLAG_BINARY;
	return 0;
}
	...
	git3_patch_parse_ctx *ctx)
{
	...
	if (!old || !new)
		return git3_parse_err("corrupt binary data without paths at line %"PRIuZ, ctx->parse_ctx.line_num);

	if (patch->base.delta->status == GIT3_DELTA_ADDED)
		old = "/dev/null";
	else if (patch->base.delta->status == GIT3_DELTA_DELETED)
		new = "/dev/null";

	if (git3_parse_advance_expected_str(&ctx->parse_ctx, "Binary files ") < 0 ||
	    git3_parse_advance_expected_str(&ctx->parse_ctx, old) < 0 ||
	    git3_parse_advance_expected_str(&ctx->parse_ctx, " and ") < 0 ||
	    git3_parse_advance_expected_str(&ctx->parse_ctx, new) < 0 ||
	    git3_parse_advance_expected_str(&ctx->parse_ctx, " differ") < 0 ||
	    git3_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git3_parse_err("corrupt git binary header at line %"PRIuZ, ctx->parse_ctx.line_num);
	...
	git3_patch_parse_ctx *ctx)
{
	...
	while (git3_parse_ctx_contains_s(&ctx->parse_ctx, "@@ -")) {
		hunk = git3_array_alloc(patch->base.hunks);
		GIT3_ERROR_CHECK_ALLOC(hunk);

		memset(hunk, 0, sizeof(git3_patch_hunk));

		hunk->line_start = git3_array_size(patch->base.lines);
		hunk->line_count = 0;
	...
	patch->base.delta->flags |= GIT3_DIFF_FLAG_NOT_BINARY;
...
static int parse_patch_body(
	git3_patch_parsed *patch, git3_patch_parse_ctx *ctx)
{
	if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "GIT binary patch"))
		return parse_patch_binary(patch, ctx);
	else if (git3_parse_ctx_contains_s(&ctx->parse_ctx, "Binary files "))
	...
	bool two_null)
{
	...
	if (two_null && strcmp(two, "/dev/null") != 0)
		return git3_parse_err("expected %s path of '/dev/null'", old_or_new);

	else if (!two_null && strcmp(one, two) != 0)
		return git3_parse_err("mismatched %s path names", old_or_new);
	...
	const char *path_start)
{
	...
	*out_len = 0;
	...
	if (remain_len || !*path)
		return git3_parse_err(
			"header filename does not contain %"PRIuZ" path components",
			prefix_len);

done:
	*out_len = (path - path_start);
	*out = git3__strndup(path_start, *out_len);

	return (*out == NULL) ? -1 : 0;
}

static int check_filenames(git3_patch_parsed *patch)
{
	const char *prefixed_new, *prefixed_old;
	size_t old_prefixlen = 0, new_prefixlen = 0;
	bool added = (patch->base.delta->status == GIT3_DELTA_ADDED);
	bool deleted = (patch->base.delta->status == GIT3_DELTA_DELETED);

	if (patch->old_path && !patch->new_path)
		return git3_parse_err("missing new path");

	if (!patch->old_path && patch->new_path)
		return git3_parse_err("missing old path");
	...
	if (!patch->base.delta->old_file.path &&
		...
		return git3_parse_err("git diff header lacks old / new paths");

	return 0;
}

static int check_patch(git3_patch_parsed *patch)
{
	git3_diff_delta *delta = patch->base.delta;
	...
	if (delta->status == GIT3_DELTA_MODIFIED &&
	    !(delta->flags & GIT3_DIFF_FLAG_BINARY) &&
	    delta->new_file.mode == delta->old_file.mode &&
	    git3_array_size(patch->base.hunks) == 0)
		return git3_parse_err("patch with no hunks");

	if (delta->status == GIT3_DELTA_ADDED) {
		git3_oid_clear(&delta->old_file.id,
			patch->base.diff_opts.oid_type);
		delta->old_file.id_abbrev = 0;
	}

	if (delta->status == GIT3_DELTA_DELETED) {
		git3_oid_clear(&delta->new_file.id,
			patch->base.diff_opts.oid_type);
...
git3_patch_parse_ctx *git3_patch_parse_ctx_init(
	const char *content,
	size_t content_len,
	const git3_patch_options *opts)
{
	git3_patch_parse_ctx *ctx;
	git3_patch_options default_opts = GIT3_PATCH_OPTIONS_INIT;

	if ((ctx = git3__calloc(1, sizeof(git3_patch_parse_ctx))) == NULL)
		return NULL;

	if ((git3_parse_ctx_init(&ctx->parse_ctx, content, content_len)) < 0) {
		git3__free(ctx);
		return NULL;
	}

	if (opts)
		memcpy(&ctx->opts, opts, sizeof(git3_patch_options));
	else
		memcpy(&ctx->opts, &default_opts, sizeof(git3_patch_options));

	GIT3_REFCOUNT_INC(ctx);
	return ctx;
}

static void patch_parse_ctx_free(git3_patch_parse_ctx *ctx)
{
	if (!ctx)
		return;

	git3_parse_ctx_clear(&ctx->parse_ctx);
	git3__free(ctx);
}

void git3_patch_parse_ctx_free(git3_patch_parse_ctx *ctx)
{
	GIT3_REFCOUNT_DEC(ctx, patch_parse_ctx_free);
}

int git3_patch_parsed_from_diff(git3_patch **out, git3_diff *d, size_t idx)
{
	git3_diff_parsed *diff = (git3_diff_parsed *)d;
	git3_patch *p;

	if ((p = git3_vector_get(&diff->patches, idx)) == NULL)
		return -1;
...
static void patch_parsed__free(git3_patch *p)
{
	git3_patch_parsed *patch = (git3_patch_parsed *)p;
	git3_diff_line *line;
	size_t i;
	...
	git3_patch_parse_ctx_free(patch->ctx);

	git3__free((char *)patch->base.binary.old_file.data);
	git3__free((char *)patch->base.binary.new_file.data);
	git3_array_clear(patch->base.hunks);
	git3_array_foreach(patch->base.lines, i, line)
		git3__free((char *) line->content);
	git3_array_clear(patch->base.lines);
	git3__free(patch->base.delta);

	git3__free(patch->old_prefix);
	git3__free(patch->new_prefix);
	git3__free(patch->header_old_path);
	git3__free(patch->header_new_path);
	git3__free(patch->rename_old_path);
	git3__free(patch->rename_new_path);
	git3__free(patch->old_path);
	git3__free(patch->new_path);
	git3__free(patch);
}

int git3_patch_parse(
	git3_patch **out,
	git3_patch_parse_ctx *ctx)
{
	git3_patch_parsed *patch;
	...
	*out = NULL;
	...
	patch = git3__calloc(1, sizeof(git3_patch_parsed));
	GIT3_ERROR_CHECK_ALLOC(patch);

	patch->ctx = ctx;
	GIT3_REFCOUNT_INC(patch->ctx);

	patch->base.free_fn = patch_parsed__free;

	patch->base.delta = git3__calloc(1, sizeof(git3_diff_delta));
	GIT3_ERROR_CHECK_ALLOC(patch->base.delta);

	patch->base.delta->status = GIT3_DELTA_MODIFIED;
	...
	patch->base.diff_opts.flags |= GIT3_DIFF_SHOW_BINARY;

	GIT3_REFCOUNT_INC(&patch->base);
	*out = &patch->base;
	...
	const git3_patch_options *opts)
{
	...
	ctx = git3_patch_parse_ctx_init(content, content_len, opts);
	GIT3_ERROR_CHECK_ALLOC(ctx);

	error = git3_patch_parse(out, ctx);
