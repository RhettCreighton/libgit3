...
{
	git3_error_set(GIT3_ERROR_ODB, "invalid multi-pack-index file - %s", message);
	return -1;
}
		...
		struct git3_midx_chunk *chunk)
{
	...
	if ((error = git3_vector_init(&idx->packfile_names, packfiles, git3__strcmp_cb)) < 0)
		return error;
	for (i = 0; i < packfiles; ++i) {
		...
		git3_vector_insert(&idx->packfile_names, packfile_name);
		if (i && strcmp(git3_vector_get(&idx->packfile_names, i - 1), packfile_name) >= 0)
			return midx_error("packfile names are not sorted");
		if (strlen(packfile_name) <= strlen(".idx") || git3__suffixcmp(packfile_name, ".idx") != 0)
...
static int midx_parse_oid_fanout(
		git3_midx_file *idx,
		const unsigned char *data,
		struct git3_midx_chunk *chunk_oid_fanout)
{
	uint32_t i, nr;
...
static int midx_parse_oid_lookup(
		git3_midx_file *idx,
		const unsigned char *data,
		struct git3_midx_chunk *chunk_oid_lookup)
{
...
static int midx_parse_object_offsets(
		git3_midx_file *idx,
		const unsigned char *data,
		struct git3_midx_chunk *chunk_object_offsets)
{
...
static int midx_parse_object_large_offsets(
		git3_midx_file *idx,
		const unsigned char *data,
		struct git3_midx_chunk *chunk_object_large_offsets)
{
...
int git3_midx_parse(
		...
		size_t size)
{
	struct git3_midx_header *hdr;
	const unsigned char *chunk_hdr;
	struct git3_midx_chunk *last_chunk;
	uint32_t i;
	off64_t last_chunk_offset, chunk_offset, trailer_offset;
	size_t checksum_size, oid_size;
	int error;
	struct git3_midx_chunk chunk_packfile_names = {0},
					 chunk_oid_fanout = {0},
					 chunk_oid_lookup = {0},
	...
	if (size < sizeof(struct git3_midx_header) + oid_size)
		return midx_error("multi-pack index is too short");

	hdr = ((struct git3_midx_header *)data);
	 ...
	 */
	last_chunk_offset =
			sizeof(struct git3_midx_header) +
			(1 + hdr->chunks) * 12;
	...
	chunk_hdr = data + sizeof(struct git3_midx_header);
	last_chunk = NULL;
	...
	git3_oid_t oid_type)
{
	...
	GIT3_ASSERT_ARG(idx_out && path && oid_type);
	...
	if (p_fstat(fd, &st) < 0) {
		p_close(fd);
		git3_error_set(GIT3_ERROR_ODB, "multi-pack-index file not found - '%s'", path);
		return -1;
	}
	...
	idx = git3__calloc(1, sizeof(git3_midx_file));
	GIT3_ERROR_CHECK_ALLOC(idx);

	idx->oid_type = oid_type;

	error = git3_str_sets(&idx->filename, path);
	if (error < 0)
		return error;

	error = git3_futils_mmap_ro(&idx->index_map, fd, 0, idx_size);
	p_close(fd);
	if (error < 0) {
		git3_midx_free(idx);
		return error;
	...
	if ((error = git3_midx_parse(idx, idx->index_map.data, idx_size)) < 0) {
		git3_midx_free(idx);
		return error;
	}
		...
		const char *path)
{
	...
	checksum_size = git3_oid_size(idx->oid_type);
		...
		size_t len)
{
	...
	oid_size = git3_oid_size(idx->oid_type);
	oid_hexsize = git3_oid_hexsize(idx->oid_type);

	hi = ntohl(idx->oid_fanout[(int)short_oid->id[0]]);
	lo = ((short_oid->id[0] == 0x0) ? 0 : ntohl(idx->oid_fanout[(int)short_oid->id[0] - 1]));

	pos = git3_pack__lookup_id(idx->oid_lookup, oid_size, lo, hi, short_oid->id, idx->oid_type);
	...
	if (!found)
		return git3_odb__error_notfound("failed to find offset for multi-pack index entry", short_oid, len);
	if (found > 1)
		return git3_odb__error_ambiguous("found multiple offsets for multi-pack index entry");
	...
	if (idx->object_large_offsets && offset & 0x80000000) {
		...
		if (object_large_offsets_pos >= idx->num_object_large_offsets)
			return git3_odb__error_notfound("invalid index into the object large offsets table", short_oid, len);
	...
	if (pack_index >= git3_vector_length(&idx->packfile_names))
		return midx_error("invalid index into the packfile names table");
	e->pack_index = pack_index;
	e->offset = offset;
	git3_oid_from_raw(&e->sha1, current, idx->oid_type);
	return 0;
}
		...
		void *data)
{
	...
	for (i = 0; i < idx->num_objects; ++i) {
		if ((error = git3_oid_from_raw(&oid, &idx->oid_lookup[i * oid_size], idx->oid_type)) < 0)
			return error;
...
int git3_midx_close(git3_midx_file *idx)
{
	...
	if (idx->index_map.data)
		git3_futils_mmap_free(&idx->index_map);

	git3_vector_dispose(&idx->packfile_names);
		...
		)
{
	...
	w = git3__calloc(1, sizeof(git3_midx_writer));
	GIT3_ERROR_CHECK_ALLOC(w);

	if (git3_str_sets(&w->pack_dir, pack_dir) < 0) {
		git3__free(w);
		return -1;
	}
	git3_fs_path_squash_slashes(&w->pack_dir);

	if (git3_vector_init(&w->packs, 0, packfile__cmp) < 0) {
		git3_str_dispose(&w->pack_dir);
		git3__free(w);
		return -1;
	}
...
void git3_midx_writer_free(git3_midx_writer *w)
{
	...
	git3_vector_foreach (&w->packs, i, p)
		git3_mwindow_put_pack(p);
	git3_vector_dispose(&w->packs);
	git3_str_dispose(&w->pack_dir);
	git3__free(w);
}

int git3_midx_writer_add(
		git3_midx_writer *w,
		const char *idx_path)
{
	git3_str idx_path_buf = GIT3_STR_INIT;
	int error;
	struct git3_pack_file *p;

	error = git3_fs_path_prettify(&idx_path_buf, idx_path, git3_str_cstr(&w->pack_dir));
	if (error < 0)
		return error;

	/* TODO: SHA256 */
	error = git3_mwindow_get_pack(&p, git3_str_cstr(&idx_path_buf), 0);
	git3_str_dispose(&idx_path_buf);
	if (error < 0)
		return error;

	error = git3_vector_insert(&w->packs, p);
	if (error < 0) {
		git3_mwindow_put_pack(p);
		return error;
	}
...
typedef git3_array_t(git3_midx_entry) object_entry_array_t;
...
static int object_entry__cb(const git3_oid *oid, off64_t offset, void *data)
{
	struct object_entry_cb_state *state = (struct object_entry_cb_state *)data;

	git3_midx_entry *entry = git3_array_alloc(*state->object_entries_array);
	GIT3_ERROR_CHECK_ALLOC(entry);

	git3_oid_cpy(&entry->sha1, oid);
	entry->offset = offset;
...
{
	const git3_midx_entry *a = (const git3_midx_entry *)a_;
	const git3_midx_entry *b = (const git3_midx_entry *)b_;
		...
		void *cb_data)
{
	...
	size_t i;
	struct git3_pack_file *p;
	struct git3_midx_header hdr = {0};
	uint32_t oid_fanout_count;
	uint32_t object_large_offsets_count;
	uint32_t oid_fanout[256];
	off64_t offset;
	git3_str packfile_names = GIT3_STR_INIT,
		oid_lookup = GIT3_STR_INIT,
		object_offsets = GIT3_STR_INIT,
		object_large_offsets = GIT3_STR_INIT;
	unsigned char checksum[GIT3_HASH_MAX_SIZE];
	size_t checksum_size, oid_size;
	git3_midx_entry *entry;
	object_entry_array_t object_entries_array = GIT3_ARRAY_INIT;
	git3_vector object_entries = GIT3_VECTOR_INIT;
	git3_hash_ctx ctx;
	...
	GIT3_ASSERT(oid_size && checksum_type && checksum_size);

	if ((error = git3_hash_ctx_init(&ctx, checksum_type)) < 0)
		return error;

	cb_data = &hash_cb_data;
	write_cb = midx_write_hash;

	git3_vector_sort(&w->packs);
	git3_vector_foreach (&w->packs, i, p) {
		git3_str relative_index = GIT3_STR_INIT;
		...
		error = git3_str_sets(&relative_index, p->pack_name);
		if (error < 0)
			goto cleanup;
		error = git3_fs_path_make_relative(&relative_index, git3_str_cstr(&w->pack_dir));
		if (error < 0) {
			git3_str_dispose(&relative_index);
			goto cleanup;
		}
		path_len = git3_str_len(&relative_index);
		if (path_len <= strlen(".pack") || git3__suffixcmp(git3_str_cstr(&relative_index), ".pack") != 0) {
			git3_str_dispose(&relative_index);
			git3_error_set(GIT3_ERROR_INVALID, "invalid packfile name: '%s'", p->pack_name);
			error = -1;
			goto cleanup;
		}
		path_len -= strlen(".pack");

		git3_str_put(&packfile_names, git3_str_cstr(&relative_index), path_len);
		git3_str_puts(&packfile_names, ".idx");
		git3_str_putc(&packfile_names, '\0');
		git3_str_dispose(&relative_index);

		error = git3_pack_foreach_entry_offset(p, object_entry__cb, &state);
		if (error < 0)
			goto cleanup;
	}

	/* Sort the object entries. */
	error = git3_vector_init(&object_entries, git3_array_size(object_entries_array), object_entry__cmp);
	if (error < 0)
		goto cleanup;
	git3_array_foreach (object_entries_array, i, entry) {
		if ((error = git3_vector_set(NULL, &object_entries, i, entry)) < 0)
			goto cleanup;
	}
	git3_vector_set_sorted(&object_entries, 0);
	git3_vector_sort(&object_entries);
	git3_vector_uniq(&object_entries, NULL);

	/* Pad the packfile names so it is a multiple of four. */
	while (git3_str_len(&packfile_names) & 3)
		git3_str_putc(&packfile_names, '\0');

	/* Fill the OID Fanout table. */
	oid_fanout_count = 0;
	for (i = 0; i < 256; i++) {
		while (oid_fanout_count < git3_vector_length(&object_entries) &&
		       ((const git3_midx_entry *)git3_vector_get(&object_entries, oid_fanout_count))->sha1.id[0] <= i)
			++oid_fanout_count;
		oid_fanout[i] = htonl(oid_fanout_count);
	}

	/* Fill the OID Lookup table. */
	git3_vector_foreach (&object_entries, i, entry) {
	...
	git3_vector_foreach (&object_entries, i, entry) {
		...
		error = git3_str_put(&object_offsets, (const char *)&word, sizeof(word));
		if (error < 0)
			goto cleanup;
	...
	/* Write the header. */
	hdr.packfiles = htonl((uint32_t)git3_vector_length(&w->packs));
	hdr.chunks = 4;
	if (git3_str_len(&object_large_offsets) > 0)
		hdr.chunks++;
	...
	if (error < 0)
		goto cleanup;
	offset += git3_str_len(&packfile_names);
	...
	error = write_cb(git3_str_cstr(&packfile_names), git3_str_len(&packfile_names), cb_data);
	if (error < 0)
		goto cleanup;
	...
	if (error < 0)
		goto cleanup;
	error = write_cb(git3_str_cstr(&object_offsets), git3_str_len(&object_offsets), cb_data);
	if (error < 0)
		goto cleanup;
	error = write_cb(git3_str_cstr(&object_large_offsets), git3_str_len(&object_large_offsets), cb_data);
	if (error < 0)
		goto cleanup;
...
cleanup:
	git3_array_clear(object_entries_array);
	git3_vector_dispose(&object_entries);
	git3_str_dispose(&packfile_names);
	git3_str_dispose(&oid_lookup);
	git3_str_dispose(&object_offsets);
	git3_str_dispose(&object_large_offsets);
		...
		git3_midx_writer *w)
{
	...
	error = git3_str_joinpath(&midx_path, git3_str_cstr(&w->pack_dir), "multi-pack-index");
	if (error < 0)
		return error;

	if (git3_repository__fsync_gitdir)
		filebuf_flags |= GIT3_FILEBUF_FSYNC;
	error = git3_filebuf_open(&output, git3_str_cstr(&midx_path), filebuf_flags, 0644);
		...
		git3_midx_writer *w)
{
	...
	if ((error = git3_buf_tostr(&str, midx)) < 0 ||
	    (error = midx_write(w, midx_write_buf, &str)) == 0)
		error = git3_buf_fromstr(midx, &str);
