...
#define GIT3_ITERATOR_FIRST_ACCESS   (1 << 15)
#define GIT3_ITERATOR_HONOR_IGNORES  (1 << 16)
#define GIT3_ITERATOR_IGNORE_DOT_GIT (1 << 17)

#define iterator__flag(I,F) ((((git3_iterator *)(I))->flags & GIT3_ITERATOR_ ## F) != 0)
...
static void iterator_set_ignore_case(git3_iterator *iter, bool ignore_case)
{
	int (*vector_cmp)(const void *a, const void *b);

	if (ignore_case)
		iter->flags |= GIT3_ITERATOR_IGNORE_CASE;
	else
		iter->flags &= ~GIT3_ITERATOR_IGNORE_CASE;

	iter->strcomp = ignore_case ? git3__strcasecmp : git3__strcmp;
	iter->strncomp = ignore_case ? git3__strncasecmp : git3__strncmp;
	iter->prefixcomp = ignore_case ? git3__prefixcmp_icase : git3__prefixcmp;
	iter->entry_srch = ignore_case ? git3_index_entry_isrch : git3_index_entry_srch;

	vector_cmp = ignore_case ? git3__strcasecmp_cb : git3__strcmp_cb;

	git3_vector_set_cmp(&iter->pathlist, vector_cmp);
}
...
{
	if (start && *start) {
		iter->start = git3__strdup(start);
		GIT3_ERROR_CHECK_ALLOC(iter->start);

		iter->start_len = strlen(iter->start);
	}

	if (end && *end) {
		iter->end = git3__strdup(end);
		GIT3_ERROR_CHECK_ALLOC(iter->end);
...
static void iterator_range_free(git3_iterator *iter)
{
	if (iter->start) {
		git3__free(iter->start);
		iter->start = NULL;
		iter->start_len = 0;
	}
...
static int iterator_pathlist_init(git3_iterator *iter, git3_strarray *pathlist)
{
	size_t i;

	if (git3_vector_init(&iter->pathlist, pathlist->count, NULL) < 0)
		return -1;
	...
	for (i = 0; i < pathlist->count; i++) {
		...
		if (git3_vector_insert(&iter->pathlist, pathlist->strings[i]) < 0)
			return -1;
	}
...
static int iterator_init_common(
	...
	git3_index *index,
	git3_iterator_options *given_opts)
{
	static git3_iterator_options default_opts = GIT3_ITERATOR_OPTIONS_INIT;
	git3_iterator_options *options = given_opts ? given_opts : &default_opts;
	bool ignore_case;
	int precompose;
	int error;
	...
	if ((iter->flags & GIT3_ITERATOR_IGNORE_CASE) != 0) {
		ignore_case = true;
	} else if ((iter->flags & GIT3_ITERATOR_DONT_IGNORE_CASE) != 0) {
		ignore_case = false;
	} else if (repo) {
		git3_index *index;

		if ((error = git3_repository_index__weakptr(&index, iter->repo)) < 0)
			return error;
		...
		if (ignore_case == 1)
			...
			iter->flags |= GIT3_ITERATOR_DONT_IGNORE_CASE;
	} else {
		ignore_case = false;
	}
	...
	if (repo &&
		(iter->flags & GIT3_ITERATOR_PRECOMPOSE_UNICODE) == 0 &&
		(iter->flags & GIT3_ITERATOR_DONT_PRECOMPOSE_UNICODE) == 0) {

		if (git3_repository__configmap_lookup(&precompose, repo, GIT3_CONFIGMAP_PRECOMPOSE) < 0)
			git3_error_clear();
		else if (precompose)
			iter->flags |= GIT3_ITERATOR_PRECOMPOSE_UNICODE;
	}

	if ((iter->flags & GIT3_ITERATOR_DONT_AUTOEXPAND))
		iter->flags |= GIT3_ITERATOR_INCLUDE_TREES;
...
static void iterator_clear(git3_iterator *iter)
{
	iter->started = false;
	iter->ended = false;
	iter->stat_calls = 0;
	iter->pathlist_walk_idx = 0;
	iter->flags &= ~GIT3_ITERATOR_FIRST_ACCESS;
}

GIT3_INLINE(bool) iterator_has_started(
	git3_iterator *iter, const char *path, bool is_submodule)
{
	size_t path_len;
...
GIT3_INLINE(bool) iterator_has_ended(git3_iterator *iter, const char *path)
{
	if (iter->end == NULL)
		return false;
	else if (iter->ended)
 ...
 */
static bool iterator_pathlist_next_is(git3_iterator *iter, const char *path)
{
	char *p;
	...
	git3_vector_sort(&iter->pathlist);
...
static iterator_pathlist_search_t iterator_pathlist_search(
	git3_iterator *iter, const char *path, size_t path_len)
{
	int (*vector_cmp)(const void *a, const void *b);
	const char *p;
	size_t idx;
	...
	vector_cmp = (iter->flags & GIT3_ITERATOR_IGNORE_CASE) != 0 ?
		git3__strcasecmp_cb : git3__strcmp_cb;

	error = git3_vector_bsearch2(&idx, &iter->pathlist, vector_cmp, path);
	 ...
	 */
	while ((p = git3_vector_get(&iter->pathlist, idx)) != NULL) {
		if (iter->prefixcomp(p, path) != 0)
			break;

		/* an exact match would have been matched by the bsearch above */
		GIT3_ASSERT_WITH_RETVAL(p[path_len], ITERATOR_PATHLIST_NONE);
...
static int empty_iterator_noop(const git3_index_entry **e, git3_iterator *i)
{
	GIT3_UNUSED(i);
...
static int empty_iterator_advance_over(
	...
	git3_iterator_status_t *s,
	git3_iterator *i)
{
	*s = GIT3_ITERATOR_STATUS_EMPTY;
	return empty_iterator_noop(e, i);
}

static int empty_iterator_reset(git3_iterator *i)
{
	GIT3_UNUSED(i);
	return 0;
}

static void empty_iterator_free(git3_iterator *i)
{
	GIT3_UNUSED(i);
}
...
int git3_iterator_for_nothing(
	git3_iterator **out,
	git3_iterator_options *options)
{
	empty_iterator *iter;

	static git3_iterator_callbacks callbacks = {
		empty_iterator_noop,
		empty_iterator_noop,
		empty_iterator_noop,
	...
	*out = NULL;
	...
	iter = git3__calloc(1, sizeof(empty_iterator));
	GIT3_ERROR_CHECK_ALLOC(iter);

	iter->base.type = GIT3_ITERATOR_EMPTY;
...
typedef struct {
	 ...
	 * parent path.
	 */
	git3_vector similar_trees;
	git3_array_t(git3_str) similar_paths;
} tree_iterator_frame;

typedef struct {
	git3_iterator base;
	git3_tree *root;
	git3_array_t(tree_iterator_frame) frames;

	git3_index_entry entry;
	git3_str entry_path;

	/* a pool of entries to reduce the number of allocations */
	git3_pool entry_pool;
} tree_iterator;

GIT3_INLINE(tree_iterator_frame *) tree_iterator_parent_frame(
	tree_iterator *iter)
{
...
GIT3_INLINE(tree_iterator_frame *) tree_iterator_current_frame(
	tree_iterator *iter)
{
	return iter->frames.size ? &iter->frames.ptr[iter->frames.size-1] : NULL;
}

GIT3_INLINE(int) tree_entry_cmp(
	const git3_tree_entry *a, const git3_tree_entry *b, bool icase)
{
	return git3_fs_path_cmp(
		a->filename, a->filename_len, a->attr == GIT3_FILEMODE_TREE,
		b->filename, b->filename_len, b->attr == GIT3_FILEMODE_TREE,
		icase ? git3__strncasecmp : git3__strncmp);
}

GIT3_INLINE(int) tree_iterator_entry_cmp_icase(
...
static int tree_iterator_entry_sort_icase(const void *ptr_a, const void *ptr_b)
{
	...
	if (!c && a->parent_path != b->parent_path)
		c = git3__strcmp(a->parent_path, b->parent_path);
	...
	tree_iterator_entry *entry)
{
	...
	if (entry->parent_path)
		git3_str_joinpath(out, entry->parent_path, entry->tree_entry->filename);
	else
		git3_str_puts(out, entry->tree_entry->filename);

	if (git3_tree_entry__is_tree(entry->tree_entry))
		git3_str_putc(out, '/');
	...
	tree_iterator_entry *frame_entry)
{
	...
	new_frame = git3_array_alloc(iter->frames);
	GIT3_ERROR_CHECK_ALLOC(new_frame);

	if ((error = git3_tree_dup(&dup, tree)) < 0)
		goto done;
	...
	if ((error = git3_vector_init(&new_frame->entries,
				     dup->entries.size, cmp)) < 0)
		goto done;

	git3_array_foreach(dup->entries, i, tree_entry) {
		if ((new_entry = git3_pool_malloc(&iter->entry_pool, 1)) == NULL) {
			git3_error_set_oom();
			error = -1;
			goto done;
		}
		...
		if ((error = git3_vector_insert(&new_frame->entries, new_entry)) < 0)
			goto done;
	}

	git3_vector_set_sorted(&new_frame->entries,
		!iterator__ignore_case(&iter->base));

done:
	if (error < 0) {
		git3_tree_free(dup);
		git3_array_pop(iter->frames);
	}

	return error;
}

GIT3_INLINE(tree_iterator_entry *) tree_iterator_current_entry(
	tree_iterator_frame *frame)
{
	return frame->current;
}

GIT3_INLINE(int) tree_iterator_frame_push_neighbors(
	tree_iterator *iter,
	...
	const char *filename)
{
	...
	while (parent_frame->next_idx < parent_frame->entries.length) {
		...
		if ((error = git3_tree_lookup(&tree,
			iter->base.repo, &entry->tree_entry->oid)) < 0)
			break;

		if (git3_vector_insert(&parent_frame->similar_trees, tree) < 0)
			break;

		path = git3_array_alloc(parent_frame->similar_paths);
		GIT3_ERROR_CHECK_ALLOC(path);

		memset(path, 0, sizeof(git3_str));

		if ((error = tree_iterator_compute_path(path, entry)) < 0)
			break;

		GIT3_ERROR_CHECK_ALLOC_ADD(&new_size,
			frame->entries.length, tree->entries.size);
		git3_vector_size_hint(&frame->entries, new_size);

		git3_array_foreach(tree->entries, i, tree_entry) {
			new_entry = git3_pool_malloc(&iter->entry_pool, 1);
			GIT3_ERROR_CHECK_ALLOC(new_entry);

			new_entry->tree_entry = tree_entry;
			new_entry->parent_path = path->ptr;

			if ((error = git3_vector_insert(&frame->entries, new_entry)) < 0)
				break;
		}
...
GIT3_INLINE(int) tree_iterator_frame_push(
...
static int tree_iterator_frame_pop(tree_iterator *iter)
{
	...
	GIT3_ASSERT(iter->frames.size);

	frame = git3_array_pop(iter->frames);

	git3_vector_dispose(&frame->entries);
	git3_tree_free(frame->tree);

	do {
		buf = git3_array_pop(frame->similar_paths);
		git3_str_dispose(buf);
	} while (buf != NULL);

	git3_array_clear(frame->similar_paths);

	git3_vector_foreach(&frame->similar_trees, i, tree)
		git3_tree_free(tree);

	git3_vector_dispose(&frame->similar_trees);

	git3_str_dispose(&frame->path);

	return 0;
}

static int tree_iterator_current(
	const git3_index_entry **out, git3_iterator *i)
{
...
static void tree_iterator_set_current(
	...
	tree_iterator_entry *entry)
{
	git3_tree_entry *tree_entry = entry->tree_entry;

	frame->current = entry;

	memset(&iter->entry, 0x0, sizeof(git3_index_entry));

	iter->entry.mode = tree_entry->attr;
	iter->entry.path = iter->entry_path.ptr;
	git3_oid_cpy(&iter->entry.id, &tree_entry->oid);
}

static int tree_iterator_advance(const git3_index_entry **out, git3_iterator *i)
{
	tree_iterator *iter = (tree_iterator *)i;
	int error = 0;

	iter->base.flags |= GIT3_ITERATOR_FIRST_ACCESS;
	...
	while (true) {
		 ...
		 */
		if (frame->next_idx == 0 && !git3_vector_is_sorted(&frame->entries))
			git3_vector_sort(&frame->entries);
		...
		is_tree = git3_tree_entry__is_tree(entry->tree_entry);
	...
	const git3_index_entry **out, git3_iterator *i)
{
	 ...
	 */
	GIT3_ASSERT(iterator__do_autoexpand(i) ^ (prev_entry != NULL));

	if (prev_entry) {
		if (!git3_tree_entry__is_tree(prev_entry->tree_entry))
			return 0;
...
static int tree_iterator_advance_over(
	...
	git3_iterator *i)
{
	*status = GIT3_ITERATOR_STATUS_NORMAL;
	return git3_iterator_advance(out, i);
}
...
static void tree_iterator_clear(tree_iterator *iter)
{
	...
	git3_array_clear(iter->frames);

	git3_pool_clear(&iter->entry_pool);
	git3_str_clear(&iter->entry_path);
...
static int tree_iterator_init(tree_iterator *iter)
{
	...
	if ((error = git3_pool_init(&iter->entry_pool, sizeof(tree_iterator_entry))) < 0 ||
...
static int tree_iterator_reset(git3_iterator *i)
{
	tree_iterator *iter = (tree_iterator *)i;
...
static void tree_iterator_free(git3_iterator *i)
{
	tree_iterator *iter = (tree_iterator *)i;

	tree_iterator_clear(iter);

	git3_tree_free(iter->root);
	git3_str_dispose(&iter->entry_path);
}

int git3_iterator_for_tree(
	git3_iterator **out,
	git3_tree *tree,
	git3_iterator_options *options)
{
	...
	*out = NULL;
	...
	if (tree == NULL)
		return git3_iterator_for_nothing(out, options);

	iter = git3__calloc(1, sizeof(tree_iterator));
	GIT3_ERROR_CHECK_ALLOC(iter);

	iter->base.type = GIT3_ITERATOR_TREE;
	iter->base.cb = &callbacks;

	if ((error = iterator_init_common(&iter->base,
			git3_tree_owner(tree), NULL, options)) < 0 ||
		(error = git3_tree_dup(&iter->root, tree)) < 0 ||
...
on_error:
	git3_iterator_free(&iter->base);
	return error;
}

int git3_iterator_current_tree_entry(
	const git3_tree_entry **tree_entry, git3_iterator *i)
{
	tree_iterator *iter;
	tree_iterator_frame *frame;
	tree_iterator_entry *entry;

	GIT3_ASSERT(i->type == GIT3_ITERATOR_TREE);
...
int git3_iterator_current_parent_tree(
	const git3_tree **parent_tree, git3_iterator *i, size_t depth)
{
	tree_iterator *iter;
	tree_iterator_frame *frame;
	...
	GIT3_ASSERT(depth < iter->frames.size);
...
typedef struct {
	...
	git3_tree *tree;
	git3_index *index;
	git3_vector index_snapshot;

	git3_oid_t oid_type;

	git3_array_t(filesystem_iterator_frame) frames;
	git3_ignores ignores;

	/* info about the current entry */
	git3_index_entry entry;
	git3_str current_path;
...
GIT3_INLINE(filesystem_iterator_frame *) filesystem_iterator_parent_frame(
...
GIT3_INLINE(filesystem_iterator_frame *) filesystem_iterator_current_frame(
	filesystem_iterator *iter)
{
	return iter->frames.size ? &iter->frames.ptr[iter->frames.size-1] : NULL;
}

GIT3_INLINE(filesystem_iterator_entry *) filesystem_iterator_current_entry(
...
static int filesystem_iterator_entry_cmp(const void *_a, const void *_b)
{
	...
	return git3__strcmp(a->path, b->path);
}
...
static int filesystem_iterator_entry_cmp_icase(const void *_a, const void *_b)
{
	...
	return git3__strcasecmp(a->path, b->path);
}
	...
	bool *out, filesystem_iterator *iter, const char *path, size_t path_len)
{
	...
	*out = false;
	...
	if (iter->tree) {
		...
		error = git3_tree_entry_bypath(&entry, iter->tree, path);

		if (error < 0 && error != GIT3_ENOTFOUND)
			return error;

		if (!error) {
			is_submodule = (entry->attr == GIT3_FILEMODE_COMMIT);
	...
	if (!is_submodule && iter->base.index) {
		...
		error = git3_index_snapshot_find(&pos,
		...
		if (!error) {
			git3_index_entry *e = git3_vector_get(&iter->index_snapshot, pos);
			is_submodule = (e->mode == GIT3_FILEMODE_COMMIT);
		}
	}
	...
	filesystem_iterator_frame *new_frame)
{
	...
	if (git3_ignore__lookup(&new_frame->is_ignored,
			&iter->ignores, path, GIT3_DIR_FLAG_TRUE) < 0) {
		git3_error_clear();
		new_frame->is_ignored = GIT3_IGNORE_NOTFOUND;
	}
	...
	if (frame_entry) {
		...
		if (new_frame->is_ignored <= GIT3_IGNORE_NOTFOUND)
			new_frame->is_ignored = previous_frame->is_ignored;

		git3_ignore__push_dir(&iter->ignores, relative_path);
	}
}
...
{
	if (iterator__honor_ignores(&iter->base))
		git3_ignore__pop_dir(&iter->ignores);
}

GIT3_INLINE(bool) filesystem_iterator_examine_path(
	bool *is_dir_out,
...
GIT3_INLINE(bool) filesystem_iterator_is_dot_git(
	filesystem_iterator *iter, const char *path, size_t path_len)
{
	...
	if (git3__tolower(path[len - 1]) != 't' ||
		git3__tolower(path[len - 2]) != 'i' ||
		git3__tolower(path[len - 3]) != 'g' ||
		git3__tolower(path[len - 4]) != '.')
		return false;
	...
	filesystem_iterator_entry *entry)
{
	...
	if (S_ISDIR(entry->st.st_mode)) {
		memset(&entry->id, 0, git3_oid_size(iter->oid_type));
		return 0;
	}

	if (iter->base.type == GIT3_ITERATOR_WORKDIR)
		return git3_repository_hashfile(&entry->id,
			iter->base.repo, entry->path, GIT3_OBJECT_BLOB, NULL);

	if (!(error = git3_str_joinpath(&fullpath, iter->root, entry->path)) &&
	    !(error = git3_path_validate_str_length(iter->base.repo, &fullpath)))
		error = git3_odb__hashfile(&entry->id, fullpath.ptr, GIT3_OBJECT_BLOB, iter->oid_type);
	...
	iterator_pathlist_search_t pathlist_match)
{
	...
	*out = NULL;
	 ...
	 */
	GIT3_ERROR_CHECK_ALLOC_ADD(&entry_size,
		sizeof(filesystem_iterator_entry), path_len);
	GIT3_ERROR_CHECK_ALLOC_ADD(&entry_size, entry_size, 2);

	entry = git3_pool_malloc(&frame->entry_pool, entry_size);
	...
	if (iter->base.flags & GIT3_ITERATOR_INCLUDE_HASH)
	...
	filesystem_iterator_entry *frame_entry)
{
	...
	git3_fs_path_diriter diriter = GIT3_FS_PATH_DIRITER_INIT;
	...
	if (frame_entry)
		git3_str_joinpath(&root, iter->root, frame_entry->path);
	else
		git3_str_puts(&root, iter->root);

	if (git3_str_oom(&root) ||
	    git3_path_validate_str_length(iter->base.repo, &root) < 0) {
		error = -1;
		goto done;
	}
	...
	if ((error = git3_fs_path_diriter_init(
			&diriter, root.ptr, iter->dirload_flags)) < 0) {
		error = GIT3_ENOTFOUND;
		goto done;
	}

	if ((error = git3_vector_init(&new_frame->entries, 64,
			iterator__ignore_case(&iter->base) ?
			filesystem_iterator_entry_cmp_icase :
			filesystem_iterator_entry_cmp)) < 0)
		goto done;

	if ((error = git3_pool_init(&new_frame->entry_pool, 1)) < 0)
		goto done;

	/* check if this directory is ignored */
	filesystem_iterator_frame_push_ignores(iter, frame_entry, new_frame);

	while ((error = git3_fs_path_diriter_next(&diriter)) == 0) {
		iterator_pathlist_search_t pathlist_match = ITERATOR_PATHLIST_FULL;
		git3_str path_str = GIT3_STR_INIT;
		bool dir_expected = false;

		if ((error = git3_fs_path_diriter_fullpath(&path, &path_len, &diriter)) < 0)
			goto done;

		path_str.ptr = (char *)path;
		path_str.size = path_len;

		if ((error = git3_path_validate_str_length(iter->base.repo, &path_str)) < 0)
			goto done;

		GIT3_ASSERT(path_len > iter->root_len);
		...
		if ((error = git3_fs_path_diriter_stat(&statbuf, &diriter)) < 0) {
			...
			statbuf.st_mode = GIT3_FILEMODE_UNREADABLE;
		...
		if (S_ISDIR(statbuf.st_mode)) {
			...
			if (submodule)
				statbuf.st_mode = GIT3_FILEMODE_COMMIT;
		}
		...
		git3_vector_insert(&new_frame->entries, entry);
	}
	...
	git3_vector_sort(&new_frame->entries);
	...
	git3_fs_path_diriter_free(&diriter);
	return error;
}

GIT3_INLINE(int) filesystem_iterator_frame_pop(filesystem_iterator *iter)
{
	filesystem_iterator_frame *frame;
	...
	git3_pool_clear(&frame->entry_pool);
	git3_vector_dispose(&frame->entries);
	...
	filesystem_iterator_entry *entry)
{
	...
	iter->entry.mode = git3_futils_canonical_mode(entry->st.st_mode);
	iter->entry.uid = entry->st.st_uid;
	iter->entry.gid = entry->st.st_gid;
	iter->entry.file_size = (uint32_t)entry->st.st_size;

	if (iter->base.flags & GIT3_ITERATOR_INCLUDE_HASH)
		git3_oid_cpy(&iter->entry.id, &entry->id);
	else
		git3_oid_clear(&iter->entry.id, iter->oid_type);

	iter->entry.path = entry->path;

	iter->current_is_ignored = GIT3_IGNORE_UNCHECKED;
}

static int filesystem_iterator_current(
	const git3_index_entry **out, git3_iterator *i)
{
	filesystem_iterator *iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);
	...
	const filesystem_iterator_entry *entry)
{
	...
	if ((error = git3_str_joinpath(&fullpath, iter->root, entry->path)) < 0 ||
	    (error = git3_path_validate_str_length(iter->base.repo, &fullpath)) < 0 ||
	...
	while (true) {
		...
		if (is_dir) {
			if (iterator__do_autoexpand(iter)) {
				...
				/* may get GIT3_ENOTFOUND due to races or permission problems
	...
	const git3_index_entry **out, git3_iterator *i)
{
	...
	if (prev_entry) {
		if (prev_entry->st.st_mode != GIT3_FILEMODE_COMMIT &&
...
int git3_iterator_current_workdir_path(git3_str **out, git3_iterator *i)
{
	filesystem_iterator *iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);
	const git3_index_entry *entry;

	if (i->type != GIT3_ITERATOR_FS &&
		i->type != GIT3_ITERATOR_WORKDIR) {
		*out = NULL;
		return 0;
	}

	git3_str_truncate(&iter->current_path, iter->root_len);

	if (git3_iterator_current(&entry, i) < 0 ||
		git3_str_puts(&iter->current_path, entry->path) < 0)
		return -1;
...
GIT3_INLINE(git3_dir_flag) entry_dir_flag(git3_index_entry *entry)
{
#if defined(GIT3_WIN32) && !defined(__MINGW32__)
	return (entry && entry->mode) ?
		(S_ISDIR(entry->mode) ? GIT3_DIR_FLAG_TRUE : GIT3_DIR_FLAG_FALSE) :
		GIT3_DIR_FLAG_UNKNOWN;
#else
	GIT3_UNUSED(entry);
	return GIT3_DIR_FLAG_UNKNOWN;
#endif
}
...
{
	filesystem_iterator_frame *frame;
	git3_dir_flag dir_flag = entry_dir_flag(&iter->entry);

	if (git3_ignore__lookup(&iter->current_is_ignored,
			&iter->ignores, iter->entry.path, dir_flag) < 0) {
		git3_error_clear();
		iter->current_is_ignored = GIT3_IGNORE_NOTFOUND;
	}

	/* use ignore from containing frame stack */
	if (iter->current_is_ignored <= GIT3_IGNORE_NOTFOUND) {
		frame = filesystem_iterator_current_frame(iter);
		iter->current_is_ignored = frame->is_ignored;
	}
}

GIT3_INLINE(bool) filesystem_iterator_current_is_ignored(
	filesystem_iterator *iter)
{
	if (iter->current_is_ignored == GIT3_IGNORE_UNCHECKED)
		filesystem_iterator_update_ignored(iter);

	return (iter->current_is_ignored == GIT3_IGNORE_TRUE);
}

bool git3_iterator_current_is_ignored(git3_iterator *i)
{
	filesystem_iterator *iter = NULL;

	if (i->type != GIT3_ITERATOR_WORKDIR)
		return false;

	iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);

	return filesystem_iterator_current_is_ignored(iter);
}

bool git3_iterator_current_tree_is_ignored(git3_iterator *i)
{
	filesystem_iterator *iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);
	filesystem_iterator_frame *frame;
	...
	frame = filesystem_iterator_current_frame(iter);
	return (frame->is_ignored == GIT3_IGNORE_TRUE);
}
	...
	git3_iterator *i)
{
	...
	*status = GIT3_ITERATOR_STATUS_NORMAL;
	...
	GIT3_ASSERT(iterator__has_been_accessed(i));

	current_frame = filesystem_iterator_current_frame(iter);
	GIT3_ASSERT(current_frame);

	current_entry = filesystem_iterator_current_entry(current_frame);
	GIT3_ASSERT(current_entry);

	if ((error = git3_iterator_current(&entry, i)) < 0)
		return error;

	if (!S_ISDIR(entry->mode)) {
		if (filesystem_iterator_current_is_ignored(iter))
			*status = GIT3_ITERATOR_STATUS_IGNORED;

		return filesystem_iterator_advance(out, i);
	}

	git3_str_clear(&iter->tmp_buf);
	if ((error = git3_str_puts(&iter->tmp_buf, entry->path)) < 0)
		return error;
	...
	*status = current_entry->match == ITERATOR_PATHLIST_IS_PARENT ?
		GIT3_ITERATOR_STATUS_FILTERED : GIT3_ITERATOR_STATUS_EMPTY;
	...
	while (entry && !iter->base.prefixcomp(entry->path, base)) {
		...
		if ((error = git3_iterator_advance(&entry, i)) < 0)
			break;
	}
...
static void filesystem_iterator_clear(filesystem_iterator *iter)
{
	...
	git3_array_clear(iter->frames);
	git3_ignore__free(&iter->ignores);

	git3_str_dispose(&iter->tmp_buf);
...
static int filesystem_iterator_init(filesystem_iterator *iter)
{
	...
	if (iterator__honor_ignores(&iter->base) &&
		(error = git3_ignore__for_path(iter->base.repo,
...
static int filesystem_iterator_reset(git3_iterator *i)
{
	filesystem_iterator *iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);
...
static void filesystem_iterator_free(git3_iterator *i)
{
	filesystem_iterator *iter = GIT3_CONTAINER_OF(i, filesystem_iterator, base);
	git3__free(iter->root);
	git3_str_dispose(&iter->current_path);
	git3_tree_free(iter->tree);
	if (iter->index)
		git3_index_snapshot_release(&iter->index_snapshot, iter->index);
	filesystem_iterator_clear(iter);
}
	...
	git3_iterator_options *options)
{
	...
	*out = NULL;
	...
	iter = git3__calloc(1, sizeof(filesystem_iterator));
	GIT3_ERROR_CHECK_ALLOC(iter);
	...
	iter->root = git3__malloc(root_len+2);
	GIT3_ERROR_CHECK_ALLOC(iter->root);
	...
	if ((error = git3_str_puts(&iter->current_path, iter->root)) < 0)
		goto on_error;

	if ((error = iterator_init_common(&iter->base, repo, index, options)) < 0)
		goto on_error;

	if (tree && (error = git3_tree_dup(&iter->tree, tree)) < 0)
		goto on_error;

	if (index &&
		(error = git3_index_snapshot_new(&iter->index_snapshot, index)) < 0)
		goto on_error;

	iter->index = index;
	iter->dirload_flags =
		(iterator__ignore_case(&iter->base) ?
			GIT3_FS_PATH_DIR_IGNORE_CASE : 0) |
		(iterator__flag(&iter->base, PRECOMPOSE_UNICODE) ?
			GIT3_FS_PATH_DIR_PRECOMPOSE_UNICODE : 0);
...
int git3_iterator_for_filesystem(
	git3_iterator **out,
	const char *root,
	git3_iterator_options *given_opts)
{
	git3_iterator_options options = GIT3_ITERATOR_OPTIONS_INIT;

	if (given_opts)
		memcpy(&options, given_opts, sizeof(git3_iterator_options));

	return iterator_for_filesystem(out,
		NULL, root, NULL, NULL, GIT3_ITERATOR_FS, &options);
}

int git3_iterator_for_workdir_ext(
	git3_iterator **out,
	git3_repository *repo,
	...
	git3_iterator_options *given_opts)
{
	...
	if (!repo_workdir) {
		if (git3_repository__ensure_not_bare(repo, "scan working directory") < 0)
			return GIT3_EBAREREPO;

		repo_workdir = git3_repository_workdir(repo);
	}
	...
	options.flags |= GIT3_ITERATOR_HONOR_IGNORES |
		GIT3_ITERATOR_IGNORE_DOT_GIT;
	...
	return iterator_for_filesystem(out,
		repo, repo_workdir, index, tree, GIT3_ITERATOR_WORKDIR, &options);
}
	...
	const char *path)
{
	...
	common_len = git3_fs_path_common_dirlen(prev_path, path);
	relative_path = path + common_len;

	if ((dirsep = strchr(relative_path, '/')) == NULL)
		return false;

	git3_str_clear(&iter->tree_buf);
	git3_str_put(&iter->tree_buf, path, (dirsep - path) + 1);

	iter->tree_entry.mode = GIT3_FILEMODE_TREE;
...
{
	GIT3_ASSERT(iterator__has_been_accessed(&iter->base));
	GIT3_ASSERT(S_ISDIR(iter->entry->mode));

	while (true) {
		const git3_index_entry *next_entry = NULL;
...
static int index_iterator_advance(
	const git3_index_entry **out, git3_iterator *i)
{
	index_iterator *iter = GIT3_CONTAINER_OF(i, index_iterator, base);
	const git3_index_entry *entry = NULL;
	bool is_submodule;
	int error = 0;
...
static int index_iterator_reset(git3_iterator *i)
{
	index_iterator *iter = GIT3_CONTAINER_OF(i, index_iterator, base);
...
static void index_iterator_free(git3_iterator *i)
{
	index_iterator *iter = GIT3_CONTAINER_OF(i, index_iterator, base);

	git3_index_snapshot_release(&iter->entries, iter->base.index);
	git3_str_dispose(&iter->tree_buf);
}

int git3_iterator_for_index(
	git3_iterator **out,
	git3_repository *repo,
	git3_index  *index,
	git3_iterator_options *options)
{
	...
	*out = NULL;
	...
	iter = git3__calloc(1, sizeof(index_iterator));
	GIT3_ERROR_CHECK_ALLOC(iter);

	iter->base.type = GIT3_ITERATOR_INDEX;
	iter->base.cb = &callbacks;

	if ((error = iterator_init_common(&iter->base, repo, index, options)) < 0 ||
		(error = git3_index_snapshot_new(&iter->entries, index)) < 0 ||
		(error = index_iterator_init(iter)) < 0)
		goto on_error;

	git3_vector_set_cmp(&iter->entries, iterator__ignore_case(&iter->base) ?
		git3_index_entry_icmp : git3_index_entry_cmp);
	git3_vector_sort(&iter->entries);
...
int git3_iterator_reset_range(
	git3_iterator *i, const char *start, const char *end)
{
...
int git3_iterator_set_ignore_case(git3_iterator *i, bool ignore_case)
{
	GIT3_ASSERT(!iterator__has_been_accessed(i));
...
void git3_iterator_free(git3_iterator *iter)
{
	if (iter == NULL)
		return;
	...
	git3_vector_dispose(&iter->pathlist);
	git3__free(iter->start);
	git3__free(iter->end);
...
int git3_iterator_foreach(
	git3_iterator *iterator,
	git3_iterator_foreach_cb cb,
	void *data)
{
	const git3_index_entry *iterator_item;
	int error = 0;

	if ((error = git3_iterator_current(&iterator_item, iterator)) < 0)
		goto done;
	...
	while (true) {
		if ((error = git3_iterator_advance(&iterator_item, iterator)) < 0)
			goto done;
...
int git3_iterator_walk(
	git3_iterator **iterators,
	size_t cnt,
	git3_iterator_walk_cb cb,
	void *data)
{
	const git3_index_entry **iterator_item;	/* next in each iterator */
	const git3_index_entry **cur_items;		/* current path in each iter */
	const git3_index_entry *first_match;
	size_t i, j;
	int error = 0;

	iterator_item = git3__calloc(cnt, sizeof(git3_index_entry *));
	cur_items = git3__calloc(cnt, sizeof(git3_index_entry *));

	GIT3_ERROR_CHECK_ALLOC(iterator_item);
	GIT3_ERROR_CHECK_ALLOC(cur_items);

	/* Set up the iterators */
	for (i = 0; i < cnt; i++) {
		error = git3_iterator_current(&iterator_item[i], iterators[i]);
	...
	while (true) {
		...
		for (i = 0; i < cnt; i++) {
			...
			if (first_match == NULL) {
				...
				int path_diff = git3_index_entry_cmp(iterator_item[i], first_match);
		...
		for (i = 0; i < cnt; i++) {
			...
			error = git3_iterator_advance(&iterator_item[i], iterators[i]);
	...
	git3__free((git3_index_entry **)iterator_item);
	git3__free((git3_index_entry **)cur_items);
