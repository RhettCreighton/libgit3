...
void hunk_message(size_t idx, const git3_blame_hunk *hunk, const char *fmt, ...)
{
	va_list arglist;
...
void check_blame_hunk_index(git3_repository *repo, git3_blame *blame, int idx,
		size_t start_line, size_t len, char boundary, const char *commit_id, const char *orig_path)
{
	char expected[GIT3_OID_SHA1_HEXSIZE+1] = {0}, actual[GIT3_OID_SHA1_HEXSIZE+1] = {0};
	const git3_blame_hunk *hunk = git3_blame_hunk_byindex(blame, idx);
	cl_assert(hunk);
	...
	if (!strncmp(commit_id, "0000", 4)) {
		...
		git3_object *obj;
		cl_git_pass(git3_revparse_single(&obj, repo, commit_id));
		git3_oid_fmt(expected, git3_object_id(obj));
	...
	git3_oid_fmt(actual, &hunk->final_commit_id);
