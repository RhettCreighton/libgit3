...
static git3_oid commit_ids[commit_count];
static git3_oid _head_id;
...
void test_revwalk_hidecb__initialize(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_head_id, commit_head, GIT3_OID_SHA1));

	for (i = 0; i < commit_count; i++)
		cl_git_pass(git3_oid_from_string(&commit_ids[i], commit_strs[i], GIT3_OID_SHA1));
...
/* Hide all commits */
static int hide_every_commit_cb(const git3_oid *commit_id, void *data)
{
	GIT3_UNUSED(commit_id);
	GIT3_UNUSED(data);
...
static int hide_none_cb(const git3_oid *commit_id, void *data)
{
	GIT3_UNUSED(commit_id);
	GIT3_UNUSED(data);
...
/* Hide some commits */
static int hide_commit_cb(const git3_oid *commit_id, void *data)
{
	GIT3_UNUSED(commit_id);
	GIT3_UNUSED(data);

	return (git3_oid_cmp(commit_id, &commit_ids[5]) == 0);
}

/* In payload data, pointer to a commit id is passed */
static int hide_commit_use_payload_cb(const git3_oid *commit_id, void *data)
{
	git3_oid *hide_commit_id = data;

	return (git3_oid_cmp(commit_id, hide_commit_id) == 0);
}
...
void test_revwalk_hidecb__hide_all_cb(void)
{
	...
	cl_git_pass(git3_revwalk_new(&walk, _repo));
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_every_commit_cb, NULL));
	cl_git_pass(git3_revwalk_push(walk, &_head_id));

	/* First call to git3_revwalk_next should return GIT3_ITEROVER */
	cl_assert_equal_i(GIT3_ITEROVER, git3_revwalk_next(&id, walk));
...
void test_revwalk_hidecb__hide_none_cb(void)
{
	...
	cl_git_pass(git3_revwalk_new(&walk, _repo));
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_none_cb, NULL));
	cl_git_pass(git3_revwalk_push(walk, &_head_id));
...
void test_revwalk_hidecb__unset_cb_before_walk(void)
{
	...
	cl_git_pass(git3_revwalk_new(&walk, _repo));
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_every_commit_cb, NULL));
	cl_git_pass(git3_revwalk_add_hide_cb(walk, NULL, NULL));
	cl_git_pass(git3_revwalk_push(walk, &_head_id));
...
void test_revwalk_hidecb__add_hide_cb_during_walking(void)
{
	...
	cl_git_pass(git3_revwalk_next(&id, walk));

	/* Now add hide callback */
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_none_cb, NULL));

	/* walk should be reset */
	error = git3_revwalk_next(&id, walk);
	cl_assert_equal_i(error, GIT3_ITEROVER);
...
void test_revwalk_hidecb__hide_some_commits(void)
{
	...
	/* Add hide callback */
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_commit_cb, NULL));
...
void test_revwalk_hidecb__test_payload(void)
{
	...
	cl_git_pass(git3_revwalk_add_hide_cb(walk, hide_commit_use_payload_cb, &commit_ids[5]));
