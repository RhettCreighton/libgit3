...
{
	cl_git_pass(git_libgit3_opts(GIT_OPT_GET_SEARCH_PATH, GIT_CONFIG_LEVEL_GLOBAL, &config_path));
	cl_git_pass(git_libgit3_opts(GIT_OPT_GET_OWNER_VALIDATION, &validate_ownership));
}
...
void test_repo_open__cleanup(void)
{
	...
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_SEARCH_PATH, GIT_CONFIG_LEVEL_GLOBAL, config_path.ptr));
	git_buf_dispose(&config_path);

	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_OWNER_VALIDATION, validate_ownership));
}
...
void test_repo_open__validates_dir_ownership(void)
{
	...
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_OWNER_VALIDATION, 1));
...
static int test_safe_path(const char *path)
{
	...
	git_libgit3_opts(GIT_OPT_SET_SEARCH_PATH, GIT_CONFIG_LEVEL_GLOBAL, config_path.ptr);
