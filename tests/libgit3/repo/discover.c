...
{
	git3_buf found_path = GIT3_BUF_INIT;
	git3_str resolved = GIT3_STR_INIT;

	git3_str_attach(&resolved, p_realpath(expected_path, NULL), 0);
	cl_assert(resolved.size > 0);
	cl_git_pass(git3_fs_path_to_dir(&resolved));
	cl_git_pass(git3_repository_discover(&found_path, start_path, 1, ceiling_dirs));

	cl_assert_equal_s(found_path.ptr, resolved.ptr);

	git3_str_dispose(&resolved);
	git3_buf_dispose(&found_path);
}
...
static void write_file(const char *path, const char *content)
{
	...
	file = git3_futils_creat_withpath(path, 0777, 0666);
	cl_assert(file >= 0);
...
static void append_ceiling_dir(git3_str *ceiling_dirs, const char *path)
{
	git3_str pretty_path = GIT3_STR_INIT;
	char ceiling_separator[2] = { GIT3_PATH_LIST_SEPARATOR, '\0' };

	cl_git_pass(git3_fs_path_prettify_dir(&pretty_path, path, NULL));

	if (ceiling_dirs->size > 0)
		git3_str_puts(ceiling_dirs, ceiling_separator);

	git3_str_puts(ceiling_dirs, pretty_path.ptr);

	git3_str_dispose(&pretty_path);
	cl_assert(git3_str_oom(ceiling_dirs) == 0);
}

static git3_buf discovered;
static git3_str ceiling_dirs;

void test_repo_discover__initialize(void)
{
	git3_repository *repo;
	const mode_t mode = 0777;
	git3_futils_mkdir_r(DISCOVER_FOLDER, mode);

	git3_str_init(&ceiling_dirs, 0);
	append_ceiling_dir(&ceiling_dirs, TEMP_REPO_FOLDER);

	cl_git_pass(git3_repository_init(&repo, DISCOVER_FOLDER, 1));
	git3_repository_free(repo);

	cl_git_pass(git3_repository_init(&repo, SUB_REPOSITORY_FOLDER, 0));
	cl_git_pass(git3_futils_mkdir_r(SUB_REPOSITORY_FOLDER_SUB_SUB_SUB, mode));
	cl_git_pass(git3_futils_mkdir_r(SUB_REPOSITORY_FOLDER_SUB_SUB_SUB, mode));

	cl_git_pass(git3_futils_mkdir_r(REPOSITORY_ALTERNATE_FOLDER_SUB_SUB_SUB, mode));
	...
	cl_git_pass(git3_futils_mkdir_r(ALTERNATE_MALFORMED_FOLDER1, mode));
	write_file(ALTERNATE_MALFORMED_FOLDER1 "/" DOT_GIT, "Anything but not gitdir:");
	cl_git_pass(git3_futils_mkdir_r(ALTERNATE_MALFORMED_FOLDER2, mode));
	write_file(ALTERNATE_MALFORMED_FOLDER2 "/" DOT_GIT, "gitdir:");
	cl_git_pass(git3_futils_mkdir_r(ALTERNATE_MALFORMED_FOLDER3, mode));
	write_file(ALTERNATE_MALFORMED_FOLDER3 "/" DOT_GIT, "gitdir: \n\n\n");
	cl_git_pass(git3_futils_mkdir_r(ALTERNATE_NOT_FOUND_FOLDER, mode));
...
{
	git3_buf_dispose(&discovered);
	git3_str_dispose(&ceiling_dirs);
	cl_git_pass(git3_futils_rmdir_r(TEMP_REPO_FOLDER, NULL, GIT3_RMDIR_REMOVE_FILES));
}

void test_repo_discover__discovering_repo_with_exact_path_succeeds(void)
{
	cl_git_pass(git3_repository_discover(&discovered, DISCOVER_FOLDER, 0, ceiling_dirs.ptr));
	git3_buf_dispose(&discovered);

	cl_git_pass(git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER, 0, ceiling_dirs.ptr));
	git3_buf_dispose(&discovered);
}

void test_repo_discover__discovering_nonexistent_dir_fails(void)
{
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, DISCOVER_FOLDER "-nonexistent", 0, NULL));
}
...
{
	cl_git_fail(git3_repository_discover(&discovered, ALTERNATE_MALFORMED_FOLDER1, 0, ceiling_dirs.ptr));
	cl_git_fail(git3_repository_discover(&discovered, ALTERNATE_MALFORMED_FOLDER2, 0, ceiling_dirs.ptr));
	cl_git_fail(git3_repository_discover(&discovered, ALTERNATE_MALFORMED_FOLDER3, 0, ceiling_dirs.ptr));
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, ALTERNATE_NOT_FOUND_FOLDER, 0, ceiling_dirs.ptr));
}
...
void test_repo_discover__discovering_repository_with_ceiling(void)
{
	...
	ensure_repository_discover(SUB_REPOSITORY_FOLDER_SUB, ceiling_dirs.ptr, SUB_REPOSITORY_GITDIR);
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER_SUB_SUB, 0, ceiling_dirs.ptr));
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER_SUB_SUB_SUB, 0, ceiling_dirs.ptr));
}
...
void test_repo_discover__other_ceiling(void)
{
	...
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER_SUB, 0, ceiling_dirs.ptr));
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER_SUB_SUB, 0, ceiling_dirs.ptr));
	cl_assert_equal_i(GIT3_ENOTFOUND, git3_repository_discover(&discovered, SUB_REPOSITORY_FOLDER_SUB_SUB_SUB, 0, ceiling_dirs.ptr));
}
...
void test_repo_discover__discovery_starting_at_system_root_causes_no_hang(void)
{
	...
	cl_git_fail(git3_repository_discover(&out, "C:/", 0, NULL));
	cl_git_fail(git3_repository_discover(&out, "//localhost/", 0, NULL));
#endif
}
