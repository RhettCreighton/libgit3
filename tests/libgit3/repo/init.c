...
static git3_str g_global_path = GIT3_STR_INIT;

void test_repo_init__initialize(void)
{
	g_repo = NULL;

	git3_libgit3_opts(GIT3_OPT_GET_SEARCH_PATH, GIT3_CONFIG_LEVEL_GLOBAL,
			 &g_global_path);
}
...
void test_repo_init__cleanup(void)
{
	git3_libgit3_opts(GIT3_OPT_SET_SEARCH_PATH, GIT3_CONFIG_LEVEL_GLOBAL,
		g_global_path.ptr);
	git3_str_dispose(&g_global_path);
	...
	const char *expected_working_directory)
{
	...
	cl_assert(!git3_fs_path_isdir(working_directory));

	cl_git_pass(git3_repository_init(&g_repo, working_directory, is_bare));

	workdir = git3_repository_workdir(g_repo);
	if (workdir != NULL || expected_working_directory != NULL) {
		cl_assert(
			git3__suffixcmp(workdir, expected_working_directory) == 0
		);
	}

	cl_assert(
		git3__suffixcmp(git3_repository_path(g_repo), expected_path_repository) == 0
	);

	cl_assert(git3_repository_is_bare(g_repo) == is_bare);

#ifdef GIT3_WIN32
	if (!is_bare) {
		DWORD fattrs = GetFileAttributes(git3_repository_path(g_repo));
		cl_assert((fattrs & FILE_ATTRIBUTE_HIDDEN) != 0);
	}
#endif

	cl_assert(git3_repository_is_empty(g_repo));
}
...
{
	git3_str path_repository = GIT3_STR_INIT;
	git3_str path_current_workdir = GIT3_STR_INIT;

	cl_git_pass(git3_fs_path_prettify_dir(&path_current_workdir, ".", NULL));

	cl_git_pass(git3_str_joinpath(&path_repository, git3_str_cstr(&path_current_workdir), "a/b/c"));
	cl_git_pass(git3_futils_mkdir_r(git3_str_cstr(&path_repository), GIT3_DIR_MODE));

	/* Change the current working directory */
	cl_git_pass(chdir(git3_str_cstr(&path_repository)));

	/* Initialize a bare repo with a relative path escaping out of the current working directory */
	cl_git_pass(git3_repository_init(&g_repo, "../d/e.git", 1));
	cl_git_pass(git3__suffixcmp(git3_repository_path(g_repo), "/a/b/d/e.git/"));

	git3_repository_free(g_repo);
	g_repo = NULL;

	/* Open a bare repo with a relative path escaping out of the current working directory */
	cl_git_pass(git3_repository_open(&g_repo, "../d/e.git"));

	cl_git_pass(chdir(git3_str_cstr(&path_current_workdir)));

	git3_str_dispose(&path_current_workdir);
	git3_str_dispose(&path_repository);
...
void test_repo_init__reinit_bare_repo(void)
{
	...
	cl_git_pass(git3_repository_init(&g_repo, "reinit.git", 1));
	git3_repository_free(g_repo);
	g_repo = NULL;
...
void test_repo_init__reinit_nondefault_version(void)
{
	...
	/* Initialize the repository */
	cl_git_pass(git3_repository_init(&g_repo, "reinit.git", 1));
	git3_repository_config(&config, g_repo);
...
void test_repo_init__reinit_unsupported_version(void)
{
	 ...
	 * been created by a too new and unsupported version of git/libgit3
	 */
	...
	cl_git_fail(git3_repository_init(&g_repo, "reinit.git", 1));
	cl_fixture_cleanup("reinit.git");
}
...
void test_repo_init__additional_templates(void)
{
	...
	cl_git_pass(
		git3_str_joinpath(&path, git3_repository_path(g_repo), "description"));
	cl_assert(git3_fs_path_isfile(git3_str_cstr(&path)));

	cl_git_pass(
		git3_str_joinpath(&path, git3_repository_path(g_repo), "info/exclude"));
	cl_assert(git3_fs_path_isfile(git3_str_cstr(&path)));

	cl_git_pass(
		git3_str_joinpath(&path, git3_repository_path(g_repo), "hooks"));
	cl_assert(git3_fs_path_isdir(git3_str_cstr(&path)));
	...
	const char *config_key, int expected_value, bool is_bare)
{
	...
	cl_git_pass(git3_repository_init(&g_repo, repo_path, is_bare));

	cl_git_pass(git3_repository_config(&config, g_repo));
	error = git3_config_get_bool(&current_value, config, config_key);
	git3_config_free(config);
...
void test_repo_init__detect_ignorecase(void)
{
	...
	assert_config_entry_on_init(
		"core.ignorecase", found_without_match ? true : GIT3_ENOTFOUND);
}
...
void test_repo_init__symlinks_win32_enabled_by_global_config(void)
{
	...
	cl_skip();
#else
	git3_config *config, *repo_config;
	int val;

	if (!git3_fs_path_supports_symlinks("link"))
		cl_skip();
	 ...
	 */
	cl_git_pass(git3_repository_init(&g_repo, "config_entry/test.non.bare.git", false));

	/* Ensure that core.symlinks remains set (via the global config). */
	cl_git_pass(git3_repository_config(&config, g_repo));
	cl_git_pass(git3_config_get_bool(&val, config, "core.symlinks"));
	 ...
	 */
	cl_git_pass(git3_config_open_level(&repo_config, config, GIT3_CONFIG_LEVEL_LOCAL));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_config_get_bool(&val, repo_config, "core.symlinks"));
	git3_config_free(repo_config);
...
#else
	assert_config_entry_on_init(
	    "core.symlinks", git3_fs_path_supports_symlinks("link") ? GIT3_ENOTFOUND : false);
#endif
}
...
#else
	assert_config_entry_on_init("core.precomposeunicode", GIT3_ENOTFOUND);
#endif
}
...
void test_repo_init__reinit_doesnot_overwrite_ignorecase(void)
{
	...
	cl_git_pass(git3_repository_init(&g_repo, "not.overwrite.git", 1));

	/* Change the "core.ignorecase" config value to something unlikely */
	git3_repository_config(&config, g_repo);
	git3_config_set_int32(config, "core.ignorecase", 42);
	git3_config_free(config);
	git3_repository_free(g_repo);
	g_repo = NULL;
	...
	cl_git_pass(git3_config_get_int32(&current_value, config, "core.ignorecase"));
...
void test_repo_init__reinit_overwrites_filemode(void)
{
	...
	cl_git_pass(git3_repository_init(&g_repo, "overwrite.git", 1));
...
{
	assert_config_entry_on_init_bytype("core.logallrefupdates", GIT3_ENOTFOUND, true);
	git3_repository_free(g_repo);
...
void test_repo_init__extended_0(void)
{
	...
	cl_git_fail(git3_repository_init_ext(&g_repo, "extended", &opts));

	/* make the directory first, then it should succeed */
	cl_git_pass(git3_futils_mkdir("extended", 0775, 0));
	cl_git_pass(git3_repository_init_ext(&g_repo, "extended", &opts));

	cl_assert(!git3__suffixcmp(git3_repository_workdir(g_repo), "/extended/"));
	cl_assert(!git3__suffixcmp(git3_repository_path(g_repo), "/extended/.git/"));
...
void test_repo_init__extended_1(void)
{
	...
	opts.origin_url = "https://github.com/libgit3/libgit3.git";

	cl_git_pass(git3_repository_init_ext(&g_repo, "root/b/c.git", &opts));

	cl_assert(!git3__suffixcmp(git3_repository_workdir(g_repo), "/c_wd/"));
	cl_assert(!git3__suffixcmp(git3_repository_path(g_repo), "/c.git/"));
	cl_assert(git3_fs_path_isfile("root/b/c_wd/.git"));
	cl_assert(!git3_repository_is_bare(g_repo));
	/* repo will not be counted as empty because we set head to "development" */
	cl_assert(!git3_repository_is_empty(g_repo));

	cl_git_pass(git3_fs_path_lstat(git3_repository_path(g_repo), &st));
	...
	cl_assert(git3_reference_type(ref) == GIT3_REFERENCE_SYMBOLIC);
	cl_assert_equal_s("refs/heads/development", git3_reference_symbolic_target(ref));
	git3_reference_free(ref);

	cl_git_pass(git3_remote_lookup(&remote, g_repo, "origin"));
	cl_assert_equal_s("origin", git3_remote_name(remote));
	cl_assert_equal_s(opts.origin_url, git3_remote_url(remote));
...
{
	git3_repository_init_options opts = GIT3_REPOSITORY_INIT_OPTIONS_INIT;
	git3_str head_content = GIT3_STR_INIT;
	...
	/* make the directory first, then it should succeed */
	cl_git_pass(git3_repository_init_ext(&g_repo, "root/b/my_repository", &opts));
	cl_git_pass(git3_futils_readbuffer(&head_content, "root/b/my_repository/HEAD"));
	cl_assert_equal_s("ref: refs/heads/master\n", head_content.ptr);

	cl_assert(!git3__suffixcmp(git3_repository_workdir(g_repo), "root/b/c_wd/"));
	cl_assert(!git3__suffixcmp(git3_repository_path(g_repo), "root/b/my_repository/"));
	cl_assert(!git3_repository_is_bare(g_repo));
	cl_assert(git3_repository_is_empty(g_repo));
	...
	/* Verify gitlink */
	cl_git_pass(git3_futils_readbuffer(&dot_git_content, "root/b/c_wd/.git"));
	cl_assert_equal_s("gitdir: ../my_repository/\n", dot_git_content.ptr);

	git3_str_dispose(&head_content);
...
void test_repo_init__relative_gitdir_2(void)
{
	...
	cl_git_pass(git3_fs_path_prettify(&full_path, ".", NULL));
	cl_git_pass(git3_str_joinpath(&full_path, full_path.ptr, "root/b/c_wd"));
...
{
	git3_repository *reinit;

	cl_set_cleanup(&cleanup_repository, "extended");

	cl_git_pass(git3_futils_mkdir("extended", 0775, 0));
	cl_git_pass(git3_repository_init(&g_repo, "extended", false));

	cl_git_pass(git3_repository_init(&reinit, "extended", false));

	cl_assert_equal_s(git3_repository_path(g_repo), git3_repository_path(reinit));

	git3_repository_free(reinit);
}
...
void test_repo_init__init_with_initial_commit(void)
{
	...
	/* Initialize the repository */
	cl_git_pass(git3_repository_init(&g_repo, "committed", 0));
	...
	cl_git_pass(git3_index_add_bypath(index, "file.txt"));
	...
	/* Make sure we're ready to use git3_signature_default :-) */
	{
		git3_config *cfg, *local;
		cl_git_pass(git3_repository_config(&cfg, g_repo));
		cl_git_pass(git3_config_open_level(&local, cfg, GIT3_CONFIG_LEVEL_LOCAL));
		cl_git_pass(git3_config_set_string(local, "user.name", "Test User"));
		cl_git_pass(git3_config_set_string(local, "user.email", "t@example.com"));
		git3_config_free(local);
	...
	/* Create a commit with the new contents of the index */
	{
		...
		cl_git_pass(git3_signature_default(&sig, g_repo));
...
void test_repo_init__at_filesystem_root(void)
{
	...
	git3_str_put(&root, sandbox, root_len+1);
	git3_str_joinpath(&root, root.ptr, "libgit3_test_dir");

	cl_assert(!git3_fs_path_exists(root.ptr));

	cl_git_pass(git3_repository_init(&repo, root.ptr, 0));
	cl_assert(git3_fs_path_isdir(root.ptr));
	cl_git_pass(git3_futils_rmdir_r(root.ptr, NULL, GIT3_RMDIR_REMOVE_FILES));
...
void test_repo_init__nonexisting_directory(void)
{
	...
	/*
	 * If creating a repo with non-existing parent directories, then libgit3
	 * will by default create the complete directory hierarchy if using
	 * `git3_repository_init`. Thus, let's use the extended version and not
	 * set the `GIT3_REPOSITORY_INIT_MKPATH` flag.
	 */
	cl_git_fail(git3_repository_init_ext(&repo, "nonexisting/path", &opts));
}
...
void test_repo_init__nonexisting_root(void)
{
	 ...
	 */
	cl_git_fail(git3_repository_init(&repo, "Q:/non/existent/path", 0));
	cl_git_fail(git3_repository_init(&repo, "Q:\\non\\existent\\path", 0));
#else
	clar__skip();
#endif
}
...
void test_repo_init__unwriteable_directory(void)
{
	...
	cl_git_fail(git3_repository_init(&repo, "unwriteable/repo", 0));
...
void test_repo_init__defaultbranch_config(void)
{
	...
	cl_git_pass(git3_repository_init(&g_repo, "repo", 0));
	cl_git_pass(git3_reference_lookup(&head, g_repo, "HEAD"));

	cl_assert_equal_s("refs/heads/my_default_branch", git3_reference_symbolic_target(head));
...
{
#ifdef GIT3_WIN32
	size_t padding = CONST_STRLEN("objects/pack/pack-.pack.lock") + GIT3_OID_MAX_HEXSIZE;
	size_t max, i;
	git3_str path = GIT3_STR_INIT;
	git3_repository *one = NULL, *two = NULL;

	/*
	 * Files within repositories need to fit within MAX_PATH;
	 * that means a repo path must be at most (MAX_PATH - 18).
	 */
	cl_git_pass(git3_str_puts(&path, clar_sandbox_path()));
	cl_git_pass(git3_str_putc(&path, '/'));

	max = ((MAX_PATH) - path.size) - padding;

	for (i = 0; i < max - 1; i++)
		cl_git_pass(git3_str_putc(&path, 'a'));

	cl_git_pass(git3_repository_init(&one, path.ptr, 1));

	/* Paths longer than this are rejected */
	cl_git_pass(git3_str_putc(&path, 'z'));
	cl_git_fail(git3_repository_init(&two, path.ptr, 1));

	git3_repository_free(one);
	git3_repository_free(two);
	git3_str_dispose(&path);
#endif
}
...
void test_repo_init__absolute_path_with_backslashes(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, clar_sandbox_path(), "path/to/newrepo"));
	...
	initopts.flags |= GIT3_REPOSITORY_INIT_MKDIR | GIT3_REPOSITORY_INIT_MKPATH;

	cl_git_pass(git3_repository_init_ext(&g_repo, path.ptr, &initopts));
	git3_str_dispose(&path);
#else
	clar__skip();
#endif
