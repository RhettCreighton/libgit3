...
void test_repo_config__cleanup(void)
{
	...
	cl_assert(!git3_fs_path_isdir("alternate"));
...
void test_repo_config__can_open_global_when_there_is_no_file(void)
{
	...
	cl_git_pass(git3_config_open_level(
		&global, config, GIT3_CONFIG_LEVEL_GLOBAL));

	cl_git_pass(git3_config_set_string(global, "test.set", "42"));
...
void test_repo_config__can_open_missing_global_with_separators(void)
{
	...
	cl_git_pass(git3_str_printf(
		&path, "%c%s", GIT3_PATH_LIST_SEPARATOR, "dummy"));
...
void test_repo_config__read_with_no_configs_at_all(void)
{
	...
	cl_assert(!git3_fs_path_isfile("empty_standard_repo/.git/config"));

	cl_git_pass(git3_repository_open(&repo, "empty_standard_repo"));
	git3_repository__configmap_lookup_cache_clear(repo);
	val = -1;
	cl_git_pass(git3_repository__configmap_lookup(&val, repo, GIT3_CONFIGMAP_ABBREV));
	cl_assert_equal_i(GIT3_ABBREV_DEFAULT, val);
	git3_repository_free(repo);
	...
	cl_git_pass(git3_str_joinpath(&path, path.ptr, "1"));
	...
	cl_assert(!git3_fs_path_isfile("alternate/1/gitconfig"));

	cl_must_pass(p_unlink("alternate/2/config"));
	cl_assert(!git3_fs_path_isfile("alternate/2/config"));

	cl_must_pass(p_unlink("alternate/3/.gitconfig"));
	cl_assert(!git3_fs_path_isfile("alternate/3/.gitconfig"));
	...
	cl_assert(!git3_fs_path_exists("empty_standard_repo/.git/config"));
	cl_assert(!git3_fs_path_exists("alternate/3/.gitconfig"));
}
