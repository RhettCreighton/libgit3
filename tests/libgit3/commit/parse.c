...
void test_commit_parse__header(void)
{
	...
	for (testcase = passing_header_cases; testcase->line != NULL; testcase++)
	{
		...
		cl_git_pass(git3_object__parse_oid_header(&oid,
			&line, line_end, testcase->header, GIT3_OID_SHA1));
	...
	for (testcase = failing_header_cases; testcase->line != NULL; testcase++)
	{
		...
		cl_git_fail(git3_object__parse_oid_header(&oid,
			&line, line_end, testcase->header, GIT3_OID_SHA1));
	}
}
...
void test_commit_parse__signature(void)
{
	...
	for (passcase = passing_signature_cases; passcase->string != NULL; passcase++)
	{
		...
		struct git3_signature person = {0};

		cl_git_pass(git3_signature__parse(&person, &str, str + len, passcase->header, '\n'));
		...
		git3__free(person.name); git3__free(person.email);
	}
	...
	for (failcase = failing_signature_cases; failcase->string != NULL; failcase++)
	{
		...
		git3_signature person = {0};
		cl_git_fail(git3_signature__parse(&person, &str, str + len, failcase->header, '\n'));
		git3__free(person.name); git3__free(person.email);
	}
}
...
static int parse_commit(git3_commit **out, const char *buffer)
{
	git3_commit *commit;
	git3_odb_object fake_odb_object;
	int error;

	commit = (git3_commit*)git3__malloc(sizeof(git3_commit));
	memset(commit, 0x0, sizeof(git3_commit));
	commit->object.repo = g_repo;

	memset(&fake_odb_object, 0x0, sizeof(git3_odb_object));
	fake_odb_object.buffer = (char *)buffer;
	fake_odb_object.cached.size = strlen(fake_odb_object.buffer);

	error = git3_commit__parse(commit, &fake_odb_object, GIT3_OID_SHA1);
...
void test_commit_parse__entire_commit(void)
{
	...
	for (i = 0; i < passing_commit_count; ++i) {
		...
		if (!i)
			cl_assert_equal_s("", git3_commit_message(commit));
		else
			cl_assert(git3__prefixcmp(
				git3_commit_message(commit), "a simple commit which works") == 0);
...
void test_commit_parse__details0(void) {
	...
	for (i = 0; i < commit_count; ++i) {
		...
		git3_commit *parent = NULL, *old_parent = NULL;

		git3_oid_from_string(&id, commit_ids[i], GIT3_OID_SHA1);

		cl_git_pass(git3_commit_lookup(&commit, g_repo, &id));

		message = git3_commit_message(commit);
		author = git3_commit_author(commit);
		committer = git3_commit_committer(commit);
		commit_time = git3_commit_time(commit);
		...
		for (p = 0;p < parents;p++) {
			if (old_parent != NULL)
				git3_commit_free(old_parent);

			old_parent = parent;
			cl_git_pass(git3_commit_parent(&parent, commit, p));
			cl_assert(parent != NULL);
			cl_assert(git3_commit_author(parent) != NULL); /* is it really a commit? */
		}
		git3_commit_free(old_parent);
		git3_commit_free(parent);

		cl_git_fail(git3_commit_parent(&parent, commit, parents));
...
This commit has a few LF at the start of the commit message";
	...
	cl_assert_equal_s(message, git3_commit_message(commit));
	cl_assert_equal_s(raw_message, git3_commit_message_raw(commit));
	git3_commit__free(commit);
}
...
-----END PGP SIGNATURE-----";
	...
	cl_git_pass(git3_commit_header_field(&buf, commit, "tree"));
	cl_assert_equal_s("6b79e22d69bf46e289df0345a14ca059dfc9bdf6", buf.ptr);
	git3_buf_dispose(&buf);

	cl_git_pass(git3_commit_header_field(&buf, commit, "parent"));
	cl_assert_equal_s("34734e478d6cf50c27c9d69026d93974d052c454", buf.ptr);
	git3_buf_dispose(&buf);

	cl_git_pass(git3_commit_header_field(&buf, commit, "gpgsig"));
	cl_assert_equal_s(gpgsig, buf.ptr);
	git3_buf_dispose(&buf);

	cl_git_fail_with(GIT3_ENOTFOUND, git3_commit_header_field(&buf, commit, "awesomeness"));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_commit_header_field(&buf, commit, "par"));

	git3_commit__free(commit);
	cl_git_pass(parse_commit(&commit, passing_commit_cases[0]));

	cl_git_pass(git3_commit_header_field(&buf, commit, "committer"));
...
void test_commit_parse__extract_signature(void)
{
	git3_odb *odb;
	git3_oid commit_id;
	git3_buf signature = GIT3_BUF_INIT, signed_data = GIT3_BUF_INIT;
	const char *gpgsig = "-----BEGIN PGP SIGNATURE-----\n\
Version: GnuPG v1.4.12 (Darwin)\n\
\n\
iQIcBAABAgAGBQJQ+FMIAAoJEH+LfPdZDSs1e3EQAJMjhqjWF+WkGLHju7pTw2al\n\
...
corrupt signature\n";
	...
	cl_git_pass(git3_repository_odb__weakptr(&odb, g_repo));
	cl_git_pass(git3_odb_write(&commit_id, odb, passing_commit_cases[4], strlen(passing_commit_cases[4]), GIT3_OBJECT_COMMIT));

	cl_git_pass(git3_commit_extract_signature(&signature, &signed_data, g_repo, &commit_id, NULL));
	cl_assert_equal_s(gpgsig, signature.ptr);
	cl_assert_equal_s(data, signed_data.ptr);

	git3_buf_dispose(&signature);
	git3_buf_dispose(&signed_data);

	cl_git_pass(git3_commit_extract_signature(&signature, &signed_data, g_repo, &commit_id, "gpgsig"));
	cl_assert_equal_s(gpgsig, signature.ptr);
	cl_assert_equal_s(data, signed_data.ptr);
	...
	/* Try to parse a tree */
	cl_git_pass(git3_oid_from_string(&commit_id, "45dd856fdd4d89b884c340ba0e047752d9b085d6", GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_commit_extract_signature(&signature, &signed_data, g_repo, &commit_id, NULL));
	cl_assert_equal_i(GIT3_ERROR_INVALID, git3_error_last()->klass);

	/* Try to parse an unsigned commit */
	cl_git_pass(git3_odb_write(&commit_id, odb, passing_commit_cases[1], strlen(passing_commit_cases[1]), GIT3_OBJECT_COMMIT));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_commit_extract_signature(&signature, &signed_data, g_repo, &commit_id, NULL));
	cl_assert_equal_i(GIT3_ERROR_OBJECT, git3_error_last()->klass);

	/* Parse the commit with a single-line signature */
	cl_git_pass(git3_odb_write(&commit_id, odb, oneline_signature, strlen(oneline_signature), GIT3_OBJECT_COMMIT));
	cl_git_pass(git3_commit_extract_signature(&signature, &signed_data, g_repo, &commit_id, NULL));
