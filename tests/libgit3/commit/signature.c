...
static int try_build_signature(const char *name, const char *email, git3_time_t time, int offset)
{
	git3_signature *sign;
	int error = 0;

	if ((error =  git3_signature_new(&sign, name, email, time, offset)) < 0)
		return error;

	git3_signature_free((git3_signature *)sign);
	...
	const char *email)
{
	...
	cl_git_pass(git3_signature_new(&sign, name, email, 1234567890, 60));
	cl_assert_equal_s(expected_name, sign->name);
	cl_assert_equal_s(expected_email, sign->email);

	git3_signature_free(sign);
}
...
void test_commit_signature__timezone_does_not_read_oob(void)
{
	...
	cl_git_pass(git3_signature__parse(sig, &header, header_end, NULL, '\0'));
...
void test_commit_signature__create_zero_char(void)
{
	...
	git3_signature *sign;
	cl_git_fail(git3_signature_new(&sign, "", "x@y.z", 1234567890, 60));
	cl_assert(sign == NULL);
}
...
void test_commit_signature__from_buf(void)
{
	...
	cl_git_pass(git3_signature_from_buffer(&sign, "Test User <test@test.tt> 1461698487 +0200"));
...
void test_commit_signature__from_buf_with_neg_zero_offset(void)
{
	...
	cl_git_pass(git3_signature_from_buffer(&sign, "Test User <test@test.tt> 1461698487 -0000"));
	cl_assert_equal_s("Test User", sign->name);
	cl_assert_equal_s("test@test.tt", sign->email);
	cl_assert_equal_i(1461698487, sign->when.time);
...
{
	git3_signature *with_neg_zero;
	git3_signature *with_pos_zero;

	cl_git_pass(git3_signature_from_buffer(&with_neg_zero, "Test User <test@test.tt> 1461698487 -0000"));
	cl_git_pass(git3_signature_from_buffer(&with_pos_zero, "Test User <test@test.tt> 1461698487 +0000"));

	cl_assert(!git3_signature__equal(with_neg_zero, with_pos_zero));

	git3_signature_free((git3_signature *)with_neg_zero);
	git3_signature_free((git3_signature *)with_pos_zero);
}
...
{
	git3_signature *author_sign, *committer_sign;
	...
	cl_setenv("GIT3_AUTHOR_NAME", NULL);
	cl_setenv("GIT3_AUTHOR_EMAIL", NULL);
	cl_setenv("GIT3_COMMITTER_NAME", NULL);
	cl_setenv("GIT3_COMMITTER_EMAIL", NULL);
	cl_git_fail(git3_signature_default_from_env(&author_sign, &committer_sign, g_repo));
	/* Name is read from configuration and email is read from fallback EMAIL
	 * environment variable */
	cl_git_pass(git3_config_set_string(local, "user.name", "Name (config)"));
	cl_setenv("EMAIL", "email-envvar@example.com");
	cl_git_pass(git3_signature_default_from_env(&author_sign, &committer_sign, g_repo));
	...
	cl_setenv("EMAIL", NULL);
	git3_signature_free(author_sign);
	git3_signature_free(committer_sign);
	/* Environment variables have precedence over configuration */
	cl_git_pass(git3_config_set_string(local, "user.email", "config@example.com"));
	cl_setenv("GIT3_AUTHOR_NAME", "Author (envvar)");
	cl_setenv("GIT3_AUTHOR_EMAIL", "author-envvar@example.com");
	cl_setenv("GIT3_COMMITTER_NAME", "Committer (envvar)");
	cl_setenv("GIT3_COMMITTER_EMAIL", "committer-envvar@example.com");
	cl_git_pass(git3_signature_default_from_env(&author_sign, &committer_sign, g_repo));
	...
	cl_setenv("GIT3_AUTHOR_DATE", "1971-02-03 04:05:06+01");
	cl_setenv("GIT3_COMMITTER_DATE", "1988-09-10 11:12:13-01");
	cl_git_pass(git3_signature_default_from_env(&author_sign, &committer_sign, g_repo));
	...
	git3_signature_free(author_sign);
	git3_signature_free(committer_sign);
	cl_setenv("GIT3_AUTHOR_DATE", NULL);
	cl_setenv("GIT3_COMMITTER_DATE", NULL);
