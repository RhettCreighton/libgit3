...
static int unlink_file(void *payload, git3_str *path)
{
	char *fn;

	cl_assert(fn = git3_fs_path_basename(path->ptr));
...
void test_checkout_crlf__initialize(void)
{
	 ...
	 * check out over it.
	 */
	cl_git_pass(git3_str_puts(&reponame, "crlf"));
	cl_git_pass(git3_fs_path_direach(&reponame, 0, unlink_file, NULL));
...
static int compare_file(void *payload, git3_str *actual_path)
{
	git3_str expected_path = GIT3_STR_INIT;
	git3_str actual_contents = GIT3_STR_INIT;
	...
	if (!git3_fs_path_isfile(expected_path.ptr) ||
		!git3_fs_path_isfile(actual_path->ptr))
		goto done;

	if (git3_futils_readbuffer(&actual_contents, actual_path->ptr) < 0 ||
		git3_futils_readbuffer(&expected_contents, expected_path.ptr) < 0)
		goto done;
	...
	if (failed) {
		...
		git3_str_printf(&details, "filename=%s, system=%s, autocrlf=%s, attrs={%s}",
			git3_fs_path_basename(actual_path->ptr), systype, cd->autocrlf, cd->attrs);
	...
	git3_str_dispose(&actual_contents);
...
static void test_checkout(const char *autocrlf, const char *attrs)
{
	...
	git3_str systype_and_direction = GIT3_STR_INIT;
	...
	cl_git_pass(git3_str_puts(&systype_and_direction, systype));
	cl_git_pass(git3_str_puts(&systype_and_direction, "_to_workdir"));

	cl_git_pass(git3_str_puts(&sandboxname, "autocrlf_"));
	cl_git_pass(git3_str_puts(&sandboxname, autocrlf));

	if (*attrs) {
		cl_git_pass(git3_str_puts(&sandboxname, ","));

		for (c = attrs; *c; c++) {
			if (*c == ' ')
				cl_git_pass(git3_str_putc(&sandboxname, ','));
			else if (*c == '=')
				cl_git_pass(git3_str_putc(&sandboxname, '_'));
			else
				cl_git_pass(git3_str_putc(&sandboxname, *c));
		}

		cl_git_pass(git3_str_printf(&attrbuf, "* %s\n", attrs));
		cl_git_mkfile("crlf/.gitattributes", attrbuf.ptr);
	}

	cl_repo_set_string(g_repo, "core.autocrlf", autocrlf);

	cl_git_pass(git3_str_joinpath(&expected_dirname, systype_and_direction.ptr, sandboxname.ptr));
	cl_git_pass(git3_str_joinpath(&expected_fixture, "crlf_data", expected_dirname.ptr));
	...
	cl_git_pass(git3_fs_path_direach(&reponame, 0, compare_file, &compare_data));
	...
	git3_str_dispose(&systype_and_direction);
	git3_str_dispose(&reponame);
}
...
static void empty_workdir(const char *name)
{
	...
	cl_git_pass(git3_fs_path_dirload(&contents, name, 0, 0));
	git3_vector_foreach(&contents, i, fn) {
		cl_assert(basename = git3_fs_path_basename(fn));
...
void test_checkout_crlf__detect_crlf_autocrlf_false(void)
{
	...
	git3_checkout_head(g_repo, &opts);
...
void test_checkout_crlf__autocrlf_false_index_size_is_unfiltered_size(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(index, "all-lf", 0)) != NULL);
	cl_assert(entry->file_size == strlen(ALL_LF_TEXT_RAW));

	cl_assert((entry = git3_index_get_bypath(index, "all-crlf", 0)) != NULL);
...
void test_checkout_crlf__detect_crlf_autocrlf_true_utf8(void)
{
	...
	git3_repository_set_head(g_repo, "refs/heads/master");
	git3_checkout_head(g_repo, &opts);
...
void test_checkout_crlf__with_ident(void)
{
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_add_bypath(index, "lf.ident"));
	cl_git_pass(git3_index_add_bypath(index, "crlf.ident"));
	cl_git_pass(git3_index_add_bypath(index, "more1.identlf"));
	cl_git_pass(git3_index_add_bypath(index, "more2.identcrlf"));
	...
	cl_assert((entry = git3_index_get_bypath(index, "lf.ident", 0)));
	cl_git_pass(git3_blob_lookup(&blob, g_repo, &entry->id));
	cl_assert_equal_s(
		ALL_LF_TEXT_RAW "\n$Id$\n", git3_blob_rawcontent(blob));
	git3_blob_free(blob);

	cl_assert((entry = git3_index_get_bypath(index, "more2.identcrlf", 0)));
	cl_git_pass(git3_blob_lookup(&blob, g_repo, &entry->id));
	cl_assert_equal_s(
		"\n$Id$\n" MORE_CRLF_TEXT_AS_LF, git3_blob_rawcontent(blob));
	git3_blob_free(blob);
...
void test_checkout_crlf__can_write_empty_file(void)
{
	...
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/empty-files"));
	cl_git_pass(git3_checkout_head(g_repo, &opts));
