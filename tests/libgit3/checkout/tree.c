...
static git3_checkout_options g_opts;
static git3_object *g_object;

static void assert_status_entrycount(git3_repository *repo, size_t count)
{
	git3_status_list *status;

	cl_git_pass(git3_status_list_new(&status, repo, NULL));
	cl_assert_equal_i(count, git3_status_list_entrycount(status));
...
void test_checkout_tree__initialize(void)
{
	...
	GIT3_INIT_STRUCTURE(&g_opts, GIT3_CHECKOUT_OPTIONS_VERSION);
	g_opts.checkout_strategy = GIT3_CHECKOUT_FORCE;
}

void test_checkout_tree__cleanup(void)
{
	git3_object_free(g_object);
	g_object = NULL;

	cl_git_sandbox_cleanup();

	if (git3_fs_path_isdir("alternative"))
		git3_futils_rmdir_r("alternative", NULL, GIT3_RMDIR_REMOVE_FILES);
}

void test_checkout_tree__cannot_checkout_a_non_treeish(void)
{
	/* blob */
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "a71586c1dfe8a71c6cbf6c129f404c5642ff31bd"));
	cl_git_fail(git3_checkout_tree(g_repo, g_object, NULL));
}
...
void test_checkout_tree__can_checkout_a_subdirectory_from_a_commit(void)
{
	...
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "subtrees"));

	cl_assert_equal_i(false, git3_fs_path_isdir("./testrepo/ab/"));

	cl_git_pass(git3_checkout_tree(g_repo, g_object, &g_opts));

	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/2.txt"));
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/fgh/1.txt"));
}
...
void test_checkout_tree__can_checkout_and_remove_directory(void)
{
	...
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/subtrees"));

	cl_assert_equal_i(true, git3_fs_path_isdir("./testrepo/ab/"));
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/2.txt"));
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/fgh/1.txt"));
	 ...
	 * current working tree
	 */
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "master"));
	cl_git_pass(git3_checkout_tree(g_repo, g_object, &g_opts));

	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/master"));
...
void test_checkout_tree__can_checkout_a_subdirectory_from_a_subtree(void)
{
	...
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "subtrees:ab"));

	cl_assert_equal_i(false, git3_fs_path_isdir("./testrepo/de/"));

	cl_git_pass(git3_checkout_tree(g_repo, g_object, &g_opts));

	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/de/2.txt"));
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/de/fgh/1.txt"));
}
...
{
	git3_oid master_oid;
	git3_oid chomped_oid;
	git3_commit* p_master_commit;
	git3_commit* p_chomped_commit;
	git3_checkout_options opts = GIT3_CHECKOUT_OPTIONS_INIT;

	git3_oid_from_string(&master_oid, "a65fedf39aefe402d3bb6e24df4d4f5fe4547750", GIT3_OID_SHA1);
	git3_oid_from_string(&chomped_oid, "e90810b8df3e80c413d903f631643c716887138d", GIT3_OID_SHA1);
	cl_git_pass(git3_commit_lookup(&p_master_commit, g_repo, &master_oid));
	cl_git_pass(git3_commit_lookup(&p_chomped_commit, g_repo, &chomped_oid));

	/* GIT3_CHECKOUT_NONE should not add any file to the working tree from the
	 * index as it is supposed to be a dry run.
	 */
	opts.checkout_strategy = GIT3_CHECKOUT_NONE;
	git3_checkout_tree(g_repo, (git3_object*)p_chomped_commit, &opts);
	cl_assert_equal_i(false, git3_fs_path_isfile("testrepo/readme.txt"));

	git3_commit_free(p_master_commit);
	git3_commit_free(p_chomped_commit);
}
...
void test_checkout_tree__can_switch_branches(void)
{
	...
	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "refs/heads/dir"));
	cl_git_pass(git3_object_lookup(&obj, g_repo, &oid, GIT3_OBJECT_ANY));

	cl_git_pass(git3_checkout_tree(g_repo, obj, &opts));
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/dir"));

	cl_assert(git3_fs_path_isfile("testrepo/README"));
	cl_assert(git3_fs_path_isfile("testrepo/branch_file.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/new.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/a/b.txt"));

	cl_assert(!git3_fs_path_isdir("testrepo/ab"));
	...
	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "refs/heads/subtrees"));
	cl_git_pass(git3_object_lookup(&obj, g_repo, &oid, GIT3_OBJECT_ANY));
	...
	cl_assert(git3_fs_path_isfile("testrepo/branch_file.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/new.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/ab/4.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/ab/c/3.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/ab/de/2.txt"));
	cl_assert(git3_fs_path_isfile("testrepo/ab/de/fgh/1.txt"));
...
void test_checkout_tree__can_remove_untracked(void)
{
	...
	cl_assert(git3_fs_path_isfile("testrepo/untracked_file"));

	cl_git_pass(git3_checkout_head(g_repo, &opts));
...
void test_checkout_tree__can_remove_ignored(void)
{
	...
	cl_git_pass(git3_ignore_add_rule(g_repo, "ignored_file\n"));

	cl_git_pass(git3_ignore_path_is_ignored(&ignored, g_repo, "ignored_file"));
	cl_assert_equal_i(1, ignored);

	cl_assert(git3_fs_path_isfile("testrepo/ignored_file"));
...
static int checkout_tree_with_blob_ignored_in_workdir(int strategy, bool isdir)
{
	...
	if (isdir) {
		...
		cl_assert(git3_fs_path_isdir("testrepo/ab/4.txt"));
	} else {
		cl_must_pass(p_mkdir("testrepo/ab", 0777));
	...
	cl_git_pass(git3_ignore_add_rule(g_repo, "ab/4.txt\n"));

	cl_git_pass(git3_ignore_path_is_ignored(&ignored, g_repo, "ab/4.txt"));
	cl_assert_equal_i(1, ignored);

	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "refs/heads/subtrees"));
	cl_git_pass(git3_object_lookup(&obj, g_repo, &oid, GIT3_OBJECT_ANY));

	error = git3_checkout_tree(g_repo, obj, &opts);
...
void test_checkout_tree__conflict_on_ignored_when_not_overwriting(void)
{
	...
	cl_git_fail(error = checkout_tree_with_blob_ignored_in_workdir(
		GIT3_CHECKOUT_DONT_OVERWRITE_IGNORED, false));

	cl_assert_equal_i(GIT3_ECONFLICT, error);
}

void test_checkout_tree__can_overwrite_ignored_by_default(void)
{
	cl_git_pass(checkout_tree_with_blob_ignored_in_workdir(GIT3_CHECKOUT_SAFE, false));
...
void test_checkout_tree__conflict_on_ignored_folder_when_not_overwriting(void)
{
	...
	cl_git_fail(error = checkout_tree_with_blob_ignored_in_workdir(
		GIT3_CHECKOUT_DONT_OVERWRITE_IGNORED, true));
...
{
	cl_git_pass(checkout_tree_with_blob_ignored_in_workdir(GIT3_CHECKOUT_SAFE, true));
...
void test_checkout_tree__can_update_only(void)
{
	...
	opts.checkout_strategy = GIT3_CHECKOUT_UPDATE_ONLY;
...
void test_checkout_tree__can_checkout_with_pattern(void)
{
	...
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "8496071c1b46c854b31185ea97743be6a8774479"));

	cl_git_pass(git3_checkout_tree(g_repo, g_object, &g_opts));
	cl_git_pass(
		git3_repository_set_head_detached(g_repo, git3_object_id(g_object)));

	git3_object_free(g_object);
	g_object = NULL;

	cl_assert(git3_fs_path_exists("testrepo/README"));
	cl_assert(!git3_fs_path_exists("testrepo/branch_file.txt"));
	cl_assert(!git3_fs_path_exists("testrepo/link_to_new.txt"));
	cl_assert(!git3_fs_path_exists("testrepo/new.txt"));
	...
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "refs/heads/master"));
...
void test_checkout_tree__pathlist_checkout_ignores_non_matches(void)
{
	...
	g_opts.checkout_strategy =
		GIT3_CHECKOUT_FORCE | GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH;
	g_opts.paths.strings = entries;
...
void test_checkout_tree__can_disable_pattern_match(void)
{
	...
	g_opts.checkout_strategy =
		GIT3_CHECKOUT_DISABLE_PATHSPEC_MATCH;
	g_opts.paths.strings = entries;
	g_opts.paths.count = 1;
	...
	g_opts.checkout_strategy = GIT3_CHECKOUT_SAFE;
...
{
	git3_index *index;
	git3_object *hack_tree;
	git3_reference *branch, *head;
	git3_str file_path = GIT3_STR_INIT;

	cl_git_pass(git3_repository_index(&index, g_repo));

	/* Create a branch pointing at the parent */
	cl_git_pass(git3_revparse_single(&g_object, g_repo, parent_sha));
	cl_git_pass(git3_branch_create(&branch, g_repo,
		"potential_conflict", (git3_commit *)g_object, 0));

	/* Make HEAD point to this branch */
	cl_git_pass(git3_reference_symbolic_create(
		&head, g_repo, "HEAD", git3_reference_name(branch), 1, NULL));
	 ...
	 */
	cl_git_pass(git3_object_peel(&hack_tree, g_object, GIT3_OBJECT_TREE));
	cl_git_pass(git3_index_read_tree(index, (git3_tree *)hack_tree));
	cl_git_pass(git3_index_write(index));
	git3_object_free(hack_tree);
	git3_object_free(g_object);
	g_object = NULL;

	/* Create a conflicting file */
	cl_git_pass(git3_str_joinpath(&file_path, "./testrepo", entry_path));
	cl_git_mkfile(git3_str_cstr(&file_path), new_content);
	git3_str_dispose(&file_path);

	/* Trying to checkout the original commit */
	cl_git_pass(git3_revparse_single(&g_object, g_repo, commit_sha));

	g_opts.checkout_strategy = GIT3_CHECKOUT_SAFE;
	cl_assert_equal_i(
		GIT3_ECONFLICT, git3_checkout_tree(g_repo, g_object, &g_opts));

	/* Stage the conflicting change */
	cl_git_pass(git3_index_add_bypath(index, entry_path));
	cl_git_pass(git3_index_write(index));
...
void test_checkout_tree__donot_update_deleted_file_by_default(void)
{
	...
	git3_commit *old_commit = NULL, *new_commit = NULL;
	git3_index *index = NULL;
	checkout_counts ct;

	memset(&ct, 0, sizeof(ct));
	opts.notify_flags = GIT3_CHECKOUT_NOTIFY_ALL;
	opts.notify_cb = checkout_count_callback;
	opts.notify_payload = &ct;

	cl_git_pass(git3_repository_index(&index, g_repo));

	cl_git_pass(git3_oid_from_string(&old_id, "be3563ae3f795b2b4353bcce3a527ad0a4f7f644", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&old_commit, g_repo, &old_id));
	cl_git_pass(git3_reset(g_repo, (git3_object *)old_commit, GIT3_RESET_HARD, NULL));

	cl_git_pass(p_unlink("testrepo/branch_file.txt"));
	cl_git_pass(git3_index_remove_bypath(index ,"branch_file.txt"));
	cl_git_pass(git3_index_write(index));

	cl_assert(!git3_fs_path_exists("testrepo/branch_file.txt"));

	cl_git_pass(git3_oid_from_string(&new_id, "099fabac3a9ea935598528c27f866e34089c2eff", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&new_commit, g_repo, &new_id));


	cl_git_fail(git3_checkout_tree(g_repo, (git3_object *)new_commit, &opts));
...
void test_checkout_tree__can_cancel_checkout_from_notify(void)
{
	...
	opts.notify_flags = GIT3_CHECKOUT_NOTIFY_UPDATED;
	...
	cl_git_fail_with(git3_checkout_tree(g_repo, obj, &opts), -5555);
	...
	if (git3_fs_path_exists("testrepo/.git/CoNfIg")) /* case insensitive */
	...
	cl_git_fail_with(git3_checkout_tree(g_repo, obj, &opts), 123);
...
void test_checkout_tree__can_checkout_with_last_workdir_item_missing(void)
{
	...
	git3_repository_index(&index, g_repo);

	opts.checkout_strategy = GIT3_CHECKOUT_FORCE;

	cl_git_pass(git3_reference_name_to_id(&commit_id, g_repo, "refs/heads/master"));
	cl_git_pass(git3_commit_lookup(&commit, g_repo, &commit_id));

	cl_git_pass(git3_checkout_tree(g_repo, (git3_object *)commit, &opts));
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/master"));

	cl_git_pass(p_mkdir("./testrepo/this-is-dir", 0777));
	cl_git_mkfile("./testrepo/this-is-dir/contained_file", "content\n");

	cl_git_pass(git3_index_add_bypath(index, "this-is-dir/contained_file"));
	cl_git_pass(git3_index_write(index));
	...
	git3_checkout_tree(g_repo, (git3_object *)tree, &opts);
...
void test_checkout_tree__issue_1397(void)
{
	...
	cl_git_pass(git3_revparse_single(&tree, g_repo, partial_oid));

	opts.checkout_strategy = GIT3_CHECKOUT_FORCE;

	cl_git_pass(git3_checkout_tree(g_repo, tree, &opts));
...
void test_checkout_tree__fails_when_dir_in_use(void)
{
	...
	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "refs/heads/master"));
	cl_git_pass(git3_object_lookup(&obj, g_repo, &oid, GIT3_OBJECT_ANY));
	...
	cl_assert(git3_fs_path_is_empty_dir("testrepo/a"));
...
void test_checkout_tree__can_continue_when_dir_in_use(void)
{
	...
	opts.checkout_strategy = GIT3_CHECKOUT_FORCE |
		GIT3_CHECKOUT_SKIP_LOCKED_DIRECTORIES;
...
void test_checkout_tree__target_directory_from_bare(void)
{
	...
	opts.checkout_strategy = GIT3_CHECKOUT_RECREATE_MISSING;

	opts.notify_flags = GIT3_CHECKOUT_NOTIFY_ALL;
	opts.notify_cb = checkout_count_callback;
	opts.notify_payload = &cts;

	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "HEAD"));
	cl_git_pass(git3_object_lookup(&g_object, g_repo, &oid, GIT3_OBJECT_ANY));

	cl_git_fail(git3_checkout_tree(g_repo, g_object, &opts));

	opts.target_directory = "alternative";
	cl_assert(!git3_fs_path_isdir("alternative"));

	cl_git_pass(git3_checkout_tree(g_repo, g_object, &opts));
	...
	cl_git_pass(git3_futils_rmdir_r(
		"alternative", NULL, GIT3_RMDIR_REMOVE_FILES));
}
...
void test_checkout_tree__extremely_long_file_name(void)
{
	...
	g_opts.checkout_strategy = GIT3_CHECKOUT_FORCE;
	cl_git_pass(git3_revparse_single(&g_object, g_repo, "long-file-name"));
	cl_git_pass(git3_checkout_tree(g_repo, g_object, &g_opts));

	sprintf(path, "testrepo/%s.txt", longname);
	cl_assert(git3_fs_path_exists(path));
...
static void create_conflict(const char *path)
{
	...
	entry.mode = 0100644;
	GIT3_INDEX_ENTRY_STAGE_SET(&entry, 1);
	git3_oid_from_string(&entry.id, "d427e0b2e138501a3d15cc376077a3631e15bd46", GIT3_OID_SHA1);
	entry.path = path;
	cl_git_pass(git3_index_add(index, &entry));

	GIT3_INDEX_ENTRY_STAGE_SET(&entry, 2);
	git3_oid_from_string(&entry.id, "ee3fa1b8c00aff7fe02065fdb50864bb0d932ccf", GIT3_OID_SHA1);
	cl_git_pass(git3_index_add(index, &entry));

	GIT3_INDEX_ENTRY_STAGE_SET(&entry, 3);
	git3_oid_from_string(&entry.id, "2bd0a343aeef7a2cf0d158478966a6e587ff3863", GIT3_OID_SHA1);
	cl_git_pass(git3_index_add(index, &entry));
...
void test_checkout_tree__filemode_preserved_in_index(void)
{
	...
	cl_git_pass(git3_oid_from_string(&executable_oid, "afe4393b2b2a965f06acf2ca9658eaa01e0cd6b6", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit, g_repo, &executable_oid));

	cl_git_pass(git3_checkout_tree(g_repo, (const git3_object *)commit, &opts));
	cl_assert(entry = git3_index_get_bypath(index, "executable.txt", 0));
	cl_assert(GIT3_PERMS_IS_EXEC(entry->mode));

	git3_commit_free(commit);


	/* Now start with a commit which has a text file */
	cl_git_pass(git3_oid_from_string(&executable_oid, "cf80f8de9f1185bf3a05f993f6121880dd0cfbc9", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit, g_repo, &executable_oid));

	cl_git_pass(git3_checkout_tree(g_repo, (const git3_object *)commit, &opts));
	cl_assert(entry = git3_index_get_bypath(index, "a/b.txt", 0));
	cl_assert(!GIT3_PERMS_IS_EXEC(entry->mode));
	...
	cl_git_pass(git3_oid_from_string(&executable_oid, "144344043ba4d4a405da03de3844aa829ae8be0e", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit, g_repo, &executable_oid));
...
static mode_t read_filemode(const char *path)
{
	...
	git3_str_joinpath(&fullpath, "testrepo", path);
	cl_must_pass(p_stat(fullpath.ptr, &st));

	result = GIT3_PERMS_IS_EXEC(st.st_mode) ?
		GIT3_FILEMODE_BLOB_EXECUTABLE : GIT3_FILEMODE_BLOB;
...
void test_checkout_tree__filemode_preserved_in_workdir(void)
{
	...
	cl_git_pass(git3_checkout_tree(g_repo, (const git3_object *)commit, &opts));
	cl_assert(GIT3_PERMS_IS_EXEC(read_filemode("executable.txt")));
	...
	cl_git_pass(git3_checkout_tree(g_repo, (const git3_object *)commit, &opts));
	cl_assert(!GIT3_PERMS_IS_EXEC(read_filemode("a/b.txt")));
...
void test_checkout_tree__removes_conflicts(void)
{
	...
	cl_git_pass(git3_oid_from_string(&commit_id, "afe4393b2b2a965f06acf2ca9658eaa01e0cd6b6", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit, g_repo, &commit_id));
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_remove(index, "executable.txt", 0));
	...
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "executable.txt", 1));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "executable.txt", 2));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "executable.txt", 3));

	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "other.txt", 1));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "other.txt", 2));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "other.txt", 3));

	cl_assert(!git3_fs_path_exists("testrepo/other.txt"));
...
void test_checkout_tree__removes_conflicts_only_by_pathscope(void)
{
	...
	cl_assert(git3_index_get_bypath(index, "other.txt", 1) != NULL);
	cl_assert(git3_index_get_bypath(index, "other.txt", 2) != NULL);
	cl_assert(git3_index_get_bypath(index, "other.txt", 3) != NULL);
...
void test_checkout_tree__case_changing_rename(void)
{
	git3_index *index;
	git3_checkout_options opts = GIT3_CHECKOUT_OPTIONS_INIT;
	git3_oid master_id, dir_commit_id, tree_id, commit_id;
	git3_commit *master_commit, *dir_commit;
	git3_tree *tree;
	git3_signature *signature;
	const git3_index_entry *index_entry;
	bool case_sensitive;
	...
	cl_git_pass(git3_reference_name_to_id(&dir_commit_id, g_repo, "refs/heads/dir"));
	cl_git_pass(git3_commit_lookup(&dir_commit, g_repo, &dir_commit_id));

	cl_git_pass(git3_checkout_tree(g_repo, (git3_object *)dir_commit, &opts));
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/dir"));

	cl_assert(git3_fs_path_isfile("testrepo/README"));
	case_sensitive = !git3_fs_path_isfile("testrepo/readme");

	cl_assert(index_entry = git3_index_get_bypath(index, "README", 0));
	cl_assert_equal_s("README", index_entry->path);

	cl_git_pass(git3_index_remove_bypath(index, "README"));
	...
	cl_git_pass(git3_index_add_bypath(index, "readme"));
	...
	cl_git_pass(git3_signature_new(&signature, "Renamer", "rename@contoso.com", time(NULL), 0));

	cl_git_pass(git3_commit_create(&commit_id, g_repo, "refs/heads/dir", signature, signature, NULL, "case-changing rename", tree, 1, (const git3_commit **)&dir_commit));
	...
	cl_git_pass(git3_reference_name_to_id(&master_id, g_repo, "refs/heads/master"));
	cl_git_pass(git3_commit_lookup(&master_commit, g_repo, &master_id));

	cl_git_pass(git3_checkout_tree(g_repo, (git3_object *)master_commit, &opts));
	cl_git_pass(git3_repository_set_head(g_repo, "refs/heads/master"));
	...
	git3_tree_free(tree);
	git3_commit_free(dir_commit);
	git3_commit_free(master_commit);
}

static void perfdata_cb(const git3_checkout_perfdata *in, void *payload)
{
	memcpy(payload, in, sizeof(git3_checkout_perfdata));
}
...
void test_checkout_tree__can_collect_perfdata(void)
{
	...
	git3_oid oid;
	git3_object *obj = NULL;
	git3_checkout_perfdata perfdata = {0};
...
static void update_attr_callback(
	...
	void *payload)
{
	GIT3_UNUSED(completed_steps);
	GIT3_UNUSED(total_steps);
	GIT3_UNUSED(payload);
...
void test_checkout_tree__caches_attributes_during_checkout(void)
{
	...
	git3_oid oid;
	git3_object *obj = NULL;
	git3_str ident1 = GIT3_STR_INIT, ident2 = GIT3_STR_INIT;
	...
	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "refs/heads/ident"));
	cl_git_pass(git3_object_lookup(&obj, g_repo, &oid, GIT3_OBJECT_ANY));

	cl_git_pass(git3_checkout_tree(g_repo, obj, &opts));

	cl_git_pass(git3_futils_readbuffer(&ident1, "testrepo/ident1.txt"));
	cl_git_pass(git3_futils_readbuffer(&ident2, "testrepo/ident2.txt"));
	...
	git3_str_dispose(&ident1);
	git3_str_dispose(&ident2);
	git3_object_free(obj);
}
...
void test_checkout_tree__can_not_update_index(void)
{
	...
	opts.checkout_strategy |=
		GIT3_CHECKOUT_FORCE | GIT3_CHECKOUT_DONT_UPDATE_INDEX;

	cl_git_pass(git3_reference_name_to_id(&oid, g_repo, "HEAD"));
	cl_git_pass(git3_object_lookup(&head, g_repo, &oid, GIT3_OBJECT_ANY));

	cl_git_pass(git3_reset(g_repo, head, GIT3_RESET_HARD, &g_opts));
	...
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/2.txt"));
	cl_git_pass(git3_status_file(&status, g_repo, "ab/de/2.txt"));
...
void test_checkout_tree__can_update_but_not_write_index(void)
{
	...
	opts.checkout_strategy |=
		GIT3_CHECKOUT_FORCE | GIT3_CHECKOUT_DONT_WRITE_INDEX;
	...
	cl_assert_equal_i(true, git3_fs_path_isfile("./testrepo/ab/de/2.txt"));
	cl_git_pass(git3_status_file(&status, g_repo, "ab/de/2.txt"));
	cl_assert_equal_i(GIT3_STATUS_INDEX_NEW, status);

	cl_git_pass(git3_repository_open(&other, "testrepo"));
	cl_git_pass(git3_status_file(&status, other, "ab/de/2.txt"));
	cl_assert_equal_i(GIT3_STATUS_WT_NEW, status);
...
void test_checkout_tree__baseline_is_empty_when_no_index(void)
{
	...
	cl_git_pass(git3_reference_peel(&obj, head, GIT3_OBJECT_COMMIT));

	cl_git_pass(git3_reset(g_repo, obj, GIT3_RESET_HARD, NULL));
	 ...
	 */
	opts.checkout_strategy &= ~GIT3_CHECKOUT_FORCE;
	opts.notify_flags = GIT3_CHECKOUT_NOTIFY_CONFLICT;
	opts.notify_cb = checkout_conflict_count_cb;
	opts.notify_payload = &conflicts;

	cl_git_fail_with(GIT3_ECONFLICT, git3_checkout_tree(g_repo, obj, &opts));
...
void test_checkout_tree__mode_change_is_force_updated(void)
{
	...
	cl_git_pass(git3_checkout_tree(g_repo, obj, &g_opts));
	assert_status_entrycount(g_repo, 0);

	/* update the mode on-disk and in the index */
	cl_must_pass(p_chmod("testrepo/README", 0755));
	cl_must_pass(git3_index_add_bypath(index, "README"));
...
{
	cl_git_pass(git3_checkout_tree(g_repo, NULL, NULL));
}

static void modify_index_ondisk(void)
{
	git3_repository *other_repo;
	git3_index *other_index;
	git3_index_entry entry = {{0}};

	cl_git_pass(git3_repository_open(&other_repo, git3_repository_workdir(g_repo)));
	cl_git_pass(git3_repository_index(&other_index, other_repo));

	cl_git_pass(git3_oid_from_string(&entry.id, "1385f264afb75a56a5bec74243be9b367ba4ca08", GIT3_OID_SHA1));
	entry.mode = 0100644;
	entry.path = "README";

	cl_git_pass(git3_index_add(other_index, &entry));
	cl_git_pass(git3_index_write(other_index));

	git3_index_free(other_index);
	git3_repository_free(other_repo);
}

static void modify_index_and_checkout_tree(git3_checkout_options *opts)
{
	git3_index *index;
	git3_reference *head;
	git3_object *obj;
...
void test_checkout_tree__no_index_refresh(void)
{
	...
	opts.checkout_strategy = GIT3_CHECKOUT_NO_REFRESH;
...
void test_checkout_tree__dry_run(void)
{
	...
	memset(&ct, 0, sizeof(ct));
	opts.checkout_strategy = GIT3_CHECKOUT_DRY_RUN;
	opts.notify_flags = GIT3_CHECKOUT_NOTIFY_ALL;
	opts.notify_cb = checkout_count_callback;
	opts.notify_payload = &ct;
