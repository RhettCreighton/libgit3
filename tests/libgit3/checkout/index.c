...
void test_checkout_index__initialize(void)
{
	...
	reset_index_to_treeish((git3_object *)tree);
	git3_tree_free(tree);
...
void test_checkout_index__cannot_checkout_a_bare_repository(void)
{
	...
	cl_git_fail(git3_checkout_index(g_repo, NULL, NULL));
}
...
void test_checkout_index__can_create_missing_files(void)
{
	...
	cl_assert_equal_i(false, git3_fs_path_isfile("./testrepo/README"));
	cl_assert_equal_i(false, git3_fs_path_isfile("./testrepo/branch_file.txt"));
	cl_assert_equal_i(false, git3_fs_path_isfile("./testrepo/new.txt"));

	opts.checkout_strategy = GIT3_CHECKOUT_RECREATE_MISSING;

	cl_git_pass(git3_checkout_index(g_repo, NULL, &opts));
...
void test_checkout_index__can_remove_untracked_files(void)
{
	...
	git3_futils_mkdir("./testrepo/dir/subdir/subsubdir", 0755, GIT3_MKDIR_PATH);
	cl_git_mkfile("./testrepo/dir/one", "one\n");
	cl_git_mkfile("./testrepo/dir/subdir/two", "two\n");

	cl_assert_equal_i(true, git3_fs_path_isdir("./testrepo/dir/subdir/subsubdir"));

	opts.checkout_strategy =
		GIT3_CHECKOUT_RECREATE_MISSING |
		GIT3_CHECKOUT_REMOVE_UNTRACKED;

	cl_git_pass(git3_checkout_index(g_repo, NULL, &opts));

	cl_assert_equal_i(false, git3_fs_path_isdir("./testrepo/dir"));
}
...
void test_checkout_index__can_disable_pathspec_match(void)
{
	...
	opts.checkout_strategy = GIT3_CHECKOUT_FORCE | GIT3_CHECKOUT_REMOVE_UNTRACKED;

	cl_git_pass(git3_revparse_single(&objects, g_repo, "8496071c1b46c854b31185ea97743be6a8774479"));
	cl_git_pass(git3_checkout_tree(g_repo, objects, &opts));
	cl_git_pass(git3_repository_set_head_detached(g_repo, git3_object_id(objects)));
	git3_object_free(objects);
	...
	cl_git_pass(git3_index_add_bypath(index, "test9.txt"));
	cl_git_pass(git3_index_add_bypath(index, "test10.txt"));
	cl_git_pass(git3_index_add_bypath(index, "test11.txt"));
	cl_git_pass(git3_index_add_bypath(index, "test12.txt"));
...
static void populate_symlink_workdir(void)
{
	...
	const char *url = git3_repository_path(g_repo);

	cl_git_pass(git3_str_joinpath(&path, clar_sandbox_path(), "symlink.git"));
	cl_git_pass(git3_repository_init(&repo, path.ptr, true));
	cl_git_pass(git3_repository_set_workdir(repo, "symlink", 1));

	/* Delete the `origin` repo (if it exists) so we can recreate it. */
	git3_remote_delete(repo, GIT3_REMOTE_ORIGIN);

	cl_git_pass(git3_remote_create(&origin, repo, GIT3_REMOTE_ORIGIN, url));
	cl_git_pass(git3_remote_fetch(origin, NULL, NULL, NULL));
	git3_remote_free(origin);

	cl_git_pass(git3_revparse_single(&target, repo, "remotes/origin/master"));
...
{
	char link_data[GIT3_PATH_MAX];
	int link_size = GIT3_PATH_MAX;

	cl_must_pass(p_mkdir("symlink", 0777));

	if (!git3_fs_path_supports_symlinks("symlink/test"))
		cl_skip();
...
void test_checkout_index__coresymlinks_set_to_true_fails_when_unsupported(void)
{
	...
	if (git3_fs_path_supports_symlinks("testrepo/test")) {
		cl_skip();
	}
	...
	opts.checkout_strategy = GIT3_CHECKOUT_RECREATE_MISSING;
	cl_git_fail(git3_checkout_index(g_repo, NULL, &opts));
}
...
void test_checkout_index__honor_coresymlinks_setting_set_to_true(void)
{
	git3_checkout_options opts = GIT3_CHECKOUT_OPTIONS_INIT;
	char link_data[GIT3_PATH_MAX];
	size_t link_size = GIT3_PATH_MAX;
...
void test_checkout_index__donot_overwrite_modified_file_by_default(void)
{
	 ...
	 */
	opts.checkout_strategy = GIT3_CHECKOUT_ALLOW_CONFLICTS;
...
void test_checkout_index__options_dir_modes(void)
{
	...
	reset_index_to_treeish((git3_object *)commit);
	...
	cl_assert_equal_i_fmt(st.st_mode & (S_IFMT | 07777), (GIT3_FILEMODE_TREE | 0701) & ~um, "%07o");

	/* File-mode test, since we're on the 'dir' branch */
	cl_git_pass(p_stat("./testrepo/a/b.txt", &st));
	cl_assert_equal_i_fmt(st.st_mode & (S_IFMT | 07777), GIT3_FILEMODE_BLOB_EXECUTABLE & ~um, "%07o");
...
void test_checkout_index__options_override_file_modes(void)
{
	...
	cl_assert_equal_i_fmt(st.st_mode & GIT3_MODE_PERMS_MASK, 0700, "%07o");
}
...
void test_checkout_index__options_open_flags(void)
{
	...
	opts.checkout_strategy =
		GIT3_CHECKOUT_FORCE | GIT3_CHECKOUT_DONT_REMOVE_EXISTING;
	...
	void *payload)
{
	...
	cl_assert_equal_i(GIT3_CHECKOUT_NOTIFY_CONFLICT, why);
	cl_assert_equal_s(expectations->file, path);
	cl_assert_equal_i(0, git3_oid_streq(&baseline->id, expectations->sha));
	cl_assert_equal_i(0, git3_oid_streq(&target->id, expectations->sha));
...
void test_checkout_index__can_overcome_name_clashes(void)
{
	...
	cl_git_pass(git3_index_add_bypath(index, "path0"));
	cl_git_pass(git3_index_add_bypath(index, "path1/file1"));

	cl_git_pass(p_unlink("./testrepo/path0"));
	cl_git_pass(git3_futils_rmdir_r(
		"./testrepo/path1", NULL, GIT3_RMDIR_REMOVE_FILES));

	cl_git_mkfile("./testrepo/path1", "content\r\n");
	cl_git_pass(p_mkdir("./testrepo/path0", 0777));
	cl_git_mkfile("./testrepo/path0/file0", "content\r\n");

	cl_assert(git3_fs_path_isfile("./testrepo/path1"));
	cl_assert(git3_fs_path_isfile("./testrepo/path0/file0"));

	opts.checkout_strategy =
		GIT3_CHECKOUT_RECREATE_MISSING |
		GIT3_CHECKOUT_ALLOW_CONFLICTS;
	cl_git_pass(git3_checkout_index(g_repo, index, &opts));
	...
	cl_assert(git3_fs_path_isfile("./testrepo/path0"));
	cl_assert(git3_fs_path_isfile("./testrepo/path1/file1"));
...
void test_checkout_index__validates_struct_version(void)
{
	...
	err = git3_error_last();
	cl_assert_equal_i(err->klass, GIT3_ERROR_INVALID);
...
void test_checkout_index__can_update_prefixed_files(void)
{
	...
	cl_assert(!git3_fs_path_exists("testrepo/READ"));
	cl_assert(!git3_fs_path_exists("testrepo/README.after"));
	cl_assert(!git3_fs_path_exists("testrepo/branch_file"));
	cl_assert(!git3_fs_path_exists("testrepo/branch_file.txt.after"));
}
...
void test_checkout_index__can_checkout_a_newly_initialized_repository(void)
{
	...
	cl_git_pass(git3_checkout_index(g_repo, NULL, NULL));
}
...
void test_checkout_index__target_directory_from_bare(void)
{
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_revparse_single(&head, g_repo, "HEAD^{tree}"));
	cl_git_pass(git3_index_read_tree(index, (const git3_tree *)head));
	cl_git_pass(git3_index_write(index));
	git3_index_free(index);
...
void test_checkout_index__can_get_repo_from_index(void)
{
	...
	cl_git_pass(git3_checkout_index(NULL, index, &opts));
...
static void add_conflict(git3_index *index, const char *path)
{
	...
	git3_oid_from_string(&entry.id, "4e886e602529caa9ab11d71f86634bd1b6e0de10", GIT3_OID_SHA1);
...
void test_checkout_index__writes_conflict_file(void)
{
	...
	cl_git_pass(git3_futils_readbuffer(&conflicting_buf, "testrepo/conflicting.txt"));
...
void test_checkout_index__adding_conflict_removes_stage_0(void)
{
	...
	cl_git_pass(git3_checkout_index(g_repo, new_index, &opts));

	cl_git_pass(git3_repository_index(&index, g_repo));

	cl_assert(git3_index_get_bypath(index, "new.txt", 0) == NULL);
	cl_assert(git3_index_get_bypath(index, "new.txt", 1) != NULL);
	cl_assert(git3_index_get_bypath(index, "new.txt", 2) != NULL);
	cl_assert(git3_index_get_bypath(index, "new.txt", 3) != NULL);
