...
void test_checkout_head__unborn_head_returns_GIT3_EUNBORNBRANCH(void)
{
	make_head_unborn(g_repo, NON_EXISTING_HEAD);

	cl_assert_equal_i(GIT3_EUNBORNBRANCH, git3_checkout_head(g_repo, NULL));
}
...
void test_checkout_head__with_index_only_tree(void)
{
	...
	cl_git_pass(git3_index_add_bypath(index, "newdir/newfile.txt"));
	cl_git_pass(git3_index_write(index));

	cl_assert(git3_fs_path_isfile("testrepo/newdir/newfile.txt"));
	cl_assert(git3_index_get_bypath(index, "newdir/newfile.txt", 0) != NULL);
...
void test_checkout_head__do_not_remove_untracked_file(void)
{
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_add_bypath(index, "tracked/tracked"));
	cl_git_pass(git3_index_write(index));
	...
	cl_assert(!git3_fs_path_isfile("testrepo/tracked/tracked"));
	cl_assert(git3_fs_path_isfile("testrepo/tracked/untracked"));
}
...
void test_checkout_head__do_not_remove_untracked_file_in_subdir(void)
{
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_add_bypath(index, "tracked/tracked"));
	cl_git_pass(git3_index_add_bypath(index, "tracked/subdir/tracked"));
	cl_git_pass(git3_index_write(index));
	...
	cl_assert(!git3_fs_path_isfile("testrepo/tracked/tracked"));
	cl_assert(!git3_fs_path_isfile("testrepo/tracked/subdir/tracked"));
	cl_assert(git3_fs_path_isfile("testrepo/tracked/subdir/untracked"));
}
...
void test_checkout_head__do_remove_tracked_subdir(void)
{
	...
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_add_bypath(index, "subdir/tracked-file"));
	cl_git_pass(git3_index_add_bypath(index, "subdir/tracked/tracked1"));
	cl_git_pass(git3_index_add_bypath(index, "subdir/tracked/tracked2"));
	cl_git_pass(git3_index_write(index));
	...
	cl_assert(!git3_fs_path_isdir("testrepo/subdir/tracked"));
	cl_assert(!git3_fs_path_isfile("testrepo/subdir/tracked-file"));
	cl_assert(git3_fs_path_isfile("testrepo/subdir/untracked-file"));
}
...
void test_checkout_head__typechange_workdir(void)
{
	...
	cl_git_pass(git3_revparse_single(&target, g_repo, "HEAD"));
	cl_git_pass(git3_reset(g_repo, target, GIT3_RESET_HARD, NULL));

	cl_must_pass(p_chmod("testrepo/new.txt", 0755));
	cl_git_pass(git3_checkout_head(g_repo, &opts));

	cl_git_pass(p_stat("testrepo/new.txt", &st));
	cl_assert(!GIT3_PERMS_IS_EXEC(st.st_mode));
...
void test_checkout_head__typechange_index_and_workdir(void)
{
	...
	cl_must_pass(p_chmod("testrepo/new.txt", 0755));
	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_add_bypath(index, "new.txt"));
	cl_git_pass(git3_index_write(index));
	cl_git_pass(git3_checkout_head(g_repo, &opts));
...
{
	git3_checkout_options opts = GIT3_CHECKOUT_OPTIONS_INIT;
	git3_object *target, *branch;

	opts.checkout_strategy = GIT3_CHECKOUT_FORCE;

	cl_git_pass(git3_revparse_single(&target, g_repo, "a38d028f71eaa590febb7d716b1ca32350cf70da"));
	cl_git_pass(git3_reset(g_repo, target, GIT3_RESET_HARD, NULL));
	 ...
	 */
	cl_git_pass(git3_revparse_single(&branch, g_repo, "099fabac3a9ea935598528c27f866e34089c2eff"));

	opts.checkout_strategy &= ~GIT3_CHECKOUT_FORCE;
	cl_git_pass(git3_checkout_tree(g_repo, branch, NULL));

	git3_object_free(branch);
	git3_object_free(target);
}
...
void test_checkout_head__obeys_filemode_true(void)
{
	...
	cl_git_pass(git3_revparse_single(&target, g_repo, "f9ed4af42472941da45a3c"));
	cl_git_pass(git3_reset(g_repo, target, GIT3_RESET_HARD, NULL));
	...
	opts.checkout_strategy &= ~GIT3_CHECKOUT_FORCE;
	cl_git_fail_with(GIT3_ECONFLICT, git3_checkout_tree(g_repo, branch, NULL));
