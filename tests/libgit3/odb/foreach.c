...
static int foreach_cb(const git3_oid *oid, void *data)
{
	int *nobj = data;
	(*nobj)++;
...
void test_odb_foreach__foreach(void)
{
	...
	git3_repository_odb(&_odb, _repo);

	cl_git_pass(git3_odb_foreach(_odb, foreach_cb, &nobj));
...
static int foreach_stop_cb(const git3_oid *oid, void *data)
{
	int *nobj = data;
	(*nobj)++;
...
static int foreach_stop_first_cb(const git3_oid *oid, void *data)
{
	int *nobj = data;
	(*nobj)++;
...
static int foreach_stop_cb_positive_ret(const git3_oid *oid, void *data)
{
	int *nobj = data;
	(*nobj)++;
...
void test_odb_foreach__interrupt_foreach(void)
{
	...
	cl_assert_equal_i(-321, git3_odb_foreach(_odb, foreach_stop_cb, &nobj));
	cl_assert(nobj == 1000);

	nobj = 0;

	cl_assert_equal_i(321, git3_odb_foreach(_odb, foreach_stop_cb_positive_ret, &nobj));
	cl_assert(nobj == 1000);

	git3_odb_free(_odb);
	git3_repository_free(_repo);

	cl_git_pass(git3_repository_init(&_repo, "onlyloose.git", true));
	git3_repository_odb(&_odb, _repo);

	cl_git_pass(git3_odb_write(&id, _odb, "", 0, GIT3_OBJECT_BLOB));
	cl_assert_equal_i(-123, git3_odb_foreach(_odb, foreach_stop_first_cb, &nobj));
}
...
void test_odb_foreach__files_in_objects_dir(void)
{
	...
	cl_git_pass(git3_str_joinpath(&buf, git3_repository_path(repo), "objects/somefile"));
	cl_git_mkfile(buf.ptr, "");
	git3_str_dispose(&buf);

	cl_git_pass(git3_repository_odb(&odb, repo));
	cl_git_pass(git3_odb_foreach(odb, foreach_cb, &nobj));
	cl_assert_equal_i(60 + 1640, nobj); /* count + in-pack */
