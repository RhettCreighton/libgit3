...
static git3_odb_object *_obj;
static git3_oid _oid;
...
static void setup_backend(const fake_object *objs)
{
	...
	cl_git_pass(git3_repository_odb__weakptr(&_odb, _repo));
	cl_git_pass(git3_odb_add_backend(_odb, backend, 10));
}

static void assert_object_contains(git3_odb_object *obj, const char *expected)
{
	const char *actual = (const char *) git3_odb_object_data(obj);
...
void test_odb_backend_simple__read_of_object_succeeds(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_pass(git3_odb_read(&_obj, _odb, &_oid));
...
void test_odb_backend_simple__read_of_nonexisting_object_fails(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, "f6ea0495187600e7b2288c8ac19c5886383a4633", GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_odb_read(&_obj, _odb, &_oid));
}
...
void test_odb_backend_simple__read_with_hash_mismatch_fails(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_EMISMATCH, git3_odb_read(&_obj, _odb, &_oid));
}
...
void test_odb_backend_simple__read_prefix_succeeds(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, "f6ea0495187600e7b2288c8ac19c5886383a4632", GIT3_OID_SHA1));
	cl_git_pass(git3_odb_read(&_obj, _odb, &_oid));
...
void test_odb_backend_simple__read_prefix_of_nonexisting_object_fails(void)
{
	...
	cl_git_pass(git3_oid_from_prefix(&_oid, hash, strlen(hash), GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_ENOTFOUND, git3_odb_read(&_obj, _odb, &_oid));
}
...
void test_odb_backend_simple__read_with_ambiguous_prefix_fails(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_EAMBIGUOUS, git3_odb_read_prefix(&_obj, _odb, &_oid, 7));
}
...
void test_odb_backend_simple__read_with_highly_ambiguous_prefix(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_ENABLE_STRICT_HASH_VERIFICATION, 0));
	cl_git_fail_with(GIT3_EAMBIGUOUS, git3_odb_read_prefix(&_obj, _odb, &_oid, 39));
	cl_git_pass(git3_odb_read_prefix(&_obj, _odb, &_oid, 40));
	assert_object_contains(_obj, objs[0].content);
}
...
void test_odb_backend_simple__exists_succeeds(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_assert(git3_odb_exists(_odb, &_oid));
}
...
void test_odb_backend_simple__exists_fails_for_nonexisting_object(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, "f6ea0495187600e7b2288c8ac19c5886383a4633", GIT3_OID_SHA1));
	cl_assert(git3_odb_exists(_odb, &_oid) == 0);
}
...
void test_odb_backend_simple__exists_prefix_succeeds(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_pass(git3_odb_exists_prefix(&found, _odb, &_oid, 12));
	cl_assert(git3_oid_equal(&found, &_oid));
}
...
void test_odb_backend_simple__exists_with_ambiguous_prefix_fails(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_fail_with(GIT3_EAMBIGUOUS, git3_odb_exists_prefix(NULL, _odb, &_oid, 7));
}
...
void test_odb_backend_simple__exists_with_highly_ambiguous_prefix(void)
{
	...
	cl_git_pass(git3_oid_from_string(&_oid, objs[0].oid, GIT3_OID_SHA1));
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_ENABLE_STRICT_HASH_VERIFICATION, 0));
	cl_git_fail_with(GIT3_EAMBIGUOUS, git3_odb_exists_prefix(&found, _odb, &_oid, 39));
	cl_git_pass(git3_odb_exists_prefix(&found, _odb, &_oid, 40));
	cl_assert(git3_oid_equal(&found, &_oid));
}
...
void test_odb_backend_simple__null_oid_is_ignored(void)
{
	...
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_ENABLE_STRICT_HASH_VERIFICATION, 0));
	cl_assert(!git3_odb_exists(_odb, &null_oid));

	cl_git_fail_with(GIT3_ENOTFOUND, git3_odb_read(&obj, _odb, &null_oid));
	cl_assert(git3_error_last() && strstr(git3_error_last()->message, "null OID"));
}
