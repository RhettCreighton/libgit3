...
void test_odb_backend_multiple__read_with_empty_first_succeeds(void)
{
	...
	cl_git_pass(git3_odb_add_backend(odb, (git3_odb_backend *)_fake_filled, 10));
	cl_git_pass(git3_odb_add_backend(odb, (git3_odb_backend *)_fake_empty, 50));

	cl_git_pass(git3_odb_read(&_obj, odb, &_existing_oid));
...
void test_odb_backend_multiple__read_with_first_matching_stops(void)
{
	...
	cl_git_pass(git3_repository_odb__weakptr(&odb, _repo));
	cl_git_pass(git3_odb_add_backend(odb, (git3_odb_backend *)_fake_empty, 10));
	cl_git_pass(git3_odb_add_backend(odb, (git3_odb_backend *)_fake_filled, 50));
...
void test_odb_backend_multiple__read_prefix_with_first_empty_succeeds(void)
{
	...
	cl_git_pass(git3_odb_read_prefix(&_obj, odb, &_existing_oid, 7));
...
void test_odb_backend_multiple__read_prefix_with_first_matching_succeeds_without_hash_verification(void)
{
	...
	git3_libgit3_opts(GIT3_OPT_ENABLE_STRICT_HASH_VERIFICATION, 0);
	...
	git3_libgit3_opts(GIT3_OPT_ENABLE_STRICT_HASH_VERIFICATION, 1);
}
