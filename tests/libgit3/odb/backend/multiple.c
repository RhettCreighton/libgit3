...
void test_odb_backend_multiple__initialize(void)
{
	...
	git_oid_from_string(&_existing_oid, EXISTING_HASH, GIT_OID_SHA1);
...
void test_odb_backend_multiple__read_prefix_with_first_matching_succeeds_without_hash_verification(void)
{
	...
	git_libgit3_opts(GIT_OPT_ENABLE_STRICT_HASH_VERIFICATION, 0);
	...
	git_libgit3_opts(GIT_OPT_ENABLE_STRICT_HASH_VERIFICATION, 1);
}
