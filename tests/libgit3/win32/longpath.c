...
static git3_str path = GIT3_STR_INIT;
...
void test_win32_longpath__errmsg_on_checkout(void)
{
	...
	cl_git_fail(git3_clone(&repo, cl_fixture("testrepo.git"), path.ptr, NULL));
	cl_assert(git3__prefixcmp(git3_error_last()->message, "path too long") == 0);
#endif
}
...
void test_win32_longpath__workdir_path_validated(void)
{
	...
	cl_git_pass(git3_repository_workdir_path(&out, repo, "a.txt"));

	/* even if the repo path is a drive letter, this is too long */
	cl_git_fail(git3_repository_workdir_path(&out, repo, LONG_FILENAME));
	cl_assert(git3__prefixcmp(git3_error_last()->message, "path too long") == 0);

	cl_repo_set_bool(repo, "core.longpaths", true);
	cl_git_pass(git3_repository_workdir_path(&out, repo, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.txt"));
	cl_git_pass(git3_repository_workdir_path(&out, repo, LONG_FILENAME));
	git3_str_dispose(&out);
#endif
}

#ifdef GIT3_WIN32
static void assert_longpath_status_and_add(git3_repository *repo, const char *wddata, const char *repodata) {
	git3_index *index;
	git3_blob *blob;
	git3_str out = GIT3_STR_INIT;
	...
	cl_git_pass(git3_status_file(&status_flags, repo, LONG_FILENAME));
	cl_assert_equal_i(GIT3_STATUS_WT_NEW, status_flags);

	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_add_bypath(index, LONG_FILENAME));

	cl_git_pass(git3_status_file(&status_flags, repo, LONG_FILENAME));
	cl_assert_equal_i(GIT3_STATUS_INDEX_NEW, status_flags);

	cl_assert((entry = git3_index_get_bypath(index, LONG_FILENAME, 0)) != NULL);
	cl_git_pass(git3_blob_lookup(&blob, repo, &entry->id));
	cl_assert_equal_s(repodata, git3_blob_rawcontent(blob));
