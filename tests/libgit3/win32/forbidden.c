...
void test_win32_forbidden__can_open_index(void)
{
	...
	cl_assert_equal_i(7, git3_index_entrycount(index));
...
void test_win32_forbidden__can_add_forbidden_filename_with_entry(void)
{
	...
	git3_oid_from_string(&entry.id, "da623abd956bb2fd8052c708c7ed43f05d192d37", GIT3_OID_SHA1);
...
#ifdef GIT3_WIN32
	cl_git_fail(git3_index_add_bypath(index, "aux."));
#else
	cl_git_pass(git3_index_add_bypath(index, "aux."));
#endif
...
void test_win32_forbidden__can_diff_tree_to_index(void)
{
	...
	cl_git_pass(git3_diff_tree_to_index(&diff, repo, tree, NULL, NULL));
...
void test_win32_forbidden__can_diff_tree_to_tree(void)
{
	...
	cl_git_pass(git3_repository_head_tree(&tree, repo));
	cl_git_pass(git3_diff_tree_to_tree(&diff, repo, tree, tree, NULL));
	cl_assert_equal_i(0, git3_diff_num_deltas(diff));
	git3_diff_free(diff);
	git3_tree_free(tree);
}
...
void test_win32_forbidden__checking_out_forbidden_index_fails(void)
{
	...
	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_fail(git3_checkout_index(repo, index, &opts));

	cl_git_pass(git3_repository_head_tree(&tree, repo));
	cl_git_pass(git3_diff_index_to_workdir(&diff, repo, index, NULL));

	num_deltas = git3_diff_num_deltas(diff);
...
{
	cl_git_pass(git3_submodule_foreach(repo, dummy_submodule_cb, NULL));
}
...
void test_win32_forbidden__can_blame_file(void)
{
	...
	cl_git_pass(git3_blame_file(&blame, repo, "aux", NULL));
