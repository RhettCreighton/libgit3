#include "clar_libgit3.h"

#include "remote.h"
#include "repository.h"

static git3_repository *repo1;
static git3_repository *repo2;
static char* repo1_path;
static char* repo2_path;

static const char *REPO1_REFNAME = "refs/heads/main";
static const char *REPO2_REFNAME = "refs/remotes/repo1/main";
static const char *REPO1_UNDERSCORE_REFNAME = "refs/heads/_branch";
static const char *REPO2_UNDERSCORE_REFNAME = "refs/remotes/repo1/_branch";
static char *FORCE_FETCHSPEC = "+refs/heads/main:refs/remotes/repo1/main";
static char *NON_FORCE_FETCHSPEC = "refs/heads/main:refs/remotes/repo1/main";
static char *NEGATIVE_SPEC = "^refs/heads/_*";

void test_remote_fetch__initialize(void) {
	git3_config *c;
	git3_str repo1_path_buf = GIT3_STR_INIT;
	git3_str repo2_path_buf = GIT3_STR_INIT;
	const char *sandbox = clar_sandbox_path();

	cl_git_pass(git3_str_joinpath(&repo1_path_buf, sandbox, "fetchtest_repo1"));
	repo1_path = git3_str_detach(&repo1_path_buf);
	cl_git_pass(git3_repository_init(&repo1, repo1_path, true));

	cl_git_pass(git3_str_joinpath(&repo2_path_buf, sandbox, "fetchtest_repo2"));
	repo2_path = git3_str_detach(&repo2_path_buf);
	cl_git_pass(git3_repository_init(&repo2, repo2_path, true));

	cl_git_pass(git3_repository_config(&c, repo1));
	cl_git_pass(git3_config_set_string(c, "user.email", "some@email"));
	cl_git_pass(git3_config_set_string(c, "user.name", "some@name"));
	git3_config_free(c);
	git3_str_dispose(&repo1_path_buf);
	git3_str_dispose(&repo2_path_buf);
}

void test_remote_fetch__cleanup(void) {
	git3_repository_free(repo1);
	git3_repository_free(repo2);

	cl_git_pass(git3_futils_rmdir_r(repo1_path, NULL, GIT3_RMDIR_REMOVE_FILES));
	git3__free(repo1_path);

	cl_git_pass(git3_futils_rmdir_r(repo2_path, NULL, GIT3_RMDIR_REMOVE_FILES));
	git3__free(repo2_path);
}


/**
 * This checks that the '+' flag on fetchspecs is respected. We create a
 * repository that has a reference to two commits, one a child of the other.
 * We fetch this repository into a second repository. Then we reset the
 * reference in the first repository and run the fetch again. If the '+' flag
 * is used then the reference in the second repository will change, but if it
 * is not then it should stay the same.
 *
 * @param commit1id A pointer to an OID which will be populated with the first
 *                  commit.
 * @param commit2id A pointer to an OID which will be populated with the second
 *                  commit, which is a descendant of the first.
 * @param force     Whether to use a spec with '+' prefixed to force the refs
 *                  to update
 */
static void do_time_travelling_fetch(git3_oid *commit1id, git3_oid *commit2id,
		bool force) {
	char *refspec_strs = {
		force ? FORCE_FETCHSPEC : NON_FORCE_FETCHSPEC,
	};
	git3_strarray refspecs = {
		.count = 1,
		.strings = &refspec_strs,
	};

	/* create two commits in repo 1 and a reference to them */
	{
		git3_oid empty_tree_id;
		git3_commit *commit1;
		git3_tree *empty_tree;
		git3_signature *sig;
		git3_treebuilder *tb;
		cl_git_pass(git3_treebuilder_new(&tb, repo1, NULL));
		cl_git_pass(git3_treebuilder_write(&empty_tree_id, tb));
		cl_git_pass(git3_tree_lookup(&empty_tree, repo1, &empty_tree_id));
		cl_git_pass(git3_signature_default(&sig, repo1));
		cl_git_pass(git3_commit_create(commit1id, repo1, REPO1_REFNAME, sig,
					sig, NULL, "one", empty_tree, 0, NULL));
		cl_git_pass(git3_commit_lookup(&commit1, repo1, commit1id));
		cl_git_pass(git3_commit_create_v(commit2id, repo1, REPO1_REFNAME, sig,
					sig, NULL, "two", empty_tree, 1, commit1));

		git3_tree_free(empty_tree);
		git3_commit_free(commit1);
		git3_signature_free(sig);
		git3_treebuilder_free(tb);
	}

	/* fetch the reference via the remote */
	{
		git3_remote *remote;

		cl_git_pass(git3_remote_create_anonymous(&remote, repo2,
					git3_repository_path(repo1)));
		cl_git_pass(git3_remote_fetch(remote, &refspecs, NULL, "some message"));

		git3_remote_free(remote);
	}

	/* assert that repo2 references the second commit */
	{
		const git3_oid *target;
		git3_reference *ref;
		cl_git_pass(git3_reference_lookup(&ref, repo2, REPO2_REFNAME));
		target = git3_reference_target(ref);
		cl_assert_equal_b(git3_oid_cmp(target, commit2id), 0);
		git3_reference_free(ref);
	}

	/* set the reference in repo1 to point to the older commit */
	{
		git3_reference *ref;
		git3_reference *ref2;
		cl_git_pass(git3_reference_lookup(&ref, repo1, REPO1_REFNAME));
		cl_git_pass(git3_reference_set_target(&ref2, ref, commit1id,
					"rollback"));
		git3_reference_free(ref);
		git3_reference_free(ref2);
	}

	/* fetch the reference again */
	{
		git3_remote *remote;

		cl_git_pass(git3_remote_create_anonymous(&remote, repo2,
					git3_repository_path(repo1)));
		cl_git_pass(git3_remote_fetch(remote, &refspecs, NULL, "some message"));

		git3_remote_free(remote);
	}
}

void test_remote_fetch__dont_update_refs_if_not_descendant_and_not_force(void) {
	const git3_oid *target;
	git3_oid commit1id;
	git3_oid commit2id;
	git3_reference *ref;

	do_time_travelling_fetch(&commit1id, &commit2id, false);

	/* assert that the reference in repo2 has not changed */
	cl_git_pass(git3_reference_lookup(&ref, repo2, REPO2_REFNAME));
	target = git3_reference_target(ref);
	cl_assert_equal_b(git3_oid_cmp(target, &commit2id), 0);

	git3_reference_free(ref);
}

void test_remote_fetch__do_update_refs_if_not_descendant_and_force(void) {
	const git3_oid *target;
	git3_oid commit1id;
	git3_oid commit2id;
	git3_reference *ref;

	do_time_travelling_fetch(&commit1id, &commit2id, true);

	/* assert that the reference in repo2 has changed */
	cl_git_pass(git3_reference_lookup(&ref, repo2, REPO2_REFNAME));
	target = git3_reference_target(ref);
	cl_assert_equal_b(git3_oid_cmp(target, &commit1id), 0);

	git3_reference_free(ref);
}

/**
 * This checks that negative refspecs are respected when fetching. We create a
 * repository with a '_' prefixed reference. A second repository is configured
 * with a negative refspec to ignore any refs prefixed with '_' and fetch the
 * first repository into the second.
 *
 * @param commit1id A pointer to an OID which will be populated with the first
 *                  commit.
 */
static void do_fetch_repo_with_ref_matching_negative_refspec(git3_oid *commit1id) {
	/* create a commit in repo 1 and a reference to it with '_' prefix */
	{
		git3_oid empty_tree_id;
		git3_tree *empty_tree;
		git3_signature *sig;
		git3_treebuilder *tb;
		cl_git_pass(git3_treebuilder_new(&tb, repo1, NULL));
		cl_git_pass(git3_treebuilder_write(&empty_tree_id, tb));
		cl_git_pass(git3_tree_lookup(&empty_tree, repo1, &empty_tree_id));
		cl_git_pass(git3_signature_default(&sig, repo1));
		cl_git_pass(git3_commit_create(commit1id, repo1, REPO1_UNDERSCORE_REFNAME, sig,
					sig, NULL, "one", empty_tree, 0, NULL));

		git3_tree_free(empty_tree);
		git3_signature_free(sig);
		git3_treebuilder_free(tb);
	}

	/* fetch the remote with negative refspec for references prefixed with '_' */
	{
		char *refspec_strs = { NEGATIVE_SPEC };
		git3_strarray refspecs = { &refspec_strs, 1 };

		git3_remote *remote;

		cl_git_pass(git3_remote_create_anonymous(&remote, repo2,
					git3_repository_path(repo1)));
		cl_git_pass(git3_remote_fetch(remote, &refspecs, NULL, "some message"));

		git3_remote_free(remote);
	}
}

void test_remote_fetch__skip_negative_refspec_match(void) {
	git3_oid commit1id;
	git3_reference *ref1;
	git3_reference *ref2;

	do_fetch_repo_with_ref_matching_negative_refspec(&commit1id);

	/* assert that the reference in exists in repo1 but not in repo2 */
	cl_git_pass(git3_reference_lookup(&ref1, repo1, REPO1_UNDERSCORE_REFNAME));
	cl_assert_equal_b(git3_reference_lookup(&ref2, repo2, REPO2_UNDERSCORE_REFNAME), GIT3_ENOTFOUND);

	git3_reference_free(ref1);
	git3_reference_free(ref2);
}
