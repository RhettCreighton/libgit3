...
{
	cl_git_pass(git3_config_open_ondisk(&cfg, cl_fixture("config/config-include")));

	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.bar.baz"));
	cl_assert_equal_s("huzzah", buf.ptr);
}

void test_config_include__absolute(void)
{
	git3_str cfgdata = GIT3_STR_INIT;
	cl_git_pass(git3_str_printf(&cfgdata, "[include]\npath = %s/config-included", cl_fixture("config")));

	cl_git_mkfile("config-include-absolute", cfgdata.ptr);
	git3_str_dispose(&cfgdata);

	cl_git_pass(git3_config_open_ondisk(&cfg, "config-include-absolute"));
...
void test_config_include__homedir(void)
{
	...
	cl_git_pass(git3_str_joinpath(&homefile, homefile.ptr, "config-included"));

	cl_git_mkfile("config-include-homedir",  "[include]\npath = ~/config-included");
	cl_git_mkfile(homefile.ptr, "[foo \"bar\"]\n\tbaz = huzzah\n");

	cl_git_pass(git3_config_open_ondisk(&cfg, "config-include-homedir"));
...
void test_config_include__ordering(void)
{
	...
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.bar.frotz"));
...
void test_config_include__depth(void)
{
	...
	cl_git_fail(git3_config_open_ondisk(&cfg, "a"));
...
{
	cl_git_mkfile("a", "[include]\npath");
	cl_git_pass(git3_config_open_ondisk(&cfg, "a"));
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "include.path"));
...
void test_config_include__missing(void)
{
	...
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.bar"));
...
void test_config_include__depth2(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&cfg, "top-level"));

	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.bar"));
	cl_assert_equal_s("baz", buf.ptr);

	git3_buf_dispose(&buf);
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.bar2"));
...
void test_config_include__removing_include_removes_values(void)
{
	...
	cl_git_fail(git3_config_get_string_buf(&buf, cfg, "foo.bar"));
...
void test_config_include__rewriting_include_refreshes_values(void)
{
	...
	cl_git_mkfile("first", "[first]\nother = value");
	cl_git_fail(git3_config_get_string_buf(&buf, cfg, "foo.bar"));
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "first.other"));
...
void test_config_include__rewriting_include_twice_refreshes_values(void)
{
	...
	cl_git_mkfile("included", "[foo]\nother = value2");
	cl_git_fail(git3_config_get_string_buf(&buf, cfg, "foo.bar"));
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.other"));
	cl_assert_equal_s(buf.ptr, "value2");

	git3_buf_dispose(&buf);
	cl_git_mkfile("included", "[foo]\nanother = bar");
	cl_git_fail(git3_config_get_string_buf(&buf, cfg, "foo.other"));
	cl_git_pass(git3_config_get_string_buf(&buf, cfg, "foo.another"));
...
void test_config_include__included_variables_cannot_be_deleted(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&cfg, "top-level"));
	cl_git_fail(git3_config_delete_entry(cfg, "foo.bar"));
...
void test_config_include__included_variables_cannot_be_modified(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&cfg, "top-level"));
	cl_git_fail(git3_config_set_string(cfg, "foo.bar", "other-value"));
...
void test_config_include__variables_in_included_override_including(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&cfg, "top-level"));
	cl_git_pass(git3_config_get_int32(&i, cfg, "foo.bar"));
