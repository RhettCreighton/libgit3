...
void test_config_stress__initialize(void)
{
	...
	cl_git_pass(git3_filebuf_open(&file, TEST_CONFIG, 0, 0666));

	git3_filebuf_printf(&file, "[color]\n\tui = auto\n");
	git3_filebuf_printf(&file, "[core]\n\teditor = \n");
...
void test_config_stress__dont_break_on_invalid_input(void)
{
	...
	cl_assert(git3_fs_path_exists(TEST_CONFIG));
...
static void assert_config_value(git3_config *config, const char *key, const char *value)
{
	git3_buf_dispose(&buf);
	cl_git_pass(git3_config_get_string_buf(&buf, config, key));
...
void test_config_stress__comments(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&config, cl_fixture("config/config12")));
...
void test_config_stress__escape_subsection_names(void)
{
	...
	cl_assert(git3_fs_path_exists("git-test-config"));
	cl_git_pass(git3_config_open_ondisk(&config, TEST_CONFIG));

	cl_git_pass(git3_config_set_string(config, "some.sec\\tion.other", "foo"));
	git3_config_free(config);
...
void test_config_stress__trailing_backslash(void)
{
	...
	cl_assert(git3_fs_path_exists("git-test-config"));
	cl_git_pass(git3_config_open_ondisk(&config, TEST_CONFIG));
	cl_git_pass(git3_config_set_string(config, "windows.path", path));
	git3_config_free(config);
...
void test_config_stress__complex(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&config, path));
...
void test_config_stress__quick_write(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&config_w, path));
	cl_git_pass(git3_config_open_ondisk(&config_r, path));

	/* Write and read in the same second (repeat to increase the chance of it happening) */
	for (i = 0; i < 10; i++) {
		int32_t val;
		cl_git_pass(git3_config_set_int32(config_w, key, i));
		cl_msleep(1);
		cl_git_pass(git3_config_get_int32(&val, config_r, key));
		cl_assert_equal_i(i, val);
	}

	git3_config_free(config_r);
	git3_config_free(config_w);
}

static int foreach_cb(const git3_config_entry *entry, void *payload)
{
	if (!strcmp(entry->name, "key.value")) {
		*(char **)payload = git3__strdup(entry->value);
		return 0;
	}
	return -1;
}
...
void test_config_stress__foreach_refreshes(void)
{
	...
	cl_git_pass(git3_config_open_ondisk(&config_w, "./cfg"));
	cl_git_pass(git3_config_open_ondisk(&config_r, "./cfg"));

	cl_git_pass(git3_config_set_string(config_w, "key.value", "1"));
	cl_git_pass(git3_config_foreach_match(config_r, "key.value", foreach_cb, &value));
...
{
	git3_config *config, *snapshot;
	char *value = NULL;

	cl_git_pass(git3_config_open_ondisk(&config, "./cfg"));

	cl_git_pass(git3_config_set_string(config, "key.value", "1"));
	cl_git_pass(git3_config_snapshot(&snapshot, config));
	cl_git_pass(git3_config_foreach_match(snapshot, "key.value", foreach_cb, &value));
...
void test_config_stress__huge_section_with_many_values(void)
{
	 ...
	 */
	cl_git_pass(git3_config_open_ondisk(&config, cl_fixture("config/config-oom")));
