...
void test_apply_workdir__generated_diff(void)
{
	...
	git3_oid_from_string(&b_oid, "7c7bf85e978f1d18c0566f702d2cb7766b9c8d4f", GIT3_OID_SHA1); cl_git_pass(git3_commit_lookup(&a_commit, repo, &a_oid));
	...
	cl_git_pass(git3_apply(repo, diff, GIT3_APPLY_LOCATION_WORKDIR, NULL));
...
void test_apply_workdir__modified_index_with_unmodified_workdir_is_ok(void)
{
	...
	cl_git_pass(git3_oid_from_string(&idx_entry.id, "ffb36e513f5fdf8a6ba850a20142676a2ac4807d", GIT3_OID_SHA1));

	cl_git_pass(git3_index_add(index, &idx_entry));
	cl_git_pass(git3_index_remove(index, "asparagus.txt", 0));
...
void test_apply_workdir__application_failure_leaves_workdir_unmodified(void)
{
	...
	cl_git_pass(diff_from_buffer(&diff, diff_file, strlen(diff_file)));
	cl_git_fail_with(GIT3_EAPPLYFAIL, git3_apply(repo, diff, GIT3_APPLY_LOCATION_WORKDIR, NULL));
