...
void test_submodule_status__unchanged(void)
{
	...
	unsigned int expected =
		...
		GIT3_SUBMODULE_STATUS_IN_CONFIG |
		GIT3_SUBMODULE_STATUS_IN_WD;

	cl_assert(GIT3_SUBMODULE_STATUS_IS_UNMODIFIED(status));
...
static void rm_submodule(const char *name)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(g_repo), name));
	cl_git_pass(git3_futils_rmdir_r(path.ptr, NULL, GIT3_RMDIR_REMOVE_FILES));
...
{
	git3_submodule *sm;
	cl_git_pass(git3_submodule_lookup(&sm, g_repo, name));
	cl_git_pass(git3_submodule_add_to_index(sm, true));
...
static void rm_submodule_from_index(const char *name)
{
	...
	cl_assert(!git3_index_find(&pos, index, name));
	cl_git_pass(git3_index_remove(index, name, 0));
	cl_git_pass(git3_index_write(index));
	git3_index_free(index);
}

/* 4 values of GIT3_SUBMODULE_IGNORE to check */
...
void test_submodule_status__ignore_none(void)
{
	...
	refute_submodule_exists(g_repo, "not", GIT3_EEXISTS);

	status = get_submodule_status(g_repo, "sm_changed_index");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_INDEX_MODIFIED) != 0);

	status = get_submodule_status(g_repo, "sm_changed_head");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_MODIFIED) != 0);

	status = get_submodule_status(g_repo, "sm_changed_file");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_WD_MODIFIED) != 0);

	status = get_submodule_status(g_repo, "sm_changed_untracked_file");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_UNTRACKED) != 0);

	status = get_submodule_status(g_repo, "sm_missing_commits");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_MODIFIED) != 0);

	status = get_submodule_status(g_repo, "sm_added_and_uncommited");
	cl_assert((status & GIT3_SUBMODULE_STATUS_INDEX_ADDED) != 0);

	/* removed sm_unchanged for deleted workdir */
	status = get_submodule_status(g_repo, "sm_unchanged");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_DELETED) != 0);

	/* now mkdir sm_unchanged to test uninitialized */
	cl_git_pass(git3_futils_mkdir_relative("sm_unchanged", "submod2", 0755, 0, NULL));
	status = get_submodule_status(g_repo, "sm_unchanged");
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_UNINITIALIZED) != 0);

	/* update sm_changed_head in index */
	add_submodule_to_index("sm_changed_head");
	status = get_submodule_status(g_repo, "sm_changed_head");
	cl_assert((status & GIT3_SUBMODULE_STATUS_INDEX_MODIFIED) != 0);

	/* remove sm_changed_head from index */
	rm_submodule_from_index("sm_changed_head");
	status = get_submodule_status(g_repo, "sm_changed_head");
	cl_assert((status & GIT3_SUBMODULE_STATUS_INDEX_DELETED) != 0);
}

void test_submodule_status__ignore_untracked(void)
{
	unsigned int status;
	git3_submodule_ignore_t ign = GIT3_SUBMODULE_IGNORE_UNTRACKED;
	...
	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_changed_index", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_INDEX_MODIFIED) != 0);

	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_changed_head", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_MODIFIED) != 0);

	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_changed_file", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_WD_MODIFIED) != 0);

	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_changed_untracked_file", ign));
	cl_assert(GIT3_SUBMODULE_STATUS_IS_UNMODIFIED(status));

	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_missing_commits", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_MODIFIED) != 0);

	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_added_and_uncommited", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_INDEX_ADDED) != 0);

	/* removed sm_unchanged for deleted workdir */
	cl_git_pass(git3_submodule_status(&status, g_repo,"sm_unchanged", ign));
	cl_assert((status & GIT3_SUBMODULE_STATUS_WD_DELETED) != 0);
...
{
	unsigned int status;
	git3_submodule_ignore_t ign = GIT3_SUBMODULE_IGNORE_DIRTY;
...
{
	unsigned int status;
	git3_submodule_ignore_t ign = GIT3_SUBMODULE_IGNORE_ALL;
...
void test_submodule_status__iterator(void)
{
	...
	static int expected_flags[] = {
		GIT3_STATUS_INDEX_MODIFIED | GIT3_STATUS_WT_MODIFIED, /* ".gitmodules" */
		-1,					    /* "just_a_dir/" will be skipped */
		GIT3_STATUS_CURRENT,     /* "just_a_dir/contents" */
		GIT3_STATUS_CURRENT,	    /* "just_a_file" */
		GIT3_STATUS_WT_NEW,      /* "not-submodule/" untracked item */
		-1,                     /* "not-submodule/README.txt" */
		GIT3_STATUS_WT_NEW,      /* "not/" untracked item */
		-1,                     /* "not/README.txt" */
		GIT3_STATUS_CURRENT,     /* "README.txt */
		GIT3_STATUS_INDEX_NEW,   /* "sm_added_and_uncommited" */
		GIT3_STATUS_WT_MODIFIED, /* "sm_changed_file" */
		GIT3_STATUS_WT_MODIFIED, /* "sm_changed_head" */
		GIT3_STATUS_WT_MODIFIED, /* "sm_changed_index" */
		GIT3_STATUS_WT_MODIFIED, /* "sm_changed_untracked_file" */
		GIT3_STATUS_WT_MODIFIED, /* "sm_missing_commits" */
		GIT3_STATUS_CURRENT,     /* "sm_unchanged" */
		0
	};
	...
	iter_opts.flags = GIT3_ITERATOR_IGNORE_CASE | GIT3_ITERATOR_INCLUDE_TREES;

	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_iterator_for_workdir(&iter, g_repo, index, NULL, &iter_opts));

	for (i = 0; !git3_iterator_advance(&entry, iter); ++i)
	...
	opts.flags = GIT3_STATUS_OPT_INCLUDE_UNTRACKED |
		...
		GIT3_STATUS_OPT_SORT_CASE_INSENSITIVELY;
...
void test_submodule_status__untracked_dirs_containing_ignored_files(void)
{
	...
	cl_git_pass(
		git3_futils_mkdir_relative("sm_unchanged/directory", "submod2", 0755, 0, NULL));
	cl_git_mkfile(
	...
	expected = GIT3_SUBMODULE_STATUS_IN_HEAD |
		GIT3_SUBMODULE_STATUS_IN_INDEX |
		GIT3_SUBMODULE_STATUS_IN_CONFIG |
		GIT3_SUBMODULE_STATUS_IN_WD;
