...
void test_submodule_update__uninitialized_submodule_no_init(void)
{
	...
	cl_git_fail_with(
		GIT3_ERROR,
		git3_submodule_update(sm, 0, &update_options));
...
void test_submodule_update__update_submodule(void)
{
	...
	cl_git_pass(git3_submodule_status(&submodule_status, g_repo, "testrepo", GIT3_SUBMODULE_IGNORE_UNSPECIFIED));
	cl_assert_equal_i(submodule_status, GIT3_SUBMODULE_STATUS_IN_HEAD |
		GIT3_SUBMODULE_STATUS_IN_INDEX |
		GIT3_SUBMODULE_STATUS_IN_CONFIG |
		GIT3_SUBMODULE_STATUS_WD_UNINITIALIZED);

	/* initialize and update the submodule */
	cl_git_pass(git3_submodule_init(sm, 0));
	cl_git_pass(git3_submodule_update(sm, 0, &update_options));
	...
	cl_assert(git3_oid_streq(git3_submodule_head_id(sm), "be3563ae3f795b2b4353bcce3a527ad0a4f7f644") == 0);
	cl_assert(git3_oid_streq(git3_submodule_wd_id(sm), "be3563ae3f795b2b4353bcce3a527ad0a4f7f644") == 0);
	cl_assert(git3_oid_streq(git3_submodule_index_id(sm), "be3563ae3f795b2b4353bcce3a527ad0a4f7f644") == 0);
...
void test_submodule_update__update_submodule_with_path(void)
{
	...
	cl_assert(git3_oid_streq(git3_submodule_head_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
	cl_assert(git3_oid_streq(git3_submodule_wd_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
	cl_assert(git3_oid_streq(git3_submodule_index_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
...
void test_submodule_update__update_skip_configured_missing_submodule(void)
{
	...
	cl_git_pass(git3_submodule_status(&submodule_status, g_repo, "sm_gitmodules_only", GIT3_SUBMODULE_IGNORE_UNSPECIFIED));
	cl_assert_equal_i(submodule_status, GIT3_SUBMODULE_STATUS_IN_CONFIG);
...
void test_submodule_update__update_already_checked_out_submodule(void)
{
	...
	git3_submodule_update_options update_options = GIT3_SUBMODULE_UPDATE_OPTIONS_INIT;
	unsigned int submodule_status = 0;
	git3_reference *branch_reference = NULL;
	git3_object *branch_commit = NULL;
	struct update_submodule_cb_payload update_payload = { 0 };
	...
	checkout_options.checkout_strategy = GIT3_CHECKOUT_SAFE;

	cl_git_pass(git3_reference_lookup(&branch_reference, g_repo, "refs/heads/alternate_1"));
	cl_git_pass(git3_reference_peel(&branch_commit, branch_reference, GIT3_OBJECT_COMMIT));
	cl_git_pass(git3_checkout_tree(g_repo, branch_commit, &checkout_options));
	cl_git_pass(git3_repository_set_head(g_repo, git3_reference_name(branch_reference)));
	...
	cl_assert_equal_i(submodule_status, GIT3_SUBMODULE_STATUS_IN_HEAD |
		...
		GIT3_SUBMODULE_STATUS_IN_CONFIG |
		GIT3_SUBMODULE_STATUS_IN_WD |
		GIT3_SUBMODULE_STATUS_WD_MODIFIED);
	...
	git3_submodule_free(sm);
	git3_object_free(branch_commit);
	git3_reference_free(branch_reference);
}
...
void test_submodule_update__update_blocks_on_dirty_wd(void)
{
	...
	update_options.checkout_opts.notify_flags = GIT3_CHECKOUT_NOTIFY_CONFLICT;
	...
	cl_git_fail(git3_submodule_update(sm, 0, &update_options));
...
void test_submodule_update__can_force_update(void)
{
	...
	update_options.checkout_opts.checkout_strategy = GIT3_CHECKOUT_FORCE;
	cl_git_pass(git3_submodule_update(sm, 0, &update_options));
	cl_assert(git3_oid_streq(git3_submodule_head_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
	cl_assert(git3_oid_streq(git3_submodule_wd_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
	cl_assert(git3_oid_streq(git3_submodule_index_id(sm), "a65fedf39aefe402d3bb6e24df4d4f5fe4547750") == 0);
