...
static int find_evil(git3_submodule *sm, const char *name, void *payload)
{
	int *foundit = (int *) payload;

	GIT3_UNUSED(sm);

	if (!git3__strcmp(EVIL_SM_NAME, name) ||
	    !git3__strcmp(EVIL_SM_NAME_WINDOWS_UNESC, name))
		*foundit = true;
...
void test_submodule_escape__from_gitdir(void)
{
	...
	foundit = 0;
	cl_git_pass(git3_submodule_foreach(g_repo, find_evil, &foundit));
	cl_assert_equal_i(0, foundit);
	cl_git_fail_with(GIT3_ENOTFOUND, git3_submodule_lookup(&sm, g_repo, EVIL_SM_NAME));
	/*
	...
	cl_git_pass(git3_submodule_location(&sm_location, sm));
	cl_assert_equal_i(GIT3_SUBMODULE_STATUS_IN_INDEX | GIT3_SUBMODULE_STATUS_IN_HEAD, sm_location);
...
void test_submodule_escape__from_gitdir_windows(void)
{
	...
	cl_git_pass(git3_submodule_foreach(g_repo, find_evil, &foundit));
	cl_assert_equal_i(0, foundit);
	cl_git_fail_with(GIT3_ENOTFOUND, git3_submodule_lookup(&sm, g_repo, EVIL_SM_NAME_WINDOWS_UNESC));
	/*
	 * We do know about this as it's in the index and HEAD, but the data is
	 * incomplete as there is no configured data for it (we pretend it
	 * doesn't exist). This leaves us with an odd situation but it's
