...
static void populate_fetchhead(git3_vector *out, git3_repository *repo)
{
	...
	cl_git_pass(git3_oid_from_string(&oid,
		...
		GIT3_OID_SHA1));
	cl_git_pass(git3_fetchhead_ref_create(&fetchhead_ref, &oid, 1,
		"refs/heads/master",
		"https://github.com/libgit3/TestGitRepository"));
	cl_git_pass(git3_vector_insert(out, fetchhead_ref));

	cl_git_pass(git3_oid_from_string(&oid,
		"0966a434eb1a025db6b71485ab63a3bfbea520b6",
		GIT3_OID_SHA1));
	cl_git_pass(git3_fetchhead_ref_create(&fetchhead_ref, &oid, 0,
	...
	cl_git_pass(git3_fetchhead_write(repo, out));
}

void test_fetchhead_nonetwork__write(void)
{
	git3_vector fetchhead_vector = GIT3_VECTOR_INIT;
	git3_fetchhead_ref *fetchhead_ref;
	git3_str fetchhead_buf = GIT3_STR_INIT;
	int equals = 0;
	size_t i;

	cl_git_pass(git3_vector_init(&fetchhead_vector, 6, NULL));
	...
	cl_git_pass(git3_futils_readbuffer(&fetchhead_buf,
	...
	git3_vector_foreach(&fetchhead_vector, i, fetchhead_ref) {
		git3_fetchhead_ref_free(fetchhead_ref);
	}

	git3_vector_dispose(&fetchhead_vector);

	cl_assert(equals);
}

typedef struct {
	git3_vector *fetchhead_vector;
	size_t idx;
} fetchhead_ref_cb_data;

static int fetchhead_ref_cb(const char *name, const char *url,
	const git3_oid *oid, unsigned int is_merge, void *payload)
{
	fetchhead_ref_cb_data *cb_data = payload;
	git3_fetchhead_ref *expected;

	cl_assert(payload);

	expected = git3_vector_get(cb_data->fetchhead_vector, cb_data->idx);
...
void test_fetchhead_nonetwork__read(void)
{
	...
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, fetchhead_ref_cb, &cb_data));
	...
	const git3_oid *oid, unsigned int is_merge, void *payload)
{
	...
	git3_oid_from_string(&expected, "49322bb17d3acc9146f98c97d078513228bbf3c0", GIT3_OID_SHA1);
...
void test_fetchhead_nonetwork__read_old_style(void)
{
	...
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, read_old_style_cb, NULL));
}
...
void test_fetchhead_nonetwork__type_missing(void)
{
	...
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, read_type_missing, NULL));
}
...
void test_fetchhead_nonetwork__name_missing(void)
{
	...
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, read_name_missing, NULL));
}

static int read_noop(const char *ref_name, const char *remote_url,
	const git3_oid *oid, unsigned int is_merge, void *payload)
{
	GIT3_UNUSED(ref_name);
	GIT3_UNUSED(remote_url);
	GIT3_UNUSED(oid);
	GIT3_UNUSED(is_merge);
	GIT3_UNUSED(payload);
...
void test_fetchhead_nonetwork__nonexistent(void)
{
	...
	cl_git_fail((error = git3_repository_fetchhead_foreach(g_repo, read_noop, NULL)));
...
void test_fetchhead_nonetwork__invalid_unterminated_last_line(void)
{
	...
	cl_git_fail(git3_repository_fetchhead_foreach(g_repo, read_noop, NULL));
}
...
void test_fetchhead_nonetwork__invalid_for_merge(void)
{
	...
	cl_assert(git3__prefixcmp(git3_error_last()->message, "invalid for-merge") == 0);
}
...
void test_fetchhead_nonetwork__invalid_description(void)
{
	...
	cl_assert(git3__prefixcmp(git3_error_last()->message, "invalid description") == 0);
}

static int assert_master_for_merge(const char *ref, const char *url, const git3_oid *id, unsigned int is_merge, void *data)
{
	GIT3_UNUSED(url);
	GIT3_UNUSED(id);
	GIT3_UNUSED(data);
...
static int assert_none_for_merge(const char *ref, const char *url, const git3_oid *id, unsigned int is_merge, void *data)
{
	GIT3_UNUSED(ref);
	GIT3_UNUSED(url);
	GIT3_UNUSED(id);
...
void test_fetchhead_nonetwork__unborn_with_upstream(void)
{
	...
	cl_git_pass(git3_remote_set_url(repo, "origin", cl_fixture("testrepo.git")));
	...
	cl_git_pass(git3_repository_fetchhead_foreach(repo, assert_master_for_merge, NULL));
...
void test_fetchhead_nonetwork__fetch_into_repo_with_symrefs(void)
{
	 ...
	 */
	cl_git_pass(git3_reference_symbolic_create(&symref, repo, "refs/heads/symref", "refs/heads/master", 0, NULL));
...
void test_fetchhead_nonetwork__fetch_into_repo_with_invalid_head(void)
{
	...
	git3_strarray refspecs = { strings, 1 };
	...
	cl_git_pass(git3_remote_create_anonymous(&remote, g_repo, cl_fixture("testrepo.git")));
	cl_git_pass(git3_remote_fetch(remote, &refspecs, NULL, NULL));
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, assert_none_for_merge, NULL));
...
void test_fetchhead_nonetwork__quote_in_branch_name(void)
{
	...
	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, read_noop, NULL));
}
...
static int find_master_haacked(const char *ref_name, const char *remote_url, const git3_oid *oid, unsigned int is_merge, void *payload)
{
	GIT3_UNUSED(remote_url);
	GIT3_UNUSED(oid);
	GIT3_UNUSED(payload);
...
void test_fetchhead_nonetwork__create_when_refpecs_given(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_path(g_repo), "FETCH_HEAD"));
	cl_git_pass(git3_remote_create(&remote, g_repo, "origin", cl_fixture("testrepo.git")));

	cl_assert(!git3_fs_path_exists(path.ptr));
	cl_git_pass(git3_remote_fetch(remote, &specs, NULL, NULL));
	cl_assert(git3_fs_path_exists(path.ptr));

	cl_git_pass(git3_repository_fetchhead_foreach(g_repo, find_master_haacked, NULL));
...
static int count_refs(const char *ref_name, const char *remote_url, const git3_oid *oid, unsigned int is_merge, void *payload)
{
	int i;
	...
	for (i = 0; prefix_counts[i].prefix; i++) {
		if (!git3__prefixcmp(ref_name, prefix_counts[i].prefix))
...
void test_fetchhead_nonetwork__create_with_multiple_refspecs(void)
{
	...
	cl_git_pass(git3_remote_create(&remote, g_repo, "origin", cl_fixture("testrepo.git")));
	git3_remote_free(remote);
	cl_git_pass(git3_remote_add_fetch(g_repo, "origin", "+refs/notes/*:refs/origin/notes/*"));
...

	{
		...
		cl_git_pass(git3_repository_fetchhead_foreach(g_repo, count_refs, &prefix_counts));
...
{
	git3_fetchhead_ref *ref;
	git3_oid oid;

	cl_git_pass(git3_oid_from_string(&oid, "49322bb17d3acc9146f98c97d078513228bbf3c0", GIT3_OID_SHA1));
	cl_git_pass(git3_fetchhead_ref_create(&ref, &oid, 0,
		"refs/tags/commit_tree", "http://foo:bar@github.com/libgit3/TestGitRepository"));
	cl_assert_equal_s(ref->remote_url, "http://github.com/libgit3/TestGitRepository");
	git3_fetchhead_ref_free(ref);

	cl_git_pass(git3_oid_from_string(&oid, "49322bb17d3acc9146f98c97d078513228bbf3c0", GIT3_OID_SHA1));
	cl_git_pass(git3_fetchhead_ref_create(&ref, &oid, 0,
		"refs/tags/commit_tree", "https://foo:bar@github.com/libgit3/TestGitRepository"));
	cl_assert_equal_s(ref->remote_url, "https://github.com/libgit3/TestGitRepository");
	git3_fetchhead_ref_free(ref);
}
