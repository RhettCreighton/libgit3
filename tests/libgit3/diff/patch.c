...
void test_diff_patch__can_properly_display_the_removal_of_a_file(void)
{
	...
	*/
	...
	cl_git_pass(git3_diff_tree_to_tree(&diff, g_repo, one, another, NULL));

	cl_git_pass(git3_diff_print(
		diff, GIT3_DIFF_FORMAT_PATCH, check_removal_cb, NULL));

	git3_diff_free(diff);

	git3_tree_free(another);
	git3_tree_free(one);
}
...
void test_diff_patch__can_cancel_diff_print(void)
{
	...
	cl_git_fail_with(git3_diff_print(
		diff, GIT3_DIFF_FORMAT_PATCH, check_removal_cb, &fail_with),
		fail_with);
...
void test_diff_patch__to_string(void)
{
	...
	cl_assert_equal_i(1, (int)git3_diff_num_deltas(diff));
	...
	cl_assert_equal_sz(31, git3_patch_size(patch, 0, 0, 0));
	cl_assert_equal_sz(31, git3_patch_size(patch, 1, 0, 0));
	cl_assert_equal_sz(31 + 16, git3_patch_size(patch, 1, 1, 0));
	cl_assert_equal_sz(strlen(expected), git3_patch_size(patch, 1, 1, 1));
...
void test_diff_patch__config_options(void)
{
	...
	cl_git_pass(git3_config_set_string(cfg, "diff.mnemonicprefix", "true"));

	cl_git_pass(git3_diff_tree_to_index(&diff, g_repo, one, NULL, &opts));
	...
	cl_git_pass(git3_config_set_string(cfg, "diff.noprefix", "true"));
	...
	cl_git_pass(git3_config_set_int32(cfg, "core.abbrev", 12));
...
void test_diff_patch__hunks_have_correct_line_numbers(void)
{
	...
	const git3_diff_line *line;
	size_t hunklen;
	git3_str old_content = GIT3_STR_INIT, actual = GIT3_STR_INIT;
	...
	git3_repository_set_config(g_repo, cfg);
	git3_config_free(cfg);

	git3_repository_reinit_filesystem(g_repo, false);
	...
	cl_git_pass(git3_diff_tree_to_workdir(&diff, g_repo, head, &opt));
	...
	cl_assert_equal_i(2, (int)git3_patch_num_hunks(patch));

	/* check hunk 0 */

	cl_git_pass(
		git3_patch_get_hunk(&hunk, &hunklen, patch, 0));
	...
	cl_assert_equal_i(18, (int)git3_patch_num_lines_in_hunk(patch, 0));

	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 0));
	cl_assert_equal_i(GIT3_DIFF_LINE_CONTEXT, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 3));
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 12));
	...
	cl_git_pass(git3_patch_get_hunk(&hunk, &hunklen, patch, 1));
	...
	cl_assert_equal_i(18, (int)git3_patch_num_lines_in_hunk(patch, 1));

	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 1, 0));
	cl_assert_equal_i(GIT3_DIFF_LINE_CONTEXT, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 1, 3));
	cl_assert_equal_i(GIT3_DIFF_LINE_DELETION, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 1, 12));
	cl_assert_equal_i(GIT3_DIFF_LINE_ADDITION, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	...
	cl_assert_equal_i(GIT3_DELTA_MODIFIED, (int)delta->status);
	cl_assert_equal_i(1, (int)git3_patch_num_hunks(patch));

	/* check hunk 0 */

	cl_git_pass(git3_patch_get_hunk(&hunk, &hunklen, patch, 0));
	...
	cl_assert_equal_i(6, (int)git3_patch_num_lines_in_hunk(patch, 0));

	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 1));
	cl_assert_equal_i(GIT3_DIFF_LINE_CONTEXT, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 2));
	cl_assert_equal_i(GIT3_DIFF_LINE_CONTEXT, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
	cl_assert_equal_s("\n", actual.ptr);
	...
	cl_git_pass(git3_patch_get_line_in_hunk(&line, patch, 0, 5));
	cl_assert_equal_i(GIT3_DIFF_LINE_DEL_EOFNL, (int)line->origin);
	cl_git_pass(git3_str_set(&actual, line->content, line->content_len));
...
static void check_single_patch_stats(
	git3_repository *repo, size_t hunks,
	...
	const char *expected)
{
	...
	cl_git_pass(git3_diff_index_to_workdir(&diff, repo, NULL, NULL));
	...
	cl_assert_equal_i((int)hunks, (int)git3_patch_num_hunks(patch));

	cl_git_pass( git3_patch_line_stats(
	...
	if (expected != NULL) {
		...
		cl_assert_equal_sz(
			strlen(expected), git3_patch_size(patch, 1, 1, 1));
	}

	if (sizes) {
		if (sizes[0])
			cl_assert_equal_sz(sizes[0], git3_patch_size(patch, 0, 0, 0));
		if (sizes[1])
			cl_assert_equal_sz(sizes[1], git3_patch_size(patch, 1, 0, 0));
		if (sizes[2])
			cl_assert_equal_sz(sizes[2], git3_patch_size(patch, 1, 1, 0));
	}
	...
	for (; hunks > 0; --hunks) {
		...
		max_i = git3_patch_num_lines_in_hunk(patch, hunks - 1);

		for (i = 0; i < max_i; ++i) {
			int expected = 1;

			cl_git_pass(
				git3_patch_get_line_in_hunk(&line, patch, hunks - 1, i));

			if (line->origin == GIT3_DIFF_LINE_ADD_EOFNL ||
				line->origin == GIT3_DIFF_LINE_DEL_EOFNL ||
				line->origin == GIT3_DIFF_LINE_CONTEXT_EOFNL)
				expected = 0;
...
void test_diff_patch__line_counts_with_eofnl(void)
{
	...
	cl_git_pass(git3_futils_readbuffer(&content, "renames/songof7cities.txt"));

	/* remove first line */

	end = git3_str_cstr(&content) + git3_str_find(&content, '\n') + 1;
	git3_str_consume(&content, end);
	...
	cl_git_pass(git3_str_putc(&content, '\n'));
	cl_git_rewritefile("renames/songof7cities.txt", content.ptr);
...
void test_diff_patch__can_strip_bad_utf8(void)
{
	...
	cl_git_pass(git3_patch_from_buffers(&patch, a, strlen(a), NULL, b, strlen(b), NULL, &opts));
	cl_git_pass(git3_patch_to_buf(&buf, patch));
