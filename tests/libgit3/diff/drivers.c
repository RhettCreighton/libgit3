...
static void overwrite_filemode(const char *expected, git3_buf *actual)
{
	size_t offset;
	char *found;
...
void test_diff_drivers__patterns(void)
{
	...
	cl_git_pass(git3_diff_tree_to_workdir(&diff, g_repo, one, NULL));
	cl_assert_equal_i(0, (int)git3_diff_num_deltas(diff));
	git3_diff_free(diff);
	...
	cl_git_pass(git3_config_set_bool(cfg, "diff.kipling0.binary", 1));
	...
	git3_diff_driver_registry_free(g_repo->diff_drivers);
	g_repo->diff_drivers = NULL;

	cl_git_pass(git3_repository_config(&cfg, g_repo));
	cl_git_pass(git3_config_set_bool(cfg, "diff.kipling0.binary", 0));
	cl_git_pass(git3_config_set_string(cfg, "diff.kipling0.xfuncname", "^H.*$"));
	git3_config_free(cfg);
...
void test_diff_drivers__long_lines(void)
{
	...
	cl_git_pass(git3_index_add_bypath(idx, "longlines.txt"));
	...
	cl_assert_equal_sz(1, git3_diff_num_deltas(diff));
	cl_git_pass(git3_patch_from_diff(&patch, diff, 0));
	cl_git_pass(git3_patch_to_buf(&actual, patch));
...
void test_diff_drivers__builtins(void)
{
	git3_diff *diff;
	git3_patch *patch;
	git3_str file = GIT3_STR_INIT, expected = GIT3_STR_INIT;
	git3_buf actual = GIT3_BUF_INIT;
	...
	cl_git_pass(git3_fs_path_dirload(&files, "userdiff/files", 9, 0));

	opts.interhunk_lines = 1;
	opts.context_lines = 1;
	opts.pathspec.count = 1;

	git3_vector_foreach(&files, i, path) {
		if (git3__prefixcmp(path, "files/file."))
			continue;
		...
		git3_str_sets(&expected, "userdiff/expected/nodriver/diff.");
		git3_str_puts(&expected, extension);
		cl_git_pass(git3_futils_readbuffer(&expected, expected.ptr));
		...
		git3_str_sets(&expected, "userdiff/expected/driver/diff.");
		git3_str_puts(&expected, extension);
		cl_git_pass(git3_futils_readbuffer(&expected, expected.ptr));
...
void test_diff_drivers__invalid_pattern(void)
{
	...
	cl_git_pass(git3_repository_config__weakptr(&cfg, g_repo));
	cl_git_pass(git3_config_set_string(cfg, "diff.storyboard.xfuncname", "<!--(.*?)-->"));

	cl_git_mkfile("userdiff/dummy.storyboard", "");
	cl_git_pass(git3_repository_index__weakptr(&idx, g_repo));
	cl_git_pass(git3_index_add_bypath(idx, "dummy.storyboard"));
