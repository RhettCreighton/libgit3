...
static git3_diff_options opts;
static git3_blob *d, *alien;

static void quick_diff_blob_to_str(
	const git3_blob *blob, const char *blob_path,
	const char *str, size_t len, const char *str_path)
{
	...
	cl_git_pass(git3_diff_blob_to_buffer(
...
void test_diff_blob__initialize(void)
{
	...
	cl_git_pass(git3_diff_options_init(&opts, GIT3_DIFF_OPTIONS_VERSION));
	opts.context_lines = 1;

	memset(&expected, 0, sizeof(expected));

	/* tests/resources/attr/root_test4.txt */
	cl_git_pass(git3_oid_from_prefix(&oid, "a0f7217a", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&d, g_repo, &oid, 8));

	/* alien.png */
	cl_git_pass(git3_oid_from_prefix(&oid, "edf3dcee", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&alien, g_repo, &oid, 8));
}
...
void test_diff_blob__patch_with_freed_blobs(void)
{
	...
	cl_git_pass(git3_oid_from_prefix(&a_oid, "45141a79", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&a, g_repo, &a_oid, 4));
	/* tests/resources/attr/root_test2 */
	cl_git_pass(git3_oid_from_prefix(&b_oid, "4d713dc4", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&b, g_repo, &b_oid, 4));

	cl_git_pass(git3_patch_from_blobs(&p, a, NULL, b, NULL, NULL));

	git3_blob_free(a);
	git3_blob_free(b);

	cl_git_pass(git3_patch_to_buf(&buf, p));
...
void test_diff_blob__can_compare_text_blobs(void)
{
	...
	cl_git_pass(git3_oid_from_prefix(&c_oid, "c96bbb2c2557a832", 16, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&c, g_repo, &c_oid, 16));

	/* Doing the equivalent of a `git diff -U1` on these files */

	/* diff on tests/resources/attr/root_test1 */
	memset(&expected, 0, sizeof(expected));
	cl_git_pass(git3_diff_blobs(
		a, NULL, b, NULL, &opts,
	...
	memset(&expected, 0, sizeof(expected));
	cl_git_pass(git3_diff_buffers(
		git3_blob_rawcontent(a), (size_t)git3_blob_rawsize(a), NULL,
		git3_blob_rawcontent(b), (size_t)git3_blob_rawsize(b), NULL, &opts,
		diff_file_cb, diff_binary_cb, diff_hunk_cb, diff_line_cb, &expected));
	assert_one_modified(1, 6, 1, 5, 0, &expected);
...
static void assert_patch_matches_blobs(
	git3_patch *p, git3_blob *a, git3_blob *b,
	int hunks, int l0, int l1, int ctxt, int adds, int dels)
{
	...
	delta = git3_patch_get_delta(p);
	cl_assert(delta != NULL);

	cl_assert_equal_i(GIT3_DELTA_MODIFIED, delta->status);
	cl_assert_equal_oid(git3_blob_id(a), &delta->old_file.id);
	cl_assert_equal_sz(git3_blob_rawsize(a), delta->old_file.size);
	cl_assert_equal_oid(git3_blob_id(b), &delta->new_file.id);
	cl_assert_equal_sz(git3_blob_rawsize(b), delta->new_file.size);

	cl_assert_equal_i(hunks, (int)git3_patch_num_hunks(p));

	if (hunks > 0)
		cl_assert_equal_i(l0, git3_patch_num_lines_in_hunk(p, 0));
	if (hunks > 1)
		cl_assert_equal_i(l1, git3_patch_num_lines_in_hunk(p, 1));

	cl_git_pass(git3_patch_line_stats(&tc, &ta, &td, p));
...
void test_diff_blob__can_compare_text_blobs_with_patch(void)
{
	...
	/* tests/resources/attr/root_test1 */
	cl_git_pass(git3_oid_from_prefix(&a_oid, "45141a79", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&a, g_repo, &a_oid, 8));

	/* tests/resources/attr/root_test2 */
	cl_git_pass(git3_oid_from_prefix(&b_oid, "4d713dc4", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&b, g_repo, &b_oid, 8));
	...
	/* diff on tests/resources/attr/root_test1 */
	cl_git_pass(git3_patch_from_blobs(&p, a, NULL, b, NULL, &opts));
	assert_patch_matches_blobs(p, a, b, 1, 6, 0, 1, 5, 0);
	git3_patch_free(p);

	/* diff on tests/resources/attr/root_test2 */
	cl_git_pass(git3_patch_from_blobs(&p, b, NULL, c, NULL, &opts));
	assert_patch_matches_blobs(p, b, c, 1, 15, 0, 3, 9, 3);
	git3_patch_free(p);

	/* diff on tests/resources/attr/root_test3 */
	cl_git_pass(git3_patch_from_blobs(&p, a, NULL, c, NULL, &opts));
	assert_patch_matches_blobs(p, a, c, 1, 13, 0, 0, 12, 1);
	git3_patch_free(p);

	/* one more */
	cl_git_pass(git3_patch_from_blobs(&p, c, NULL, d, NULL, &opts));
...
void test_diff_blob__can_compare_against_null_blobs(void)
{
	...
	cl_assert_equal_i(1, expected.file_status[GIT3_DELTA_DELETED]);
	...
	cl_assert_equal_i(1, expected.files);
	cl_assert_equal_i(1, expected.file_status[GIT3_DELTA_ADDED]);
	cl_assert_equal_i(0, expected.files_binary);
...
void test_diff_blob__can_compare_against_null_blobs_with_patch(void)
{
	...
	cl_git_pass(git3_patch_from_blobs(&p, d, NULL, e, NULL, &opts));

	cl_assert(p != NULL);

	delta = git3_patch_get_delta(p);
	cl_assert(delta != NULL);
	cl_assert_equal_i(GIT3_DELTA_DELETED, delta->status);
	cl_assert_equal_oid(git3_blob_id(d), &delta->old_file.id);
	cl_assert_equal_sz(git3_blob_rawsize(d), delta->old_file.size);
	cl_assert(git3_oid_is_zero(&delta->new_file.id));
	cl_assert_equal_sz(0, delta->new_file.size);

	cl_assert_equal_i(1, (int)git3_patch_num_hunks(p));
	cl_assert_equal_i(14, git3_patch_num_lines_in_hunk(p, 0));

	max_l = git3_patch_num_lines_in_hunk(p, 0);
	for (l = 0; l < max_l; ++l) {
		cl_git_pass(git3_patch_get_line_in_hunk(&line, p, 0, l));
		cl_assert_equal_i(GIT3_DIFF_LINE_DELETION, (int)line->origin);
	}
	...
	delta = git3_patch_get_delta(p);
	cl_assert(delta != NULL);
	cl_assert_equal_i(GIT3_DELTA_ADDED, delta->status);
	cl_assert(git3_oid_is_zero(&delta->old_file.id));
	cl_assert_equal_sz(0, delta->old_file.size);
	cl_assert_equal_oid(git3_blob_id(d), &delta->new_file.id);
	cl_assert_equal_sz(git3_blob_rawsize(d), delta->new_file.size);
	...
	for (l = 0; l < max_l; ++l) {
		cl_git_pass(git3_patch_get_line_in_hunk(&line, p, 0, l));
		cl_assert_equal_i(GIT3_DIFF_LINE_ADDITION, (int)line->origin);
	}
	...
	cl_git_pass(git3_patch_from_blobs(&p, alien, NULL, NULL, NULL, &opts));
	...
	cl_assert_equal_i(0, (int)git3_patch_num_hunks(p));

	git3_patch_free(p);

	cl_git_pass(git3_patch_from_blobs(&p, NULL, NULL, alien, NULL, &opts));
...
{
	cl_assert_equal_i(1, expected->files);
	cl_assert_equal_i(1, expected->file_status[GIT3_DELTA_UNMODIFIED]);
	cl_assert_equal_i(0, expected->hunks);
	cl_assert_equal_i(0, expected->lines);
}
...
{
	opts.flags |= GIT3_DIFF_INCLUDE_UNMODIFIED;
...
void test_diff_blob__can_compare_identical_blobs_with_patch(void)
{
	...
	cl_git_pass(git3_patch_from_blobs(&p, d, NULL, d, NULL, &opts));
	cl_assert(p != NULL);

	delta = git3_patch_get_delta(p);
	cl_assert(delta != NULL);
	cl_assert_equal_i(GIT3_DELTA_UNMODIFIED, delta->status);
	cl_assert_equal_sz(delta->old_file.size, git3_blob_rawsize(d));
	cl_assert_equal_oid(git3_blob_id(d), &delta->old_file.id);
	cl_assert_equal_sz(delta->new_file.size, git3_blob_rawsize(d));
	cl_assert_equal_oid(git3_blob_id(d), &delta->new_file.id);

	cl_assert_equal_i(0, (int)git3_patch_num_hunks(p));
	git3_patch_free(p);

	cl_git_pass(git3_patch_from_blobs(&p, NULL, NULL, NULL, NULL, &opts));
	cl_assert(p != NULL);
	...
	cl_git_pass(git3_patch_from_blobs(&p, alien, NULL, alien, NULL, &opts));
	cl_assert(p != NULL);
	cl_assert_equal_i(GIT3_DELTA_UNMODIFIED, git3_patch_get_delta(p)->status);
	cl_assert_equal_i(0, (int)git3_patch_num_hunks(p));
	git3_patch_free(p);
}
...
static void assert_binary_blobs_comparison(diff_expects *expected)
{
	...
	cl_assert_equal_i(1, expected->files);
	cl_assert_equal_i(1, expected->file_status[GIT3_DELTA_MODIFIED]);
	cl_assert_equal_i(0, expected->hunks);
	cl_assert_equal_i(0, expected->lines);
}
...
void test_diff_blob__can_compare_two_binary_blobs(void)
{
	...
	/* heart.png */
	cl_git_pass(git3_oid_from_prefix(&h_oid, "de863bff", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&heart, g_repo, &h_oid, 8));
...
void test_diff_blob__comparing_two_text_blobs_honors_interhunkcontext(void)
{
	...
	cl_git_pass(git3_oid_from_prefix(&old_d_oid, "fe773770", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&old_d, g_repo, &old_d_oid, 8));
...
{
	/* alien.png */
	cl_assert_equal_i(true, git3_blob_is_binary(alien));
}

void test_diff_blob__can_correctly_detect_binary_blob_data_as_binary(void)
{
	/* alien.png */
	const char *content = git3_blob_rawcontent(alien);
	size_t len = (size_t)git3_blob_rawsize(alien);
	cl_assert_equal_i(true, git3_blob_data_is_binary(content, len));
}

void test_diff_blob__can_correctly_detect_a_textual_blob_as_non_binary(void)
{
	/* tests/resources/attr/root_test4.txt */
	cl_assert_equal_i(false, git3_blob_is_binary(d));
}

void test_diff_blob__can_correctly_detect_textual_blob_data_as_non_binary(void)
{
	/* tests/resources/attr/root_test4.txt */
	const char *content = git3_blob_rawcontent(d);
	size_t len = (size_t)git3_blob_rawsize(d);
	cl_assert_equal_i(false, git3_blob_data_is_binary(content, len));
}

/*
 * git3_diff_blob_to_buffer tests
 */

static void assert_changed_single_one_line_file(
	diff_expects *expected, git3_delta_t mod)
{
	cl_assert_equal_i(1, expected->files);
	...
	if (mod == GIT3_DELTA_ADDED)
		cl_assert_equal_i(1, expected->line_adds);
	else if (mod == GIT3_DELTA_DELETED)
...
void test_diff_blob__can_compare_blob_to_buffer(void)
{
	...
	assert_changed_single_one_line_file(&expected, GIT3_DELTA_ADDED);

	/* diff from blob a to NULL buffer */
	memset(&expected, 0, sizeof(expected));
	quick_diff_blob_to_str(a, NULL, NULL, 0, NULL);
	assert_changed_single_one_line_file(&expected, GIT3_DELTA_DELETED);
...
void test_diff_blob__can_compare_blob_to_buffer_with_patch(void)
{
	...
	/* diff from blob a to content of b */
	cl_git_pass(git3_patch_from_blob_and_buffer(
		&p, a, NULL, b_content, strlen(b_content), NULL, &opts));

	cl_assert(p != NULL);
	cl_assert_equal_i(GIT3_DELTA_MODIFIED, git3_patch_get_delta(p)->status);
	cl_assert_equal_i(1, (int)git3_patch_num_hunks(p));
	cl_assert_equal_i(6, git3_patch_num_lines_in_hunk(p, 0));
	...
	cl_assert(p != NULL);
	cl_assert_equal_i(GIT3_DELTA_ADDED, git3_patch_get_delta(p)->status);
	cl_assert_equal_i(1, (int)git3_patch_num_hunks(p));
	cl_assert_equal_i(1, git3_patch_num_lines_in_hunk(p, 0));
	git3_patch_free(p);
	...
	cl_assert(p != NULL);
	cl_assert_equal_i(GIT3_DELTA_DELETED, git3_patch_get_delta(p)->status);
	cl_assert_equal_i(1, (int)git3_patch_num_hunks(p));
	cl_assert_equal_i(1, git3_patch_num_lines_in_hunk(p, 0));
	git3_patch_free(p);
...
void test_diff_blob__binary_data_comparisons(void)
{
	...
	cl_git_pass(git3_oid_from_prefix(&oid, "45141a79", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&nonbin, g_repo, &oid, 8));

	cl_git_pass(git3_oid_from_prefix(&oid, "b435cd56", 8, GIT3_OID_SHA1));
	cl_git_pass(git3_blob_lookup_prefix(&bin, g_repo, &oid, 8));
	...
	opts.flags |= GIT3_DIFF_FORCE_TEXT;
...
void test_diff_blob__using_path_and_attributes(void)
{
	...
	cl_git_pass(git3_config_set_bool(cfg, "diff.iam_binary.binary", 1));
	cl_git_pass(git3_config_set_bool(cfg, "diff.iam_text.binary", 0));
	cl_git_pass(git3_config_set_string(
		cfg, "diff.iam_alphactx.xfuncname", "^[A-Za-z].*$"));
	cl_git_pass(git3_config_set_bool(cfg, "diff.iam_textalpha.binary", 0));
	cl_git_pass(git3_config_set_string(
		cfg, "diff.iam_textalpha.xfuncname", "^[A-Za-z].*$"));
	cl_git_pass(git3_config_set_string(
		cfg, "diff.iam_numctx.funcname", "^[0-9][0-9]*"));
	cl_git_pass(git3_config_set_bool(cfg, "diff.iam_textnum.binary", 0));
	cl_git_pass(git3_config_set_string(
