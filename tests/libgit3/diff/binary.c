...
static void test_patch(
	...
	const char *expected)
{
	git3_oid id_one, id_two;
	git3_index *index = NULL;
	git3_commit *commit_one, *commit_two = NULL;
	git3_tree *tree_one, *tree_two;
	git3_diff *diff;
	git3_patch *patch;
	git3_buf actual = GIT3_BUF_INIT;

	cl_git_pass(git3_oid_from_string(&id_one, one, GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit_one, repo, &id_one));
	cl_git_pass(git3_commit_tree(&tree_one, commit_one));

	if (two) {
		cl_git_pass(git3_oid_from_string(&id_two, two, GIT3_OID_SHA1));
		cl_git_pass(git3_commit_lookup(&commit_two, repo, &id_two));
		cl_git_pass(git3_commit_tree(&tree_two, commit_two));
	} else {
		cl_git_pass(git3_repository_index(&index, repo));
		cl_git_pass(git3_index_write_tree(&id_two, index));
		cl_git_pass(git3_tree_lookup(&tree_two, repo, &id_two));
	}

	cl_git_pass(git3_diff_tree_to_tree(&diff, repo, tree_one, tree_two, opts));

	cl_git_pass(git3_patch_from_diff(&patch, diff, 0));
	cl_git_pass(git3_patch_to_buf(&actual, patch));

	cl_assert_equal_s(expected, actual.ptr);

	git3_buf_dispose(&actual);
	cl_git_pass(git3_diff_print(diff, GIT3_DIFF_FORMAT_PATCH, git3_diff_print_callback__to_buf, &actual));
	...
	git3_diff_free(diff);
	git3_tree_free(tree_one);
	git3_tree_free(tree_two);
	git3_commit_free(commit_one);
	git3_commit_free(commit_two);
...
void test_diff_binary__delta(void)
{
	...
	opts.flags = GIT3_DIFF_SHOW_BINARY | GIT3_DIFF_FORCE_BINARY;
	opts.id_abbrev = GIT3_OID_SHA1_HEXSIZE;

	repo = cl_git_sandbox_init("renames");
	cl_git_pass(git3_repository_index(&index, repo));

	cl_git_pass(git3_futils_readbuffer(&contents, "renames/songof7cities.txt"));
...
void test_diff_binary__empty_for_no_diff(void)
{
	...
	cl_git_pass(git3_oid_from_string(&id, "19dd32dfb1520a64e5bbaae8dce6ef423dfa2f13", GIT3_OID_SHA1));
	cl_git_pass(git3_commit_lookup(&commit, repo, &id));
	cl_git_pass(git3_commit_tree(&tree, commit));

	cl_git_pass(git3_diff_tree_to_tree(&diff, repo, tree, tree, &opts));
	cl_git_pass(git3_diff_print(diff, GIT3_DIFF_FORMAT_PATCH, git3_diff_print_callback__to_buf, &actual));
...
void test_diff_binary__index_to_workdir(void)
{
	...
	cl_git_pass(git3_diff_index_to_workdir(&diff, repo, index, &opts));
...
static int print_cb(
	...
	void *payload)
{
	git3_str *buf = (git3_str *)payload;

	GIT3_UNUSED(delta);

	if (hunk)
		git3_str_put(buf, hunk->header, hunk->header_len);

	if (line)
		git3_str_put(buf, line->content, line->content_len);
...
void test_diff_binary__print_patch_from_diff(void)
{
	...
	cl_git_pass(git3_diff_print(diff, GIT3_DIFF_FORMAT_PATCH, print_cb, &actual));
...
struct diff_data {
	char *old_path;
	git3_oid old_id;
	git3_str old_binary_base85;
	size_t old_binary_inflatedlen;
	git3_diff_binary_t old_binary_type;

	char *new_path;
	git3_oid new_id;
	git3_str new_binary_base85;
	size_t new_binary_inflatedlen;
	git3_diff_binary_t new_binary_type;
};
	...
	void *payload)
{
	...
	if (delta->old_file.path)
		diff_data->old_path = git3__strdup(delta->old_file.path);

	if (delta->new_file.path)
		diff_data->new_path = git3__strdup(delta->new_file.path);

	git3_oid_cpy(&diff_data->old_id, &delta->old_file.id);
	git3_oid_cpy(&diff_data->new_id, &delta->new_file.id);
	...
	void *payload)
{
	...
	git3_str_encode_base85(&diff_data->old_binary_base85,
	...
	git3_str_encode_base85(&diff_data->new_binary_base85,
...
void test_diff_binary__blob_to_blob(void)
{
	...
	git3_oid_from_string(&old_id, "9a69d960ae94b060f56c2a8702545e2bb1abb935", GIT3_OID_SHA1);
	git3_oid_from_string(&new_id, "1111d4f11f4b35bf6759e0fb714fe09731ef0840", GIT3_OID_SHA1);

	cl_git_pass(git3_blob_lookup(&old_blob, repo, &old_id));
	cl_git_pass(git3_blob_lookup(&new_blob, repo, &new_id));

	cl_git_pass(git3_diff_blobs(old_blob,
	...
	cl_assert_equal_i(GIT3_DIFF_BINARY_DELTA, diff_data.old_binary_type);
	...
	cl_assert_equal_i(GIT3_DIFF_BINARY_DELTA, diff_data.new_binary_type);
	...
	git3_blob_free(old_blob);
	git3_blob_free(new_blob);

	git3__free(diff_data.old_path);
	git3__free(diff_data.new_path);

	git3_str_dispose(&diff_data.old_binary_base85);
	git3_str_dispose(&diff_data.new_binary_base85);
}
