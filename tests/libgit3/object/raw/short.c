...
void test_object_raw_short__oid_shortener_no_duplicates(void)
{
	...
	os = git3_oid_shorten_new(0);
	cl_assert(os != NULL);

	git3_oid_shorten_add(os, "22596363b3de40b06f981fb85d82312e8c0ed511");
	git3_oid_shorten_add(os, "ce08fe4884650f067bd5703b6a59a8b3b3c99a09");
	git3_oid_shorten_add(os, "16a0123456789abcdef4b775213c23a8bd74f5e0");
	min_len = git3_oid_shorten_add(os, "ce08fe4884650f067bd5703b6a59a8b3b3c99a09");

	cl_assert(min_len == GIT3_OID_SHA1_HEXSIZE + 1);

	git3_oid_shorten_free(os);
}

static int insert_sequential_oids(
	char ***out, git3_oid_shorten *os, int n, int fail)
{
	int i, min_len = 0;
	char numbuf[16];
	git3_oid oid;
	unsigned char hashbuf[GIT3_HASH_SHA1_SIZE];
	char **oids = git3__calloc(n, sizeof(char *));
	cl_assert(oids != NULL);

	for (i = 0; i < n; ++i) {
		p_snprintf(numbuf, sizeof(numbuf), "%u", (unsigned int)i);
		git3_hash_buf(hashbuf, numbuf, strlen(numbuf), GIT3_HASH_ALGORITHM_SHA1);

		git3_oid_from_raw(&oid, hashbuf, GIT3_OID_SHA1);

		oids[i] = git3__malloc(GIT3_OID_SHA1_HEXSIZE + 1);
		cl_assert(oids[i]);
		git3_oid_nfmt(oids[i], GIT3_OID_SHA1_HEXSIZE + 1, &oid);

		min_len = git3_oid_shorten_add(os, oids[i]);

		/* After "fail", we expect git3_oid_shorten_add to fail */
		if (fail >= 0 && i >= fail)
            cl_assert(min_len < 0);
		else
            cl_assert(min_len >= 0);
