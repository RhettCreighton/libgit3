	...
	size_t expected_parents)
{
	...
	if (!datalen)
		datalen = strlen(data);
	cl_git_pass(git3_object__from_raw((git3_object **) &commit, data, datalen, GIT3_OBJECT_COMMIT, oid_type));

	if (expected_author) {
		git3_signature *author;
		cl_git_pass(git3_signature_from_buffer(&author, expected_author));
		cl_assert(git3_signature__equal(author, commit->author));
	...
	if (expected_committer) {
		...
		cl_git_pass(git3_signature_from_buffer(&committer, expected_committer));
	...
	git3_oid_t oid_type)
{
	...
	if (!datalen)
		datalen = strlen(data);
	cl_git_fail(git3_object__from_raw(&object, data, datalen, GIT3_OBJECT_COMMIT, oid_type));
}
...
void test_object_commit_parse__sha1_parsing_commit_succeeds(void)
{
	const char *commit =
		...
		"\n"
		"Message";
	assert_commit_parses(commit, 0, GIT3_OID_SHA1,
...
void test_object_commit_parse__sha1_parsing_commit_with_invalid_tree_fails(void)
{
	const char *commit =
		...
		"\n"
		"Message";
	assert_commit_fails(commit, 0, GIT3_OID_SHA1);
}
...
void test_object_commit_parse__sha256_parsing_commit_succeeds(void)
{
	...
	const char *commit =
		...
		"\n"
		"Message";
	assert_commit_parses(commit, 0, GIT3_OID_SHA256,
...
void test_object_commit_parse__sha256_parsing_commit_with_invalid_tree_fails(void)
{
	...
	const char *commit =
		...
		"\n"
		"Message";
	assert_commit_fails(commit, 0, GIT3_OID_SHA256);
#endif
}
