...
static void assert_blob_creation(const char *path_to_file, const char *blob_from_path, blob_creator_fn creator)
{
	...
	cl_assert(git3_oid_streq(&oid, "da5e4f20c91c81b44a7e298f3d3fb3fe2f178e32") == 0);
}
...
void test_object_blob_write__can_create_a_blob_in_a_standard_repo_from_a_file_located_in_the_working_directory(void)
{
	...
	assert_blob_creation(WORKDIR "/test.txt", "test.txt", &git3_blob_create_from_workdir);
}
...
void test_object_blob_write__can_create_a_blob_in_a_standard_repo_from_a_absolute_filepath_pointing_outside_of_the_working_directory(void)
{
	...
	cl_must_pass(git3_fs_path_prettify_dir(&full_path, ELSEWHERE, NULL));
	cl_must_pass(git3_str_puts(&full_path, "test.txt"));

	assert_blob_creation(ELSEWHERE "/test.txt", git3_str_cstr(&full_path), &git3_blob_create_from_disk);

	git3_str_dispose(&full_path);
	cl_must_pass(git3_futils_rmdir_r(ELSEWHERE, NULL, GIT3_RMDIR_REMOVE_FILES));
}
