...
void test_object_tree_walk__0(void)
{
	...
	ct = 0;
	cl_git_pass(git3_tree_walk(tree, GIT3_TREEWALK_PRE, treewalk_count_cb, &ct));
	cl_assert_equal_i(3, ct);

	ct = 0;
	cl_git_pass(git3_tree_walk(tree, GIT3_TREEWALK_POST, treewalk_count_cb, &ct));
	cl_assert_equal_i(3, ct);
...
void test_object_tree_walk__1(void)
{
	...
	ct = 0;
	cl_assert_equal_i(
		-123, git3_tree_walk(tree, GIT3_TREEWALK_PRE, treewalk_stop_cb, &ct));
	cl_assert_equal_i(2, ct);

	ct = 0;
	cl_assert_equal_i(
		-123, git3_tree_walk(tree, GIT3_TREEWALK_POST, treewalk_stop_cb, &ct));
	cl_assert_equal_i(2, ct);

	cl_assert_equal_i(
		-100, git3_tree_walk(
			tree, GIT3_TREEWALK_PRE, treewalk_stop_immediately_cb, NULL));

	cl_assert_equal_i(
		-100, git3_tree_walk(
			tree, GIT3_TREEWALK_POST, treewalk_stop_immediately_cb, NULL));
	...
	const char *root, const git3_tree_entry *entry, void *payload)
{
	...
	const char *name = git3_tree_entry_name(entry);

	GIT3_UNUSED(root);

	if (git3_tree_entry_type(entry) == GIT3_OBJECT_TREE)
		data->dirs++;
	else
		data->files++;
...
void test_object_tree_walk__2(void)
{
	...
	/* look up a deep tree */
	git3_oid_from_string(&id, "ae90f12eea699729ed24555e40b9fd669da12a12", GIT3_OID_SHA1);
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &id));

	memset(&data, 0, sizeof(data));
	data.skip = "de";

	cl_assert_equal_i(0, git3_tree_walk(
		tree, GIT3_TREEWALK_PRE, treewalk_skip_de_cb, &data));
	...
	cl_assert_equal_i(-1, git3_tree_walk(
		tree, GIT3_TREEWALK_PRE, treewalk_skip_de_cb, &data));
