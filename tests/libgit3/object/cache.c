...
void test_object_cache__cleanup(void)
{
	...
	git_libgit3_opts(GIT_OPT_SET_CACHE_OBJECT_LIMIT, (int)GIT_OBJECT_BLOB, (size_t)0);
	git_libgit3_opts(GIT_OPT_SET_CACHE_OBJECT_LIMIT, (int)GIT_OBJECT_TREE, (size_t)4096);
	git_libgit3_opts(GIT_OPT_SET_CACHE_OBJECT_LIMIT, (int)GIT_OBJECT_COMMIT, (size_t)4096);
}
...
void test_object_cache__cache_counts(void)
{
	...
	git_libgit3_opts(GIT_OPT_SET_CACHE_OBJECT_LIMIT, object_type, cache_limit);
	...
	for (i = 0; g_data[i].sha != NULL; ++i) {
		...
		cl_git_pass(git_oid_from_string(&oid, g_data[i].sha, GIT_OID_SHA1));
...
static void *cache_quick(void *arg)
{
	...
	cl_git_pass(git_oid_from_string(&oid, g_data[4].sha, GIT_OID_SHA1));
	cl_git_pass(git_object_lookup(&obj, g_repo, &oid, GIT_OBJECT_ANY));
