...
void test_object_tag_write__basic(void)
{
	...
	git3_tag *tag;
	git3_oid target_id, tag_id;
	git3_signature *tagger;
	const git3_signature *tagger1;
	git3_reference *ref_tag;
	git3_object *target;

	git3_oid_from_string(&target_id, tagged_commit, GIT3_OID_SHA1);
	cl_git_pass(git3_object_lookup(&target, g_repo, &target_id, GIT3_OBJECT_COMMIT));

	/* create signature */
	cl_git_pass(git3_signature_new(&tagger, tagger_name, tagger_email, 123456789, 60));

	cl_git_pass(
		git3_tag_create(&tag_id, g_repo,
	...
	cl_git_pass(git3_tag_lookup(&tag, g_repo, &tag_id));
	cl_assert(git3_oid_cmp(git3_tag_target_id(tag), &target_id) == 0);

	/* Check attributes were set correctly */
	tagger1 = git3_tag_tagger(tag);
	...
	cl_assert_equal_s(git3_tag_message(tag), tagger_message);

	cl_git_pass(git3_reference_lookup(&ref_tag, g_repo, "refs/tags/the-tag"));
	cl_assert(git3_oid_cmp(git3_reference_target(ref_tag), &tag_id) == 0);
	cl_git_pass(git3_reference_delete(ref_tag));
	git3_reference_free(ref_tag);
...
void test_object_tag_write__overwrite(void)
{
	...
	cl_assert_equal_i(GIT3_EEXISTS, git3_tag_create(
                              &tag_id, /* out id */
                              g_repo,
                              "e90810b",
                              target,
...
void test_object_tag_write__replace(void)
{
	...
	git3_oid target_id, tag_id, old_tag_id;
	git3_signature *tagger;
	git3_reference *ref_tag;
	git3_object *target;
	...
	cl_git_pass(git3_reference_lookup(&ref_tag, g_repo, "refs/tags/e90810b"));
	git3_oid_cpy(&old_tag_id, git3_reference_target(ref_tag));
	git3_reference_free(ref_tag);

	/* create signature */
	cl_git_pass(git3_signature_new(&tagger, tagger_name, tagger_email, 123456789, 60));

	cl_git_pass(git3_tag_create(
                              &tag_id, /* out id */
                              g_repo,
                              "e90810b",
                              target,
	...
	cl_git_pass(git3_reference_lookup(&ref_tag, g_repo, "refs/tags/e90810b"));
	cl_assert(git3_oid_cmp(git3_reference_target(ref_tag), &tag_id) == 0);
	cl_assert(git3_oid_cmp(git3_reference_target(ref_tag), &old_tag_id) != 0);
...
void test_object_tag_write__lightweight(void)
{
	...
	cl_git_pass(git3_tag_create_lightweight(
                                          &object_id,
                                          g_repo,
                                          "light-tag",
                                          target,
	...
	cl_assert(git3_oid_cmp(&object_id, &target_id) == 0);

	cl_git_pass(git3_reference_lookup(&ref_tag, g_repo, "refs/tags/light-tag"));
	cl_assert(git3_oid_cmp(git3_reference_target(ref_tag), &target_id) == 0);

	cl_git_pass(git3_tag_delete(g_repo, "light-tag"));
...
void test_object_tag_write__lightweight_over_existing(void)
{
	...
	git3_oid target_id, object_id, existing_object_id;
	git3_object *target;

	git3_oid_from_string(&target_id, tagged_commit, GIT3_OID_SHA1);
	cl_git_pass(git3_object_lookup(&target, g_repo, &target_id, GIT3_OBJECT_COMMIT));

	cl_assert_equal_i(GIT3_EEXISTS, git3_tag_create_lightweight(
                                          &object_id,
                                          g_repo,
                                          "e90810b",
                                          target,
	...
	git3_oid_from_string(&existing_object_id, tag2_id, GIT3_OID_SHA1);
	cl_assert(git3_oid_cmp(&object_id, &existing_object_id) == 0);
...
void test_object_tag_write__delete(void)
{
	...
	cl_git_pass(git3_tag_delete(g_repo, "e90810b"));

	cl_git_fail(git3_reference_lookup(&ref_tag, g_repo, "refs/tags/e90810b"));
...
void test_object_tag_write__creating_with_an_invalid_name_returns_EINVALIDSPEC(void)
{
	...
	cl_assert_equal_i(GIT3_EINVALIDSPEC,
		git3_tag_create_lightweight(&tag_id, g_repo,
		  "Inv@{id", target, 0)
	);
...
{
	cl_assert_equal_i(GIT3_EINVALIDSPEC, git3_tag_delete(g_repo, "Inv@{id"));
}

static void create_annotation(git3_oid *tag_id, const char *name)
{
	git3_object *target;
	git3_oid target_id;
	git3_signature *tagger;
	...
	cl_git_pass(git3_tag_annotation_create(tag_id, g_repo, name, target, tagger, "boom!"));
	git3_object_free(target);
	git3_signature_free(tagger);
}
...
void test_object_tag_write__creating_an_annotation_stores_the_new_object_in_the_odb(void)
{
	...
	cl_git_pass(git3_tag_lookup(&tag, g_repo, &tag_id));
	cl_assert_equal_s("new_tag", git3_tag_name(tag));
...
void test_object_tag_write__creating_an_annotation_does_not_create_a_reference(void)
{
	...
	create_annotation(&tag_id, "new_tag");
	cl_git_fail_with(git3_reference_lookup(&tag_ref, g_repo, "refs/tags/new_tag"), GIT3_ENOTFOUND);
}
