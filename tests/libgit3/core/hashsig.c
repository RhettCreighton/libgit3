...
void test_core_hashsig__similarity_metric(void)
{
	...
	cl_git_pass(git3_str_sets(&buf, SIMILARITY_TEST_DATA_1));
	cl_git_pass(git3_hashsig_create(&a, buf.ptr, buf.size, GIT3_HASHSIG_NORMAL));
	cl_git_pass(git3_hashsig_create(&b, buf.ptr, buf.size, GIT3_HASHSIG_NORMAL));

	cl_assert_equal_i(100, git3_hashsig_compare(a, b));
	...
	cl_git_pass(git3_futils_mkdir("scratch", 0755, GIT3_MKDIR_PATH));
	cl_git_mkfile("scratch/testdata", SIMILARITY_TEST_DATA_1);
	cl_git_pass(git3_hashsig_create_fromfile(
		&b, "scratch/testdata", GIT3_HASHSIG_NORMAL));
	...
	git3_futils_rmdir_r("scratch", NULL, GIT3_RMDIR_REMOVE_FILES);
}
...
void test_core_hashsig__similarity_metric_whitespace(void)
{
	...
	for (opt = GIT3_HASHSIG_NORMAL; opt <= GIT3_HASHSIG_SMART_WHITESPACE; ++opt) {
		for (i = 0; i < 3; ++i) {
			for (j = 0; j < 3; ++j) {
				cl_git_pass(git3_str_sets(&buf, text[i]));
				cl_git_pass(git3_hashsig_create(&a, buf.ptr, buf.size, opt));

				cl_git_pass(git3_str_sets(&buf, text[j]));
				cl_git_pass(git3_hashsig_create(&b, buf.ptr, buf.size, opt));

				sim = git3_hashsig_compare(a, b);

				if (opt == GIT3_HASHSIG_NORMAL) {
					if (i == j)
