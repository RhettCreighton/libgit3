...
{
	cl_git_pass(git_libgit3_opts(GIT_OPT_GET_USER_AGENT, &default_ua));
	cl_git_pass(git_libgit3_opts(GIT_OPT_GET_USER_AGENT_PRODUCT, &default_product));
}

void test_core_useragent__cleanup(void)
{
	git_libgit3_opts(GIT_OPT_SET_USER_AGENT, NULL);
	git_libgit3_opts(GIT_OPT_SET_USER_AGENT_PRODUCT, NULL);
...
{
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT, "foo bar 4.24"));
	...
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT_PRODUCT, "baz/2.2.3"));
	cl_assert_equal_s("foo bar 4.24", git_settings__user_agent());
	cl_assert_equal_s("baz/2.2.3", git_settings__user_agent_product());

	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT, ""));
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT_PRODUCT, ""));
	cl_assert_equal_s("", git_settings__user_agent());
	cl_assert_equal_s("", git_settings__user_agent_product());

	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT, NULL));
	cl_git_pass(git_libgit3_opts(GIT_OPT_SET_USER_AGENT_PRODUCT, NULL));
