...
static void _rename_helper(
	git3_repository *repo, const char *from, const char *to, const char *extra)
{
	git3_str oldpath = GIT3_STR_INIT, newpath = GIT3_STR_INIT;

	cl_git_pass(git3_str_joinpath(
		&oldpath, git3_repository_workdir(repo), from));
	cl_git_pass(git3_str_joinpath(
		&newpath, git3_repository_workdir(repo), to));
...
static void check_status(
	...
	size_t expected_len)
{
	const git3_status_entry *actual;
	const struct status_entry *expected;
	const char *oldname, *newname;
	size_t i, files_in_status = git3_status_list_entrycount(status_list);

	cl_assert_equal_sz(expected_len, files_in_status);

	for (i = 0; i < expected_len; i++) {
		actual = git3_status_byindex(status_list, i);
		...
		if (actual->status & (GIT3_STATUS_INDEX_RENAMED|GIT3_STATUS_WT_RENAMED)) {
			if (expected->newname) {
...
void test_status_renames__head2index_one(void)
{
	...
	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_RENAMED, "ikeepsix.txt", "newname.txt" },
	};

	opts.flags |= GIT3_STATUS_OPT_RENAMES_HEAD_TO_INDEX;
	...
	cl_git_pass(git3_index_add_bypath(index, "newname.txt"));
	cl_git_pass(git3_index_write(index));

	cl_git_pass(git3_status_list_new(&statuslist, g_repo, &opts));
...
void test_status_renames__head2index_two(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_INDEX_MODIFIED,
		  "sixserving.txt", "aaa.txt" },
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_INDEX_MODIFIED,
		  "untimely.txt", "bbb.txt" },
		{ GIT3_STATUS_INDEX_RENAMED, "songof7cities.txt", "ccc.txt" },
		{ GIT3_STATUS_INDEX_RENAMED, "ikeepsix.txt", "ddd.txt" },
	};
	...
	cl_git_pass(git3_index_remove_bypath(index, "ikeepsix.txt"));
	cl_git_pass(git3_index_remove_bypath(index, "sixserving.txt"));
	cl_git_pass(git3_index_remove_bypath(index, "songof7cities.txt"));
	cl_git_pass(git3_index_remove_bypath(index, "untimely.txt"));
	cl_git_pass(git3_index_add_bypath(index, "ddd.txt"));
	cl_git_pass(git3_index_add_bypath(index, "aaa.txt"));
	cl_git_pass(git3_index_add_bypath(index, "ccc.txt"));
	cl_git_pass(git3_index_add_bypath(index, "bbb.txt"));
	cl_git_pass(git3_index_write(index));
...
void test_status_renames__head2index_no_rename_from_rewrite(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_MODIFIED, "ikeepsix.txt", "ikeepsix.txt" },
		{ GIT3_STATUS_INDEX_MODIFIED, "sixserving.txt", "sixserving.txt" },
	};
	...
	cl_git_pass(git3_index_add_bypath(index, "sixserving.txt"));
	cl_git_pass(git3_index_write(index));
...
void test_status_renames__head2index_rename_from_rewrite(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_RENAMED, "sixserving.txt", "ikeepsix.txt" },
		{ GIT3_STATUS_INDEX_RENAMED, "ikeepsix.txt", "sixserving.txt" },
	};

	opts.flags |= GIT3_STATUS_OPT_RENAMES_HEAD_TO_INDEX;
	opts.flags |= GIT3_STATUS_OPT_RENAMES_FROM_REWRITES;
...
void test_status_renames__index2workdir_one(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_WT_RENAMED, "ikeepsix.txt", "newname.txt" },
	};

	opts.flags |= GIT3_STATUS_OPT_INCLUDE_UNTRACKED;
	opts.flags |= GIT3_STATUS_OPT_RENAMES_INDEX_TO_WORKDIR;
...
void test_status_renames__index2workdir_two(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_WT_RENAMED | GIT3_STATUS_WT_MODIFIED,
		  "sixserving.txt", "aaa.txt" },
		{ GIT3_STATUS_WT_RENAMED | GIT3_STATUS_WT_MODIFIED,
		  "untimely.txt", "bbb.txt" },
		{ GIT3_STATUS_WT_RENAMED, "songof7cities.txt", "ccc.txt" },
		{ GIT3_STATUS_WT_RENAMED, "ikeepsix.txt", "ddd.txt" },
	};
...
void test_status_renames__index2workdir_rename_from_rewrite(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_WT_RENAMED, "sixserving.txt", "ikeepsix.txt" },
		{ GIT3_STATUS_WT_RENAMED, "ikeepsix.txt", "sixserving.txt" },
	};
...
void test_status_renames__both_one(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_WT_RENAMED,
	...
	cl_git_pass(git3_index_remove_bypath(index, "ikeepsix.txt"));
	cl_git_pass(git3_index_add_bypath(index, "newname-index.txt"));
	cl_git_pass(git3_index_write(index));
...
void test_status_renames__both_two(void)
{
	...
	cl_git_pass(git3_index_remove_bypath(index, "sixserving.txt"));
	cl_git_pass(git3_index_remove_bypath(index, "untimely.txt"));
	cl_git_pass(git3_index_add_bypath(index, "ikeepsix-index.txt"));
	cl_git_pass(git3_index_add_bypath(index, "sixserving-index.txt"));
	cl_git_pass(git3_index_add_bypath(index, "untimely-index.txt"));
	cl_git_pass(git3_index_write(index));
...
void test_status_renames__rewrites_only_for_renames(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected[] = {
		{ GIT3_STATUS_WT_MODIFIED, "ikeepsix.txt", "ikeepsix.txt" },
	};
...
void test_status_renames__both_casechange_one(void)
{
	...
	check_status(statuslist, (index_caps & GIT3_INDEX_CAPABILITY_IGNORE_CASE) ?
...
void test_status_renames__both_casechange_two(void)
{
	...
	struct status_entry expected_icase[] = {
		...
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_WT_MODIFIED,
	...
	struct status_entry expected_case[] = {
		{ GIT3_STATUS_INDEX_RENAMED |
		  GIT3_STATUS_WT_MODIFIED | GIT3_STATUS_WT_RENAMED,
		  "songof7cities.txt", "SONGOF7.txt" },
		{ GIT3_STATUS_INDEX_MODIFIED | GIT3_STATUS_WT_RENAMED,
		  "sixserving.txt", "SixServing.txt" },
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_INDEX_MODIFIED |
		  GIT3_STATUS_WT_RENAMED | GIT3_STATUS_WT_MODIFIED,
		  "ikeepsix.txt", "ikeepsix.txt" },
	...
	cl_git_pass(git3_index_add_bypath(index, "IKeepSix.txt"));
	cl_git_pass(git3_index_add_bypath(index, "sixserving.txt"));
	cl_git_pass(git3_index_add_bypath(index, "songof7.txt"));
	cl_git_pass(git3_index_add_bypath(index, "untimelier.txt"));
	cl_git_pass(git3_index_write(index));
...
void test_status_renames__zero_byte_file_does_not_fail(void)
{
	...
	struct status_entry expected[] = {
		{ GIT3_STATUS_WT_DELETED, "ikeepsix.txt", "ikeepsix.txt" },
		{ GIT3_STATUS_WT_NEW, "zerobyte.txt", "zerobyte.txt" },
	};
...
void test_status_renames__precomposed_unicode_rename(void)
{
	...
	struct status_entry expected0[] = {
		{ GIT3_STATUS_WT_NEW, nfd, NULL },
		{ GIT3_STATUS_WT_DELETED, "sixserving.txt", NULL },
	};
	struct status_entry expected1[] = {
		{ GIT3_STATUS_WT_RENAMED, "sixserving.txt", nfd },
	};
	struct status_entry expected2[] = {
		{ GIT3_STATUS_WT_DELETED, "sixserving.txt", NULL },
		{ GIT3_STATUS_WT_NEW, nfc, NULL },
	};
	struct status_entry expected3[] = {
		{ GIT3_STATUS_WT_RENAMED, "sixserving.txt", nfc },
	};
...
void test_status_renames__precomposed_unicode_toggle_is_rename(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	struct status_entry expected0[] = {
		{ GIT3_STATUS_INDEX_RENAMED, "ikeepsix.txt", nfd },
	};
	struct status_entry expected1[] = {
		{ GIT3_STATUS_WT_RENAMED, nfd, nfc },
	};
	struct status_entry expected2[] = {
		{ GIT3_STATUS_INDEX_RENAMED, nfd, nfc },
	};
	struct status_entry expected3[] = {
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_WT_RENAMED, nfd, nfd },
	};
...

	{
		...
		cl_git_pass(git3_repository_index(&index, g_repo));
		cl_git_pass(git3_index_remove_bypath(index, "ikeepsix.txt"));
		cl_git_pass(git3_index_add_bypath(index, nfd));
		cl_git_pass(git3_index_write(index));
		git3_index_free(index);
	}
	...
	cl_git_pass(git3_status_list_new(&statuslist, g_repo, &opts));
	cl_assert_equal_sz(0, git3_status_list_entrycount(statuslist));
	git3_status_list_free(statuslist);
...

	{
		git3_index *index;
		cl_git_pass(git3_repository_index(&index, g_repo));
		cl_git_pass(git3_index_remove_bypath(index, nfd));
		cl_git_pass(git3_index_add_bypath(index, nfc));
		cl_git_pass(git3_index_write(index));
		git3_index_free(index);
	}
...
void test_status_renames__rename_threshold(void)
{
	...
	{
		struct status_entry expected[] = {
			{ GIT3_STATUS_WT_RENAMED | GIT3_STATUS_WT_MODIFIED, "ikeepsix.txt", "newname.txt" },
		};
	...
	{
		struct status_entry expected[] = {
			{ GIT3_STATUS_WT_DELETED, "ikeepsix.txt", NULL },
			{ GIT3_STATUS_WT_NEW, "newname.txt", NULL }
		};
...
void test_status_renames__case_insensitive_h2i_and_i2wc(void)
{
	git3_status_list *statuslist;
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	git3_reference *head, *test_branch;
	git3_checkout_options checkout_opts = GIT3_CHECKOUT_OPTIONS_INIT;
	git3_str path_to_delete = GIT3_STR_INIT;
	git3_str path_to_edit = GIT3_STR_INIT;
	git3_index *index;
	git3_strarray paths = { NULL, 0 };

	struct status_entry expected[] = {
		{ GIT3_STATUS_INDEX_RENAMED | GIT3_STATUS_WT_MODIFIED, "sixserving.txt", "sixserving-renamed.txt" },
		{ GIT3_STATUS_INDEX_DELETED, "Wow.txt", "Wow.txt" }
	};
	...
	cl_git_pass(git3_checkout_head(g_repo, &checkout_opts));
	...
	cl_git_pass(git3_str_joinpath(
		&path_to_delete, git3_repository_workdir(g_repo), "Wow.txt"));
	cl_git_rmfile(path_to_delete.ptr);

	cl_git_pass(git3_index_add_all(index, &paths, GIT3_INDEX_ADD_FORCE, NULL, NULL));
	cl_git_pass(git3_index_write(index));


	/* Change content of sixserving-renamed.txt */
	cl_git_pass(git3_str_joinpath(
		&path_to_edit, git3_repository_workdir(g_repo), "sixserving-renamed.txt"));
	...
	git3_str_dispose(&path_to_delete);
	git3_str_dispose(&path_to_edit);

	git3_reference_free(head);
	git3_reference_free(test_branch);
}
