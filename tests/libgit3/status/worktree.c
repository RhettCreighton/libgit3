...
void test_status_worktree__whole_repository(void)
{
	...
	cl_git_pass(
		git3_status_foreach(repo, cb_status__normal, &counts)
	);
	...
	unsigned int extra_flags)
{
	...
	opts.flags = GIT3_STATUS_OPT_DEFAULTS | extra_flags;
	opts.show = show;

	cl_git_pass(
		git3_status_foreach_ext(repo, &opts, cb_status__normal, &counts)
	);
...
{
	int count = 0;
	git3_repository *repo = cl_git_sandbox_init("empty_standard_repo");

	cl_git_pass(git3_status_foreach(repo, cb_status__count, &count));

	cl_assert_equal_i(0, count);
}

static int remove_file_cb(void *data, git3_str *file)
{
	const char *filename = git3_str_cstr(file);

	GIT3_UNUSED(data);

	if (git3__suffixcmp(filename, ".git") == 0)
		return 0;

	if (git3_fs_path_isdir(filename))
		cl_git_pass(git3_futils_rmdir_r(filename, NULL, GIT3_RMDIR_REMOVE_FILES));
	else
		cl_git_pass(p_unlink(git3_str_cstr(file)));
...
void test_status_worktree__purged_worktree(void)
{
	status_entry_counts counts;
	git3_repository *repo = cl_git_sandbox_init("status");
	git3_str workdir = GIT3_STR_INIT;

	/* first purge the contents of the worktree */
	cl_git_pass(git3_str_sets(&workdir, git3_repository_workdir(repo)));
	cl_git_pass(git3_fs_path_direach(&workdir, 0, remove_file_cb, NULL));
...
void test_status_worktree__swap_subdir_and_file(void)
{
	...
	ignore_case = (git3_index_caps(index) & GIT3_INDEX_CAPABILITY_IGNORE_CASE) != 0;
...
void test_status_worktree__swap_subdir_with_recurse_and_pathspec(void)
{
	...
	cl_git_pass(git3_futils_mkdir_r("status/zzz_new_dir", 0777));
...
static void stage_and_commit(git3_repository *repo, const char *path)
{
	git3_index *index;

	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_add_bypath(index, path));
...
void test_status_worktree__single_file(void)
{
	...
	for (i = 0; i < (int)entry_count0; i++) {
		cl_git_pass(
			git3_status_file(&status_flags, repo, entry_paths0[i])
		);
...
void test_status_worktree__single_nonexistent_file(void)
{
	...
	error = git3_status_file(&status_flags, repo, "nonexistent");
	cl_git_fail(error);
...
void test_status_worktree__single_file_empty_repo(void)
{
	...
	cl_git_pass(git3_status_file(&status_flags, repo, "new_file"));
	cl_assert(status_flags == GIT3_STATUS_WT_NEW);
}
...
void test_status_worktree__single_folder(void)
{
	...
	error = git3_status_file(&status_flags, repo, "subdir");
	cl_git_fail(error);
	cl_assert(error != GIT3_ENOTFOUND);
}
...
void test_status_worktree__ignores(void)
{
	...
	for (i = 0; i < (int)entry_count0; i++) {
		cl_git_pass(
			git3_status_should_ignore(&ignored, repo, entry_paths0[i])
		);
		cl_assert(ignored == (entry_statuses0[i] == GIT3_STATUS_IGNORED));
	}

	cl_git_pass(
		git3_status_should_ignore(&ignored, repo, "nonexistent_file")
	);
	cl_assert(!ignored);

	cl_git_pass(
		git3_status_should_ignore(&ignored, repo, "ignored_nonexistent_file")
	);
	cl_assert(ignored);
}
...
void test_status_worktree__issue_592(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(repo), "l.txt"));
	cl_git_pass(p_unlink(git3_str_cstr(&path)));
	cl_assert(!git3_fs_path_exists("issue_592/l.txt"));

	cl_git_pass(git3_status_foreach(repo, cb_status__check_592, "l.txt"));
...
void test_status_worktree__issue_592_2(void)
{
	...
	repo = cl_git_sandbox_init("issue_592");
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(repo), "c/a.txt"));
	cl_git_pass(p_unlink(git3_str_cstr(&path)));
	cl_assert(!git3_fs_path_exists("issue_592/c/a.txt"));

	cl_git_pass(git3_status_foreach(repo, cb_status__check_592, "c/a.txt"));
...
void test_status_worktree__issue_592_3(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(repo), "c"));
	cl_git_pass(git3_futils_rmdir_r(git3_str_cstr(&path), NULL, GIT3_RMDIR_REMOVE_FILES));
	cl_assert(!git3_fs_path_exists("issue_592/c/a.txt"));
...
void test_status_worktree__issue_592_4(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(repo), "t/b.txt"));
	cl_git_pass(p_unlink(git3_str_cstr(&path)));

	cl_git_pass(git3_status_foreach(repo, cb_status__check_592, "t/b.txt"));
...
void test_status_worktree__issue_592_5(void)
{
	...
	cl_git_pass(git3_str_joinpath(&path, git3_repository_workdir(repo), "t"));
	cl_git_pass(git3_futils_rmdir_r(git3_str_cstr(&path), NULL, GIT3_RMDIR_REMOVE_FILES));
	cl_git_pass(p_mkdir(git3_str_cstr(&path), 0777));

	cl_git_pass(git3_status_foreach(repo, cb_status__check_592, NULL));
...
void test_status_worktree__issue_592_ignores_0(void)
{
	...
	git3_repository *repo = cl_git_sandbox_init("issue_592");
	...
	/* This is a situation where the behavior of libgit3 is
	...
	cl_git_pass(git3_status_foreach(repo, cb_status__single, &st));
	cl_assert_equal_i(1, st.count);
	cl_assert(st.status == GIT3_STATUS_IGNORED);
...
{
	int count = 0;
	git3_repository *repo = cl_git_sandbox_init("issue_592b");
...
void test_status_worktree__conflict_with_diff3(void)
{
	...
	git3_oid_from_string(&ancestor_entry.id,
		"452e4244b5d083ddf0460acf1ecc74db9dcfa11a",
		GIT3_OID_SHA1);

	our_entry.path = "modified_file";
	our_entry.mode = 0100644;
	git3_oid_from_string(&our_entry.id,
		"452e4244b5d083ddf0460acf1ecc74db9dcfa11a",
		GIT3_OID_SHA1);

	their_entry.path = "modified_file";
	their_entry.mode = 0100644;
	git3_oid_from_string(&their_entry.id,
		"452e4244b5d083ddf0460acf1ecc74db9dcfa11a",
		GIT3_OID_SHA1);

	cl_git_pass(git3_status_file(&status, repo, "modified_file"));
	cl_assert_equal_i(GIT3_STATUS_WT_MODIFIED, status);

	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_remove(index, "modified_file", 0));
	cl_git_pass(git3_index_conflict_add(
	...
	cl_assert_equal_i(GIT3_STATUS_CONFLICTED, status);
}
...
{
	git3_repository *repo = cl_git_sandbox_init("filemodes");
	status_entry_counts counts;
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	...
	if (cl_is_chmod_supported())
		...
		for (i = 0; i < filemode_count; ++i)
			if (filemode_statuses[i] == GIT3_STATUS_WT_MODIFIED)
				filemode_statuses[i] = GIT3_STATUS_CURRENT;
	}
...
void test_status_worktree__filemode_non755(void)
{
	...
	status_entry_counts counts;
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	git3_str executable_path = GIT3_STR_INIT;
	git3_str nonexecutable_path = GIT3_STR_INIT;
	...
	git3_str_joinpath(&executable_path, git3_repository_workdir(repo), "exec_on");
	cl_must_pass(p_chmod(git3_str_cstr(&executable_path), 0744));
	git3_str_dispose(&executable_path);

	git3_str_joinpath(&nonexecutable_path, git3_repository_workdir(repo), "exec_off");

	cl_must_pass(p_chmod(git3_str_cstr(&nonexecutable_path), 0655));
	git3_str_dispose(&nonexecutable_path);
...
void test_status_worktree__interruptable_foreach(void)
{
	...
	cl_assert_equal_i(
		-111, git3_status_foreach(repo, cb_status__interrupt, &count)
	);
...
void test_status_worktree__line_endings_dont_count_as_changes_with_autocrlf(void)
{
	...
	cl_git_pass(git3_status_file(&status, repo, "current_file"));
...
{
	git3_repository *repo = cl_git_sandbox_init("issue_1397");
	unsigned int status;

	cl_repo_set_bool(repo, "core.autocrlf", true);

	cl_git_pass(git3_status_file(&status, repo, "crlf_file.txt"));

	cl_assert_equal_i(GIT3_STATUS_CURRENT, status);
}
...
void test_status_worktree__conflict_has_no_oid(void)
{
	...
	const git3_status_entry *status;
	git3_oid zero_id = GIT3_OID_SHA1_ZERO;

	entry.mode = 0100644;
	entry.path = "modified_file";
	git3_oid_from_string(&entry.id, "452e4244b5d083ddf0460acf1ecc74db9dcfa11a", GIT3_OID_SHA1);

	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_conflict_add(index, &entry, &entry, &entry));

	git3_status_list_new(&statuslist, repo, NULL);

	cl_assert_equal_i(16, git3_status_list_entrycount(statuslist));

	status = git3_status_byindex(statuslist, 2);

	cl_assert_equal_i(GIT3_STATUS_CONFLICTED, status->status);
	cl_assert_equal_s("modified_file", status->head_to_index->old_file.path);
	cl_assert(!git3_oid_equal(&zero_id, &status->head_to_index->old_file.id));
	...
	cl_assert(
		!git3_oid_equal(&zero_id, &status->index_to_workdir->new_file.id) ||
		!(status->index_to_workdir->new_file.flags & GIT3_DIFF_FLAG_VALID_ID));
...
{
	unsigned int status;
	git3_str lower_case_path = GIT3_STR_INIT, camel_case_path = GIT3_STR_INIT;
	git3_repository *repo, *repo2;
	...
	cl_git_pass(git3_str_joinpath(&lower_case_path,
		git3_repository_workdir(repo), "plop"));

	cl_git_mkfile(git3_str_cstr(&lower_case_path), "");

	stage_and_commit(repo, "plop");

	cl_git_pass(git3_repository_open(&repo2, "./empty_standard_repo"));

	cl_git_pass(git3_status_file(&status, repo2, "plop"));
	cl_assert_equal_i(GIT3_STATUS_CURRENT, status);

	cl_git_pass(git3_str_joinpath(&camel_case_path,
		git3_repository_workdir(repo), "Plop"));

	cl_git_pass(p_rename(git3_str_cstr(&lower_case_path), git3_str_cstr(&camel_case_path)));
	...
	git3_str_dispose(&lower_case_path);
	git3_str_dispose(&camel_case_path);
}

void test_status_worktree__file_status_honors_core_ignorecase_true(void)
{
	assert_ignore_case(true, GIT3_STATUS_CURRENT, GIT3_STATUS_CURRENT);
}

void test_status_worktree__file_status_honors_core_ignorecase_false(void)
{
	assert_ignore_case(false, GIT3_STATUS_WT_DELETED, GIT3_STATUS_WT_NEW);
}
...
void test_status_worktree__file_status_honors_case_ignorecase_regarding_untracked_files(void)
{
    ...
    /* Actually returns GIT3_STATUS_IGNORED on Windows */
    cl_git_fail_with(git3_status_file(&status, repo, "NEW_FILE"), GIT3_ENOTFOUND);

    cl_git_pass(git3_repository_index(&index, repo));

    cl_git_pass(git3_index_add_bypath(index, "new_file"));
    cl_git_pass(git3_index_write(index));
    git3_index_free(index);
...
{
    git3_repository *repo = cl_git_sandbox_init("renames");
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	int count;

	opts.flags = GIT3_STATUS_OPT_INCLUDE_UNTRACKED |
		GIT3_STATUS_OPT_DISABLE_PATHSPEC_MATCH |
		GIT3_STATUS_OPT_EXCLUDE_SUBMODULES |
		GIT3_STATUS_OPT_RECURSE_UNTRACKED_DIRS;

	count = 0;
	cl_git_pass(
		git3_status_foreach_ext(repo, &opts, cb_status__count, &count) );
	cl_assert_equal_i(0, count);
...
void test_status_worktree__simple_delete_indexed(void)
{
	...
	cl_git_pass(git3_status_list_new(&status, repo, &opts));
	cl_assert_equal_sz(0, git3_status_list_entrycount(status));
	git3_status_list_free(status);

	cl_must_pass(p_unlink("renames/untimely.txt"));

	cl_git_pass(git3_status_list_new(&status, repo, &opts));
	cl_assert_equal_sz(1, git3_status_list_entrycount(status));
	cl_assert_equal_i(
		GIT3_STATUS_WT_DELETED, git3_status_byindex(status, 0)->status);
	git3_status_list_free(status);
}

static const char *icase_paths[] = { "B", "c", "g", "H" };
static unsigned int icase_statuses[] = {
	GIT3_STATUS_WT_MODIFIED, GIT3_STATUS_WT_DELETED,
	GIT3_STATUS_WT_MODIFIED, GIT3_STATUS_WT_DELETED,
};
...
static unsigned int case_statuses[] = {
	GIT3_STATUS_WT_MODIFIED, GIT3_STATUS_WT_DELETED,
	GIT3_STATUS_WT_DELETED, GIT3_STATUS_WT_MODIFIED,
};

void test_status_worktree__sorting_by_case(void)
{
	git3_repository *repo = cl_git_sandbox_init("icase");
	git3_index *index;
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	...
	cl_git_pass(git3_repository_index(&index, repo));
	native_ignore_case =
		(git3_index_caps(index) & GIT3_INDEX_CAPABILITY_IGNORE_CASE) != 0;
	git3_index_free(index);
	...
	opts.flags = GIT3_STATUS_OPT_SORT_CASE_SENSITIVELY;
	...
	opts.flags = GIT3_STATUS_OPT_SORT_CASE_INSENSITIVELY;
...
void test_status_worktree__long_filenames(void)
{
	...
	const unsigned int expected_statuses[] = {GIT3_STATUS_WT_NEW};
	...
	cl_git_pass(git3_futils_mkdir_r(path, 0777));
	...
	opts.show = GIT3_STATUS_SHOW_WORKDIR_ONLY;
...
static void check_status0(git3_status_list *status)
{
	size_t i, max_i = git3_status_list_entrycount(status);
	cl_assert_equal_sz(entry_count0, max_i);
	for (i = 0; i < max_i; ++i) {
		const git3_status_entry *entry = git3_status_byindex(status, i);
...
void test_status_worktree__update_stat_cache_0(void)
{
	...
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	git3_status_list *status;
	git3_diff_perfdata perf = GIT3_DIFF_PERFDATA_INIT;
	git3_index *index;
	...
	cl_git_pass(git3_status_list_get_perfdata(&perf, status));
	...
	opts.flags |= GIT3_STATUS_OPT_UPDATE_INDEX;
...
void test_status_worktree__unreadable(void)
{
	...
	const unsigned int expected_statuses[] = {GIT3_STATUS_WT_UNREADABLE};
	...
	cl_git_pass(git3_futils_mkdir_r("empty_standard_repo/no_permission", 0777));
	...
	opts.show = GIT3_STATUS_SHOW_WORKDIR_ONLY;
	opts.flags = GIT3_STATUS_OPT_DEFAULTS | GIT3_STATUS_OPT_INCLUDE_UNREADABLE;
...
void test_status_worktree__unreadable_not_included(void)
{
	...
	opts.show = GIT3_STATUS_SHOW_WORKDIR_ONLY;
	opts.flags = (GIT3_STATUS_OPT_INCLUDE_IGNORED | GIT3_STATUS_OPT_INCLUDE_UNTRACKED);
...
{
	git3_repository *repo = cl_git_sandbox_init("testrepo");
	git3_reference *head;
	...
	opts.flags = GIT3_STATUS_OPT_DEFAULTS | GIT3_STATUS_OPT_UPDATE_INDEX;
	...
	cl_assert(idx_entry = git3_index_get_bypath(index, "link_to_new.txt", 0));
...
{
	git3_repository *repo = cl_git_sandbox_init("testrepo2");
	git3_index *index;
	git3_status_options opts = GIT3_STATUS_OPTIONS_INIT;
	git3_status_list *statuslist;
	const git3_status_entry *status;
	...
	opts.flags =
			GIT3_STATUS_OPT_DEFAULTS |
			GIT3_STATUS_OPT_INCLUDE_UNMODIFIED |
			GIT3_STATUS_OPT_DISABLE_PATHSPEC_MATCH;

	opts.show = GIT3_STATUS_SHOW_WORKDIR_ONLY;
	git3_status_list_new(&statuslist, repo, &opts);

	entrycount = git3_status_list_entrycount(statuslist);
	cl_assert_equal_i(4, entrycount);

	for (i = 0; i < entrycount; i++) {
		status = git3_status_byindex(statuslist, i);
	...
	opts.show = GIT3_STATUS_SHOW_INDEX_ONLY;
	git3_status_list_new(&statuslist, repo, &opts);
...
void test_status_worktree__at_head_parent(void)
{
	...
	cl_git_pass(git3_repository_head_tree(&parent_tree, repo));
	...
	cl_git_pass(git3_status_list_new(&statuslist, repo, &opts));

	cl_assert_equal_sz(1, git3_status_list_entrycount(statuslist));
	status = git3_status_byindex(statuslist, 0);
	cl_assert(status != NULL);
	cl_assert_equal_s("file2", status->index_to_workdir->old_file.path);
	cl_assert_equal_i(GIT3_STATUS_WT_MODIFIED | GIT3_STATUS_INDEX_NEW, status->status);
...
{
	git3_repository *repo = cl_git_sandbox_init("status_skiphash");
	git3_index *index;
