...
void test_status_worktree_init__cannot_retrieve_the_status_of_a_bare_repository(void)
{
	...
	cl_assert_equal_i(GIT3_EBAREREPO, git3_status_file(&status, repo, "dummy"));
...
void test_status_worktree_init__first_commit_in_progress(void)
{
	...
	cl_git_pass(git3_repository_init(&repo, "getting_started", 0));
	cl_git_mkfile("getting_started/testfile.txt", "content\n");

	memset(&result, 0, sizeof(result));
	cl_git_pass(git3_status_foreach(repo, cb_status__single, &result));
	cl_assert_equal_i(1, result.count);
	cl_assert(result.status == GIT3_STATUS_WT_NEW);

	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_add_bypath(index, "testfile.txt"));
	cl_git_pass(git3_index_write(index));

	memset(&result, 0, sizeof(result));
	cl_git_pass(git3_status_foreach(repo, cb_status__single, &result));
	cl_assert_equal_i(1, result.count);
	cl_assert(result.status == GIT3_STATUS_INDEX_NEW);
...
void test_status_worktree_init__status_file_without_index_or_workdir(void)
{
	...
	cl_git_pass(git3_repository_open(&repo, cl_fixture("testrepo.git")));
	cl_git_pass(git3_repository_set_workdir(repo, "wd", false));

	cl_git_pass(git3_index_open(&index, "empty-index"));
	cl_assert_equal_i(0, (int)git3_index_entrycount(index));
	git3_repository_set_index(repo, index);

	cl_git_pass(git3_status_file(&status, repo, "branch_file.txt"));

	cl_assert_equal_i(GIT3_STATUS_INDEX_DELETED, status);
...
static void fill_index_wth_head_entries(git3_repository *repo, git3_index *index)
{
	git3_oid oid;
	git3_commit *commit;
	git3_tree *tree;
...
void test_status_worktree_init__status_file_with_clean_index_and_empty_workdir(void)
{
	...
	cl_git_pass(git3_index_open(&index, "my-index"));
	...
	cl_assert_equal_i(GIT3_STATUS_WT_DELETED, status);
...
void test_status_worktree_init__bracket_in_filename(void)
{
	...
	cl_git_pass(git3_repository_init(&repo, "with_bracket", 0));
	...
	cl_git_pass(git3_status_file(&status_flags, repo, FILE_WITH_BRACKET));
	...
	cl_assert(result.status == GIT3_STATUS_IGNORED);

	cl_git_pass(git3_status_file(&status_flags, repo, FILE_WITH_BRACKET));
	cl_assert(status_flags == GIT3_STATUS_IGNORED);
	...
	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_add_bypath(index, FILE_WITH_BRACKET));
	cl_git_pass(git3_index_write(index));
	...
	cl_git_pass(git3_status_file(&status_flags, repo, FILE_WITH_BRACKET));
	cl_assert(status_flags == GIT3_STATUS_INDEX_NEW);

	/* Create file without bracket */

	cl_git_mkfile("with_bracket/" FILE_WITHOUT_BRACKET, "I have no bracket in my name!\n");

	cl_git_pass(git3_status_file(&status_flags, repo, FILE_WITHOUT_BRACKET));
	cl_assert(status_flags == GIT3_STATUS_WT_NEW);

	cl_git_fail_with(git3_status_file(&status_flags, repo, "LICENSE\\[1\\].md"), GIT3_ENOTFOUND);
...
#define FILE_WITH_SPACE "LICENSE - copy.md"
	...
	cl_git_pass(git3_repository_init(&repo, "with_space", 0));
	...
	cl_git_pass(git3_status_file(&status_flags, repo, FILE_WITH_SPACE));
	cl_assert(status_flags == GIT3_STATUS_WT_NEW);
	...
	cl_git_pass(git3_repository_index(&index, repo));
	cl_git_pass(git3_index_add_bypath(index, FILE_WITH_SPACE));
	cl_git_pass(git3_index_write(index));
...
void test_status_worktree_init__disable_pathspec_match(void)
{
	...
	cl_git_pass(git3_repository_init(&repo, "pathspec", 0));
	...
	cl_git_pass(
		git3_status_foreach_ext(repo, &opts, cb_status__expected_path,
		file_with_bracket)
	);
	...
	cl_git_pass(
		git3_status_foreach_ext(repo, &opts, cb_status__expected_path, NULL)
	);
...
void test_status_worktree_init__new_staged_file_must_handle_crlf(void)
{
	...
	cl_git_pass(git3_status_file(&status, repo, "testfile.txt"));
