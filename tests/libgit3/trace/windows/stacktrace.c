...
static void a(void)
{
	...
	cl_assert(git3_win32_leakcheck_stack(buf, sizeof(buf), 0, NULL, NULL) == 0);
...
void test_trace_windows_stacktrace__leaks(void)
{
	 ...
	 */
	before = git3_win32_leakcheck_stacktrace_dump(
		GIT3_WIN32_LEAKCHECK_STACKTRACE_QUIET |
		GIT3_WIN32_LEAKCHECK_STACKTRACE_LEAKS_TOTAL |
		GIT3_WIN32_LEAKCHECK_STACKTRACE_SET_MARK,
		NULL);

	p1 = git3__malloc(5);
	leaks = git3_win32_leakcheck_stacktrace_dump(
		GIT3_WIN32_LEAKCHECK_STACKTRACE_QUIET |
		GIT3_WIN32_LEAKCHECK_STACKTRACE_LEAKS_SINCE_MARK,
		"p1");
	...
	error = git3_win32_leakcheck_stacktrace_dump(
		GIT3_WIN32_LEAKCHECK_STACKTRACE_SET_MARK,
		NULL);
	...
	after = git3_win32_leakcheck_stacktrace_dump(
		GIT3_WIN32_LEAKCHECK_STACKTRACE_QUIET |
		GIT3_WIN32_LEAKCHECK_STACKTRACE_LEAKS_TOTAL,
		"total");
...
{
#if defined(GIT3_DEBUG_LEAKCHECK_WIN32)
	git3_win32_leakcheck_stack_set_aux_cb(aux_cb_alloc__1, aux_cb_lookup__1);
	c();
	c();
	c();
	c();
	git3_win32_leakcheck_stack_set_aux_cb(NULL, NULL);
#endif
}
