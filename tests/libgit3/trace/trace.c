...
static void trace_callback(git3_trace_level_t level, const char *message)
{
	GIT3_UNUSED(level);
...
void test_trace_trace__initialize(void)
{
	...
	git3_trace_set(GIT3_TRACE_INFO, trace_callback);
	written = 0;
}
...
{
	cl_assert(git3_trace_level() == GIT3_TRACE_INFO);
}

void test_trace_trace__can_reset(void)
{
	cl_assert(git3_trace_level() == GIT3_TRACE_INFO);
	cl_git_pass(git3_trace_set(GIT3_TRACE_ERROR, trace_callback));

	cl_assert(written == 0);
	git3_trace(GIT3_TRACE_INFO, "Hello %s!", "world");
	cl_assert(written == 0);

	git3_trace(GIT3_TRACE_ERROR, "Hello %s!", "world");
	cl_assert(written == 1);
}
...
{
	cl_assert(git3_trace_level() == GIT3_TRACE_INFO);
	cl_git_pass(git3_trace_set(GIT3_TRACE_NONE, NULL));

	cl_assert(git3_trace_level() == GIT3_TRACE_NONE);

	cl_assert(written == 0);
	git3_trace(GIT3_TRACE_FATAL, "Hello %s!", "world");
	cl_assert(written == 0);
}
...
{
	cl_assert(written == 0);
	git3_trace(GIT3_TRACE_DEBUG, "Hello %s!", "world");
	cl_assert(written == 0);
}
