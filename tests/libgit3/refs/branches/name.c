...
void test_refs_branches_name__can_get_local_branch_name(void)
{
	...
	cl_git_pass(git3_branch_lookup(&ref,repo,"master",GIT3_BRANCH_LOCAL));
	cl_git_pass(git3_branch_name(&name,ref));
...
void test_refs_branches_name__can_get_remote_branch_name(void)
{
	...
	cl_git_pass(git3_branch_lookup(&ref,repo,"test/master",GIT3_BRANCH_REMOTE));
	cl_git_pass(git3_branch_name(&name,ref));
...
void test_refs_branches_name__error_when_ref_is_no_branch(void)
{
	...
	cl_git_pass(git3_reference_lookup(&ref,repo,"refs/notes/fanout"));
	cl_git_fail(git3_branch_name(&name,ref));
}

static int name_is_valid(const char *name)
{
	int valid;
	cl_git_pass(git3_branch_name_is_valid(&valid, name));
	return valid;
}
