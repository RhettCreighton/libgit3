...
void test_refs_branches_ishead__can_tell_if_a_branch_is_pointed_at_by_HEAD(void)
{
	...
	cl_assert_equal_i(true, git3_branch_is_head(branch));
}
...
void test_refs_branches_ishead__can_properly_handle_unborn_HEAD(void)
{
	...
	cl_assert_equal_i(false, git3_branch_is_head(branch));
}
...
{
	cl_git_pass(git3_reference_lookup(&branch, repo, "refs/heads/br2"));
...
{
	git3_reference *linked, *super, *head;

	cl_git_pass(git3_reference_symbolic_create(&linked, repo, "refs/heads/linked", "refs/heads/master", 0, NULL));
	cl_git_pass(git3_reference_symbolic_create(&super, repo, "refs/heads/super", "refs/heads/linked", 0, NULL));
	cl_git_pass(git3_reference_symbolic_create(&head, repo, GIT3_HEAD_FILE, "refs/heads/super", 1, NULL));

	cl_assert_equal_i(false, git3_branch_is_head(linked));
	cl_assert_equal_i(false, git3_branch_is_head(super));

	cl_git_pass(git3_repository_head(&branch, repo));
	cl_assert_equal_s("refs/heads/master", git3_reference_name(branch));

	git3_reference_free(linked);
	git3_reference_free(super);
