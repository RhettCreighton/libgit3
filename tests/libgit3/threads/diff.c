...
#  define git3_thread_yield() Sleep(0)
# elif defined(__FreeBSD__) || defined(__MidnightBSD__) || defined(__DragonFly__)
#  define git3_thread_yield() pthread_yield()
# else
#  define git3_thread_yield() sched_yield()
# endif

#else
# define git3_thread_yield() (void)0
#endif

static git3_repository *_repo;
static git3_tree *_a, *_b;
static git3_atomic32 _counts[4];
...
{
#ifdef GIT3_WIN32
	_retries = git3_win32__retries;
	git3_win32__retries = 1;
#endif
}
...
static void setup_trees(void)
{
	...
	cl_git_pass(git3_repository_index(&idx, _repo));
	git3_index_free(idx);

	cl_git_pass(git3_revparse_single(
		(git3_object **)&_a, _repo, "0017bd4ab1^{tree}"));
	cl_git_pass(git3_revparse_single(
		(git3_object **)&_b, _repo, "26a125ee1b^{tree}"));
...
{
	git3_tree_free(_a); _a = NULL;
	git3_tree_free(_b); _b = NULL;

	if (_check_counts) {
		cl_assert_equal_i(288, git3_atomic32_get(&_counts[0]));
		cl_assert_equal_i(112, git3_atomic32_get(&_counts[1]));
		cl_assert_equal_i( 80, git3_atomic32_get(&_counts[2]));
		cl_assert_equal_i( 96, git3_atomic32_get(&_counts[3]));
	}
}
...
static void *run_index_diffs(void *arg)
{
	...
	case 1: /* diff tree 'a' to index */;
		cl_git_pass(git3_diff_tree_to_index(&diff, repo, _a, NULL, &opts));
		break;
	case 2: /* diff tree 'b' to index */;
		cl_git_pass(git3_diff_tree_to_index(&diff, repo, _b, NULL, &opts));
		break;
	case 3: /* diff index to workdir (explicit index) */;
		{
			git3_index *idx;
			cl_git_pass(git3_repository_index(&idx, repo));
			cl_git_pass(git3_diff_index_to_workdir(&diff, repo, idx, &opts));
			git3_index_free(idx);
			break;
		}
	}
	...
	i = git3_diff_num_deltas(diff);
	git3_atomic32_add(&_counts[0], (int32_t)i);
	exp[0] = (int)i;

	while (i > 0) {
		switch (git3_diff_get_delta(diff, --i)->status) {
		case GIT3_DELTA_MODIFIED: exp[1]++; git3_atomic32_inc(&_counts[1]); break;
		case GIT3_DELTA_ADDED:    exp[2]++; git3_atomic32_inc(&_counts[2]); break;
		case GIT3_DELTA_DELETED:  exp[3]++; git3_atomic32_inc(&_counts[3]); break;
		default: break;
		}
	}
...
static void *run_index_diffs_with_modifier(void *arg)
{
	...
	if (thread == 0) {
		...
		for (i = 0; i < 300; ++i) {
			switch (i & 0x03) {
			case 0: (void)git3_index_add_bypath(idx, "new_file"); break;
			case 1: (void)git3_index_remove_bypath(idx, "modified_file"); break;
			case 2: (void)git3_index_remove_bypath(idx, "new_file"); break;
			case 3: (void)git3_index_add_bypath(idx, "modified_file"); break;
			}
			git3_thread_yield();
		}
	...
	case 0: /* diff index to workdir */;
		...
		break;
	case 1: /* diff tree 'a' to index */;
		cl_git_pass(git3_diff_tree_to_index(&diff, repo, _a, idx, &opts));
		break;
	case 2: /* diff tree 'b' to index */;
		cl_git_pass(git3_diff_tree_to_index(&diff, repo, _b, idx, &opts));
		break;
