...
void test_index_tests__empty_index(void)
{
   ...
   cl_git_pass(git3_index_open(&index, "in-memory-index"));
...
void test_index_tests__default_test_index(void)
{
   ...
   cl_git_pass(git3_index_open(&index, TEST_INDEX_PATH));
   cl_assert(index->on_disk);

   cl_assert(git3_index_entrycount(index) == index_entry_count);
   cl_assert(git3_vector_is_sorted(&index->entries));
...
void test_index_tests__gitgit_index(void)
{
   ...
   cl_git_pass(git3_index_open(&index, TEST_INDEX2_PATH));
   cl_assert(index->on_disk);

   cl_assert(git3_index_entrycount(index) == index_entry_count_2);
   cl_assert(git3_vector_is_sorted(&index->entries));
...
void test_index_tests__find_in_empty(void)
{
   ...
   cl_git_pass(git3_index_open(&index, "fake-index"));
...
void test_index_tests__find_prefix(void)
{
   ...
   cl_git_pass(git3_index_find_prefix(&pos, index, "src"));
   entry = git3_index_get_byindex(index, pos);
   cl_assert(git3__strcmp(entry->path, "src/block-sha1/sha1.c") == 0);

   cl_git_pass(git3_index_find_prefix(&pos, index, "src/co"));
   entry = git3_index_get_byindex(index, pos);
   cl_assert(git3__strcmp(entry->path, "src/commit.c") == 0);
...
void test_index_tests__write(void)
{
   ...
   cl_git_pass(git3_index_open(&index, "index_rewrite"));
   cl_assert(index->on_disk);
...
void test_index_tests__add(void)
{
	...
	cl_git_pass(git3_repository_init(&repo, "./myrepo", 0));
	 ...
	 */
	cl_git_pass(git3_oid_from_string(&id1, "a8233120f6ad708f843d861ce2b7228ec4e3dec6", GIT3_OID_SHA1));
...
void test_index_tests__add_issue_1397(void)
{
	 ...
	 */
	cl_git_pass(git3_oid_from_string(&id1, "8312e0889a9cbab77c732b6bc39b51a683e3a318", GIT3_OID_SHA1));

	/* Make sure the initial SHA-1 is correct */
	cl_assert((entry = git3_index_get_bypath(index, "crlf_file.txt", 0)) != NULL);
	cl_assert_equal_oid(&id1, &entry->id);

	/* Update the index */
	cl_git_pass(git3_index_add_bypath(index, "crlf_file.txt"));
...
static void assert_add_fails(git3_repository *repo, const char *fn)
{
	...
	entry.path = fn;
	entry.mode = GIT3_FILEMODE_BLOB;
	cl_git_pass(git3_oid_from_string(&entry.id, "e69de29bb2d1d6434b8b29ae775ad8c2e48c5391", GIT3_OID_SHA1));
...
void test_index_tests__write_tree_invalid_unowned_index(void)
{
	...
	cl_git_pass(git3_oid_from_string(&entry.id, "8312e0a89a9cbab77c732b6bc39b51a783e3a318", GIT3_OID_SHA1));
	entry.path = "foo";
	entry.mode = GIT3_FILEMODE_BLOB;
	cl_git_pass(git3_index_add(idx, &entry));

	cl_git_pass(git3_repository_init(&repo, "./invalid-id", 0));
...
void test_index_tests__reload_from_disk(void)
{
	...
	cl_git_pass(git3_index_open(&read_index, write_index->index_file_path));
...
void test_index_tests__corrupted_extension(void)
{
	...
	cl_git_fail_with(git3_index_open(&index, TEST_INDEXBAD_PATH), GIT3_ERROR);
}
...
void test_index_tests__reload_while_ignoring_case(void)
{
	...
	cl_git_pass(git3_vector_verify_sorted(&index->entries));
	cl_assert(git3_index_get_bypath(index, ".HEADER", 0));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, ".header", 0));
...
void test_index_tests__change_icase_on_instance(void)
{
	...
	cl_git_pass(git3_vector_verify_sorted(&index->entries));
	cl_assert(e = git3_index_get_bypath(index, "src/common.h", 0));
	cl_assert_equal_p(NULL, e = git3_index_get_bypath(index, "SRC/Common.h", 0));
	cl_assert(e = git3_index_get_bypath(index, "COPYING", 0));
	cl_assert_equal_p(NULL, e = git3_index_get_bypath(index, "copying", 0));
...
void test_index_tests__can_modify_while_iterating(void)
{
	 ...
	 */
	cl_git_pass(git3_oid_from_string(&new_entry.id, "8312e0a89a9cbab77c732b6bc39b51a783e3a318", GIT3_OID_SHA1));
	new_entry.path = "newfile";
	new_entry.mode = GIT3_FILEMODE_BLOB;
	cl_git_pass(git3_index_add(index, &new_entry));

	cl_git_pass(git3_oid_from_string(&new_entry.id, "4141414141414141414141414141414141414141", GIT3_OID_SHA1));
