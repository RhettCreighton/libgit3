...
static git3_index *g_idx;

void test_index_bypath__initialize(void)
{
	g_repo = setup_fixture_submod2();
	cl_git_pass(git3_repository_index__weakptr(&g_idx, g_repo));
}
...
{
	cl_git_fail_with(GIT3_EDIRECTORY, git3_index_add_bypath(g_idx, "just_a_dir"));
}
...
void test_index_bypath__add_submodule(void)
{
	...
	cl_git_pass(git3_submodule_status(&status, g_repo, sm_name, 0));
	cl_assert_equal_i(GIT3_SUBMODULE_STATUS_WD_MODIFIED, status & GIT3_SUBMODULE_STATUS_WD_MODIFIED);
	cl_git_pass(git3_index_add_bypath(g_idx, sm_name));
	cl_git_pass(git3_submodule_status(&status, g_repo, sm_name, 0));
	cl_assert_equal_i(0, status & GIT3_SUBMODULE_STATUS_WD_MODIFIED);
}
...
void test_index_bypath__add_submodule_unregistered(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(g_idx, sm_name, 0));
	cl_assert_equal_s(sm_head, git3_oid_tostr_s(&entry->id));
...
void test_index_bypath__add_hidden(void)
{
	...
	cl_git_pass(git3_win32__hidden(&hidden, "submod2/hidden_file"));
	cl_assert(!hidden);

	cl_git_pass(git3_win32__set_hidden("submod2/hidden_file", true));

	cl_git_pass(git3_win32__hidden(&hidden, "submod2/hidden_file"));
	cl_assert(hidden);

	cl_git_pass(git3_index_add_bypath(g_idx, "hidden_file"));

	cl_assert(entry = git3_index_get_bypath(g_idx, "hidden_file", 0));
	cl_assert_equal_i(GIT3_FILEMODE_BLOB, entry->mode);
#endif
}
...
void test_index_bypath__add_keeps_existing_case(void)
{
	...
	cl_git_pass(git3_index_add_bypath(g_idx, "just_a_dir/file1.txt"));

	cl_assert(entry = git3_index_get_bypath(g_idx, "just_a_dir/file1.txt", 0));
...
void test_index_bypath__add_honors_existing_case(void)
{
	...
	cl_git_pass(git3_index_add_bypath(g_idx, "just_a_dir/File1.txt"));
	cl_git_pass(git3_index_add_bypath(g_idx, "JUST_A_DIR/file2.txt"));
	cl_git_pass(git3_index_add_bypath(g_idx, "Just_A_Dir/FILE3.txt"));

	cl_assert(entry = git3_index_get_bypath(g_idx, "just_a_dir/File1.txt", 0));
	cl_assert_equal_s("just_a_dir/File1.txt", entry->path);

	cl_assert(entry = git3_index_get_bypath(g_idx, "JUST_A_DIR/file2.txt", 0));
	cl_assert_equal_s("just_a_dir/file2.txt", entry->path);

	cl_assert(entry = git3_index_get_bypath(g_idx, "Just_A_Dir/FILE3.txt", 0));
...
{
	git3_index_entry dummy = { { 0 } };
	const git3_index_entry *entry;

	if (!cl_repo_get_bool(g_repo, "core.ignorecase"))
		clar__skip();

	dummy.mode = GIT3_FILEMODE_BLOB;
	cl_git_pass(git3_oid_from_string(&dummy.id, "f990a25a74d1a8281ce2ab018ea8df66795cd60b", GIT3_OID_SHA1));

	/* note that `git3_index_add` does no checking to canonical directories */
	dummy.path = "Just_a_dir/file0.txt";
	cl_git_pass(git3_index_add(g_idx, &dummy));
	...
	cl_git_pass(git3_index_add_bypath(g_idx, "JUST_A_DIR/file2.txt"));
	cl_git_pass(git3_index_add_bypath(g_idx, "Just_A_Dir/FILE3.txt"));
	cl_git_pass(git3_index_add_bypath(g_idx, "JusT_A_DIR/FILE4.txt"));
...
void test_index_bypath__add_honors_existing_case_3(void)
{
	...
	cl_git_pass(git3_index_add_bypath(g_idx, "just_a_dir/fILEE.txt"));

	cl_assert(entry = git3_index_get_bypath(g_idx, "JUST_A_DIR/fILEE.txt", 0));
...
void test_index_bypath__add_honors_existing_case_4(void)
{
	...
	cl_git_pass(git3_index_add_bypath(g_idx, "just_a_dir/A/b/Z/y/X/foo.txt"));

	cl_assert(entry = git3_index_get_bypath(g_idx, "just_a_dir/A/b/Z/y/X/foo.txt", 0));
...
void test_index_bypath__add_honors_mode(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(g_idx, "README.txt", 0)) != NULL);

	memcpy(&new_entry, entry, sizeof(git3_index_entry));
	new_entry.path = "README.txt";
	new_entry.mode = GIT3_FILEMODE_BLOB_EXECUTABLE;

	cl_must_pass(p_chmod("submod2/README.txt", GIT3_FILEMODE_BLOB_EXECUTABLE));

	cl_git_pass(git3_index_add(g_idx, &new_entry));
	cl_git_pass(git3_index_write(g_idx));

	cl_git_rewritefile("submod2/README.txt", "Modified but still executable");

	cl_git_pass(git3_index_add_bypath(g_idx, "README.txt"));
	cl_git_pass(git3_index_write(g_idx));

	cl_assert((entry = git3_index_get_bypath(g_idx, "README.txt", 0)) != NULL);
	cl_assert_equal_i(GIT3_FILEMODE_BLOB_EXECUTABLE, entry->mode);
}
...
void test_index_bypath__add_honors_conflict_mode(void)
{
	...
	cl_git_pass(git3_index_remove_bypath(g_idx, "README.txt"));

	for (stage = 1; stage <= 3; stage++) {
		new_entry.flags = stage << GIT3_INDEX_ENTRY_STAGESHIFT;
		cl_git_pass(git3_index_add(g_idx, &new_entry));
	}
...
void test_index_bypath__add_honors_symlink(void)
{
	...
	memcpy(&new_entry, entry, sizeof(git3_index_entry));
	new_entry.path = "README.txt";
	new_entry.mode = GIT3_FILEMODE_LINK;
	...
	cl_assert((entry = git3_index_get_bypath(g_idx, "README.txt", 0)) != NULL);
	cl_assert_equal_i(GIT3_FILEMODE_LINK, entry->mode);
}
