...
static git3_odb *g_odb;
static git3_index *g_index;
static git3_oid g_empty_id;
...
void test_index_collision__initialize(void)
{
	...
	cl_git_pass(git3_odb_write(&g_empty_id, g_odb, "", 0, GIT3_OBJECT_BLOB));
}
...
void test_index_collision__add_blob_with_conflicting_file(void)
{
	...
	entry.mode  = 0100644;
	entry.file_size = 0;
	git3_oid_cpy(&entry.id, &g_empty_id);

	entry.path = "a/b";
	cl_git_pass(git3_index_add(g_index, &entry));

	/* Check a/b exists here */
	cl_git_pass(git3_index_write_tree(&tree_id, g_index));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &tree_id));
	cl_git_pass(git3_tree_entry_bypath(&tentry, tree, "a/b"));
	...
	cl_git_pass(git3_index_write_tree(&tree_id, g_index));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &tree_id));
	cl_git_pass(git3_tree_entry_bypath(&tentry, tree, "a/b/c"));
	git3_tree_entry_free(tentry);
	git3_tree_free(tree);
}
...
void test_index_collision__add_blob_with_conflicting_dir(void)
{
	...
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &tree_id));
	cl_git_pass(git3_tree_entry_bypath(&tentry, tree, "a/b"));
	cl_git_fail(git3_tree_entry_bypath(&tentry, tree, "a/b/c"));
	git3_tree_entry_free(tentry);
	git3_tree_free(tree);
}
