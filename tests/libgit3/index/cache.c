#include "clar_libgit3.h"
#include "git3.h"
#include "index.h"
#include "tree-cache.h"

static git3_repository *g_repo;

void test_index_cache__initialize(void)
{
	g_repo = cl_git_sandbox_init("testrepo");
}

void test_index_cache__cleanup(void)
{
	cl_git_sandbox_cleanup();
	g_repo = NULL;
}

void test_index_cache__write_extension_at_root(void)
{
	git3_index *index;
	git3_tree *tree;
	git3_oid id;
	const char *tree_id_str = "45dd856fdd4d89b884c340ba0e047752d9b085d6";
	const char *index_file = "index-tree";

	cl_git_pass(git3_index_open_ext(&index, index_file, NULL));
	cl_assert(index->tree == NULL);
	cl_git_pass(git3_oid_from_string(&id, tree_id_str, GIT3_OID_SHA1));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &id));
	cl_git_pass(git3_index_read_tree(index, tree));
	git3_tree_free(tree);

	cl_assert(index->tree);
	cl_git_pass(git3_index_write(index));
	git3_index_free(index);

	cl_git_pass(git3_index_open_ext(&index, index_file, NULL));
	cl_assert(index->tree);

	cl_assert_equal_i(git3_index_entrycount(index), index->tree->entry_count);
	cl_assert_equal_i(0, index->tree->children_count);

	cl_assert(git3_oid_equal(&id, &index->tree->oid));

	cl_git_pass(p_unlink(index_file));
	git3_index_free(index);
}

void test_index_cache__write_extension_invalidated_root(void)
{
	git3_index *index;
	git3_tree *tree;
	git3_oid id;
	git3_index_options index_opts = GIT3_INDEX_OPTIONS_INIT;
	const char *tree_id_str = "45dd856fdd4d89b884c340ba0e047752d9b085d6";
	const char *index_file = "index-tree-invalidated";
	git3_index_entry entry;

	cl_git_pass(git3_index_open_ext(&index, index_file, &index_opts));
	cl_assert(index->tree == NULL);
	cl_git_pass(git3_oid_from_string(&id, tree_id_str, GIT3_OID_SHA1));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &id));
	cl_git_pass(git3_index_read_tree(index, tree));
	git3_tree_free(tree);

	cl_assert(index->tree);

	memset(&entry, 0x0, sizeof(git3_index_entry));
	git3_oid_cpy(&entry.id, &git3_index_get_byindex(index, 0)->id);
	entry.mode = GIT3_FILEMODE_BLOB;
	entry.path = "some-new-file.txt";

	cl_git_pass(git3_index_add(index, &entry));

	cl_assert_equal_i(-1, index->tree->entry_count);

	cl_git_pass(git3_index_write(index));
	git3_index_free(index);

	index_opts.oid_type = GIT3_OID_SHA1;

	cl_git_pass(git3_index_open_ext(&index, index_file, &index_opts));
	cl_assert(index->tree);

	cl_assert_equal_i(-1, index->tree->entry_count);
	cl_assert_equal_i(0, index->tree->children_count);

	cl_assert(git3_oid_cmp(&id, &index->tree->oid));

	cl_git_pass(p_unlink(index_file));
	git3_index_free(index);
}

void test_index_cache__read_tree_no_children(void)
{
	git3_index *index;
	git3_index_entry entry;
	git3_tree *tree;
	git3_oid id;

	cl_git_pass(git3_index_new(&index));
	cl_assert(index->tree == NULL);
	cl_git_pass(git3_oid_from_string(&id, "45dd856fdd4d89b884c340ba0e047752d9b085d6", GIT3_OID_SHA1));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &id));
	cl_git_pass(git3_index_read_tree(index, tree));
	git3_tree_free(tree);

	cl_assert(index->tree);
	cl_assert(git3_oid_equal(&id, &index->tree->oid));
	cl_assert_equal_i(0, index->tree->children_count);
	cl_assert_equal_i(git3_index_entrycount(index), index->tree->entry_count);

	memset(&entry, 0x0, sizeof(git3_index_entry));
	entry.path = "new.txt";
	entry.mode = GIT3_FILEMODE_BLOB;
	git3_oid_from_string(&entry.id, "d4bcc68acd4410bf836a39f20afb2c2ece09584e", GIT3_OID_SHA1);

	cl_git_pass(git3_index_add(index, &entry));
	cl_assert_equal_i(-1, index->tree->entry_count);

	git3_index_free(index);
}

void test_index_cache__two_levels(void)
{
	git3_tree *tree;
	git3_oid tree_id;
	git3_index *index;
	git3_index_entry entry;
	const git3_tree_cache *tree_cache;

	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_clear(index));

	memset(&entry, 0x0, sizeof(entry));
	entry.mode = GIT3_FILEMODE_BLOB;
	cl_git_pass(git3_oid_from_string(&entry.id, "a8233120f6ad708f843d861ce2b7228ec4e3dec6", GIT3_OID_SHA1));
	entry.path = "top-level.txt";
	cl_git_pass(git3_index_add(index, &entry));

	entry.path = "subdir/file.txt";
	cl_git_pass(git3_index_add(index, &entry));

	/* the read-tree fills the tree cache */
	cl_git_pass(git3_index_write_tree(&tree_id, index));
	cl_git_pass(git3_tree_lookup(&tree, g_repo, &tree_id));
	cl_git_pass(git3_index_read_tree(index, tree));
	git3_tree_free(tree);
	cl_git_pass(git3_index_write(index));

	/* we now must have cache entries for "" and "subdir" */
	cl_assert(index->tree);
	cl_assert(git3_tree_cache_get(index->tree, "subdir"));

	cl_git_pass(git3_index_read(index, true));
	/* we must still have cache entries for "" and "subdir", since we wrote it out */
	cl_assert(index->tree);
	cl_assert(git3_tree_cache_get(index->tree, "subdir"));

	entry.path = "top-level.txt";
	cl_git_pass(git3_oid_from_string(&entry.id, "3697d64be941a53d4ae8f6a271e4e3fa56b022cc", GIT3_OID_SHA1));
	cl_git_pass(git3_index_add(index, &entry));

	/* writ out the index after we invalidate the root */
	cl_git_pass(git3_index_write(index));
	cl_git_pass(git3_index_read(index, true));

	/* the cache for the subtree must still be valid, even if the root isn't */
	cl_assert(index->tree);
	cl_assert_equal_i(-1, index->tree->entry_count);
	cl_assert_equal_i(1, index->tree->children_count);
	tree_cache = git3_tree_cache_get(index->tree, "subdir");
	cl_assert(tree_cache);
	cl_assert_equal_i(1, tree_cache->entry_count);

	git3_index_free(index);
}

void test_index_cache__read_tree_children(void)
{
	git3_index *index;
	git3_index_entry entry;
	git3_tree *tree;
	const git3_tree_cache *cache;
	git3_oid tree_id;

	cl_git_pass(git3_repository_index(&index, g_repo));
	cl_git_pass(git3_index_clear(index));
	cl_assert(index->tree == NULL);


	/* add a bunch of entries at different levels */
	memset(&entry, 0x0, sizeof(git3_index_entry));
	entry.path = "top-level";
	entry.mode = GIT3_FILEMODE_BLOB;
	git3_oid_from_string(&entry.id, "ee3fa1b8c00aff7fe02065fdb50864bb0d932ccf", GIT3_OID_SHA1);
	cl_git_pass(git3_index_add(index, &entry));


	entry.path = "subdir/some-file";
	cl_git_pass(git3_index_add(index, &entry));

	entry.path = "subdir/even-deeper/some-file";
	cl_git_pass(git3_index_add(index, &entry));

	entry.path = "subdir2/some-file";
	cl_git_pass(git3_index_add(index, &entry));

	cl_git_pass(git3_index_write_tree(&tree_id, index));
	cl_git_pass(git3_index_clear(index));
	cl_assert(index->tree == NULL);

	cl_git_pass(git3_tree_lookup(&tree, g_repo, &tree_id));
	cl_git_pass(git3_index_read_tree(index, tree));
	git3_tree_free(tree);

	cl_assert(index->tree);
	cl_assert_equal_i(2, index->tree->children_count);

	/* override with a slightly different id, also dummy */
	entry.path = "subdir/some-file";
	git3_oid_from_string(&entry.id, "ee3fa1b8c00aff7fe02065fdb50864bb0d932ccf", GIT3_OID_SHA1);
	cl_git_pass(git3_index_add(index, &entry));

	cl_assert_equal_i(-1, index->tree->entry_count);

	cache = git3_tree_cache_get(index->tree, "subdir");
	cl_assert(cache);
	cl_assert_equal_i(-1, cache->entry_count);

	cache = git3_tree_cache_get(index->tree, "subdir/even-deeper");
	cl_assert(cache);
	cl_assert_equal_i(1, cache->entry_count);

	cache = git3_tree_cache_get(index->tree, "subdir2");
	cl_assert(cache);
	cl_assert_equal_i(1, cache->entry_count);

	git3_index_free(index);
}
