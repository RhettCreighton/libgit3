...
void test_index_filemodes__read(void)
{
	...
	cl_assert_equal_i(6, (int)git3_index_entrycount(index));
	...
	const char *filename, const char *backup, unsigned int create_mode)
{
	...
	cl_git_pass(git3_str_joinpath(&path, "filemodes", filename));
	cl_git_pass(git3_str_printf(&content, "%s as %08u (%d)",
...
static void add_and_check_mode_(
	git3_index *index, const char *filename, unsigned int expect_mode,
	const char *file, const char *func, int line)
{
	...
	clar__assert(!git3_index_find(&pos, index, filename),
...
void test_index_filemodes__untrusted(void)
{
	...
	add_and_check_mode(index, "new_off", GIT3_FILEMODE_BLOB);

	/* 6 - add new 0755 -> expect 0644 if core.filemode == false */
	cl_git_write2file("filemodes/new_on", "blah", 0,
		O_WRONLY | O_CREAT | O_TRUNC, 0755);
	add_and_check_mode(index, "new_on", GIT3_FILEMODE_BLOB);
...
void test_index_filemodes__trusted(void)
{
	...
	replace_file_with_mode("exec_on", "filemodes/exec_on.0", 0644);
	add_and_check_mode(index, "exec_on", GIT3_FILEMODE_BLOB);

	/* 3 - add 0755 over existing 0644 -> expect 0755 */
	replace_file_with_mode("exec_off", "filemodes/exec_off.1", 0755);
	add_and_check_mode(index, "exec_off", GIT3_FILEMODE_BLOB_EXECUTABLE);
	...
	cl_git_write2file("filemodes/new_on", "blah", 0,
		O_WRONLY | O_CREAT | O_TRUNC, 0755);
	add_and_check_mode(index, "new_on", GIT3_FILEMODE_BLOB_EXECUTABLE);
...
static void add_entry_and_check_mode_(
	git3_index *index, bool from_file, git3_filemode_t mode,
	const char *file, const char *func, int line)
{
	size_t pos;
	const git3_index_entry* entry;
	 ...
	 * git3_index_add(), otherwise create a new entry testing git3_index_add_from_buffer
	 */
	if (from_file)
	{
		clar__assert(!git3_index_find(&pos, index, "exec_off"),
		...
		memset(&new_entry, 0x0, sizeof(git3_index_entry));
	...
	if (from_file)
	{
		clar__assert(!git3_index_add(index, &new_entry),
			file, func, line, "Cannot add index entry", NULL, 1);
	}
	else
	{
		const char *content = "hey there\n";
		clar__assert(!git3_index_add_from_buffer(index, &new_entry, content, strlen(content)),
			file, func, line, "Cannot add index entry from buffer", NULL, 1);
	}

	clar__assert(!git3_index_find(&pos, index, "filemodes/explicit_test"),
...
void test_index_filemodes__explicit(void)
{
	 ...
	 * given explicitly to git3_index_add or git3_index_add_from_buffer
	 */
	cl_repo_set_bool(g_repo, "core.filemode", false);
	...
	add_entry_and_check_mode(index, true, GIT3_FILEMODE_BLOB);

	/* 2 - add 0755 entry over existing 0644 */
	add_entry_and_check_mode(index, true, GIT3_FILEMODE_BLOB_EXECUTABLE);

	/* 3 - add 0644 entry over existing 0755 */
	add_entry_and_check_mode(index, true, GIT3_FILEMODE_BLOB);

	/* 4 - add 0755 buffer entry over existing 0644  */
	add_entry_and_check_mode(index, false, GIT3_FILEMODE_BLOB_EXECUTABLE);

	/* 5 - add 0644 buffer entry over existing 0755 */
	add_entry_and_check_mode(index, false, GIT3_FILEMODE_BLOB);
...
void test_index_filemodes__invalid(void)
{
	...
	const git3_index_entry *dummy;

	cl_git_pass(git3_repository_index(&index, g_repo));

	/* add a dummy file so that we have a valid id */
	cl_git_mkfile("./filemodes/dummy-file.txt", "new-file\n");
	cl_git_pass(git3_index_add_bypath(index, "dummy-file.txt"));
	cl_assert((dummy = git3_index_get_bypath(index, "dummy-file.txt", 0)));

	GIT3_INDEX_ENTRY_STAGE_SET(&entry, 0);
	entry.path = "foo";
	entry.mode = GIT3_OBJECT_BLOB;
	git3_oid_cpy(&entry.id, &dummy->id);
...
void test_index_filemodes__frombuffer_requires_files(void)
{
	git3_index *index;
	git3_index_entry new_entry;
	const git3_index_entry *ret_entry;
	const char *content = "hey there\n";
	...
	cl_git_pass(git3_index_add_from_buffer(index,
		&new_entry, content, strlen(content)));

	cl_assert((ret_entry = git3_index_get_bypath(index, "dummy-file.txt", 0)));
	cl_assert_equal_s("dummy-file.txt", ret_entry->path);
	cl_assert_equal_i(GIT3_FILEMODE_BLOB, ret_entry->mode);
	...
	cl_assert((ret_entry = git3_index_get_bypath(index, "dummy-file.txt", 0)));
	cl_assert_equal_s("dummy-file.txt", ret_entry->path);
	cl_assert_equal_i(GIT3_FILEMODE_BLOB_EXECUTABLE, ret_entry->mode);
	...
	cl_assert((ret_entry = git3_index_get_bypath(index, "dummy-link.txt", 0)));
	cl_assert_equal_s("dummy-link.txt", ret_entry->path);
	cl_assert_equal_i(GIT3_FILEMODE_LINK, ret_entry->mode);

	/* trees are rejected */
	new_entry.path = "invalid_mode.txt";
	new_entry.mode = GIT3_FILEMODE_TREE;

	cl_git_fail(git3_index_add_from_buffer(index,
		&new_entry, content, strlen(content)));
	cl_assert_equal_p(NULL, git3_index_get_bypath(index, "invalid_mode.txt", 0));

	/* submodules are rejected */
	new_entry.path = "invalid_mode.txt";
	new_entry.mode = GIT3_FILEMODE_COMMIT;
