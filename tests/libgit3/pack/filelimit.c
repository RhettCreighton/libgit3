#include "clar_libgit3.h"
#include "mwindow.h"

#include <git3.h>
#include "git3/sys/commit.h"
#include "git3/sys/mempack.h"

static size_t expected_open_mwindow_files = 0;
static size_t original_mwindow_file_limit = 0;

extern git3_mutex git3_mwindow__mutex;
extern git3_mwindow_ctl git3_mwindow__mem_ctl;

void test_pack_filelimit__initialize_tiny(void)
{
	expected_open_mwindow_files = 1;
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_GET_MWINDOW_FILE_LIMIT, &original_mwindow_file_limit));
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_SET_MWINDOW_FILE_LIMIT, expected_open_mwindow_files));
}

void test_pack_filelimit__initialize_medium(void)
{
	expected_open_mwindow_files = 10;
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_GET_MWINDOW_FILE_LIMIT, &original_mwindow_file_limit));
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_SET_MWINDOW_FILE_LIMIT, expected_open_mwindow_files));
}

void test_pack_filelimit__initialize_unlimited(void)
{
	expected_open_mwindow_files = 15;
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_GET_MWINDOW_FILE_LIMIT, &original_mwindow_file_limit));
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_SET_MWINDOW_FILE_LIMIT, 0));
}

void test_pack_filelimit__cleanup(void)
{
	git3_str path = GIT3_STR_INIT;
	cl_git_pass(git3_libgit3_opts(GIT3_OPT_SET_MWINDOW_FILE_LIMIT, original_mwindow_file_limit));

	cl_git_pass(git3_str_joinpath(&path, clar_sandbox_path(), "repo.git"));
	cl_fixture_cleanup(path.ptr);
	git3_str_dispose(&path);
}

/*
 * Create a packfile with one commit, one tree, and two blobs. The first blob
 * (README.md) has the same content in all commits, but the second one
 * (file.txt) has a different content in each commit.
 */
static void create_packfile_commit(
		git3_repository *repo,
		git3_oid *out_commit_id,
		git3_oid *parent_id,
		size_t commit_index,
		size_t commit_count)
{
	git3_str file_contents = GIT3_STR_INIT;
	git3_treebuilder *treebuilder;
	git3_packbuilder *packbuilder;
	git3_signature *s;
	git3_oid oid, tree_id, commit_id;
	const git3_oid *parents[] = { parent_id };
	size_t parent_count = parent_id ? 1 : 0;

	cl_git_pass(git3_treebuilder_new(&treebuilder, repo, NULL));

	cl_git_pass(git3_blob_create_from_buffer(&oid, repo, "", 0));
	cl_git_pass(git3_treebuilder_insert(NULL, treebuilder, "README.md", &oid, 0100644));

	cl_git_pass(git3_str_printf(&file_contents, "Commit %zd/%zd", commit_index, commit_count));
	cl_git_pass(git3_blob_create_from_buffer(&oid, repo, file_contents.ptr, file_contents.size));
	cl_git_pass(git3_treebuilder_insert(NULL, treebuilder, "file.txt", &oid, 0100644));

	cl_git_pass(git3_treebuilder_write(&tree_id, treebuilder));
	cl_git_pass(git3_signature_now(&s, "alice", "alice@example.com"));
	cl_git_pass(git3_commit_create_from_ids(&commit_id, repo, "refs/heads/master", s, s,
			NULL, file_contents.ptr, &tree_id, parent_count, parents));

	cl_git_pass(git3_packbuilder_new(&packbuilder, repo));
	cl_git_pass(git3_packbuilder_insert_commit(packbuilder, &commit_id));
	cl_git_pass(git3_packbuilder_write(packbuilder, NULL, 0, NULL, NULL));

	cl_git_pass(git3_oid_cpy(out_commit_id, &commit_id));

	git3_str_dispose(&file_contents);
	git3_treebuilder_free(treebuilder);
	git3_packbuilder_free(packbuilder);
	git3_signature_free(s);
}

void test_pack_filelimit__open_repo_with_multiple_packfiles(void)
{
	git3_str path = GIT3_STR_INIT;
	git3_mwindow_ctl *ctl = &git3_mwindow__mem_ctl;
	git3_repository *repo;
	git3_revwalk *walk;
	git3_oid id, *parent_id = NULL;
	size_t i;
	const size_t commit_count = 16;
	unsigned int open_windows;

	/*
	 * Create a repository and populate it with 16 commits, each in its own
	 * packfile.
	 */
	cl_git_pass(git3_str_joinpath(&path, clar_sandbox_path(), "repo.git"));
	cl_git_pass(git3_repository_init(&repo, path.ptr, true));
	for (i = 0; i < commit_count; ++i) {
		create_packfile_commit(repo, &id, parent_id, i + 1, commit_count);
		parent_id = &id;
	}

	cl_git_pass(git3_revwalk_new(&walk, repo));
	cl_git_pass(git3_revwalk_sorting(walk, GIT3_SORT_TOPOLOGICAL));
	cl_git_pass(git3_revwalk_push_ref(walk, "refs/heads/master"));

	/* Walking the repository requires eventually opening each of the packfiles. */
	i = 0;
	while (git3_revwalk_next(&id, walk) == 0)
		++i;
	cl_assert_equal_i(commit_count, i);

	cl_git_pass(git3_mutex_lock(&git3_mwindow__mutex));
	/*
	 * Adding an assert while holding a lock will cause the whole process to
	 * deadlock. Copy the value and do the assert after releasing the lock.
	 */
	open_windows = ctl->open_windows;
	cl_git_pass(git3_mutex_unlock(&git3_mwindow__mutex));

	cl_assert_equal_i(expected_open_mwindow_files, open_windows);

	git3_str_dispose(&path);
	git3_revwalk_free(walk);
	git3_repository_free(repo);
}
