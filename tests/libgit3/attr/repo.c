...
void test_attr_repo__get_one(void)
{
	...
	for (i = 0; i < (int)ARRAY_SIZE(get_one_test_cases); ++i) {
		...
		cl_git_pass(git3_attr_get(&value, g_repo, 0, scan->path, scan->attr));
		attr_check_expected(
			scan->expected, scan->expected_str, scan->attr, value);
	}

	cl_assert(git3_attr_cache__is_cached(
		g_repo, GIT3_ATTR_FILE_SOURCE_FILE, ".git/info/attributes"));
	cl_assert(git3_attr_cache__is_cached(
		g_repo, GIT3_ATTR_FILE_SOURCE_FILE, ".gitattributes"));
	cl_assert(git3_attr_cache__is_cached(
		g_repo, GIT3_ATTR_FILE_SOURCE_FILE, "sub/.gitattributes"));
}
...
void test_attr_repo__get_many(void)
{
	...
	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "root_test1", 4, names));

	cl_assert(GIT3_ATTR_IS_TRUE(values[0]));
	cl_assert(GIT3_ATTR_IS_TRUE(values[1]));
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(values[2]));
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(values[3]));

	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "root_test2", 4, names));
	...
	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "sub/subdir_test1", 4, names));
...
void test_attr_repo__get_many_in_place(void)
{
	...
	cl_git_pass(git3_attr_get_many(vals, g_repo, 0, "sub/subdir_test1", 4, vals));

	cl_assert(GIT3_ATTR_IS_TRUE(vals[0]));
	cl_assert(GIT3_ATTR_IS_TRUE(vals[1]));
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(vals[2]));
...
void test_attr_repo__foreach(void)
{
	...
	count = 0;
	cl_git_pass(git3_attr_foreach(
		g_repo, 0, "root_test1", &count_attrs, &count));
	cl_assert(count == 2);

	count = 0;
	cl_git_pass(git3_attr_foreach(g_repo, 0, "sub/subdir_test1",
		&count_attrs, &count));
	cl_assert(count == 4); /* repoattr, rootattr, subattr, negattr */

	count = 0;
	cl_git_pass(git3_attr_foreach(g_repo, 0, "sub/subdir_test2.txt",
		&count_attrs, &count));
	cl_assert(count == 6); /* repoattr, rootattr, subattr, reposub, negattr, another */

	count = 2;
	cl_assert_equal_i(
		CANCEL_VALUE, git3_attr_foreach(
...
void test_attr_repo__manpage_example(void)
{
	...
	cl_git_pass(git3_attr_get(&value, g_repo, 0, "sub/abc", "foo"));
	cl_assert(GIT3_ATTR_IS_TRUE(value));

	cl_git_pass(git3_attr_get(&value, g_repo, 0, "sub/abc", "bar"));
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(value));

	cl_git_pass(git3_attr_get(&value, g_repo, 0, "sub/abc", "baz"));
	cl_assert(GIT3_ATTR_IS_FALSE(value));

	cl_git_pass(git3_attr_get(&value, g_repo, 0, "sub/abc", "merge"));
	cl_assert_equal_s("filfre", value);

	cl_git_pass(git3_attr_get(&value, g_repo, 0, "sub/abc", "frotz"));
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(value));
}
...
static void add_to_workdir(const char *filename, const char *content)
{
	...
	cl_git_pass(git3_str_joinpath(&buf, "attr", filename));
	cl_git_rewritefile(git3_str_cstr(&buf), content);

	git3_str_dispose(&buf);
}

static void assert_proper_normalization(git3_index *index, const char *filename, const char *expected_sha)
{
	size_t index_pos;
	const git3_index_entry *entry;

	add_to_workdir(filename, CONTENT);
	cl_git_pass(git3_index_add_bypath(index, filename));

	cl_assert(!git3_index_find(&index_pos, index, filename));

	entry = git3_index_get_byindex(index, index_pos);
	cl_assert_equal_i(0, git3_oid_streq(&entry->id, expected_sha));
}
...
void test_attr_repo__bare_repo_with_index(void)
{
	...
	cl_git_pass(git3_index_add_bypath(index, ".gitattributes"));
	git3_index_free(index);

	cl_must_pass(p_unlink("attr/.gitattributes"));
	cl_assert(!git3_fs_path_exists("attr/.gitattributes"));

	cl_git_pass(git3_repository_set_bare(g_repo));

	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "file.txt", 4, names));
	...
	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "trial.txt", 4, names));

	cl_assert(GIT3_ATTR_IS_FALSE(values[0]));
	cl_assert_equal_s("barfoo", values[1]);
	cl_assert(GIT3_ATTR_IS_UNSPECIFIED(values[2]));
	cl_assert(GIT3_ATTR_IS_TRUE(values[3]));

	cl_git_pass(git3_attr_get_many(values, g_repo, 0, "sub/sub/subdir.txt", 4, names));
...
void test_attr_repo__inmemory_repo_without_index(void)
{
	...
	cl_git_pass(git3_repository_new(&inmemory));
	cl_assert(git3_repository_is_bare(inmemory));

	/* verify repo isn't given an index upfront in future */
	git3_repository_index(&index, inmemory);
	cl_assert(!index);

	/* check attributes can be queried without error due to missing index */
	cl_git_pass(git3_attr_get_many(values, inmemory, 0, "fake.txt", 1, names));
...
{
	git3_str sysdir = GIT3_STR_INIT;
	const char *value;

	cl_git_pass(p_mkdir("system", 0777));
	cl_git_rewritefile("system/gitattributes", "file merge=foo");
	cl_git_pass(git3_str_joinpath(&sysdir, clar_sandbox_path(), "system"));
	cl_git_pass(git3_sysdir_set(GIT3_SYSDIR_SYSTEM, sysdir.ptr));
	g_repo = cl_git_sandbox_reopen();

	cl_git_pass(git3_attr_get(&value, g_repo, 0, "file", "merge"));
	cl_assert_equal_s(value, "foo");

	cl_git_pass(p_unlink("system/gitattributes"));
	cl_git_pass(p_rmdir("system"));
	git3_str_dispose(&sysdir);
}
...
void test_attr_repo__sysdir_with_session(void)
{
	...
	cl_git_pass(git3_attr_session__init(&session, g_repo));
	cl_git_pass(git3_attr_get_many_with_session(values, g_repo, &session, NULL, "file", ARRAY_SIZE(attrs), attrs));
	...
	cl_git_pass(p_rmdir("system"));
	git3_str_dispose(&sysdir);
	git3_attr_session__free(&session);
}
...
void test_attr_repo__rewrite(void)
{
	...
	cl_git_pass(git3_attr_get(&value, g_repo, 0, "file.txt", "foo"));
...
void test_attr_repo__rewrite_sysdir(void)
{
	...
	cl_git_pass(git3_attr_get(&value, g_repo, 0, "file", "foo"));
	cl_assert_equal_s(value, "first");
