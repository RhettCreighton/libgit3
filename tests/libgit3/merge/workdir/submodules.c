...
void test_merge_workdir_submodules__automerge(void)
{
	...
	cl_git_pass(git3_reference_lookup(&our_ref, repo, "refs/heads/" SUBMODULE_MAIN_BRANCH));
	cl_git_pass(git3_commit_lookup(&our_commit, repo, git3_reference_target(our_ref)));
	cl_git_pass(git3_reset(repo, (git3_object *)our_commit, GIT3_RESET_HARD, NULL));

	cl_git_pass(git3_reference_lookup(&their_ref, repo, "refs/heads/" SUBMODULE_OTHER_BRANCH));
...
void test_merge_workdir_submodules__take_changed(void)
{
	...
	cl_git_pass(git3_reference_lookup(&their_ref, repo, "refs/heads/" SUBMODULE_OTHER2_BRANCH));
	cl_git_pass(git3_annotated_commit_from_ref(&their_head, repo, their_ref));
...
void test_merge_workdir_submodules__update_delete_conflict(void)
{
	...
	cl_git_pass(git3_reference_lookup(&our_ref, repo, "refs/heads/" SUBMODULE_DELETE_BRANCH));
	cl_git_pass(git3_commit_lookup(&our_commit, repo, git3_reference_target(our_ref)));
	cl_git_pass(git3_reset(repo, (git3_object *)our_commit, GIT3_RESET_HARD, NULL));

	cl_git_pass(git3_reference_lookup(&their_ref, repo, "refs/heads/" SUBMODULE_MAIN_BRANCH));
	cl_git_pass(git3_annotated_commit_from_ref(&their_head, repo, their_ref));
