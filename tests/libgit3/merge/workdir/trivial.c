...
static int merge_trivial(const char *ours, const char *theirs)
{
	...
	git3_reference *our_ref, *their_ref;
	...
	cl_git_pass(git3_reference_symbolic_create(&our_ref, repo, "HEAD", branch_buf.ptr, 1, NULL));

	cl_git_pass(git3_checkout_head(repo, &checkout_opts));

	git3_str_clear(&branch_buf);
	git3_str_printf(&branch_buf, "%s%s", GIT3_REFS_HEADS_DIR, theirs);
	cl_git_pass(git3_reference_lookup(&their_ref, repo, branch_buf.ptr));
	cl_git_pass(git3_annotated_commit_from_ref(&their_heads[0], repo, their_ref));
...
void test_merge_workdir_trivial__2alt(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "new-in-branch.txt", 0));
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);
...
void test_merge_workdir_trivial__3alt(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "new-in-3alt.txt", 0));
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);
	cl_assert(merge_trivial_conflict_entrycount() == 0);
}
...
void test_merge_workdir_trivial__4(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "new-and-different.txt", 0)) == NULL);
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);

	cl_assert(merge_trivial_conflict_entrycount() == 2);
	cl_assert(entry = git3_index_get_bypath(repo_index, "new-and-different.txt", 2));
	cl_assert(entry = git3_index_get_bypath(repo_index, "new-and-different.txt", 3));
}
...
void test_merge_workdir_trivial__5alt_1(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "new-and-same.txt", 0));
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);
	cl_assert(merge_trivial_conflict_entrycount() == 0);
}
...
void test_merge_workdir_trivial__5alt_2(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-to-same.txt", 0));
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);
	cl_assert(merge_trivial_conflict_entrycount() == 0);
}
...
void test_merge_workdir_trivial__6(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "removed-in-both.txt", 0)) == NULL);
	cl_assert(git3_index_reuc_entrycount(repo_index) == 1);
	cl_assert(reuc = git3_index_reuc_get_bypath(repo_index, "removed-in-both.txt"));
...
void test_merge_workdir_trivial__8(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "removed-in-8.txt", 0)) == NULL);

	cl_assert(git3_index_reuc_entrycount(repo_index) == 1);
	cl_assert(reuc = git3_index_reuc_get_bypath(repo_index, "removed-in-8.txt"));
...
void test_merge_workdir_trivial__7(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "removed-in-7.txt", 0)) == NULL);
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);

	cl_assert(merge_trivial_conflict_entrycount() == 2);
	cl_assert(entry = git3_index_get_bypath(repo_index, "removed-in-7.txt", 1));
	cl_assert(entry = git3_index_get_bypath(repo_index, "removed-in-7.txt", 3));
}
...
void test_merge_workdir_trivial__10(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "removed-in-10-branch.txt", 0)) == NULL);

	cl_assert(git3_index_reuc_entrycount(repo_index) == 1);
	cl_assert(reuc = git3_index_reuc_get_bypath(repo_index, "removed-in-10-branch.txt"));
...
void test_merge_workdir_trivial__9(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "removed-in-9-branch.txt", 0)) == NULL);
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);

	cl_assert(merge_trivial_conflict_entrycount() == 2);
	cl_assert(entry = git3_index_get_bypath(repo_index, "removed-in-9-branch.txt", 1));
	cl_assert(entry = git3_index_get_bypath(repo_index, "removed-in-9-branch.txt", 2));
}
...
void test_merge_workdir_trivial__13(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-in-13.txt", 0));
...
void test_merge_workdir_trivial__14(void)
{
	...
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-in-14-branch.txt", 0));
...
void test_merge_workdir_trivial__11(void)
{
	...
	cl_assert((entry = git3_index_get_bypath(repo_index, "modified-in-both.txt", 0)) == NULL);
	cl_assert(git3_index_reuc_entrycount(repo_index) == 0);

	cl_assert(merge_trivial_conflict_entrycount() == 3);
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-in-both.txt", 1));
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-in-both.txt", 2));
	cl_assert(entry = git3_index_get_bypath(repo_index, "modified-in-both.txt", 3));
}
