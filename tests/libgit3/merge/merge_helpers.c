...
{
	git3_commit *our_commit, *their_commit, *ancestor_commit = NULL;
	git3_tree *our_tree, *their_tree, *ancestor_tree = NULL;
	git3_oid our_oid, their_oid, ancestor_oid;
	git3_str branch_buf = GIT3_STR_INIT;
	int error;

	git3_str_printf(&branch_buf, "%s%s", GIT3_REFS_HEADS_DIR, ours_name);
	...
	git3_str_printf(&branch_buf, "%s%s", GIT3_REFS_HEADS_DIR, theirs_name);
	...
	error = git3_merge_base(&ancestor_oid, repo, git3_commit_id(our_commit), git3_commit_id(their_commit));
	...
	error = git3_merge_trees(index, repo, ancestor_tree, our_tree, their_tree, opts);
...
int merge_commits_from_branches(
	...
	git3_merge_options *opts)
{
	git3_commit *our_commit, *their_commit;
	git3_oid our_oid, their_oid;
	git3_str branch_buf = GIT3_STR_INIT;
	int error;
	...
	error = git3_merge_commits(index, repo, our_commit, their_commit, opts);
...
{
	git3_reference *head_ref, *theirs_ref;
	git3_annotated_commit *theirs_head;
	git3_checkout_options head_checkout_opts = GIT3_CHECKOUT_OPTIONS_INIT;

	head_checkout_opts.checkout_strategy = GIT3_CHECKOUT_FORCE;

	cl_git_pass(git3_reference_symbolic_create(&head_ref, repo, "HEAD", ours_branch, 1, NULL));
	cl_git_pass(git3_checkout_head(repo, &head_checkout_opts));

	cl_git_pass(git3_reference_lookup(&theirs_ref, repo, theirs_branch));
	cl_git_pass(git3_annotated_commit_from_ref(&theirs_head, repo, theirs_ref));

	cl_git_pass(git3_merge(repo, (const git3_annotated_commit **)&theirs_head, 1, merge_opts, checkout_opts));

	git3_reference_free(head_ref);
	git3_reference_free(theirs_ref);
	git3_annotated_commit_free(theirs_head);
...
void merge__dump_index_entries(git3_vector *index_entries)
{
	...
	for (i = 0; i < index_entries->length; i++) {
		...
		printf("%s ", git3_oid_allocfmt(&index_entry->id));
		printf("%d ", git3_index_entry_stage(index_entry));
...
void merge__dump_names(git3_index *index)
{
	...
	for (i = 0; i < git3_index_name_entrycount(index); i++) {
		conflict_name = git3_index_name_get_byindex(index, i);
...
void merge__dump_reuc(git3_index *index)
{
	...
	printf ("\nREUC:\n");
	for (i = 0; i < git3_index_reuc_entrycount(index); i++) {
		reuc = git3_index_reuc_get_byindex(index, i);

		printf("%s ", reuc->path);
		printf("%o ", reuc->mode[0]);
		printf("%s\n", git3_oid_allocfmt(&reuc->oid[0]));
		printf("          %o ", reuc->mode[1]);
		printf("          %s\n", git3_oid_allocfmt(&reuc->oid[1]));
		printf("          %o ", reuc->mode[2]);
		printf("          %s ", git3_oid_allocfmt(&reuc->oid[2]));
		printf("\n");
	}
	printf("\n");
}

static int index_entry_eq_merge_index_entry(const struct merge_index_entry *expected, const git3_index_entry *actual)
{
	git3_oid expected_oid;
	bool test_oid;

	if (strlen(expected->oid_str) != 0) {
		cl_git_pass(git3_oid_from_string(&expected_oid, expected->oid_str, GIT3_OID_SHA1));
		test_oid = 1;
	} else
		test_oid = 0;

	if (actual->mode != expected->mode ||
		(test_oid && git3_oid_cmp(&actual->id, &expected_oid) != 0) ||
		git3_index_entry_stage(actual) != expected->stage)
		return 0;
...
static int name_entry_eq_merge_name_entry(const struct merge_name_entry *expected, const git3_index_name_entry *actual)
{
...
static int index_conflict_data_eq_merge_diff(const struct merge_index_conflict_data *expected, git3_merge_diff *actual)
{
...
int merge_test_index(git3_index *index, const struct merge_index_entry expected[], size_t expected_len)
{
	...
	*/
	...
	if (git3_index_entrycount(index) != expected_len)
		return 0;

	for (i = 0; i < expected_len; i++) {
		if ((index_entry = git3_index_get_byindex(index, i)) == NULL)
			return 0;
...
int merge_test_names(git3_index *index, const struct merge_name_entry expected[], size_t expected_len)
{
	...
	*/
	...
	if (git3_index_name_entrycount(index) != expected_len)
		return 0;

	for (i = 0; i < expected_len; i++) {
		if ((name_entry = git3_index_name_get_byindex(index, i)) == NULL)
			return 0;
...
int merge_test_reuc(git3_index *index, const struct merge_reuc_entry expected[], size_t expected_len)
{
	...
	*/
	...
	if (git3_index_reuc_entrycount(index) != expected_len)
		return 0;

	for (i = 0; i < expected_len; i++) {
		if ((reuc_entry = git3_index_reuc_get_byindex(index, i)) == NULL)
			return 0;
		...
		if (expected[i].ancestor_mode > 0) {
			cl_git_pass(git3_oid_from_string(&expected_oid, expected[i].ancestor_oid_str, GIT3_OID_SHA1));

			if (git3_oid_cmp(&reuc_entry->oid[0], &expected_oid) != 0)
				return 0;
		}

		if (expected[i].our_mode > 0) {
			cl_git_pass(git3_oid_from_string(&expected_oid, expected[i].our_oid_str, GIT3_OID_SHA1));

			if (git3_oid_cmp(&reuc_entry->oid[1], &expected_oid) != 0)
				return 0;
		}

		if (expected[i].their_mode > 0) {
			cl_git_pass(git3_oid_from_string(&expected_oid, expected[i].their_oid_str, GIT3_OID_SHA1));

			if (git3_oid_cmp(&reuc_entry->oid[2], &expected_oid) != 0)
				return 0;
		}
	}
...
static int dircount(void *payload, git3_str *pathbuf)
{
	size_t *entries = payload;
	size_t len = git3_str_len(pathbuf);

	if (len < 5 || strcmp(pathbuf->ptr + (git3_str_len(pathbuf) - 5), "/.git") != 0)
		(*entries)++;
...
int merge_test_workdir(git3_repository *repo, const struct merge_index_entry expected[], size_t expected_len)
{
	...
	git3_oid actual_oid, expected_oid;
	git3_str wd = GIT3_STR_INIT;

	git3_str_puts(&wd, repo->workdir);
	git3_fs_path_direach(&wd, 0, dircount, &actual_len);

	if (actual_len != expected_len)
		return 0;

	for (i = 0; i < expected_len; i++) {
		git3_blob_create_from_workdir(&actual_oid, repo, expected[i].path);
		git3_oid_from_string(&expected_oid, expected[i].oid_str, GIT3_OID_SHA1);

		if (git3_oid_cmp(&actual_oid, &expected_oid) != 0)
			return 0;
	}
