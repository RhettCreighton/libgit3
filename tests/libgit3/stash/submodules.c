...
void test_stash_submodules__initialize(void)
{
	...
	cl_git_pass(git3_submodule_lookup(&sm, repo, "testrepo"));
}
...
{
	static git3_index *smindex;
	static git3_repository *smrepo;

	assert_status(repo, "modified", GIT3_STATUS_WT_MODIFIED);

	/* modify file in submodule */
	cl_git_rewritefile("submodules/testrepo/README", "heyheyhey");
	assert_status(repo, "testrepo", GIT3_STATUS_WT_MODIFIED);

	/* add file to index in submodule */
	cl_git_pass(git3_submodule_open(&smrepo, sm));
	cl_git_pass(git3_repository_index(&smindex, smrepo));
	cl_git_pass(git3_index_add_bypath(smindex, "README"));
	...
	assert_status(repo, "testrepo", GIT3_STATUS_WT_MODIFIED);
	assert_status(repo, "modified", GIT3_STATUS_CURRENT);
...
void test_stash_submodules__stash_is_empty_with_modified_submodules(void)
{
	...
	cl_git_fail_with(git3_stash_save(&stash_tip_oid, repo, signature, NULL, GIT3_STASH_DEFAULT), GIT3_ENOTFOUND);
