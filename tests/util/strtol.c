...
{
	int32_t i;
	cl_git_pass(git3__strntol32(&i, string, strlen(string), NULL, base));
...
{
	int32_t i;
	cl_git_fail(git3__strntol32(&i, string, strlen(string), NULL, base));
}

static void assert_l64_parses(const char *string, int64_t expected, int base)
{
	int64_t i;
	cl_git_pass(git3__strntol64(&i, string, strlen(string), NULL, base));
	cl_assert_equal_i(i, expected);
}
...
{
	int64_t i;
	cl_git_fail(git3__strntol64(&i, string, strlen(string), NULL, base));
}
...
void test_strtol__buffer_length_with_autodetection_truncates(void)
{
	...
	cl_git_pass(git3__strntol64(&i64, "011", 2, NULL, 0));
	cl_assert_equal_i(i64, 1);
	cl_git_pass(git3__strntol64(&i64, "0x11", 3, NULL, 0));
	cl_assert_equal_i(i64, 1);
}
...
void test_strtol__buffer_length_truncates(void)
{
	...
	cl_git_pass(git3__strntol32(&i32, "11", 1, NULL, 10));
	cl_assert_equal_i(i32, 1);

	cl_git_pass(git3__strntol64(&i64, "11", 1, NULL, 10));
	cl_assert_equal_i(i64, 1);
}
...
void test_strtol__buffer_length_with_leading_ws_truncates(void)
{
	...
	cl_git_fail(git3__strntol64(&i64, " 1", 1, NULL, 10));

	cl_git_pass(git3__strntol64(&i64, " 11", 2, NULL, 10));
	cl_assert_equal_i(i64, 1);
}
...
void test_strtol__error_message_cuts_off(void)
{
	...
	cl_assert(strstr(git3_error_last()->message, "2147483657") != NULL);
	cl_assert(strstr(git3_error_last()->message, "foobar") == NULL);
}
