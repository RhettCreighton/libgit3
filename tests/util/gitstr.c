...
void test_gitstr__0(void)
{
	...
	git3_str_puts(&buf, test_string);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(test_string, git3_str_cstr(&buf));

	git3_str_puts(&buf, test_string);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(test_string_x2, git3_str_cstr(&buf));

	git3_str_dispose(&buf);
}

/* test git3_str_printf */
void test_gitstr__1(void)
{
	git3_str buf = GIT3_STR_INIT;

	git3_str_printf(&buf, "%s %s %d ", "shoop", "da", 23);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s("shoop da 23 ", git3_str_cstr(&buf));

	git3_str_printf(&buf, "%s %d", "woop", 42);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s("shoop da 23 woop 42", git3_str_cstr(&buf));
...
void test_gitstr__2(void)
{
	...
	cl_assert_equal_s("", git3_str_cstr(&buf));
	cl_assert(buf.size == 0);
	/* cl_assert(buf.asize == 0); -- should not assume what git3_str does */
	...
	/* add data */
	git3_str_put(&buf, "xo", 2);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s("xo", git3_str_cstr(&buf));

	/* add letter again */
	git3_str_put(&buf, "xo", 2);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s("xoxo", git3_str_cstr(&buf));
	...
	git3_str_puts(&buf, REP4("0123456789"));
	cl_assert(git3_str_oom(&buf) == 0);

	git3_str_copy_cstr(data, sizeof(data), &buf);
	cl_assert_equal_s(REP4("0123456789"), data);
	git3_str_copy_cstr(data, 11, &buf);
	cl_assert_equal_s("0123456789", data);
	git3_str_copy_cstr(data, 3, &buf);
	cl_assert_equal_s("01", data);
	git3_str_copy_cstr(data, 1, &buf);
	cl_assert_equal_s("", data);

	git3_str_copy_cstr(data, sizeof(data), &buf);
	cl_assert_equal_s(REP4("0123456789"), data);

	git3_str_sets(&buf, REP256("x"));
	git3_str_copy_cstr(data, sizeof(data), &buf);
...
void test_gitstr__3(void)
{
	...
	/* set to string */
	git3_str_set(&buf, test_4096, 4096);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(test_4096, git3_str_cstr(&buf));

	/* append string */
	git3_str_puts(&buf, test_4096);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(test_8192, git3_str_cstr(&buf));
...
void test_gitstr__4(void)
{
	...
	for (i = 0; i < 10; ++i) {
		git3_str_puts(&buf, "1234"); /* add 4 */
		cl_assert(git3_str_oom(&buf) == 0);
		git3_str_consume(&buf, buf.ptr + 2); /* eat the first two */
		cl_assert(strlen(git3_str_cstr(&buf)) == (size_t)((i + 1) * 2));
	}
	/* we have appended 1234 10x and removed the first 20 letters */
	cl_assert_equal_s("12341234123412341234", git3_str_cstr(&buf));

	git3_str_consume(&buf, NULL);
	cl_assert_equal_s("12341234123412341234", git3_str_cstr(&buf));

	git3_str_consume(&buf, "invalid pointer");
	cl_assert_equal_s("12341234123412341234", git3_str_cstr(&buf));

	git3_str_consume(&buf, buf.ptr);
	cl_assert_equal_s("12341234123412341234", git3_str_cstr(&buf));

	git3_str_consume(&buf, buf.ptr + 1);
	cl_assert_equal_s("2341234123412341234", git3_str_cstr(&buf));

	git3_str_consume(&buf, buf.ptr + buf.size);
	cl_assert_equal_s("", git3_str_cstr(&buf));
	...
	size_t expected_asize)
{
	...
	git3_str_sets(&tgt, data_a);
	cl_assert(git3_str_oom(&tgt) == 0);
	git3_str_puts(&tgt, data_b);
	cl_assert(git3_str_oom(&tgt) == 0);
	cl_assert_equal_s(expected_data, git3_str_cstr(&tgt));
	...
	const char* expected_abcabc)
{
	...
	git3_str_sets(&buf, buf_a);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(buf_a, git3_str_cstr(&buf));

	git3_str_puts(&buf, buf_b);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected_ab, git3_str_cstr(&buf));

	git3_str_puts(&buf, buf_c);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected_abc, git3_str_cstr(&buf));

	git3_str_puts(&buf, buf_a);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected_abca, git3_str_cstr(&buf));

	git3_str_puts(&buf, buf_b);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected_abcab, git3_str_cstr(&buf));

	git3_str_puts(&buf, buf_c);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected_abcabc, git3_str_cstr(&buf));
...
{
	git3_str a = GIT3_STR_INIT;
	git3_str b = GIT3_STR_INIT;

	git3_str_sets(&a, "foo");
	cl_assert(git3_str_oom(&a) == 0);
	git3_str_sets(&b, "bar");
	cl_assert(git3_str_oom(&b) == 0);

	cl_assert_equal_s("foo", git3_str_cstr(&a));
	cl_assert_equal_s("bar", git3_str_cstr(&b));

	git3_str_swap(&a, &b);

	cl_assert_equal_s("bar", git3_str_cstr(&a));
	cl_assert_equal_s("foo", git3_str_cstr(&b));
...
void test_gitstr__7(void)
{
	...
	b = git3__strdup(fun);
	git3_str_attach(&a, b, strlen(fun) + 1);
	...
	const char *expected)
{
	...
	git3_str_sets(&buf, oldval);
	git3_str_join(&buf, sep, buf.ptr + ofs_a, b);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected, git3_str_cstr(&buf));
	git3_str_dispose(&buf);
}
	...
	const char *expected)
{
	char sep = ';';
	git3_str buf = GIT3_STR_INIT;
	git3_str_join_n(&buf, sep, 4, a, b, c, d);
	cl_assert(git3_str_oom(&buf) == 0);
	cl_assert_equal_s(expected, git3_str_cstr(&buf));
	git3_str_dispose(&buf);
}
...
void test_gitstr__8(void)
{
	...
	git3_str_join_n(&a, '/', 1, "foo");
	cl_assert(git3_str_oom(&a) == 0);
	cl_assert_equal_s("foo", git3_str_cstr(&a));

	git3_str_join_n(&a, '/', 1, "bar");
	cl_assert(git3_str_oom(&a) == 0);
	cl_assert_equal_s("foo/bar", git3_str_cstr(&a));

	git3_str_join_n(&a, '/', 1, "baz");
	cl_assert(git3_str_oom(&a) == 0);
	cl_assert_equal_s("foo/bar/baz", git3_str_cstr(&a));
	...
	for (s = 0; s < sizeof(sep) / sizeof(char); ++s) {
		...
		for (j = 0; j < sizeof(b) / sizeof(char*); ++j) {
			for (i = 0; i < sizeof(a) / sizeof(char*); ++i) {
				git3_str_join(&buf, separator, a[i], b[j]);
...
void test_gitstr__10(void)
{
	...
	cl_git_pass(git3_str_join_n(&a, '/', 1, "test"));
	cl_assert_equal_s(a.ptr, "test");
	cl_git_pass(git3_str_join_n(&a, '/', 1, "string"));
	cl_assert_equal_s(a.ptr, "test/string");
	git3_str_clear(&a);
	cl_git_pass(git3_str_join_n(&a, '/', 3, "test", "string", "join"));
	cl_assert_equal_s(a.ptr, "test/string/join");
	cl_git_pass(git3_str_join_n(&a, '/', 2, a.ptr, "more"));
...
void test_gitstr__join3(void)
{
	...
	cl_git_pass(git3_str_join3(&a, '/', "test", "string", "join"));
	...
	cl_git_pass(git3_str_join3(&a, '/', "", "string", "join"));
...
void test_gitstr__11(void)
{
	...
	cl_git_pass(git3_str_common_prefix(&a, t1, 3));
	cl_assert_equal_s(a.ptr, "");

	cl_git_pass(git3_str_common_prefix(&a, t2, 3));
	cl_assert_equal_s(a.ptr, "some");

	cl_git_pass(git3_str_common_prefix(&a, t3, 3));
	cl_assert_equal_s(a.ptr, "");

	cl_git_pass(git3_str_common_prefix(&a, t4, 3));
	cl_assert_equal_s(a.ptr, "happ");

	cl_git_pass(git3_str_common_prefix(&a, t5, 3));
	cl_assert_equal_s(a.ptr, "happ");

	cl_git_pass(git3_str_common_prefix(&a, t6, 3));
	cl_assert_equal_s(a.ptr, "");

	cl_git_pass(git3_str_common_prefix(&a, t7, 3));
	cl_assert_equal_s(a.ptr, "");
...
void test_gitstr__rfind_variants(void)
{
	...
	cl_git_pass(git3_str_sets(&a, "/this/is/it/"));

	len = (ssize_t)git3_str_len(&a);

	cl_assert(git3_str_rfind(&a, '/') == len - 1);
	cl_assert(git3_str_rfind_next(&a, '/') == len - 4);

	cl_assert(git3_str_rfind(&a, 'i') == len - 3);
	cl_assert(git3_str_rfind_next(&a, 'i') == len - 3);

	cl_assert(git3_str_rfind(&a, 'h') == 2);
	cl_assert(git3_str_rfind_next(&a, 'h') == 2);

	cl_assert(git3_str_rfind(&a, 'q') == -1);
	cl_assert(git3_str_rfind_next(&a, 'q') == -1);
...
void test_gitstr__puts_escaped(void)
{
	...
	git3_str_clear(&a);
	cl_git_pass(git3_str_puts_escaped(&a, "this is a test", "", ""));
	cl_assert_equal_s("this is a test", a.ptr);

	git3_str_clear(&a);
	cl_git_pass(git3_str_puts_escaped(&a, "this is a test", "t", "\\"));
	cl_assert_equal_s("\\this is a \\tes\\t", a.ptr);

	git3_str_clear(&a);
	cl_git_pass(git3_str_puts_escaped(&a, "this is a test", "i ", "__"));
	cl_assert_equal_s("th__is__ __is__ a__ test", a.ptr);

	git3_str_clear(&a);
	cl_git_pass(git3_str_puts_escape_regex(&a, "^match\\s*[A-Z]+.*"));
...
static void assert_unescape(char *expected, char *to_unescape) {
	...
	cl_git_pass(git3_str_sets(&buf, to_unescape));
	git3_str_unescape(&buf);
...
void test_gitstr__encode_base64(void)
{
	 ...
	 *     d  G  h  p  c  w
	 */
	cl_git_pass(git3_str_encode_base64(&buf, "this", 4));
	cl_assert_equal_s("dGhpcw==", buf.ptr);

	git3_str_clear(&buf);
	cl_git_pass(git3_str_encode_base64(&buf, "this!", 5));
	cl_assert_equal_s("dGhpcyE=", buf.ptr);

	git3_str_clear(&buf);
	cl_git_pass(git3_str_encode_base64(&buf, "this!\n", 6));
...
void test_gitstr__decode_base64(void)
{
	...
	cl_git_pass(git3_str_decode_base64(&buf, "dGhpcw==", 8));
	cl_assert_equal_s("this", buf.ptr);

	git3_str_clear(&buf);
	cl_git_pass(git3_str_decode_base64(&buf, "dGhpcyE=", 8));
	cl_assert_equal_s("this!", buf.ptr);

	git3_str_clear(&buf);
	cl_git_pass(git3_str_decode_base64(&buf, "dGhpcyEK", 8));
	cl_assert_equal_s("this!\n", buf.ptr);

	cl_git_fail(git3_str_decode_base64(&buf, "This is not a valid base64 string!!!", 36));
	cl_assert_equal_s("this!\n", buf.ptr);
...
void test_gitstr__encode_base85(void)
{
	...
	cl_git_pass(git3_str_encode_base85(&buf, "this", 4));
	cl_assert_equal_s("bZBXF", buf.ptr);
	git3_str_clear(&buf);

	cl_git_pass(git3_str_encode_base85(&buf, "two rnds", 8));
	cl_assert_equal_s("ba!tca&BaE", buf.ptr);
	git3_str_clear(&buf);

	cl_git_pass(git3_str_encode_base85(&buf, "this is base 85 encoded",
...
void test_gitstr__decode_base85(void)
{
	...
	cl_git_pass(git3_str_decode_base85(&buf, "bZBXF", 5, 4));
	cl_assert_equal_sz(4, buf.size);
	cl_assert_equal_s("this", buf.ptr);
	git3_str_clear(&buf);

	cl_git_pass(git3_str_decode_base85(&buf, "ba!tca&BaE", 10, 8));
	cl_assert_equal_sz(8, buf.size);
	cl_assert_equal_s("two rnds", buf.ptr);
	git3_str_clear(&buf);

	cl_git_pass(git3_str_decode_base85(&buf, "bZBXFAZc?TVqtS-AUHK3Wo~0{WMyOk", 30, 23));
...
void test_gitstr__decode_base85_fails_gracefully(void)
{
	...
	git3_str_puts(&buf, "foobar");

	cl_git_fail(git3_str_decode_base85(&buf, "invalid charsZZ", 15, 42));
	cl_git_fail(git3_str_decode_base85(&buf, "invalidchars__ ", 15, 42));
	cl_git_fail(git3_str_decode_base85(&buf, "overflowZZ~~~~~", 15, 42));
	cl_git_fail(git3_str_decode_base85(&buf, "truncated", 9, 42));
...
void test_gitstr__classify_with_utf8(void)
{
	...
	cl_assert(!git3_str_is_binary(&b));
	cl_assert(!git3_str_contains_nul(&b));
...
{
	git3_str src = GIT3_STR_INIT, tgt = GIT3_STR_INIT;

	/* LF source */

	git3_str_sets(&src, "lf\nlf\nlf\nlf\n");

	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
	check_buf("lf\r\nlf\r\nlf\r\nlf\r\n", tgt);

	cl_git_pass(git3_str_crlf_to_lf(&tgt, &src));
	...
	git3_str_sets(&src, "\rcr\r");
	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
	check_buf(src.ptr, tgt);
	cl_git_pass(git3_str_crlf_to_lf(&tgt, &src));
	check_buf("\rcr\r", tgt);

	git3_str_dispose(&src);
	git3_str_dispose(&tgt);

	/* blob correspondence tests */

	git3_str_sets(&src, ALL_CRLF_TEXT_RAW);
	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
	...
	git3_str_sets(&src, ALL_LF_TEXT_RAW);
	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
	...
	git3_str_sets(&src, MORE_CRLF_TEXT_RAW);
	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
	...
	git3_str_sets(&src, MORE_LF_TEXT_RAW);
	cl_git_pass(git3_str_lf_to_crlf(&tgt, &src));
...
void test_gitstr__dont_grow_borrowed(void)
{
	...
	git3_str_attach_notowned(&buf, somestring, strlen(somestring) + 1);
	cl_assert_equal_p(somestring, buf.ptr);
	cl_assert_equal_i(0, buf.asize);
	cl_assert_equal_i(strlen(somestring) + 1, buf.size);

	cl_git_fail_with(GIT3_EINVALID, git3_str_grow(&buf, 1024));
}
...
void test_gitstr__dont_hit_infinite_loop_when_resizing(void)
{
	 ...
	 * actually returns.
	 */
	(void)git3_str_try_grow(&buf, SIZE_MAX, true);
...
void test_gitstr__avoid_printing_into_oom_buffer(void)
{
	 ...
	 * all. As it didn't explicitly check for `git3_str__oom`
	 * in earlier versions, this would've resulted in it
	 * returning successfully and thus `git3_str_puts` would
	 * just print into the `git3_str__oom` array.
	 */
