...
void test_init__returns_count(void)
{
	 ...
	 * initialization.
	 */
	cl_assert_equal_i(2, git_libgit3_init());
	cl_assert_equal_i(3, git_libgit3_init());

	cl_assert_equal_i(2, git_libgit3_shutdown());
	cl_assert_equal_i(1, git_libgit3_shutdown());
}

void test_init__reinit_succeeds(void)
{
	cl_assert_equal_i(0, git_libgit3_shutdown());
	cl_assert_equal_i(1, git_libgit3_init());
...
static void *reinit(void *unused)
{
	...
	for (i = 0; i < 20; i++) {
		cl_assert(git_libgit3_init() > 0);
		cl_assert(git_libgit3_shutdown() >= 0);
	}
