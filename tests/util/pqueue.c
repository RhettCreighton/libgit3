...
void test_pqueue__items_are_put_in_order(void)
{
	...
	cl_git_pass(git3_pqueue_init(&pq, 0, 20, cmp_ints));
	...
	for (i = 0; i < 20; ++i) {
		...
		cl_git_pass(git3_pqueue_insert(&pq, &vals[i]));
	}

	cl_assert_equal_i(20, git3_pqueue_size(&pq));

	for (i = 1; i <= 20; ++i) {
		void *p = git3_pqueue_pop(&pq);
		cl_assert(p);
		cl_assert_equal_i(i, *(int *)p);
	}

	cl_assert_equal_i(0, git3_pqueue_size(&pq));
...
void test_pqueue__interleave_inserts_and_pops(void)
{
	...
	for (v = 0, chunk = 20; chunk <= 200; chunk += 20) {
		...
		for (; v < chunk; ++v) {
			vals[v] = (v & 1) ? 200 - v : v;
			cl_git_pass(git3_pqueue_insert(&pq, &vals[v]));
		}
	...
	cl_assert_equal_i(100, git3_pqueue_size(&pq));
...
void test_pqueue__max_heap_size(void)
{
	...
	cl_git_pass(git3_pqueue_init(&pq, GIT3_PQUEUE_FIXED_SIZE, 50, cmp_ints));
	...
	cl_assert_equal_i(50, git3_pqueue_size(&pq));
...
void test_pqueue__max_heap_size_without_comparison(void)
{
	...
	cl_git_pass(git3_pqueue_init(&pq, GIT3_PQUEUE_FIXED_SIZE, 50, NULL));
...
void test_pqueue__interleaved_pushes_and_pops(void)
{
	...
	cl_git_pass(git3_pqueue_init(&pq, 0, 10, cmp_ints_like_commit_time));

	for (i = 0, j = 0; commands[i] >= 0; ++i) {
		if (!commands[i]) {
			cl_assert((val = git3_pqueue_pop(&pq)) != NULL);
			cl_assert_equal_i(expected[j], *val);
			++j;
		} else {
			cl_git_pass(git3_pqueue_insert(&pq, &commands[i]));
		}
	}
