...
static void setup(walk_data *d)
{
	...
	cl_git_pass(git3_str_sets(&d->path, d->sub));

	state_loc = d;

	for (n = d->names; n->name; n++) {
		git3_file fd = p_creat(n->name, 0666);
		cl_assert(fd >= 0);
		p_close(fd);
		n->count = 0;
	}
...
static int one_entry(void *state, git3_str *path)
{
...
void test_dirent__dont_traverse_dot(void)
{
	...
	cl_git_pass(git3_fs_path_direach(&dot.path, 0, one_entry, &dot));
...
void test_dirent__traverse_subfolder(void)
{
	...
	cl_git_pass(git3_fs_path_direach(&sub.path, 0, one_entry, &sub));
...
void test_dirent__traverse_slash_terminated_folder(void)
{
	...
	cl_git_pass(git3_fs_path_direach(&sub_slash.path, 0, one_entry, &sub_slash));
...
void test_dirent__dont_traverse_empty_folders(void)
{
	...
	cl_git_pass(git3_fs_path_direach(&empty.path, 0, one_entry, &empty));

	check_counts(&empty);

	/* make sure callback not called */
	cl_assert(git3_fs_path_is_empty_dir(empty.path.ptr));
}
...
void test_dirent__traverse_weird_filenames(void)
{
	...
	cl_git_pass(git3_fs_path_direach(&odd.path, 0, one_entry, &odd));
...
static void handle_next(git3_fs_path_diriter *diriter, walk_data *walk)
{
	const char *fullpath, *filename;
	size_t fullpath_len, filename_len;

	cl_git_pass(git3_fs_path_diriter_fullpath(&fullpath, &fullpath_len, diriter));
	cl_git_pass(git3_fs_path_diriter_filename(&filename, &filename_len, diriter));
...
void test_dirent__diriter_with_fullname(void)
{
	...
	cl_git_pass(git3_fs_path_diriter_init(&diriter, sub.path.ptr, 0));
...
void test_dirent__diriter_at_directory_root(void)
{
	...
	cl_assert((root_offset = git3_fs_path_root(sandbox_path)) >= 0);

	cl_assert(root_path = git3__calloc(1, root_offset + 2));
	strncpy(root_path, sandbox_path, root_offset + 1);

	cl_git_pass(git3_fs_path_diriter_init(&diriter, root_path, 0));

	while ((error = git3_fs_path_diriter_next(&diriter)) == 0) {
		cl_git_pass(git3_fs_path_diriter_fullpath(&path, &path_len, &diriter));
