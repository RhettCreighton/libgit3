...
static git3_hash_win32_provider_t orig_provider;
#endif

void test_sha1__initialize(void)
{
#ifdef GIT3_SHA1_WIN32
	orig_provider = git3_hash_win32_provider();
#endif
...
{
#ifdef GIT3_SHA1_WIN32
	git3_hash_win32_set_provider(orig_provider);
#endif
...
static int sha1_file(unsigned char *out, const char *filename)
{
	...
	while ((read_len = p_read(fd, buf, 2048)) > 0)
		cl_git_pass(git3_hash_update(&ctx, buf, (size_t)read_len));

	cl_assert_equal_i(0, read_len);
	p_close(fd);

	ret = git3_hash_final(out, &ctx);
...
#ifdef GIT3_SHA1_BUILTIN
	GIT3_UNUSED(&expected);
	cl_git_fail(sha1_file(actual, FIXTURE_DIR "/shattered-1.pdf"));
	cl_assert_equal_s("SHA1 collision attack detected", git3_error_last()->message);
#else
...
void test_sha1__win32_providers(void)
{
	...
	git3_hash_win32_set_provider(GIT3_HASH_WIN32_CRYPTOAPI);
	cl_git_pass(sha1_file(actual, FIXTURE_DIR "/shattered-1.pdf"));
	cl_assert_equal_i(0, memcmp(expected, actual, GIT3_HASH_SHA1_SIZE));

	git3_hash_win32_set_provider(GIT3_HASH_WIN32_CNG);
	cl_git_pass(sha1_file(actual, FIXTURE_DIR "/shattered-1.pdf"));
	cl_assert_equal_i(0, memcmp(expected, actual, GIT3_HASH_SHA1_SIZE));
#endif
}
