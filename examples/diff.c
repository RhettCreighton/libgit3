/*
 * libgit3 "diff" example - shows how to use the diff API
 *
...
/**
 * This example demonstrates the use of the libgit3 diff APIs to
 * create `git3_diff` objects and display them, emulating a number of
 ...
 * the core libgit3 diff APIs, including various types of diffs and
...
struct diff_options {
	git3_diff_options diffopts;
	git3_diff_find_options findopts;
	int color;
	int no_index;
	int cache;
	int output;
...
static void diff_print_stats(git3_diff *diff, struct diff_options *o);
static void compute_diff_no_index(git3_diff **diff, struct diff_options *o);

int lg2_diff(git3_repository *repo, int argc, char *argv[])
{
	git3_tree *t1 = NULL, *t2 = NULL;
	git3_diff *diff;
	struct diff_options o = {
		GIT3_DIFF_OPTIONS_INIT, GIT3_DIFF_FIND_OPTIONS_INIT,
		-1, -1, 0, 0, GIT3_DIFF_FORMAT_PATCH, NULL, NULL, "."
	};
	...
	if (o.no_index >= 0) {
		...
		if (t1 && t2)
			check_lg2(
				git3_diff_tree_to_tree(&diff, repo, t1, t2, &o.diffopts),
				"diff trees", NULL);
			...
			if (o.cache == CACHE_NONE)
				check_lg2(
					git3_diff_tree_to_workdir(&diff, repo, t1, &o.diffopts),
					"diff tree to working directory", NULL);
			else
				check_lg2(
					git3_diff_tree_to_index(&diff, repo, t1, NULL, &o.diffopts),
					"diff tree to index", NULL);
		}
		else if (t1)
			check_lg2(
				git3_diff_tree_to_workdir_with_index(&diff, repo, t1, &o.diffopts),
				"diff tree to working directory", NULL);
		else
			check_lg2(
				git3_diff_index_to_workdir(&diff, repo, NULL, &o.diffopts),
				"diff index to working directory", NULL);

		/** Apply rename and copy detection if requested. */

		if ((o.findopts.flags & GIT3_DIFF_FIND_ALL) != 0)
			check_lg2(
				git3_diff_find_similar(diff, &o.findopts),
				"finding renames and copies", NULL);
	}

	/** Generate simple output using libgit3 display helper. */
	...
	if ((o.output & OUTPUT_DIFF) != 0) {
		...
		check_lg2(
			git3_diff_print(diff, o.format, color_printer, &o.color),
...
static void compute_diff_no_index(git3_diff **diff, struct diff_options *o) {
	...
	check_lg2(
		git3_patch_from_buffers(&patch, file1_str, strlen(file1_str), o->treeish1, file2_str, strlen(file2_str), o->treeish2, &o->diffopts),
		"patch buffers", NULL);
	check_lg2(
		git3_patch_to_buf(&buf, patch),
		"patch to buf", NULL);

	check_lg2(
		git3_diff_from_buffer(diff, buf.ptr, buf.size),
	...
	void *data)
{
	...
	if (*last_color >= 0) {
		switch (line->origin) {
		case GIT3_DIFF_LINE_ADDITION:  color = 3; break;
		case GIT3_DIFF_LINE_DELETION:  color = 2; break;
		case GIT3_DIFF_LINE_ADD_EOFNL: color = 3; break;
		case GIT3_DIFF_LINE_DEL_EOFNL: color = 2; break;
		case GIT3_DIFF_LINE_FILE_HDR:  color = 1; break;
		case GIT3_DIFF_LINE_HUNK_HDR:  color = 4; break;
		default: break;
		}
		...
		if (a[0] != '-') {
			if (o->treeish1 == NULL)
				...
				usage("Only one or two tree identifiers can be provided", NULL);
				 ...
				 !strcmp(a, "--patch")) {
			o->output |= OUTPUT_DIFF;
			o->format = GIT3_DIFF_FORMAT_PATCH;
		}
			...
			o->format = GIT3_DIFF_FORMAT_NAME_ONLY;
		else if (!strcmp(a, "--name-status") ||
				!strcmp(a, "--format=name-status"))
			o->format = GIT3_DIFF_FORMAT_NAME_STATUS;
		else if (!strcmp(a, "--raw") || !strcmp(a, "--format=raw"))
			o->format = GIT3_DIFF_FORMAT_RAW;
			...
			o->diffopts.flags |= GIT3_DIFF_REVERSE;
		else if (!strcmp(a, "-a") || !strcmp(a, "--text"))
			o->diffopts.flags |= GIT3_DIFF_FORCE_TEXT;
		else if (!strcmp(a, "--ignore-space-at-eol"))
			o->diffopts.flags |= GIT3_DIFF_IGNORE_WHITESPACE_EOL;
		else if (!strcmp(a, "-b") || !strcmp(a, "--ignore-space-change"))
			o->diffopts.flags |= GIT3_DIFF_IGNORE_WHITESPACE_CHANGE;
		else if (!strcmp(a, "-w") || !strcmp(a, "--ignore-all-space"))
			o->diffopts.flags |= GIT3_DIFF_IGNORE_WHITESPACE;
		else if (!strcmp(a, "--ignored"))
			o->diffopts.flags |= GIT3_DIFF_INCLUDE_IGNORED;
		else if (!strcmp(a, "--untracked"))
			o->diffopts.flags |= GIT3_DIFF_INCLUDE_UNTRACKED;
		else if (!strcmp(a, "--patience"))
			o->diffopts.flags |= GIT3_DIFF_PATIENCE;
		else if (!strcmp(a, "--minimal"))
			o->diffopts.flags |= GIT3_DIFF_MINIMAL;
			...
			o->findopts.flags |= GIT3_DIFF_FIND_RENAMES;
		else if (match_uint16_arg(
				&o->findopts.copy_threshold, &args, "-C") ||
			match_uint16_arg(
				&o->findopts.copy_threshold, &args, "--find-copies"))
			o->findopts.flags |= GIT3_DIFF_FIND_COPIES;
		else if (!strcmp(a, "--find-copies-harder"))
			o->findopts.flags |= GIT3_DIFF_FIND_COPIES_FROM_UNMODIFIED;
		else if (is_prefixed(a, "-B") || is_prefixed(a, "--break-rewrites"))
			/* TODO: parse thresholds */
			o->findopts.flags |= GIT3_DIFF_FIND_REWRITES;
...
static void diff_print_stats(git3_diff *diff, struct diff_options *o)
{
	git3_diff_stats *stats;
	git3_buf b = GIT3_BUF_INIT;
	git3_diff_stats_format_t format = 0;

	check_lg2(
		git3_diff_get_stats(&stats, diff), "generating stats for diff", NULL);

	if (o->output & OUTPUT_STAT)
		format |= GIT3_DIFF_STATS_FULL;
	if (o->output & OUTPUT_SHORTSTAT)
		format |= GIT3_DIFF_STATS_SHORT;
	if (o->output & OUTPUT_NUMSTAT)
		format |= GIT3_DIFF_STATS_NUMBER;
	if (o->output & OUTPUT_SUMMARY)
		format |= GIT3_DIFF_STATS_INCLUDE_SUMMARY;

	check_lg2(
		git3_diff_stats_to_buf(&b, stats, format, 80), "formatting stats", NULL);
