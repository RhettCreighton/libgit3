 ...
 */
typedef enum {
	GIT3_RESET_SOFT  = 1, /**< Move the head to the given commit */
	GIT3_RESET_MIXED = 2, /**< SOFT plus reset index to the commit */
	GIT3_RESET_HARD  = 3  /**< MIXED plus changes in working tree discarded */
} git3_reset_t;
 ...
 * must belong to the given `repo` and can either be a git3_commit or a
 * git3_tag. When a git3_tag is being passed, it should be dereferenceable
 * to a git3_commit which oid will be used as the target of the branch.
 *
 ...
 * resets the index and working tree to match.
 *
 * This behaves like `git3_reset()` but takes an annotated commit,
 * which lets you specify which extended sha syntax string was
 * specified by a user, allowing for more exact reflog messages.
 *
 * See the documentation for `git3_reset()`.
 *
 * @param repo Repository where to perform the reset operation.
 *
 ...
 * @return 0 on success or an error code
 */
GIT3_EXTERN(int) git3_reset_from_annotated(
	git3_repository *repo,
 ...
 */
GIT3_EXTERN(int) git3_reset_default(
	git3_repository *repo,
	const git3_object *target,
