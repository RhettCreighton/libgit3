...
/**
 * @file git3/sys/commit.h
 * @brief Low-level Git commit creation
 * @defgroup git3_commit Low-level Git commit creation
 * @ingroup Git
 * @{
 */
GIT3_BEGIN_DECL
...
/**
 * Create new commit in the repository from a list of `git3_oid` values.
 *
 * See documentation for `git3_commit_create()` for information about the
 * parameters, as the meaning is identical excepting that `tree` and
 * `parents` now take `git3_oid`.  This is a dangerous API in that nor
 * the `tree`, neither the `parents` list of `git3_oid`s are checked for
 * validity.
 *
 ...
 */
GIT3_EXTERN(int) git3_commit_create_from_ids(
	git3_oid *id,
 ...
 * along with the user supplied payload.  This should return a git3_oid of
 ...
 */
typedef const git3_oid * GIT3_CALLBACK(git3_commit_parent_callback)(size_t idx, void *payload);
 ...
 * `git3_oid` and doesn't check for validity, and `parent_cb` is invoked
 * with `parent_payload` and should return `git3_oid` values or NULL to
 ...
 *	the given reference will be updated to point to it
 */
GIT3_EXTERN(int) git3_commit_create_from_callback(
	git3_oid *id,
	git3_repository *repo,
	const char *update_ref,
	const git3_signature *author,
